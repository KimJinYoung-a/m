...
function getDefaultSearchDeamonPort()
	getDefaultSearchDeamonPort = "6167"
end function
...
Class SearchItemCls
	...
	dim FarrCateForAnyString'복수 카테고리 + anystring -- 다이어리스토리 6공 하위 카테고리에 사용
	dim FisTenOnly			'텐바이텐 전용상품
	dim FisLimit			'한정판매상품
	dim FisFreeBeasong
	dim FSubShopCd			'서브샵(100:다이어리스토리)
	dim FsubShopGroupCode	'서브샵 그룹코드 (2021 다이어리 스토리 신규 추가)
	...
	FUNCTION getSearchQuery(byref query)
		...
		'@ 하위 카테고리 2021 다이어리 스토리 사용
		IF FarrCateForAnyString<>"" THEN
			dim arrDt
			if right(FarrCateForAnyString,1) = "," then FarrCateForAnyString=left(FarrCateForAnyString,len(FarrCateForAnyString)-1)
			arrDt = split(FarrCateForAnyString,",")
			if ubound(arrDt)>0 then
				'다중 카테코드 적용
				strQue = strQue & " (idx_catecodeExt = '"& replace(FarrCateForAnyString,","," ") &"' anystring) "
			else
				'단일 카테코드 적용
				strQue = strQue & " (idx_catecodeExt = '"& FarrCateForAnyString &"' anystring) "
			end if
		END IF
		...
		'@ 서브샵 그룹코드
		IF FsubShopGroupCode <> "" THEN
			strQue= strQue & getQrCon(strQue) & "subshopgrouplist = '"& FsubShopGroupCode &"' "
		END IF
...
'				strQue = strQue & ")"
	...
	'				strQue = strQue & ")"
		...
		IF FRectSearchTxt<>"" and (scn="scn_dt_itemDisp" or scn="") then
		' WHERE조건을 다 만들었는데, GROUP 하는 쿼리에서 WHERE조건문자열 뒤에 추가로 넣는 부분이 있어서 애내들은 뺀다. 숫자가 좀 다를수 있겠으나 큰 문제는 없을듯 or scn="scn_dt_itemDispCateGroup"  or scn="scn_dt_itemDispBrandGroup"  or scn="scn_dt_itemDispColorGroup"  or scn="scn_dt_itemDispStyleGroup" or scn="scn_dt_itemDispAttribGroup"
	...
	Sub getSortQuery(byref query)
		...
		IF FRectSortMethod="ne" THEN '신상품
			strQue = strQue & " ORDER BY regdate desc"
	...
	PUBLIC SUB getSearchList()
		...
		Logs = Logs & "ggsn="&iggsnkey&"&page="&FCurrPage  ''2019/11/06 추가
		...
		FOR iRows=0 to FResultCount -1
			...
			SET FItemList(iRows) = NEW CCategoryPrdItem
				...
				if (FcolorCode="" or FcolorCode="0") Then
    				...
    				if (arrData(47)<>"") then ''2015 추가
    				    FItemList(iRows).FAddimage      = getItemImageUrl & "/contentsimage/" & GetImageSubFolderByItemid(FItemList(iRows).FItemid) & "/" & db2html(arrData(47))
    			    end if
				else
				    retMatchCd  = ""
				    retMatchVal = ""
				...
				'// 바로배송,Class,해외직구 2018/06/18
				FItemList(iRows).FDeliverFixDay = arrData(54)
				FItemList(iRows).FadultType = arrData(55)
				FItemList(iRows).FSubShopList = arrData(56) '2019-08-28 subshop 추가컬럼
				FItemList(iRows).FGiftDiv = arrData(57) '2019-08-28 subshop 추가컬럼
				FItemList(iRows).FNewYn = arrData(58) '2019-08-28 subshop 추가컬럼
				FItemList(iRows).FBestYn = arrData(59) '2019-08-28 subshop 추가컬럼
...
'' 검색추가로그 = 상품클릭
public Sub AddSearchAddLogItemClickWithChannel(ichannelGbn)
    Dim SvrAddr
	if (ichannelGbn="a") then
		SvrAddr = getTimeChkAddr(G_3RDSCH_ADDR)  '' app
	else
		SvrAddr = getTimeChkAddr(G_4THSCH_ADDR)  '' mob
	end if

    Dim SvrPort : SvrPort = getDefaultSearchDeamonPort()
    Dim AuthCode

    Dim iitemid : iitemid=getNumeric(request("itemid"))
    Dim iptype
    Dim ipval

    if (request("pRtr")<>"") then
        iptype="pRtr"
        ipval=request("pRtr")
    elseif (request("pCtr")<>"") then
        iptype="pCtr"
        ipval=request("pCtr")
    elseif (request("pEtr")<>"") then
        iptype="pEtr"
        ipval=request("pEtr")
    elseif (request("pBtr")<>"") then
        iptype="pBtr"
        ipval=request("pBtr")
    end if

    Dim SCN : SCN= "scn_dt_itemDispClick"       '상품클릭
    ...
    Dim SearchQuery : SearchQuery=" idx_itemid="&iitemid
    Dim SortQuery : SortQuery=""
    Dim RectSearchTxt : RectSearchTxt=iitemid
    Dim StartNum : StartNum = 1
    Dim FPageSize : FPageSize = 1


    Dim Logs : Logs= ""
    Logs = Logs & "ggsn="&iggsnkey&"&page="
    Logs = Logs & "&sid="&RIGHT(application("Svr_Info"),3)&"_"&session.sessionid ''2019/11/07 추가 세션ID값추가
    Logs = Logs & "&p="&ichannelGbn
    Logs = Logs & "&ptype="&iptype          ''– <VALUE> : pBtr 브랜드 , pCtr 카테고리, pEtr 기획전 , pRtr 검색어
    Logs = Logs & "&pval="&ipval            ''– <VALUE> : ptype에 상응하는 값
	...
	'' DocSetOption
	IF (Docruzer.SetOption(Docruzer.OPTION_REQUEST_CHARSET_UTF8,1)<0) THEN
		SET Docruzer = Nothing
		EXIT Sub
	End If
    ...
    ret = Docruzer.SubmitQuery(SvrAddr, SvrPort, _
                    AuthCode, Logs, Scn, _
                    SearchQuery,SortQuery, _
                    RectSearchTxt,StartNum, FPageSize, _
                    Docruzer.LC_KOREAN, Docruzer.CS_UTF8)
