...
function addDeviceLog(appKey,deviceid,userid,appVer,lastact)
  Exit function  ''더이상 로그쌓지 않음. 2014/12/02
    ...
    sqlStr = " insert into db_contents.dbo.tbl_app_regInfo_log" & vbCrLf
...
function addUUIDInfo(appKey,deviceid,uuID)
    ...
    sqlStr = "exec db_contents.dbo.sp_Ten_save_App_UUID_INFO "&appKey&",'"&deviceid&"','"&uuID&"'"
    dbget.Execute(sqlStr)
end function
...
function addUUIDNidInfo(appKey,deviceid,uuID,nid)
    ...
    ''sqlStr = "exec db_contents.dbo.sp_Ten_save_App_UUID_NID_INFO "&appKey&",'"&deviceid&"','"&uuID&"','"&nid&"'"
    ''dbget.Execute(sqlStr)
    
    ''2017/04/26 프로시져 분리.
    ''기존 uuid 쌓는곳.  
    if (uuID<>"") then
        CALL addUUIDInfo(appKey,deviceid,uuID)
    end if
    
    if (nid<>"") then
        sqlStr = "exec db_contents.dbo.sp_Ten_save_App_NID_INFO "&appKey&",'"&deviceid&"','"&uuID&"','"&nid&"'"
        dbget.Execute(sqlStr)
    end if
end function

'// android ad-id 추가 2017/03/25
function updateAdIDInfo(appKey,nid,adid)
    dim sqlStr
    if (nid="") then Exit function
    '' appKey 의미없음.
    sqlStr = "exec db_contents.dbo.sp_Ten_save_App_ADID_INFO '"&nid&"','"&adid&"'"
    dbget.Execute(sqlStr)    
end function
...
'// 필터 파징 (v3.2프로토콜)
Sub getParseFilterV32(sJson, byRef keyWd, byRef sscp, byRef delitp, byRef sflag, byRef pojok, byRef mde, byRef srtmt, byRef catCd, byRef mkrid, byRef styCd, byRef ColrCd, byRef prcMin, byRef prcMax, byRef rstxt, byRef offst, byRef listopt)
	Dim oFlt, i
	Set oFlt = sJson

	keyWd = requestCheckVar(oFlt.keyword,100)
	sscp = requestCheckVar(oFlt.sscp,10)
	delitp = requestCheckVar(oFlt.delitp,2)
	sflag = requestCheckVar(oFlt.sflag,10)
	pojok = requestCheckVar(oFlt.pojangok,1)
	mde = requestCheckVar(oFlt.mode,1)
	srtmt = requestCheckVar(oFlt.displaytype,2)
	catCd = ReplaceRequestSpecialChar(oFlt.categoryid)
	mkrid = ReplaceRequestSpecialChar(oFlt.brandid)
	prcMin = getNumeric(requestCheckVar(oFlt.pricelimitlow,8))
	prcMax = chkIIF(oFlt.pricelimithigh="-1","-1",getNumeric(requestCheckVar(oFlt.pricelimithigh,8)))
	rstxt = requestCheckVar(oFlt.rstxt,100)
	styCd = ReplaceRequestSpecialChar(oFlt.stylecd)
	ColrCd = ReplaceRequestSpecialChar(oFlt.colorcd)
	offst = ReplaceRequestSpecialChar(oFlt.offset)
	listopt = ReplaceRequestSpecialChar(oFlt.listoption)
