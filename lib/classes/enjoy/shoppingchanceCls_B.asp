...
Class ClsShoppingChance
 ...
 Public Fgname '// gnbname
	...
	Dim  strSql, strSqlCnt
		...
		strSqlCnt ="EXEC [db_event].[dbo].sp_Ten_event_shoppingchance_listCnt_Mobile_new '"&FSCType&"', '"&FSCTypegb&"', '"&FSCategory&"','"&FSCateMid&"','"&FEScope&"'" & CHKIIF(FUserID<>"",",'"&FUserID&"'",",''") & "" & CHKIIF(Fis2014renew="o",",'o'","") & ""
		...
		IF FTotCnt > 0 THEN
			strSql = "EXEC [db_event].[dbo].sp_Ten_event_shoppingchance_list_Mobile_2021 "&FCPage&", "&FPSize&",'"&FSCType&"', '"&FSCTypegb&"','"&FSCategory&"','"&FSCateMid&"','"&FEScope&"','"&FselOp&"'" & CHKIIF(FUserID<>"",",'"&FUserID&"'",",''") & "" & CHKIIF(Fis2014renew="o",",'o'","") & ""
	...
	Dim  strSql, strSqlCnt
		...
		strSqlCnt ="EXEC [db_event].[dbo].sp_Ten_event_shoppingchance_listCnt_App_new '"&FSCType&"', '"&FSCTypegb&"','"&FSCategory&"','"&FSCateMid&"','"&FEScope&"'" & CHKIIF(FUserID<>"",",'"&FUserID&"'",",''") & "" & CHKIIF(Fis2014renew="o",",'o'","") & ""
		...
		IF FTotCnt > 0 THEN
			strSql = "EXEC [db_event].[dbo].sp_Ten_event_shoppingchance_list_App_2021 "&FCPage&","&FPSize&",'"&FSCType&"', '"&FSCTypegb&"','"&FSCategory&"','"&FSCateMid&"','"&FEScope&"','"&FselOp&"'" & CHKIIF(FUserID<>"",",'"&FUserID&"'",",''") & "" & CHKIIF(Fis2014renew="o",",'o'","") & ""
	...
	Dim  strSql, strSqlCnt
		...
		strSqlCnt ="EXEC [db_event].[dbo].[sp_Ten_cate_event_listCnt] '"&FSCType&"','"&FSCategory&"','"&FEScope&"'" & CHKIIF(Fis2014renew="o",",'o'","") & " , '"& Fdevice & "'"
		...
		IF FTotCnt > 0 THEN
			strSql = "EXEC [db_event].[dbo].[sp_Ten_cate_event_list] "&FCPage&","&FPSize&",'"&FSCType&"','"&FSCategory&"','"&FEScope&"','"&FselOp&"'" & CHKIIF(Fis2014renew="o",",'o'","") & " , '"& Fdevice & "'"
...
Dim strSql
	...
	strSql = "EXEC db_sitemaster.dbo.usp_WWW_EventList_BestItem_Get @eventid=" & eventid
...
'// 기획전 메뉴 상단 상품 아이템 추가 (Best 상품 노출 추가) 1 6 11 .... 5개 단위로 해당 이벤트의 상품 이 있을경우 itemscore 순으로 3개 노출 3개 미만시 노출 안함 (2018-01-29 이종화)
Public Function fnGetDiaryEventListItem(eventid)
Dim strSql

	strSql = "EXEC db_sitemaster.dbo.usp_WWW_EventList_NewBestItem_Get2 @eventid=" & eventid

	dim rsMem : set rsMem = getDBCacheSQL(dbget,rsget,"DIEAITEM",strSql,60*5)
	if (rsMem is Nothing) then Exit Function ''추가

	IF Not (rsMem.EOF OR rsMem.BOF) THEN
		fnGetDiaryEventListItem = rsMem.GetRows()
	END IF
	rsMem.close
End Function

'// 상품 리스트노출
Public Function fngetDiaryEvtItemHtml(eventid)
	Dim arrListItem : arrListItem = fnGetDiaryEventListItem(eventid)
	Dim resultHtml , intLoop
	Dim link , imgsrc , price , discount , itemname
	...
	IF isArray(arrListItem) Then
		...
		resultHtml = "<div class=""prd_list type_basic"">"
	For intLoop =0 To UBound(arrListItem,2)
		...
		resultHtml = resultHtml &"	<article class=""prd_item"">"
		resultHtml = resultHtml &"		<figure class=""prd_img"">"
		resultHtml = resultHtml &"			<img src=""" & getThumbImgFromURL(imgsrc,200,200,"true","false") & """ alt=""" & arrListItem(2,intLoop) & """>"
		resultHtml = resultHtml &"		</figure>"
		resultHtml = resultHtml &"		<div class=""prd_info"">"
		resultHtml = resultHtml &"			<div class=""prd_price"">"
		resultHtml = resultHtml &"				<span class=""set_price""><dfn>판매가</dfn>" & price & "</span>"
		resultHtml = resultHtml &"				<span class=""discount""><dfn>할인율</dfn>" & discount & "</span>"
		resultHtml = resultHtml &"			</div>"
		resultHtml = resultHtml &"			<div class=""prd_badge"">"
		'if fnGetGiftiCon(arrListItem(13,intLoop),arrListItem(4,intLoop),arrListItem(0,intLoop)) then
		'resultHtml = resultHtml &"				<i class=""badge_gift"">선물</i>"
		'End If
		if fnGetDeliveryFreeiCon(arrListItem(13,intLoop),arrListItem(4,intLoop),arrListItem(14,intLoop)) then
		resultHtml = resultHtml &"				<i class=""badge_delivery"">무료배송</i>"
		End If
		resultHtml = resultHtml &"			</div>"
		resultHtml = resultHtml &"		</div>"
		If isapp = "1" Then
		resultHtml = resultHtml &"		<a href=""javascript:fnAPPpopupAutoUrl('"& link &"');"" class=""prd_link""><span class=""blind"">상품 바로가기</span></a>"
		Else
		resultHtml = resultHtml &"		<a href="""& link &""" class=""prd_link""><span class=""blind"">상품 바로가기</span></a>"
		End If
	...
	fngetDiaryEvtItemHtml = resultHtml
End Function

'// 상품 리스트노출
Public Function fngetDiary2022EvtItemHtml(eventid)
	Dim arrListItem : arrListItem = fnGetDiaryEventListItem(eventid)
	Dim resultHtml , intLoop
	Dim link , imgsrc , price , discount , itemname
	...
	For intLoop =0 To UBound(arrListItem,2)
		...
		resultHtml = resultHtml &"		</div>"
		If isapp = "1" Then
		resultHtml = resultHtml &"		<a href=""javascript:TnGotoAPPDiaryAutoUrl('"& link &"');"" onclick=""fnAmplitudeEventMultiPropertiesAction('click_diarystory_event_item','event_code|item_id','" & eventid & "|" & arrListItem(0,intLoop) & "');"" class=""prd_link""><span class=""blind"">상품 바로가기</span></a>"
		Else
		resultHtml = resultHtml &"		<a href=""javascript:TnGotoDiaryLinkUrl('"& link &"');"" onclick=""fnAmplitudeEventMultiPropertiesAction('click_diarystory_event_item','event_code|item_id','" & eventid & "|" & arrListItem(0,intLoop) & "');"" class=""prd_link""><span class=""blind"">상품 바로가기</span></a>"
		End If
		resultHtml = resultHtml &"	</article>"
	...
	fngetDiary2022EvtItemHtml = resultHtml
End Function
