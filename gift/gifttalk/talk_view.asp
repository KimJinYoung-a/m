...
function gotalkPage(page){
	$.ajax({
		url: "/gift/gifttalk/talk_view_ajax.asp?talkidx=<%=vTalkIdx%>&cpg="+page,
		cache: false,
		async: false,
...
function talkreg(talkidx){
	<%IF not(IsUserLoginOK) THEN%>
		if(confirm("로그인을 하셔야 글을 남길 수 있습니다.\n로그인 하시겠습니까?") == true) {
			parent.location.href = "<%=M_SSLUrl%>/login/login.asp?backpath=/gift/gifttalk/talk_view.asp?talkidx=<%=vTalkIdx%>";
			return true;
		} else {
			return false;
		}
	...
	if (str.length=='2'){
		if (str=='i1'){
			...
			fnOpenModal('/gift/gifttalk/talk_view.asp?talkidx='+talkidx+'&wagubun=a');
			return;
		}else if (str=='99'){
			alert('로그인을 해주세요.');
			return;
		...
		<div class="content bgGry" id="layerScroll">
			<div id="scrollarea">
				...
				<div class="inner5 tMar15 cmtCont">
					...
					<form name="frmactNew" id="frmactNew" method="post" action="/gift/gifttalk/iframe_talk_comment_proc.asp" target="iframecproc" style="margin:0px;">
					<input type="hidden" name="talkidx" value="<%=vTalkIdx%>">
