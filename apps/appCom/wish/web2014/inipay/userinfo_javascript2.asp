...
$(function() {
	...
	<%'<!--테스트 코드 15일로 바꿔야됨 -->%>
	<% If currentDate >= #07/08/2020 00:00:00# and currentDate <= #07/16/2020 23:59:59# and G_PG_CHAIPAYNEW_ENABLE Then %>
	// 결제수단 > 차이 기본 선택
	// 결제수단 > 페이코 기본 선택 2020-07-08~19
	setTimeout(function(){
...
function CheckPayMethod(comp){
	...
	else if(paymethod == "100")
	{
		<%'<!--테스트 코드 15일로 바꿔야됨 -->%>		
		<% If currentDate >= #07/08/2020 00:00:00# and currentDate <= #07/16/2020 23:59:59# and G_PG_CHAIPAYNEW_ENABLE Then %>
		$("#paymethodtab > ul > li").eq(1).addClass("current");
		<% else %>
		$("#paymethodtab > ul > li").eq(0).addClass("current");
		<% end if %>
		//PopMobileOrder(paymethod);
...
function PayNext(frm, iErrMsg){
			...
			*/
			...
			<% if (G_USE_BAGUNITEMP) then %>
				<% if (G_PG_KAKAOPAYNEW_ENABLE) Then %>
					// 신버전 카카오페이 연결
					frm.action = "<%=M_SSLUrl%>/inipay/kakaoapi/ordertemp_kakao.asp";
				<% Else %>			
					frm.action = "<%=M_SSLUrl%>/inipay/kakao/ordertemp_kakao.asp";
				<% End If %>
            <% else %>
    ...
    <% if (G_PG_TOSSPAYNEW_ENABLE) then %>
	    //토스페이
	    if (paymethod=="980"){
	        if(document.frmorder.value*1 >= 2000000){
			...
			*/
			...
			<% Else %>
				frm.action = "<%=M_SSLUrl%>/inipay/tosspay/ordertemp_toss.asp";
			<% End If %>
		    frm.submit();
	    }
    <% end if %>	
	...
	<% if (G_PG_CHAIPAYNEW_ENABLE) then %>
	    ...
	    if (paymethod=="990"){
			...
			*/
			...
			<% Else %>
				frm.action = "<%=M_SSLUrl%>/inipay/chaipay/ordertemp_chai.asp";
			<% End If %>
		    frm.submit();
	    }	
    <% end if %>	
    ...
    <% if (G_PG_NAVERPAY_ENABLE) then %>
	    ...
	    if (paymethod=="900"){
	    	/* 네이버페이 결제금액 제한 없음
	        if(document.frmorder.price.value*1 >= 5000000){
				alert("네이버페이 결제 최대 금액이 500만원 미만입니다.");
				return;
			}else if(document.frmorder.price.value*1 <1000){
				alert("네이버페이 결제 최소 금액은 1000원 이상입니다.");
				return;
			}
			*/
            ...
            <% if (G_USE_BAGUNITEMP) then %>
            frm.action = "<%=M_SSLUrl%>/inipay/naverpay/ordertemp_npay.asp";
            <% else %>
            frm.action = "<%=M_SSLUrl%>/inipay/naverpay/order_temp_save_npay.asp";
            <% end if %>
            frm.submit();
	    }
    <% end if %>
    ...
    <% if (G_PG_PAYCO_ENABLE) then %>
	    ...
	    if (paymethod=="950"){
            ...
            <% if (G_USE_BAGUNITEMP) then %>
	            frm.action = "<%=M_SSLUrl%>/inipay/payco/ordertemp_payco.asp";
	        <% else %>
			    frm.action = "<%=M_SSLUrl%>/inipay/payco/order_temp_save_payco.asp";
		    <% end if %>
			frm.submit();
	    }
    <% end if %>
