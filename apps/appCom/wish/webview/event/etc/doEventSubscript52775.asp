...
If userid = "" Then
	...
	dbget.close() : Response.End
End IF
If not(left(currenttime,10)>="2014-06-23" and left(currenttime,10)<"2014-06-30") Then
...
if mode="couponreg" then
	if not(geteventisusingyn) then
		Response.Write "<script type='text/javascript'>alert('종료 되었습니다.'); parent.top.location.href='/apps/appcom/wish/webview/event/eventmain.asp?eventid="&eCode&"'</script>"
		dbget.close() : Response.End
	end if
	...
	if subscriptcount <> 0 or bonuscouponcount <> 0 then
		Response.Write "<script type='text/javascript'>alert('쿠폰은 한 개의 아이디당 한 번만 다운 받으실 수 있습니다.'); parent.top.location.href='/apps/appcom/wish/webview/event/eventmain.asp?eventid="&eCode&"'</script>"
		dbget.close() : Response.End
	end if
	...
	if Hour(currenttime) < 09 then
		Response.Write "<script type='text/javascript'>alert('쿠폰은 오전 9시부터 다운 받으실수 있습니다.'); parent.top.location.href='/apps/appcom/wish/webview/event/eventmain.asp?eventid="&eCode&"'</script>"
		dbget.close() : Response.End
	end if	
	...
	Response.Write "<script type='text/javascript'>alert('쿠폰이 발급 되었습니다. 24시간 이내에 app에서 사용하세요!'); parent.top.location.href='/apps/appcom/wish/webview/event/eventmain.asp?eventid="&eCode&"'</script>"
	dbget.close() : Response.End
else
