...
For jj=0 To UBound(arr,2)
	If arr(0,jj) > 6 Then
		Response.Write "<script>alert('이미 출고가 완료되어\n선물포장 메세지를 수정할 수 없습니다.');location.href='/apps/appCom/wish/web2014/my10x10/order/myorderdetail.asp?idx="&orderserial&"&packopen=packopen';</script>"
		dbget.close
		Response.End
	End If
Next
...
set opackmaster = new Cpack
	...
	If opackmaster.FResultCount < 1 Then
		Response.Write "<script>alert('잘못된 경로입니다.3');location.href='/apps/appCom/wish/web2014/my10x10/order/myorderdetail.asp?idx="&orderserial&"&packopen=packopen';</script>"
		dbget.close
		Response.End
	End If
...
<script>
$(function() {
	setTimeout("fnAPPchangPopCaption('선물포장상품확인')",200);
...
function jsGoReturnPage(){
	parent.location.href='/apps/appCom/wish/web2014/my10x10/order/myorderdetail.asp?idx=<%=orderserial%>&packopen=packopen';
	fnAPPclosePopup();
}
</script>
<style>
...
<div class="heightGrid">
	<div class="container popWin">
		...
		<div class="content" id="contentArea">
			...
			<form name="frmMsg" action="/apps/appCom/wish/web2014/inipay/pack/pack_message_edit_proc.asp" method="post" style="margin:0px;">
			<input type="hidden" name="idx" value="<%=orderserial%>">
