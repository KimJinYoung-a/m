...
if (mode="cancelorder") or (mode="socancelorder") then
	...
	On Error Resume Next
		...
		If (Err.Number = 0) and (ScanErr="") Then
			...
			response.write "<script>location.href='/apps/appCom/wish/web2014/my10x10/order/myorderlist.asp?aflag=XX';</script>"
		Else
			dbget.RollBackTrans
...
elseif (mode="stockoutcancel") then	'### 품절 취소(부분취소일때) 프로세스 ###
	...
	On Error Resume Next
		...
		If (Err.Number = 0) and (ScanErr="") Then
			...
			response.write "<script>location.href='/apps/appCom/wish/web2014/my10x10/order/myorderlist.asp';</script>"
			dbget.close()	:	response.End
		Else
			dbget.RollBackTrans
