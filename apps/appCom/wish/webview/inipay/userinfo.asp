...
if IsPresentOrder then
	if oshoppingbag.isPresentItemOrderLimitOver(userid,2) then
...
if IsEventOrderItem then
	if oshoppingbag.isEventOrderItemLimitOver(userid,1) then
...
<!-- #include virtual="/apps/appCom/wish/webview/inipay/userinfo_javascript.asp" -->
</head>
<body class="shop">
...
<%
    dbget.close()
    response.redirect "/apps/appCom/wish/webview/inipay/shoppingbag.asp"
    response.end
%>
<% else %>
			<!--주문결제내역 시작-->
            ...
            <div class="inner">
                ...
                <div class="input-block email-block">
                    ...
                    <div class="input-controls email-type-b">
						<input type="text" id="email1" name="buyemail_Pre" maxlength="40" value="<%= Splitvalue(oUserInfo.FOneItem.FUserMail,"@",0) %>" class="form">
                        @
                        <% call DrawEamilBoxHTML_App("document.frmorder","buyemail_Tx","buyemail_Bx",Splitvalue(oUserInfo.FOneItem.FUserMail,"@",1),"form","form bordered","","onchange=""jsShowMailBox('document.frmorder','buyemail_Bx','buyemail_Tx');""") %>
					</div>
                </div>
                ...
                <div class="input-block">
                    ...
                    <div class="input-controls zipcode">
                        ...
                        <button type="button" class="btn type-c btn-findzipcode side-btn" onclick="searchzipBuyer('/apps/appcom/wish/webview/lib/layer_searchzipNew.asp?strMode=appcheck1&target=frmorder');">우편번호검색</button>
                    </div>
                </div>
				...
				<%	elseif IsRsvSiteOrder or (IsTicketOrder and TicketDlvType="1") then %>
					...
					%>
						...
						<% if (IsUserLoginOK) then %>
		                    <button type="button" onClick="chgRSVSel();copyDefaultinfo('O');" id="addressOption1">주문자 동일</button>
		                    <button type="button" onClick="chgRSVSel();copyDefaultinfo('R');" id="addressOption2">주소록</button>
		                    <button type="button" onClick="chgRSVSel();copyDefaultinfo('N');" id="addressOption3">새주소</button>
						<% else %>
							<button type="button" onClick="chgRSVSel();copyDefaultinfo('O');" id="addressOption1">주문고객과 동일</button>
							<button type="button" onClick="chgRSVSel();copyDefaultinfo('N');" id="addressOption3">새로운 주소</button>
						<% End If %>
							<button type="button" onClick="chgRSVSel('R');" id="addressOption4" class="active">현장수령</button>
		                </div>
						<p id="lyRsvDec">※ 자택으로 배송을 원하시면 주소를 선택해주세요.</p>
						...
						<div class="input-block no-label my-address-list" id="myaddress" style="display:none;">
							...
							<div class="input-controls">
								...
								<span>등록된 나의 주소록이 없습니다.</span>
								<% Else %>
								<select name="chgmyaddr" id="chgmyaddr" class="form full-size" title="저장된 나의 주소록">
									<option value="">주소를 선택 해주세요</option>
									<%For i = 1 To UBound(obj.Items) %>
									<option value="<%=i%>" tReqname="<%=obj.Items(i).reqName%>" tTxAddr1="<%=obj.Items(i).reqZipaddr%>" tTxAddr2="<%=obj.Items(i).reqAddress%>" tReqPhone="<%=obj.Items(i).reqPhone%>" tReqHp="<%=obj.Items(i).reqHp%>" tReqZipcode="<%=obj.Items(i).reqZipcode%>" tReqemail="<%=obj.Items(i).reqEmail%>" tCountryCode="<%=obj.Items(i).countryCode%>" tEmsAreaCode="<%=obj.Items(i).emsAreaCode%>"  ><%=obj.Items(i).reqPlace%> | <%=obj.Items(i).reqName%> | <%=obj.Items(i).reqZipaddr%> <%=obj.Items(i).reqAddress%></option>
									<% Next %>
								</select>
								<% End If %>
							</div>
					...
					<%	end if %>
		                ...
		                <div class="input-block">
		                    ...
		                    <div class="input-controls zipcode">
		                        ...
		                        <button type="button" class="btn type-c btn-findzipcode side-btn" onclick="searchzipBuyer('/apps/appcom/wish/webview/lib/layer_searchzipNew.asp?strMode=appcheck2&target=frmorder');">우편번호검색</button>
		                    </div>
		                </div>
		                <div class="input-block no-label">
		                    <label for="address1" class="input-label">주소2</label>
		                ...
		                <% end if %>
				<% else %>
		                <div class="toggle form type-c grey <%=chkIIF(IsUserLoginOK,"three","two")%>">
							<% if (IsUserLoginOK) then %>
                    ...
                    <div class="input-controls">
                        <select class="form full-size" name="appcoupon" onChange="RecalcuSubTotal(this);" <% if oAppCoupon.FResultCount>0 then %>onblur="chkCouponDefaultSelect(this);"<% end if %>>
							<% if oAppCoupon.FResultCount<1 then %>
								<option value="">사용 가능한 모바일 쿠폰이 없습니다.</option>
							<% else %>
								<option value="">사용 하실 모바일 쿠폰을 선택하세요!</option>
							<% end if %>
							<!-- Valid App Coupon -->
				...
				<%
				'20140825 이벤트 54469 기승전 쇼핑		'/2014.08.21 한용민 추가
				if date>="2014-08-25" and date<"2014-09-04" then
				%>
					<p>
						<img src="http://webimage.10x10.co.kr/eventIMG/2014/54470/img_bnr.jpg" onclick="location.href='/apps/appCom/wish/webview/event/eventmain.asp?eventid=54471';" alt="APP에서 쇼핑하면 엄청난 일이 벌어진다! 텐바이텐 APP에서 3만원 이상 쇼핑하시면 한정수량으로 제작된 텐바이텐X덴스 쇼퍼백을 선물로 드립니다. 텐바이텐 배송상품을 반드시 포함해야 사은품을 받을 수 있습니다." style="width:100%;" />
					</p>
				<% end if %>
				...
				%>
					<p>
						<a href="/apps/appCom/wish/webview/event/eventmain.asp?eventid=54872"><img src="http://imgstatic.10x10.co.kr/offshop/temp/2014/201409/order_before_m.jpg" alt="" style="width:100%;" /></a>
					</p>
				<% end if %>

				<div class="diff"></div>

                <% IF ((OpenGiftExists) and Not(IsTicketOrder)) or (DiaryOpenGiftExists) then %>
                <!-- bonus -->
                <div class="main-title">
                ...
                <div class="well type-b full-width">
					<h4>13주년 사은품 선택</h4>
					<p style="font-size:11px; margin-bottom:5px;">이벤트 기간 : 2014.10.06 ~ 10.20</p>
                    <%=replace(evtDesc,"class=""","class=""txt-list ")%>
                </div>
				<%
					for i=0 to oOpenGift.FResultCount-1
				...
				%>
						<%
							if giftRowNo=1 Then
								Response.Write "<div class=""diff-10""></div>"
								Response.Write "<div class=""bordered-box"">"
								Response.Write "	<a href=""#bonusBox"&i+1&""" class=""bordered-title t-l no-margin show-toggle-box"" closed=""true"" target=""#bonusBox"&i+1&""">" & oOpenGift.FItemList(i).getRadioName & "<i class=""icon-arrow-up-down absolute-right""></i></a>"
								Response.Write "	<ul class=""bonus-list"" id=""bonusBox"&i+1&""">"
						...
						%>
	                        <li>
								...
								<% if oOpenGift.FItemList(i).IsGiftItemSoldOut or (optAllsoldOut) then %><img src="http://fiximage.10x10.co.kr/m/2012/order/wrap_gift_soldout.png"  style="width:100%; position:absolute; left:0; top:0;" /><% end if %>
								<% if  Not IsNULL(oOpenGift.FItemList(i).Fimage120) then %>
								<img src="<%=oOpenGift.FItemList(i).Fimage120 %>" style="width:100%" OnError="this.src='http://webimage.10x10.co.kr/images/no_image.gif'" width="100" height="100"/>
								<% end if %>
								</p>

	                            <span class="product-name"><%= oOpenGift.FItemList(i).Fgiftkind_name %><% if oOpenGift.FItemList(i).getGiftLimitStr<>"" then %> <%= oOpenGift.FItemList(i).getGiftLimitStr %><% end if %></span>
				...
				<%
							...
							if (oOpenGift.FResultCount-1)>i then
								if oOpenGift.FItemList(i).Fgift_range1=oOpenGift.FItemList(i+1).Fgift_range1 then
									Response.Write chkIIF((giftRowNo mod 2)=0,"","</li>")
									giftRowNo = giftRowNo+1
								else
									Response.Write "</li></ul><div class=""clear""></div></div>"
									giftRowNo = 1
								end if
							end if
				...
				%>
                <div class="well type-b full-width">
					<h4>2015 다이어리 구매 사은품 선택</h4>
					<% if Not (isNULL(Diary_evtStDT) or isNULL(Diary_evtEdDT)) then %>
					<p style="font-size:11px; margin-bottom:5px;">이벤트 기간 : <%= replace(Diary_evtStDT,"-",".") %> ~ <%= replace(replace(Diary_evtEdDT,"-","."),Left(Diary_evtEdDT,4)&".","") %></p>
					<% end if %>
                    <ul class="txt-list tMar10 listTxt">
						<li>DIARY STORY 다이어리 포함 텐바이텐 배송상품 <br />1/2/3만5천원 이상 구매시 증정</li>
						<li>쿠폰, 할인카드 등 사용 후 구매확정금액 기준</li>
					</ul>
                </div>
				<div class="diff-10"></div>
				...
				%>
						<li>
							...
							<div>
								...
								<% end if %>
								<div class="pic">
									<% if oDiaryOpenGift.FItemList(i).IsGiftItemSoldOut or (optAllsoldOut) then %><img src="http://fiximage.10x10.co.kr/m/2012/order/wrap_gift_soldout.png" class="soldout" alt="품절되었습니다" /><% end if %>
									...
									<p class="ftMidSm tMar05 lh12 red"><%=oDiaryOpenGift.FItemList(i).getRadioName%></p>
                ...
                <div class="input-controls" id="i_paymethod">
                	<table>
                	<tr>
                		<td style="padding:0 10px 20px 0;"><input type="radio" name="Tn_paymethod" id="credit" value="100" OnClick="CheckPayMethod(this);" class="form type-c"> <span class="small"><label for="credit">신용카드</label></span></td>
		                <td style="padding:0 10px 20px 0;"><input type="radio" name="Tn_paymethod" id="mobile" value="400" OnClick="CheckPayMethod(this);" class="form type-c"> <span class="small"><label for="mobile">모바일결제</label></span></td>
            ...
            <div class="form-actions highlight">
                ...
                <div class="two-btns" id="nextbutton1" name="nextbutton1" style="display: block;">
                    <div class="col"><button type="button" class="btn type-a" onClick="location.href='<%=wwwURL%>/apps/appCom/wish/webview/inipay/ShoppingBag.asp';">쇼핑계속하기</button></div>
