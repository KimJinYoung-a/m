...
<script type="application/x-javascript" src="/deal/itemPrdDetail.js?v=1.1"></script>
...
$(function() {
	...
	$(window).scroll(function() {
		...
		if (window_top + $(".header_wrap").outerHeight() >= div_top) {
			$(".commonTabV16a").css({"position":"fixed","top":"4.1rem"});
		} else {
...
<script>
	if(confirm('이 상품은 성인 인증이 필요한 상품입니다. 성인 확인을 위해 성인 인증을 진행합니다.')){
		var url = '/login/login_adult.asp?backpath=<%=Server.URLencode(CurrURLQ())%>';
		location.href = url;
	} else {
		history.back();
	}
	...
	<div id="content" class="content">
		...
		<section class="items item-detail">
			<div class="desc">
				...
				<span class="brand" id="brandshow" style="display:none">
					<a href="/brand/brand_detail2020.asp?brandid=<%=oItem.Prd.Fmakerid%>"><%=oItem.Prd.FBrandName%></a>
				</span>
		...
		<div class="btnAreaV16a">
			...
			<% Else %>
				...
				<% Else %>
					...
					<%else%>
						<p class="actCart"><button type="button" class="btnV16a btnRed1V16a" onclick="confirmAdultAuth('<%=Server.URLencode(CurrURLQ())%>');" disabled="disabled">장바구니</button></p>
					<%End if%>
					...
					<%else%>
						<p class="actNow"><button type="button" class="btnV16a btnRed2V16a" onclick="confirmAdultAuth('<%=Server.URLencode(CurrURLQ())%>');" disabled="disabled">바로구매</button></p>
