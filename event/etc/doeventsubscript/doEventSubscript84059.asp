...
Dim urlchg, CheckUser
	...
	vQuery = "select userid from db_temp.dbo.tbl_temp_Send_UserSMS with (noLock) where yyyymmdd='20180201' and userid='" + CStr(userid) + "'"
	...
	If Not(CheckUser <> "") Then
		Response.write "<script>alert('VVIP 등급을 최초로 달성한 고객만 참여 하실 수 있습니다.'); "& urlchg &"'</script>"
		dbget.close() : Response.End
	End If
