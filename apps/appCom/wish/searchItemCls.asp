...
ELSE
     ...
     G_ORGSCH_ADDR = "192.168.0.106"		''192.168.0.106  :: 인덱싱 및 배포
END IF

function getTimeChkAddr()
...
Class SearchItemCls
		...
		IF FarrCate<>"" THEN
			...
			for lpCt=0 to ubound(arrCt)
				if FRectSearchCateDep="X" then
					...
					strQue = strQue & " idx_catecodelist='" & LCase(trim(arrCt(lpCt))) & "' "
				end if
	...
	PUBLIC Function getSearchListJson()
		...
		FOR iRows=0 to FResultCount -1
			...
			objRst(Null)("currentprice") = cStr(FItemList(iRows).FsellPrice)	'현재판매가(할인등)
	...
	PUBLIC Function getGroupbyCategoryListJson()
		...
		FOR iRows = 0 to FResultcount -1
			...
			if Not(arrData(41)="" or isNull(arrData(41))) then
				if (FGroupScope-1)>ubound(split(arrData(41),"^^")) then FGroupScope=ubound(split(arrData(41),"^^"))+1
				objRst(null)("name") = cStr(replace(split(arrData(41),"^^")(FGroupScope-1),"&nbsp;",""))	'카테고리명
			else
				objRst(null)("name") = ""
			end if
