...
If mode = "add" Then '//응모버튼 클릭
	if not( left(currenttime,10)>="2015-03-16" and left(currenttime,10)<"2015-03-21" ) then
	...
	winnumber = getdaterandomize(left(currenttime,10))
	'winnumber = 7
	...
	IF Not(oItem.Prd.isSoldout or oItem.Prd.isTempSoldOut or oItem.Prd.IsMileShopitem) Then
		...
		If result1 = "" And result2="" Then '//응모 내역이 없음
			'// 당첨일때 
			If cstr(renloop) < cstr(winnumber) Then
