...
<%
	Dim vIdx, vContentsIdx, vTitle, vViewNo, vViewNoTxt, vChkFav
	vIdx = requestCheckVar(Request("idx"),10)
	vContentsIdx = requestCheckVar(Request("contentsidx"),10)
	
	If vIdx = "" Then
		Response.Write "<script>alert('잘못된 경로입니다.');location.href='/play/';</script>"
		dbget.close()
		Response.End
	Else
	...
	If vContentsIdx = "" Then
		...
		Response.End
	Else
		If isNumeric(vContentsIdx) = False Then
			Response.Write "<script>alert('잘못된 경로입니다.');location.href='/play/';</script>"
			dbget.close()
			Response.End
		End If
	...
	<div class="mainSection">
		<div class="container">
			...
			<div class="content dFingers bgGry" id="contentArea">
				<div class="playCont">
					<span class="type" onclick="location.href='/play/index.asp?type=4'; return false;">DESIGN FINGERS</span>
					<p class="tit"><strong><%=vTitle%></strong><span><%=vViewNo%></span></p>
					<p id="mywish<%=iDFSeq%>" class="circleBox wishView <%=chkiif(vChkFav > 0 ,"wishActive","")%>" <% If getloginuserid <> "" Then %>onclick="TnAddPlaymywish('4','<%=iDFSeq%>','');"<% Else %>onclick="jsChklogin_mobile('','<%=Server.URLencode(CurrURLQ())%>');"<% End If %>><span>찜하기</span></p>
				</div>
				...
				<div class="inner10">
					...
					<div class="pdtListWrap">
						...
						<% For i = 0 To (clsDF.FResultCount-1) %>
							<li onclick="location.href='/category/category_itemPrd.asp?itemid=<% = clsDF.FCategoryPrdList(i).FItemID %>';" class="soldOut">
								<div class="pPhoto">
				...
				<div class="inner5">
					<div class="replyList box1">
						...
						<div class="btnWrap ct tPad15">
							<span class="button btM2 btRed cWh1 w30p"><a href="" onClick="fnOpenModal('/play/popComment.asp?view=w&contentsidx=<%=vContentsIdx%>'); return false;">쓰기</a></span>
							<span class="button btM2 btRedBdr cRd1 w30p"><a href="" onClick="fnOpenModal('/play/popComment.asp?view=l&contentsidx=<%=vContentsIdx%>'); return false;">전체보기</a></span>
						</div>
					</div>
				</div>
				...
				<form name="frmact" method="post" action="/play/lib/dozfcomment.asp" style="margin:0px;" target="iframeDB">
				...
				<input type="hidden" name="returnurl" value="/play/playDesignFingers.asp?idx=<%=vIdx%>&contentsidx=<%=vContentsIdx%>&comm=o">
				</form>
