	...
	<div class="mainSection">
		<div class="container bgGry">
			...
			<div class="content evtDetailV15" id="contentArea">
				<% If eCode <> "62313" and eCode <> "61319" and eCode <> "63082" And eCode <> "63504" And eCode <> "63793" And eCode <> "64274" And eCode <> "64256" Then '//기획전 제외 임시 %>
				<div class="evtHeadV15">
					...
					<% End If %>
					<div class="btnWrap">
						<p class="evtShareV15"><a href="" onClick="popSNSShare('<%=eName%>','<%=wwwUrl&CurrURLQ()%>','10x10 이벤트','<%=emimg_mo%>'); return false;">공유</a></p>
				...
				<%
					...
					set cEvent = nothing
				%>
				<% If eCode <> "62313" And eCode <> "61319" and eCode <> "63082" And eCode <> "63793" And eCode <> "64256" Then '//기획전 제외 임시 %>
				...
				<div class="inner5">
					<div class="anotherV15 box1">
						...
						<% For intCLoop = 0 To UBound(vArrIssue,2)
							...
							If vArrIssue(4,intCLoop) Or vArrIssue(5,intCLoop) Then '//issale ,  iscoupon
								if ubound(Split(vArrIssue(1,intCLoop),"|"))> 0 Then
									If vArrIssue(4,intCLoop) Or (vArrIssue(4,intCLoop) And vArrIssue(5,intCLoop)) then
										vName	= cStr(Split(vArrIssue(1,intCLoop),"|")(0)) &" <span style=color:red>"&cStr(Split(vArrIssue(1,intCLoop),"|")(1))&"</span>"
									ElseIf vArrIssue(5,intCLoop) Then
										vName	= cStr(Split(vArrIssue(1,intCLoop),"|")(0)) &" <span style=color:green>"&cStr(Split(vArrIssue(1,intCLoop),"|")(1))&"</span>"
									End If 			
								Else
							...
							END IF
						%>
							<li><a href="<%=vLink%>"><%=db2html(vName)%></a></li>
						<% Next %>
						</ul>
					</div>
				</div>
