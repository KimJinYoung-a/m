...
'// 방어코드 추가
if userage<>"" and Not(isNumeric(userage)) then
    userage = ""
end if
...
<% If IsUserLoginOK() Then ''ErBValue.value -> 공통파일의 구분값 (cate는 1) %>
	var wishpop = window.open('/common/popWishFolder.asp?ErBValue=4&ecode='+ecd+'&gb=search2017&itemid='+i+'','wishpop','')
<% Else %>
	...
	<div id="content" class="content">
		...
		<% If atype = "ne" Or atype = "be" Or atype = "ws" Or atype = "hs" Or atype = "st" Or atype = "dt" Or atype = "lv" Or atype = "ag"  Or atype = "mz" Or atype = "fo" Then %>
			<div class="items type-list tenten-best-default tenten-best-new">
				...
				%>
						...
						<% 'response.write oaward.FItemList(i).FadultType & oaward.FItemList(i).FItemID %>
		...
		<% elseif atype = "br" Then %>
			<div class="items type-list tenten-best-default tenten-best-new tenten-best-brand">
				...
				%>
								<li>
									<a href="/street/street_brand.asp?makerid=<%=oaward.FItemList(i).FMakerID %><%=gaparam & i+1 %>">
...
Function fnATYPErandom()
	...
	SELECT CASE i
		Case 1, 2 : vResult = "vi"
		Case 3, 4, 5, 6 : vResult = "be"
		Case 7, 8, 9, 10 : vResult = "ne"
		Case Else : vResult = "be"
	END SELECT

	fnATYPErandom = vResult
End Function
%>
