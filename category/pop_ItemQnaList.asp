...
function fnGoQnaPage(page) {
	$.ajax({
		url: "pop_ItemQnaList.asp?itemid=<%=itemid%>&cpg="+page,
		cache: false,
		async: false,
...
function fnGoQnaTab(mode) {
	...
	<%
		If Not(IsUserLoginOK) Then
			...
			Response.Write "jsChklogin_mobile('','" & Server.URLencode("/category/pop_itemqnalist.asp?itemid="&itemid&"&mode=wr") & "');" & vbCrLf
...
<header class="tenten-header header-popup">
	<div class="title-wrap">
		<h1>상품문의</h1>
		<button type="button" class="btn-close" onclick="location.replace('<%=wwwUrl%>/category/category_itemPrd.asp?itemid=<%=itemid%>'); return false;">닫기</button>
	</div>
</header>
<!-- content area -->
