let store = new Vuex.Store({
    ...
    mutations : {
        ...
        SET_EVENTS_RESULT(state, data) { // 이벤트 리스트 SET
            state.event_count = data.totalCount;
    ...
    actions : {
        READ_SEARCH_EVENTS(context) { // READ 검색결과
            const parameter = context.getters.parameter;

            if( parameter.keyword == null || parameter.keyword === '' ) {
                history.back();
            } else {
                let search_event_apiurl = apiurlv2 + '/search/eventSearch'
                ...
                if( parameter.correct_keyword !== '' ) { // 교정무시 키워드
                    search_event_apiurl += '&ignoreCorrectTypos=true&ignoreCorrectTyposKeyword=' + parameter.correct_keyword;
                }

                $.ajax({
                    type: "GET",
                    url: search_event_apiurl,
                    ContentType: "json",
                    crossDomain: true,
                    xhrFields: {
                    ...
                    success: function (data) {
                        console.log(data);
                        context.commit('SET_EVENTS_RESULT', data);
