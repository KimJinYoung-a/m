...
<!-- #include virtual="/event/etc/event57029Cls.asp" -->
<!-- #include virtual="/lib/inc/head.asp" -->

<%
dim eCode, userid, subscriptcount, bonuscouponcount, totalsubscriptcount, totalbonuscouponcount
dim ename, cEvent, emimg, smssubscriptcount, usercell, folderimgselect , noticetxt , appurl
	eCode=getevt_code
...
If left(Trim(currenttime),10)>="2014-12-05" And left(Trim(currenttime),10) < "2014-12-06" Then
	folderimgselect = "57029"
	noticetxt = "12/5(금)"
	appurl = "http://m.10x10.co.kr/apps/link/?2120141204"
ElseIf left(Trim(currenttime),10)>="2014-12-10" And left(Trim(currenttime),10) < "2014-12-11" Then
	folderimgselect = "57246"
	noticetxt = "12/10(수)"
	appurl = "http://m.10x10.co.kr/apps/link/?2220141204"
Else
	folderimgselect = "57029"
	noticetxt = "12/5(금)"
	appurl = "http://m.10x10.co.kr/apps/link/?2120141204"
...
.mEvt<%=folderimgselect%> .evtNoti {padding:27px 14px; background:#fff; margin-bottom:-50px;}
...
@media all and (min-width:480px){
	.mEvt<%=folderimgselect%> .evtNoti {padding:40px 21px;}
	...
	function jseventSubmit(frm){
				...
				<% if subscriptcount=0 and bonuscouponcount=0 then %>
					...
					<% else %>
						<% if Hour(currenttime) < 09 then %>
							...
							return;
						<% else %>
							frm.action="/event/etc/doEventSubscript57029.asp";
		...
		<% Else %>
			if(confirm("로그인후 쿠폰 발급이 가능 합니다. 로그인 하시겠습니까?") == true) {
				top.location.href = "/login/login.asp?backpath=/event/eventmain.asp?eventid=<%=folderimgselect%>";
			 }
			return  ;
		<% End IF %>
	}
...
<div class="evtCont">
	...
	<div class="mEvt<%=folderimgselect%>">
		<h2><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/tit_sorry.png" alt="미안하다 생색이다 - 센스있는 연말선물 걱정마세요~ 텐바이텐이 있잖아요! 오늘 하루 생색나는 APP쿠폰을 자신있게 쏩니다" /></h2>
		<div class="appCoupon">
			<p><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/img_app_coupon.png" alt="오늘 하루 APP에서 5,000원 쿠폰" /></p>
		...
		<div class="appInstall">
			<p><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/txt_app_install.png" alt="텐바이텐 APP 설치하고 쿠폰쓰러가자!" /></p>
			<a href="<%=appurl%>" target="_blank"><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/btn_app_download.png" alt="쿠폰 다운받기" /></a>
		</div>
		<p><a href="/member/join.asp" target="_top"><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/btn_join.png" alt="텐바이텐 가입하고 첫구매 하러가기" /></a></p>
		<dl class="evtNoti">
			<dt>이벤트 유의사항</dt>
			<dd>
				<ul>
