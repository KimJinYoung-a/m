...
<%
	...
	dim vItem(3), lp, i
	dim mx, my, startNo
	Dim sFolder , CtrlDate , mainFile , CtrlTime
	...
	vXmlUrl = server.MapPath(sFolder & mainFile)	'// 접수 파일
	...
	If vRstCont<>"" Then
		...
		For each subNodes in cSub
			If CDate(CtrlTime) >= CDate(subNodes.getElementsByTagName("starttime").item(0).text) AND CDate(CtrlTime) <= CDate(subNodes.getElementsByTagName("endtime").item(0).text) Then
			...
			vItem(lp) = vItem(lp) & "		<span>"& chrbyte(subNodes.getElementsByTagName("itemname").item(0).text,"16","Y")&"</span>" & vbCrLf
