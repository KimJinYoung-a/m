...
<div class="inner10">
	...
	<ul class="myWishList wishFolderList">
		<li class="subTit">
			...
			<p><a href="/my10x10/myWish/myWish.asp?ucid=<%=Server.UrlEncode(tenEnc(vUserId))%>&vlg=wlist">전체 위시보기</a></p>
		</li>
		<% IF oDoc.FResultCount >0 then %>
			...
			%>
				<li>
					<a href="/my10x10/myWish/myWish.asp?fIdx=<%=oDoc.FWishPrdList(i).FFolderIdx%>&ucid=<%=Server.UrlEncode(tenEnc(vUserId))%>&vlg=wlist">
						<p class="folderInfo">
							<span class="folderName"><%=oDoc.FWishPrdList(i).FFolderName%></span>
							<% IF Trim(oDoc.FWishPrdList(i).FFolderName) = "넣어둬 넣어둬" AND Now() < #02/23/2015 00:00:00# then %><em class="crMint fs11" style="display:block; margin-bottom:5px;">| 이벤트 진행 중 |</em><% end if %>
