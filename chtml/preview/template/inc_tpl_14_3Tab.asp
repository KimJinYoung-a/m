<%
	...
	if (application("Svr_Info")="137" or application("Svr_Info")="082" or application("Svr_Info")="Dev") then
		...
		if not(fso.FileExists(server.MapPath(sFolder & "sub_fingers.xml"))) or dateDiff("h",Application("chk_main_fingers_update"),now())>0 then
			Server.Execute("/chtml/preview/make_main_fingersXML.asp")
		end if
		set fso = Nothing
	end if
