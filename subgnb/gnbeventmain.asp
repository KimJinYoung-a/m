...
Dim etc_itemid, arrGiftBox, isNew, isOnePlusOne, isOnlyTen, evt_html, contentsAlign
dim endlessView, eventtype_mo, videoFullLink, CopyHide
...
IF egCode = "" THEN egCode = 0
	...
	If Now() >= "2018-03-10" And Now() < "2018-03-12" Then
		If Trim(eCode)="84780" Then
			session("chkDisneyHomeMove") = "checkHomeDisney"
		End If
	End If
...
$(function(){
	...
	<%' amplitude 이벤트 로깅 %>
		//setTimeout("tagScriptSend('gnbEvent', '<%=eCode%>', '', 'amplitudeProperties')", 100);
	<%'// amplitude 이벤트 로깅 %>
});

function goWishPop(i,ecd){
<% If IsUserLoginOK() Then ''ErBValue.value -> 공통파일의 구분값 (cate는 1) %>
	var wishpop = window.open('/common/popWishFolder.asp?ErBValue=12&ecode='+ecd+'&gb=search2017&itemid='+i+'','wishpop','')
...
function jsGNBEventlink(ecode,gcode){
	var url = '/subgnb/gnbeventmain.asp?eventid='+ecode;
	...
	<div id="content" class="content">
        ...
        <% CASE "11" 'MD 등록 이벤트 I형 %>
			...
			$(function(){
				...
				var menuTop=0;
				if ($(".exhibition-list-wrap").length) {
					menuTop = $(".exhibition-list-wrap").offset().top;
				}
...
<script src="/event/lib/eventgroupitemlist2021.js?v=1.03"></script>
</body>
</html>
