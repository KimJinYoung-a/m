...
<%
	...
	Dim ABTestValue : ABTestValue=RequestCheckVar(request("ABTestValue"),10)
	if page="" then page=1
	...
	oEval.FRectItemID = itemid
		...
		if oItem.Prd.FEvalCnt>0  And tabno = 3 then
		    ...
		    If ABTestValue="1" Then
				oEval.getItemEvalListTestUser
			Else
...
<div class="bxWt1V16a">
	<ul class="btnBarV16a">
		<% If oItem.Prd.Ftestercnt > 0 Then %>
			<li <%=CHKIIF(tabno="3","class='current'","")%> name="cmtTab01" style="width:33.3%;"><div><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_review_tab_in_product','type|tab_index|tab_name','tester|0|<%=chkIIF(Not(IsTicketItem),"상품후기","후기")%>');tabchange('category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=3&tnm=<%=tabname%>');return false;"><%=chkIIF(Not(IsTicketItem),"상품후기","후기")%><span>(<%=oItem.Prd.FEvalCnt%>)</span></a></div></li>
			<li <%=CHKIIF(tabno="31","class='current'","")%> name="cmtTab02" style="width:33.3%;"><div><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_review_tab_in_product','type|tab_index|tab_name','tester|1|포토');tabchange('category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=31&tnm=<%=tabname%>');return false;">포토후기<span>(<%=oItem.Prd.FEvalCnt_photo%>)</span></a></div></li>
			<% If oItem.Prd.Ftestercnt > 0 Then %>
			<li <%=CHKIIF(tabno="32","class='current'","")%> name="cmtTab03" style="width:33.4%;"><div><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_review_tab_in_product','type|tab_index|tab_name','tester|2|테스터후기');tabchange('category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=32&tnm=<%=tabname%>');return false;">테스터후기<span>(<%=oItem.Prd.Ftestercnt%>)</span></a></div></li>
			<% End If %>
		<% Else %>
			<li <%=CHKIIF(tabno="3","class='current'","")%> name="cmtTab01" style="width:50%;"><div><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_review_tab_in_product','type|tab_index|tab_name','normal|0|<%=chkIIF(Not(IsTicketItem),"상품후기","후기")%>');tabchange('category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=3&tnm=<%=tabname%>');return false;"><%=chkIIF(Not(IsTicketItem),"상품후기","후기")%><span>(<%=oItem.Prd.FEvalCnt%>)</span></a></div></li>
			<li <%=CHKIIF(tabno="31","class='current'","")%> name="cmtTab02" style="width:50%;"><div><a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_review_tab_in_product','type|tab_index|tab_name','normal|1|포토');tabchange('category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=31&tnm=<%=tabname%>');return false;">포토후기<span>(<%=oItem.Prd.FEvalCnt_photo%>)</span></a></div></li>
		<% End If %>
	</ul>
</div>
...
<% IF oItem.Prd.FEvalCnt>0 Or oItem.Prd.Ftestercnt>0 then %>
	...
	<div class="btnAreaV16a">
		<p><button type="button" onclick="fnOpenModal('pop_ItemEvalList.asp?itemid=<%=itemid%>&sortMtd=<% If tabno="31" Then Response.write "ph" End If %><% If tabno="32" Then Response.write "ne" End If %>'); fnAmplitudeEventMultiPropertiesAction('click_review_all_in_product','itemid','<%=itemid%>'); return false;" class="btnV16a btnRed1V16a">전체보기</button></p>
	</div>
	<% end if %>
<% End If %>
