...
%>
<script>
	function jsphDivView(d){
		if(d == "l"){
			$("#allCmt").css("display","block");
			$("#photoCmt").css("display","none");
			$("#alist").addClass("current");
			$("#plist").removeClass("current");
		}else{
			$("#allCmt").css("display","none");
			$("#photoCmt").css("display","block");
			$("#alist").removeClass("current");
			$("#plist").addClass("current");
		}
	}
</script>
<div class="tab01">
	<ul class="tabNav tNum2">
		<li class="current" id="alist"><a href="" onClick="jsphDivView('l'); return false;">전체(<%= oEval.FTotalCount %>)<span></span></a></li>
		<li id="plist"><a href="" onClick="jsphDivView('p'); return false;">포토후기(<%= oEvPhoto.FTotalCount %>)<span></span></a></li>
	</ul>
	<div class="tabContainer box1 mar0 evtReviewV15">
		<div class="rt"><p class="goWriteV15" onclick="fnAPPpopupBrowserURL('상품후기','<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/goodsusing.asp'); return false;"><span>후기쓰기</span></p></div>
		<!-- 전체 후기 -->
		<div id="allCmt" class="tabContent">
			<div class="postscript">
				<ul class="replyList">
					...
					%>
					<li>
						<p class="pic"><a href="" onclick="fnAPPpopupProduct(<%=oEval.FItemList(i).FItemID%>);return false;"><img src="<%=oEval.FItemList(i).FImageList120%>" alt="<%=oEval.FItemList(i).FItemID%>" /></a></p>
						<div class="replyCont">
				...
				<% if oEval.FResultCount > 0 Then %>
				<p class="goViewV15" onclick="fnAPPpopupBrowserURL('후기','<%=wwwUrl%>/apps/appCom/wish/web2014/event/event_usingpost.asp?view=l&eventid=<%=eCode%>&linkevt=<%=LinkEvtCode%>&blnB=<%=blnBlogURL%>'); return false;"><span>더보기</span></p>
				<% End If %>
			</div>
		</div>
		<!--// 전체 후기 -->
		...
		<div id="photoCmt" class="tabContent">
			<div class="postscript">
				<ul class="replyList">
					...
					%>
					<li>
						<p class="pic"><a href="" onclick="fnAPPpopupProduct(<%=oEvPhoto.FItemList(i).FItemID%>);return false;"><img src="<%=oEvPhoto.FItemList(i).FImageList120%>" alt="<%=oEvPhoto.FItemList(i).FItemID%>" /></a></p>
						<div class="replyCont">
				...
				<% if oEvPhoto.FResultCount > 0 Then %>
				<p class="goViewV15" onclick="fnAPPpopupBrowserURL('후기','<%=wwwUrl%>/apps/appCom/wish/web2014/event/event_usingpost.asp?view=p&eventid=<%=eCode%>&linkevt=<%=LinkEvtCode%>&blnB=<%=blnBlogURL%>'); return false;"><span>더보기</span></p>
				<% End If %>
			</div>
		</div>
		<!--// 포토후기 -->
