...
<%
Dim vUserID, beforepageminidx, beforepagedispyn
			...
			%>
				<div class="box1 select<%=CHKIIF(cTalk.FItemList(i).FTheme="2","AB","YN")%>">
					...
					<div class="desc descAB">
						<div class="thumb">
							<a href="" onclick="fnAPPpopupProduct('<%= vtmpitemid %>'); return false;">
							<img src="<%=getThumbImgFromURL(vtmpImageBasic,300,300,"true","false")%>" alt="<%= vtmpitemid %>" /></a>
							<a href="" onclick="fnAPPpopupProduct('<%= cTalk.FItemList(i).fitemid %>'); return false;">
					...
					<div class="comment">
						<div class="replyList" id="commentcnt<%= cTalk.FItemList(i).FTalkIdx %>" commentcnt="<%= cTalk.FItemList(i).FCommCnt %>">
							...
							<% Else %>
								<h4><a href="" onClick="fnAPPpopupBrowserURL('코멘트','<%=wwwUrl%>/apps/appCom/wish/web2014/gift/gifttalk/talk_view.asp?talkidx=<%=cTalk.FItemList(i).FTalkIdx%>&wagubun=w'); return false;">코멘트 쓰기</a></h4>
							<% End if %>
