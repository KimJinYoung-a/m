...
{
	$.ajax({
		url: "/apps/appcom/wish/web2014/event/event_commajax.asp?view=l&iCC="+page+"&eventid=<%=eCode%>&linkevt=<%=LinkEvtCode%>&blnB=<%=blnBlogURL%>",
		cache: false,
		async: false,
...
function DelCommentsNew(v){
	if (confirm('삭제 하시겠습니까?')){
		document.frmactNew.mode.value= "del";
		document.frmactNew.Cidx.value = v;
		...
		<div class="inner5 cmtCont">
			...
			<div class="tab01 noMove" id="writediv" style="display:<%=CHKIIF(vView="w","block","none")%>;">
				<ul class="tabNav tNum2">
					<% If Trim(epdate)<>"" Then %>
						<% If Left(epdate, 10) <= Left(Now(), 10) Then %>
							<li id="wtabw" class="current"><a href="" onClick="alert('당첨자 발표일이 지난 이벤트 입니다.'); return false;">쓰기<span></span></a></li>
						<% Else %>
				...
				<div class="tabContainer box1">
					<div id="cmtWrite" class="tabContent">
						<form method="post" action="/apps/appCom/wish/web2014/event/lib/doEventComment.asp" name="upcomment" target="iframeDB" style="margin:0px;">
						...
						<% If blnBlogURL Then %><p class="tip">입력하신 블로그 주소는 개인정보 유출로 인한 피해를 막고자 비공개로 텐바이텐에 접수됩니다.</p><% end if %>
			...
			<div class="tab01 noMove" id="listdiv" style="display:<%=CHKIIF(vView="l","block","none")%>;">
				<ul class="tabNav tNum2">
					<% If Trim(epdate)<>"" Then %>
						<% If Left(epdate, 10) <= Left(Now(), 10) Then %>
							<li id="ltabw"><a href="" onClick="alert('당첨자 발표일이 지난 이벤트 입니다.'); return false;">쓰기<span></span></a></li>
						<% Else %>
			...
			<!--// 전체보기 -->
			<form name="frmactNew" method="post" action="/apps/appCom/wish/web2014/event/lib/doEventComment.asp" target="iframeDB" style="margin:0px;">
