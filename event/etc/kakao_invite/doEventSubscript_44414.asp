...
'	'response.end
	...
	ElseIf resultGo = "G" Then '즉석당첨 시작
		...
		Else
			...
			if rndPt>0 and rndPt<=1 then
				'#로드맨 클래식 자전거 (1명)
				przNo = 1
				lmtCnt = 1
			elseif rndPt>1 and rndPt<=100 then
				'#아이띵소 캡슐 백팩 (5명)
				przNo = 2
				lmtCnt = 5
			elseif rndPt>100 and rndPt<=300 then
				'#체크 피크닉 매트 + 텀블러 2단 도시락 세트 (50명)
				przNo = 3
				lmtCnt = 50
			elseIf rndPt>300 and rndPt<=2300 Then
					If IsUserLoginOK and (rndPt > 1100 And rndPt<=2300) then
						przNo = 4
						lmtCnt = 100
					Elseif IsUserLoginOK Or rndPt <= 1100 Then
						przNo = 4
						lmtCnt = 100
					else
						przNo = 9
...
'			response.write sqlStr
	...
	Else '응모하기가 아닌 일반 초대에응하기 일때
		If returnIdx <> "" And returnPhone <> "" Then '초대에응하기 일때 // upin이 있을때
			...
			'초대받은 사람의 응모 여부 체크
			Dim tempidx , tempinvitecnt
			sqlStr = "Select top 1 idx , invitecnt  " &_
					" From db_temp.dbo.kakao_invite " &_
					" WHERE evt_code='" & evt_code & "'" &_
			...
			Else 
			tempidx = rsget(0)
			tempinvitecnt = rsget(1)
			End if
			rsget.Close

			'동일 ip접속시 튕김
			If uip = Request.ServerVariables("REMOTE_ADDR") Then 
				Response.Write	"<script language='javascript'>" &_
			...
			If invitecnt = 2 Then '2명 풀 일때
				If tempidx <> "" Then '응모내역이 있고
					Response.Write	"<script>" &_
							"if (confirm('아직 다른 사람의 초대에 응할 수 없습니다.\n2명 초대 완료하고 당첨 여부 확인한 후에 다른 분의 초대에 참여할 수 있습니다.\n이벤트 페이지에서 이름과 핸드폰을 입력하면 현재 초대에 응한 사람의 수를 확인 할 수 있습니다 : ) ')){ top.document.location.href = 'http://m.10x10.co.kr/event/eventmain.asp?eventid="& evt_code &"'}else{ top.document.location.href = 'http://m.10x10.co.kr' 	} " &_
							"</script>"
					response.end		
				Else
					Response.Write	"<script>" &_
									"if (confirm('당신을 초대한 친구가 목표 초대수(2명)를 이미 달성하였습니다.\n 이제 직접 이벤트에 참여해 <클래식 자전거>에 도전하고 싶으면 아래의 확인 버튼을 누르세요.')){ top.document.location.href = 'http://m.10x10.co.kr/event/eventmain.asp?eventid="& evt_code &"'}else{ top.document.location.href = 'http://m.10x10.co.kr' 	} " &_
									"</script>"
					response.end					
				End if
			Else '2명이 아님
				If tempidx <> "" Then '응모내역이 있고
					...
					response.end
				Else
					sqlStr = "update db_temp.dbo.kakao_invite set invitecnt = "& invitecnt + 1 &" where idx = "& returnIdx &" and uphone = '" &  returnPhone & "'  "
					dbget.execute(sqlStr)
					'초대한사람 카운트 update 후 본인 등록
					sqlStr = " insert into db_temp.dbo.kakao_invite "&_
								" (evt_code , uName , uPhone , uip , invitecnt , ridx) "&_
								" values "&_
								" ('"& evt_code &"' , '"& uname  &"' , '"& uphone &"' , '"& Request.ServerVariables("REMOTE_ADDR") &"' , 0 , "& returnIdx &") "
					dbget.execute(sqlStr)
					...
					Response.Write	"<script language='javascript'>" &_
											"alert('친구의 초대에 응하셨네요!! ^^ \n 감사합니다! \n\n 이제 직접 이벤트에 참여하셔서 클래식 자전거에 도전하세요~!');" &_
											"</script>"
					response.end
				End if
			End If 
