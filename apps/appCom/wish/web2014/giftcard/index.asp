...
<%
	...
	'// APP 이미지 업로드 분기를 위한 정보 취합
	dim iAndOrIos, iappVer, vProcess
	iappVer = getAppVerByAgent(iAndOrIos)
	if (iAndOrIos="a") then
	    if (FnIsAndroidKiKatUp) then
	        if (iappVer>="1.48") then
	            vProcess = "A"	''Android App Upload 방식
	        else
	            vProcess = "I"	''어플리케이션 1.48 이상 버전업이 필요하다.
	        end if
	    else
	        vProcess = "I"	''Webview Upload 방식
	    end if
	else
	    vProcess = "I"		''Webview Upload 방식
	end if
			...
			<div class="content" id="contentArea">
				...
				<input type="hidden" name="rdsite" value="mobile">
									...
									<%
										...
										for i = 0 to giftCardImgList.FResultCount - 1
											...
											<div class="swiper-slide swiper-slide-00" bno="photo">
												...
												<div class="file">
												<% if vProcess="A" then %>
													<label onclick="fnAPPuploadImage(document.frmUpload.preimg,document.frmUpload.tuid.value);" class="button btB2 btRed cWh1">사진 등록</label>
												<% else %>
					...
					<div class="usageNotice">
						...
						<div class="btnGroup">
						    <a href="#" onclick="fnAPPpopupBrowserURL('기프트카드','<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/giftcard/giftcardRegist.asp','right');return false;" class="button btBck cWh1"><span>인증번호 등록</span></a>
						    <a href="#" onclick="fnAPPpopupBrowserURL('이용안내 및 유의사항','<%=wwwUrl%>/apps/appCom/wish/web2014/giftcard/usageNotice.asp','right');return false;" class="button btBck cWh1"><span>이용안내 및 유의사항</span></a>
						</div>
					</div>
					...
					<div class="giftcardPayV15a">
						<fieldset>
							...
							<div class="agree">
								...
								<span class="button btS2 btGryBdr cGy1"><a href="#" onclick="fnAPPpopupBrowserURL('이용약관','<%=wwwUrl%>/apps/appCom/wish/web2014/giftcard/giftCardTerms.asp');return false;">약관보기</a></span>
							</div>
