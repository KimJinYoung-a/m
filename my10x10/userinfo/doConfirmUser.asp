...
'''if (Session("InfoConfirmFlag")<>userid) then
    ''패스워드없이 쿠키로만 들어온경우
    if (userpass="") then
    	response.redirect M_SSLUrl & "/my10x10/userinfo/confirmuser.asp" & vParam
        response.end    
    end if
    ...
    if (Not checkedPass) then
        response.write "<script>location.replace('" & M_SSLUrl & "/my10x10/userinfo/confirmuser.asp?errcode=1" & Replace(vParam,"?","&") & "');</script>"
    ...
    if (userdiv="02") or (userdiv="03") or (userdiv="09") then
        response.write "<script>location.replace('" & M_SSLUrl & "/biz/membermodify.asp" & vParam & "');</script>"
    else
        response.write "<script>location.replace('" & M_SSLUrl & "/my10x10/userinfo/membermodify.asp" & vParam & "');</script>"
    end if
