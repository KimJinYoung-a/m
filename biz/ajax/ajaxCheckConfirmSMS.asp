...
'// 회원 정보 변경(인증처리)
	...
	'// 회원정보 변경
	sqlStr = "UPDATE db_user.dbo.tbl_user_c SET soccell = '" & CnfCell & "' WHERE userid='" & txUserId & "'"
	dbget.execute(sqlStr)
	sqlStr = "UPDATE db_user.dbo.tbl_user_c_addinfo SET isMobileChk = 'Y', isMobileChkdate = GETDATE() WHERE userid='" & txUserId & "'"
	dbget.execute(sqlStr)
