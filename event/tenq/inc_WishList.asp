		...
		<li>
			<div class="wishPdtWrapV15a">
				<div class="wishPicV15a">
					<% if isApp="1" then %>
					<a href="" onclick="fnAPPpopupBrowser(OpenType.FROM_RIGHT, [], '100원의 기적', [BtnType.SHARE, BtnType.CART], '<%=wwwUrl%>/apps/appCom/wish/web2014/category/category_itemprd.asp?itemid=<%=cPopular.FItemList(i).FItemID%>&pEtr=85144');return false;" id="Hlink"><img src="<% = cPopular.FItemList(i).FImageBasic %>" alt="<%= cPopular.FItemList(i).FItemName %>"></a>
					<% else %>
					<a href="/category/category_itemprd.asp?itemid=<%=cPopular.FItemList(i).FItemID%>&pEtr=85144"  id="Hlink"><img src="<% = cPopular.FItemList(i).FImageBasic %>" alt="<%= cPopular.FItemList(i).FItemName %>"></a>
					<% end if %>
					<% ''// 세일 마커 redCpn , grnCpn %>
