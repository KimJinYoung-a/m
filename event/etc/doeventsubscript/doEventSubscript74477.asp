...
Dim snsno , rvalue, evtUserCell, device
...
'	prizecnt4 = 0		'꽝 쿠폰
	...
	prizecnt3 = 791		'히트템 레이디 (12개)
	prizecnt4 = 674		'눈꽃 전구
'	prizecnt5 = 700		'꽝 쿠폰

	IF application("Svr_Info") = "Dev" THEN
		eCode = "66240"
		icpn1 = "11141" '리플렉트 에코 히터 쿠폰
		icpn2 = "11144" '인스탁스 미니 8 카메라 (그레이프)
		icpn3 = "11142" '히트템 레이디 (12개)
		icpn4 = "11143" '눈꽃 전구
		icpn5 = "2827"	'30000원 구매시 4000원 할인(보너스 쿠폰)
	Else
		eCode = "74477"
		icpn1 = "12099" '리플렉트 에코 히터 쿠폰
		icpn2 = "12100" '인스탁스 미니 8 카메라 (그레이프)
		icpn3 = "12101" '히트템 레이디 (12개)
		icpn4 = "12102" '눈꽃 전구
		
		icpn5 = "930"	'30000원 구매시 4000원 할인(보너스 쿠폰)
	End If
	...
	'// 이벤트 기간 확인
	if mode<>"S" then
		If Not(Now() > #11/23/2016 00:00:00# and Now() < #11/30/2016 23:59:59#) Then
			Response.Write "{ "
...
'###########################################################################################################################################
	...
	Sub fnGetCoupon(v)
		...
		If v = 5 Then '꽝
			...
			sqlstr = sqlstr & " 	SELECT idx, '"& userid &"',coupontype,couponvalue,couponname,minbuyprice,targetitemlist,'2016-11-23 00:00:00','2016-11-30 23:59:59',couponmeaipprice,validsitename" + vbcrlf
...
'###########################################################################################################################################
	...
	Sub fnGetWinner()
		...
		If renloop >= 1 And renloop <= 50 Then 							'1등 0.1%
			If wincnt1 < prizecnt1 Then					'물량있음
				Call fnGetCoupon(1)
			Else
				Call fnGetCoupon(5)
			End If 
		ElseIf renloop >= 51 And renloop <= 100 Then							'2등 0.1%
			If wincnt2 < prizecnt2 Then					'물량있음
				Call fnGetCoupon(2)
			Else
				Call fnGetCoupon(5)
			End If 
		ElseIf renloop >= 101 And renloop <= 500 Then		'3등 39.8%
			If wincnt3 < prizecnt3 Then					'물량있음
				Call fnGetCoupon(3)
			Else
				Call fnGetCoupon(5)
			End If 
		ElseIf renloop >= 501 And renloop <= 1000 Then	'4등 40%
			If wincnt4 < prizecnt4 Then					'물량있음
				Call fnGetCoupon(4)
			Else
				Call fnGetCoupon(5)
			End If
'		ElseIf renloop >= 801 And renloop <= 1000 Then	'5등 19.8%
'			Call fnGetCoupon(5)
