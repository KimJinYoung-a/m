...
Class ClsEvtBBS
	...
	public Function fnGetBBSList
		...
		IF FTotCnt = -1 THEN
			strSqlcnt ="[db_event].[dbo].sp_Ten_event_bbs_listcnt ("&FECode&")"
		...
		END IF
		IF FTotCnt > 0 THEN
			strSql ="[db_event].[dbo].sp_Ten_event_bbs_list ("&FECode&","&FCPage&","&FPSize&")"
...
Class ClsEvtComment
	...
	public Function fnGetComment
		...
		strSql ="[db_event].[dbo].sp_Ten_event_comment ("&FECode&",'"&FComGroupCode&"',"&FEBidx&","&FCPage&","&FPSize&","&FTotCnt&")"
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdStoredProc
		IF Not (rsget.EOF OR rsget.BOF) THEN
...
Class ClsEvtUtilComment
	...
	public FLikeId
	public FIsMyComment
 	public FcommtxtOption

	public Function fnGetComment
		Dim strSql, arrList		
		strSql =" exec [db_event].[dbo].sp_Ten_event_util_comment "&FECode&", "&FCPage&", "&FPSize&", "&FTotCnt&", '"& FUserID &"', '"& FLikeId &"', '"& FIsMyComment &"', '" & FcommtxtOption & "'"
	...
	public Function getTop3Comments
		Dim SqlStr	

		SqlStr = " select top 3 option1, count(*) AS CNT "  
		SqlStr = SqlStr & " from db_event.dbo.tbl_event_util_comment with(nolock) "
		SqlStr = SqlStr & " where evt_code='" + CStr(FECode) + "' AND option1 <> '10' and evtcom_using = 'Y' "
        SqlStr = SqlStr & " GROUP BY option1 order by CNT desc "        
