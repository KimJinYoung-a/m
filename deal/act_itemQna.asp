...
<!--
	...
	function delItemQna(idx){
		if(confirm("상품문의를 삭제 하시겠습니까?")){
			location.href="/deal/doitemqna.asp?id="+idx+"&itemid=<%=itemid%>&mode=del&dealitemid=<%=dealitemid%>";
		}
	}
//-->
</script>
						<div class="btnAreaV16a">
							<p><button type="button" class="btnV16a btnRed2V16a" onclick="location.href='pop_ItemQnaList.asp?itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&mode=wr';">상품문의 하기</button></p>
						</div>
						...
						<div class="btnAreaV16a">
							<p><button type="button" class="btnV16a btnRed1V16a" onclick="location.href='pop_ItemQnaList.asp?itemid=<%=itemid%>&dealitemid=<%=dealitemid%>';">전체보기</button></p>
						</div>
						<% end if %>
<%
	Set oItem = Nothing
