...
<%
'#######################################################
' PageName : make_todaywish_xml.asp
...
for j=1 to cInt(vTerm2)
	...
	sqlStr = "select * "
	sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_mobile_main_todaywish_list as l "
	sqlStr = sqlStr + " inner join [db_sitemaster].[dbo].tbl_mobile_main_todaywish_item as s "
...
For j=1 to cInt(vTerm2)
	'// 메인 데이터 접수
	sqlStr = "Select  s.subidx , s.listidx , s.itemid , s.isusing as itemusing , s.sortnum , isnull(s.itemname,i.itemname) as itemname, i.basicimage , l.startdate , l.enddate , l.wishtitle , i.sellCash , i.orgPrice , i.sailyn , i.itemcouponYn, i.itemcouponvalue ,i.limitYN ,i.itemcoupontype"
	...
	sqlStr = sqlStr & "	left join [db_sitemaster].dbo.tbl_mobile_main_todaywish_list as l "
	sqlStr = sqlStr & "	on s.listidx = l.idx  "
	sqlStr = sqlStr & " Where s.isusing = 'Y' and l.enddate >= getdate() "
...
'	response.end
	...
	savePath = server.mappath("/chtml/main/xml/todaywish") + "\"
	FileName = "main_todaywish.xml"
	...
	if vTotalCount>0 then
		...
		Do Until rsget.EOF
			...
			objXMLv.childNodes(3).text = rsget("startdate")
			objXMLv.childNodes(4).text = rsget("enddate")
			objXMLv.childNodes(5).text = db2Html(rsget("wishtitle"))
