...
function getList() {
	var str = $.ajax({
			type: "GET",
	        url: "/apps/appcom/wish/web2014/diarystory2018/act_list.asp",
	        data: $("#sFrm").serialize(),
	        dataType: "text",
	        async: false
	}).responseText;
...
function searchlink(v){
	if (v == "")
	{
		document.location = "<%=wwwUrl%>/apps/appCom/wish/web2014/diarystory2018/list.asp";
	}else{
...
function goWishPop(i){
	<% If IsUserLoginOK() Then ''ErBValue.value -> 공통파일의 구분값 (cate는 1)%>
	   fnAPPpopupBrowserURL("위시폴더","<%=wwwUrl%>/apps/appcom/wish/web2014/common/popWishFolder.asp?itemid="+i+"&ErBValue=9&gb=search2017",'','wishfolder');
	<% Else %>
		calllogin();
		return false;
	<% End If %>
...
*/
	document.sFrm.action = "<%=wwwUrl%>/apps/appCom/wish/web2014/diarystory2018/list.asp";
	document.sFrm.submit();
}
...
function fncatesearch(ccd){
	location.href="/apps/appcom/wish/web2014/diarystory2018/list.asp?cpg=1&gnbflag=1&srm=<%=SortMet%>&arrds="+ccd+"&<%=vParaMeterCG%>";
}
	...
	<input type="hidden" name="itemid" value=""/>
		...
		<% If PrdBrandList.FResultCount > 0 Then %>
			<div class="items type-grid">
				...
				%>
						<li>
							<a href="" onclick="fnAPPpopupProduct('<%=PrdBrandList.FItemList(i).FItemID%>'); return false;">
							...
							<div class="etc">
								...
								<button class="tag wish btn-wish" onclick="goWishPop('<%= PrdBrandList.FItemList(i).FItemID %>'); return false;">
									<%
