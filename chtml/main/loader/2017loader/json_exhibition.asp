...
Dim gaParam : gaParam = "&gaparam=today_enjoy_" '//GA 체크 변수
...
If timer > 10 And Cint(timer/60) < 6 Then
	cTime = 60*1
	dummyName = "exhbt_"&Cint(timer/60)
Else
	cTime = 60*5
	dummyName = "exhbt"
End If
	...
	sqlStr = sqlStr & ", (CASE WHEN aw.currrank < 1000 THEN 'Y' ELSE 'N' END) AS bestyn "
	...
	sqlStr = sqlStr & " LEFT JOIN db_const.dbo.tbl_const_award aw on aw.awardgubun='b' and aw.itemid=i.itemid "
	sqlStr = sqlStr & "Where s.isusing = 'Y' and ('" & CtrlDate & "' between convert(varchar(10),l.startdate,120) and "
	sqlStr = sqlStr & "convert(varchar(10),l.enddate,120)) and ('"& CtrlTime &"' between datepart(hh, l.startdate) and "
	sqlStr = sqlStr & "datepart(hh, l.enddate)) and l.isusing = 'Y' "
	sqlStr = sqlStr & "and s.listidx in ("
	sqlStr = sqlStr & "	Select top 1 idx from [db_sitemaster].[dbo].[tbl_mobile_main_exhibition_list] "
	sqlStr = sqlStr & "	where ('" & CtrlDate & "' between convert(varchar(10),startdate,120) and "
	sqlStr = sqlStr & "	convert(varchar(10),enddate,120)) and ('"& CtrlTime &"' between datepart(hh, startdate) and "
	sqlStr = sqlStr & "	datepart(hh, enddate)) and isusing = 'Y' order by topview desc ) "
	sqlStr = sqlStr & " and i.sellyn='Y' and i.isusing='Y' "
	...
	If isArray(arrExhibition) and not(isnull(arrExhibition)) Then
		...
		FOR i=0 to ubound(arrExhibition,2)
			...
			itemname	= db2Html(arrExhibition(5,i))
			...
			bestyn		= db2Html(arrExhibition(24,i))

			tagname = ""
			If newyn = "Y" Then tagname = "NEW"
			If bestyn = "Y" Then tagname = "BEST"
			...
			Set obj("exhibition"&ii&"")(null) = jsObject()
				'//이미지루프
				If isapp ="1" then
					obj("exhibition"&ii&"")(null)("link") = ""& itemid & gaParam & i+1 &""
				Else
					obj("exhibition"&ii&"")(null)("link") = "/category/category_itemPrd.asp?itemid="& itemid & gaParam & i+1 &""
				End If 
				If application("Svr_Info") = "Dev" Then
					obj("exhibition"&ii&"")(null)("imgsrc") = ""& chkiif(Not(isnull(tentenimg400) Or tentenimg400 = "" ),tentenimg400,image) &""
				else
					obj("exhibition"&ii&"")(null)("imgsrc") = ""& chkiif(Not(isnull(tentenimg400) Or tentenimg400 = "" ),tentenimg400,getThumbImgFromURL(image,400,400,"true","false")) &""
				End if
				...
				If sailYN = "Y" and couponYn = "N" Then
					...
					End If
						obj("exhibition"&ii&"")(null)("sale_coupontag") = "1"
				elseif couponYn = "Y" And couponvalue>0 Then
					If coupontype = "1" Then
						obj("exhibition"&ii&"")(null)("sale") = ""& CStr(couponvalue) &"%"
					ElseIf coupontype = "2" Then
						obj("exhibition"&ii&"")(null)("sale") = "쿠폰"
					ElseIf coupontype = "3" Then
						obj("exhibition"&ii&"")(null)("sale") = ""
					Else
						obj("exhibition"&ii&"")(null)("sale") = ""& couponvalue &"%"
					End If
						obj("exhibition"&ii&"")(null)("sale_coupontag") = "2"
				Else
						obj("exhibition"&ii&"")(null)("sale") = ""
						obj("exhibition"&ii&"")(null)("sale_coupontag") = ""
				End If
				If LimitYn="Y" And limitdispyn = "Y" then
						obj("exhibition"&ii&"")(null)("itemno") = ""& limitno &""
				End If
						'//tag 유무
						obj("exhibition"&ii&"")(null)("tagname") = ""& tagname &""

						'// amplitude 셋팅
						obj("exhibition"&ii&"")(null)("ampevt") 	= "click_mainenjoy"
						obj("exhibition"&ii&"")(null)("ampevtp") 	= "enjoynumber|itemid"
						obj("exhibition"&ii&"")(null)("ampevtpv") 	= i+1 &"|"& itemid

			'//타이틀영역
			If tmpgubun <> listidx Then
					obj("extitle"&ii&"") = ""& extitle &""
					If linkurl <> "" then
					obj("link"&ii&"") = ""& linkurl & gaParam &"0"
					Else
					obj("link"&ii&"") = ""
					End If 
			End if
