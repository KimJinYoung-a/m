...
<body class="default-font body-main">
	<div id="content" class="content gnb-category gnb-living">
		...
		<div class="exhibition-list">
			...
			<div class="list-card type-align-left">
				<ul id="exhibition1">
					<item-list v-for="(item , index) in items" :item="item" :index="index" :isapp="1" :key="item.id" v-if="item.gubun == 1 && index < 4"></item-list>
				</ul>
			</div>
		</div>
		...
		<div class="exhibition-list">
			...
			<div class="list-card type-align-left">
				<ul id="exhibition2">
					<item-list v-for="(item , index) in items" :item="item" :index="index" :isapp="1" :key="item.id" v-if="item.gubun == 2 && index < 9"></item-list>
				</ul>
			</div>
		</div>
		...
		<div class="exhibition-list">
			...
			<div class="list-card type-align-left">
				<ul id="exhibition3">
					<item-list v-for="(item , index) in items" :item="item" :index="index" :isapp="1" :key="item.id" v-if="item.gubun == 3 && index < 15"></item-list>
				</ul>
			</div>
		</div>
			...
			<div id="livingCtgy" class="ctgy-list search-filter">
				<div id="fashionType1" class="panel current">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							...
							<% Call fnPrntDispCateNavi_2DepthLiving("121","E","fnAPPpopupCategory") %>
						</ul>
					</div>
				</div>
				...
				<div id="fashionType2" class="panel">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							<%'정렬상자 호출; sDisp:전시카테고리, sType:확장여부, sCallback:콜백함수명 (via functions.asp)%>
							<% Call fnPrntDispCateNavi_2DepthLiving("120","E","fnAPPpopupCategory") %>
						</ul>
					</div>
				</div>
				...
				<div id="fashionType3" class="panel">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							<%'정렬상자 호출; sDisp:전시카테고리, sType:확장여부, sCallback:콜백함수명 (via functions.asp)%>
							<% Call fnPrntDispCateNavi_2DepthLiving("122","E","fnAPPpopupCategory") %>
						</ul>
					</div>
				</div>
				<div id="fashionType4" class="panel">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							<%'정렬상자 호출; sDisp:전시카테고리, sType:확장여부, sCallback:콜백함수명 (via functions.asp)%>
							<% Call fnPrntDispCateNavi_2DepthLiving("112","E","fnAPPpopupCategory") %>
						</ul>
					</div>
				</div>
			</div>
	...
	<script src="/vue/livingapp.js?v=0.1"></script>
	<script>
	$(function(){
		livingitems(1);
