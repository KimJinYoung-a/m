			...
			If arrList(11,intLoop) Or arrList(13,intLoop) Then
				if ubound(Split(arrList(10,intLoop),"|"))> 0 Then
					If arrList(11,intLoop) Or (arrList(11,intLoop) And arrList(13,intLoop)) then
						...
						eNameredsale	= " <b class='discount color-red'>"&cStr(Split(arrList(10,intLoop),"|")(1))&"</b>"
				...
				<li<% If arrList(28,intLoop)<>"" Then %> class="has-vod"<% End If %>>
					<a href="" onclick="<%=vLink%> return false;">
						...
						<div class="desc">
							<p>
								<b class="headline">
									<span class="ellipsis" <% IF arrList(11,intLoop) THEN %>style="width:80%;"<% else %>style="width:100%;"<% end if %>><%=Replace(stripHTML(db2html(eName)),"[☆2015 다이어리]","")%></span>
									<% IF arrList(11,intLoop) THEN %><% if eNameredsale <> "" then %><%=eNameredsale%><% end if %><% end if %></b>
