...
<%
Dim vDisp : vDisp = requestCheckVar(request("sdisp"),15)
'Dim vDepth, i,TotalCnt
'	vDepth = "1"
dim PageSize	: PageSize = getNumeric(requestCheckVar(request("spsz"),9))
'dim searchFlag 	: searchFlag = "sale"
dim CurrPage 	: CurrPage = getNumeric(requestCheckVar(request("scpg"),9))
'dim SortMet		: SortMet = requestCheckVar(request("srm"),2)
'dim CurrPage2 	: CurrPage2 = getNumeric(requestCheckVar(request("scpg"),9))
Dim SortMet	: SortMet = requestCheckVar(request("sord"),1)
...
%>
	...
	<li id="<%=vDisp%>ListA<%=i%>">
		<div <% If oDoc.FCategoryPrdList(i).IsSoldOut Then %> class="soldout" <% Else %><% IF oDoc.FCategoryPrdList(i).isSaleItem then %> class="sale"<% End If %><% End If %> >
			<a href="category_itemPrd.asp?itemid=<%=oDoc.FCategoryPrdList(i).FItemID%>&dispCate=<%=vDisp%>" id="Hlink" class="<%=vDisp%>ListA<%=i%>">
