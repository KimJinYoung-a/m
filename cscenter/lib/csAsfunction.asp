...
function regDeliveryChange(iAsid, reqname, reqphone, reqhp, reqzip, reqaddr1, reqaddr2, reqetc)
    ...
    sqlStr = sqlStr & " where asid=" & iAsid
    ...
    if (resultRows=0) then
        ...
        sqlStr = sqlStr & " values(" + CStr(iAsid) + VbCrlf
...
function checkDuplicateRecallChangeExist(orderserial)
    dim sqlStr

    sqlStr = " select top 1 id " & vbCrLf
    sqlStr = sqlStr & " from [db_cs].[dbo].[tbl_new_as_list] " & vbCrLf
    sqlStr = sqlStr & " where orderserial = '" & orderserial & "' and divcd in ('A002', 'A200') and deleteyn = 'N' " & vbCrLf

    checkDuplicateRecallChangeExist = False
    rsget.Open sqlStr,dbget,1
    if Not rsget.Eof then
        checkDuplicateRecallChangeExist = True
    end if
    rsget.close
end function
...
function RegWebCSDetailStockoutCancel(byval CsId, orderserial)
    ...
    sqlStr = sqlStr + " 		((d.itemid <> 0) and (IsNull(m.code, '') in ('05') or IsNull(m.code, '') in ('06'))) "
    sqlStr = sqlStr + " 		or "
    ...
    sqlStr = sqlStr + " 				sum(case when d.itemid <> 0 then d.itemno else 0 end) = sum(case when d.itemid <> 0 and (IsNull(m.code, '') in ('05') or IsNull(m.code, '') in ('06')) then IsNull(m.itemlackno,0) else 0 end) "
    sqlStr = sqlStr + " 		))) "
    sqlStr = sqlStr + " 	) "
	...
	'// 품절 잔여상품 출고지연 전환
	sqlStr = " update T "
    sqlStr = sqlStr + " set T.code = '03', T.itemno = (ad.orderitemno - ad.regitemno), T.itemlackno = (T.itemlackno - ad.regitemno), T.state = 0, T.reqaddstr = '품절상품 고객취소' "
    sqlStr = sqlStr + " from "
    ...
    sqlStr = sqlStr + " 	and T.code in ('05') "

	dbget.Execute sqlStr

	'// 택배파업 잔여상품 출고지연 전환
	sqlStr = " update T "
    sqlStr = sqlStr + " set T.code = '03', T.itemno = (ad.orderitemno - ad.regitemno), T.itemlackno = (T.itemlackno - ad.regitemno), T.state = 0, T.reqaddstr = '택배파업 고객취소' "
    sqlStr = sqlStr + " from "
    sqlStr = sqlStr + " 	[db_cs].[dbo].tbl_new_as_list a "
    sqlStr = sqlStr + " 	join [db_order].[dbo].tbl_order_detail d on a.orderserial = d.orderserial "
    sqlStr = sqlStr + " 	join [db_cs].[dbo].tbl_new_as_detail ad on a.id = ad.masterid and ad.orderdetailidx = d.idx "
    ...
    sqlStr = sqlStr + " 	and a.id = " & CsId
    'sqlStr = sqlStr + " 	and (T.code in ('05') or (T.code in ('03') and d.isupchebeasong='N'))"
    sqlStr = sqlStr + " 	and T.code in ('06') "
