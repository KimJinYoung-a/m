...
function fnWeddingInfoSet(){
	fnAPPpopupBrowser(OpenType.FROM_RIGHT, [], '디데이 등록하기', [BtnType.CART], '<%=wwwUrl%>/apps/appCom/wish/web2014/wedding/wedding_info_reg.asp');
}
...
function fnWeddingEventView2(eventid){
...
function fnEvtItemList(ecode, gcode, menuid){
	$.ajax({
		url: "/apps/appCom/wish/web2014/wedding/act_evt_itemlist.asp?eventid="+ecode+"&eGC="+gcode,
		cache: false,
...
<body class="default-font body-<%=chkiif(gnbflag,"sub","main")%>">
	...
	<div id="content" class="content wedding2018" >
		<div class="wed-head">
			<h2><img src="http://fiximage.10x10.co.kr/m/2018/wedding2018/tit_wedding.jpg" alt="상견례 예비 신랑, 신부와 양가 가족이 공식적으로 만나 인사를 나누고 혼인 절차를 의논하는 자리에요.긴장되는 자리인 만큼 미리미리 준비하는 게 좋겠죠?" usemap="#map-d100-2" /></h2>
			<% If UserName="" Then %>
				<% If userid<>"" Then %>
				<button class="wd-day" onclick="fnWeddingInfoSet();return false;">나의 웨딩일<em>등록하기</em><s>+Coupon</s></button>
				<% Else %>
				<button class="wd-day" onclick="parent.calllogin();">나의 웨딩일<em>등록하기</em><s>+Coupon</s></button>
				<% End If %>
			<% Else %>
