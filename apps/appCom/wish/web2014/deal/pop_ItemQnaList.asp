	...
	if mode="wr" and (Not IsUserLoginOK) then
		dim checklogin_backpath
	  	dim strBackPath, strGetData, strPostData	
	        ...
	        checklogin_backpath = "backpath="+ server.URLEncode(strBackPath) + "&strGD=" +  server.URLEncode(strGetData)
...
function fnGoQnaPage(page) {
	$.ajax({
		url: "/apps/appcom/wish/web2014/deal/pop_ItemQnaList.asp?itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&cpg="+page,
		cache: false,
		async: false,
		success: function(message) {
			$(".prdDetailCont").empty().html($(message).find(".prdDetailCont").html());
			fnQnaActionSet();
		}
	});
}
...
function fnGoQnaTab(mode) {
	...
	<%
		If Not(IsUserLoginOK) Then
			'' for App // Response.Write "calllogin();" & vbCrLf
			Response.Write "jsChklogin_mobile('','" & Server.URLencode("/apps/appcom/wish/web2014/deal/pop_itemqnalist.asp?itemid="&itemid&"&dealitemid="&dealitemid&"&mode=wr") & "');" & vbCrLf
...
//		}
		...
		if(confirm("상품에 대해 문의 하시겠습니까?")){
			frm.action = "/apps/appcom/wish/web2014/deal/doitemqna.asp";
			frm.submit();
			return true;
		} else {
			return false;
...
function modiItemQna(idx) {
	location.href="/apps/appcom/wish/web2014/deal/pop_itemQnAList.asp?itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&id="+idx;
}
// 상품 문의 삭제
function delItemQna(idx){
	if(confirm("상품문의를 삭제 하시겠습니까?")){
		location.href="/apps/appcom/wish/web2014/deal/doitemqna.asp?id="+idx+"&itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&mode=del";
	}
}
	...
	<div class="inner5 tMar15 cmtCont">
		<div class="tab01 noMove">
			...
			<!-- 쓰기 -->
				...
				<input type="hidden" name="dealitemid" value="<%= dealitemid %>" >
					<div class="qnaWrite">
...
<!-- 								</div> -->
						...
						<ul class="notiList tMar15">
							<li>주문 후 반품, 교환, 취소 등에 관한 문의는 1:1 게시판을 이용해 주시기 바랍니다.</li>
							<li>고객님이 작성하신 문의 및 답변은 마이텐바이텐 &gt; 상품 Q&amp;A 에서도 확인이 가능합니다.</li>
							<li>상품과 관련없는 문의는 강제 삭제 될 수 있습니다.</li>
						</ul>
					</div>
					<div class="btnWrap tMar15">
