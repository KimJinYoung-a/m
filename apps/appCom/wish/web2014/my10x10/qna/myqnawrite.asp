...
dim orderserial, itemid, qadiv, userid, i, tmpqadivname, tmpqadivcnt
...
function jsReActWithOrderserial(orderserial, odt, oprc, oacc) {
	var vCont = '';

	if (odt != undefined) {
		vCont = '<p class="tMar05 fs10 lh12 cRd1">';
		vCont += '주문일 ' + odt + '<br />';
		vCont += '총 결제금액 ' + oprc + ' 원';
		vCont += '(' + oacc + ')';
		vCont += '</p>';
...
function jsReActWithItemID(orderserial, itemid, inm, iprc) {
	var vCont = '';

	if (inm != undefined) {
		vCont = '<p class="tMar05 fs10 cRd1">';
		vCont += inm + '&nbsp;';
		vCont += iprc + ' 원';
		vCont += '</p>';
	}
	...
	frm.s_orderserial.value = orderserial;
	frm.s_itemid.value = itemid;
	return false;
}
