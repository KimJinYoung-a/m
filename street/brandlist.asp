...
function goorder(order){
	location.replace("/street/brandlist.asp?page=1&chrCd=<%=vChar%>&Lang=<%=vLang%>&flag="+order+"&imgsize=<%=vImgSize%>");
}

function goChar(ch){
	location.replace("/street/brandlist.asp?page=1&chrCd="+ch+"&Lang=<%=vLang%>&flag=<%=vFlag%>&imgsize=<%=vImgSize%>");
}

function goPage(page){
	location.replace("/street/brandlist.asp?page="+page+"&chrCd=<%=vChar%>&Lang=<%=vLang%>&flag=<%=vFlag%>&imgsize=<%=vImgSize%>");
}
				...
				<div class="brandList">
					...
					%>
						...
						<div class="brand box1">
							<div class="info">
							    <a href="" onclick="location.href='/brand/brand_detail2020.asp?brandid=<%=cBrandList.FItemList(lp).Fmakerid%>'; return false;">
