...
Class CEvaluateSearcher
	...
	Public Sub NotEvalutedItemList()
		...
		sqlStr = "" &_
				...
				" 	and d.itemid<>0  "&_
				"	and d.currstate>=Case When d.oitemdiv='75' then 3 else 7 end " &_
				" JOIN [db_item].[dbo].tbl_Item i WITH(READUNCOMMITTED) "&_
...
'e.idx is null
		sqlStr = " " &_
				...
				" JOIN [db_item].[dbo].tbl_Item i WITH(READUNCOMMITTED) "&_
				" 	on d.itemid=i.itemid and i.isusing = 'Y'  "&_
				" LEFT JOIN [db_item].[dbo].tbl_item_Contents c WITH(READUNCOMMITTED) "&_
	...
	Public Sub getNotEvaluatedItem()
		...
		sqlStr = " " &_
			...
			" ,(Case When d.itemoptionname<>'' then d.itemoptionname else o.optionname end) as optionname " &_
			...
			"		and d.currstate>=Case When d.oitemdiv='75' then 2 else 7 end " &_
			" JOIN db_item.[dbo].tbl_item i WITH(READUNCOMMITTED) " &_
	...
	Public Sub getNotEvaluatedOffShopItem()
		...
		sqlStr = sqlStr + " ,i.itemid,i.sellcash,i.itemname,i.brandname , i.listimage , i.listImage120, i.basicimage  " & vbcrlf
		...
		sqlStr = sqlStr + " , e.Contents as UsedContents , e.imgcontents1 , e.imgcontents2 , e.imgcontents3 , e.imgcontents4 , e.imgcontents5, i.icon2image " & vbcrlf
		...
		sqlStr = sqlStr + " JOIN db_item.[dbo].tbl_item i WITH(READUNCOMMITTED) " & vbcrlf
		sqlStr = sqlStr + " 	on d.itemid=i.itemid  " & vbcrlf
		sqlStr = sqlStr + " LEFT JOIN db_item.[dbo].tbl_item_option o WITH(READUNCOMMITTED) " & vbcrlf
		sqlStr = sqlStr + " 	on d.itemid=o.itemid and d.itemoption = o.itemoption  " & vbcrlf
		sqlStr = sqlStr + " left join db_board.[dbo].tbl_item_evaluate e WITH(READUNCOMMITTED)" & vbcrlf
		...
		sqlStr = sqlStr + " and datediff(m,m.regdate ,getdate()) between 0 and 6" & vbcrlf
		If FRectOption <> "" Then
	...
	Public Sub getEvaluatedOffShopItem()
		...
		sqlStr = " " &_
			" SELECT top 1 " &_
			" e.idx , e.gubun , e.orderserial, e.itemoption , o.optionname, i.itemdiv, i.icon2image, i.basicimage  " &_
			" , e.TotalPoint , e.Point_Function , e.Point_Design , e.Point_Price , e.Point_Satisfy " &_
			" , e.icon1, e.icon2 , e.file1 , e.file2 , e.file3 , e.file4 , e.file5" &_
			" ,e.title , e.Contents as UsedContents , e.imgcontents1 , e.imgcontents2 , e.imgcontents3 , e.imgcontents4 , e.imgcontents5 " &_
			" ,i.itemid,i.sellcash,i.itemname,i.brandname , i.listimage , i.listImage120 " &_
			" FROM db_board.[dbo].tbl_item_evaluate_Offshop e WITH(READUNCOMMITTED)" &_
			" JOIN db_item.[dbo].tbl_item i WITH(READUNCOMMITTED)" &_
			" 	on i.itemid= e.itemid " &_
			" LEFT JOIN db_item.[dbo].tbl_item_option o WITH(READUNCOMMITTED)" &_
			" 	on e.itemid=o.itemid and e.itemoption = o.itemoption " &_
...
'response.write sqlStr
	...
	Public Function getDoubleEvtEvaluatedTotalMileCnt()
		dim sqlStr
		if (FRectUserID="") then Exit function ''2015/04/13 추가

			sqlStr = "EXEC [db_board].[dbo].[usp_WWW_My10x10_ItemNotEvaluted_EventDoubleMileCheck_Get] " & Cstr(FRectMileage) & ", '" & Cstr(FRectUserID) & "'"
			rsget.CursorLocation = adUseClient
        	rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
'response.write sqlStr
			IF not rsget.eof THEN
				getDoubleEvtEvaluatedTotalMileCnt = rsget.GetRows()
			END IF
			rsget.close
	End Function
...
'				FItemList(i).Fitemid    = rsget("itemid")
	...
	Public Sub MyNotEvalutedItem_Top1()
        ...
        sqlStr = "exec db_board.[dbo].[sp_Ten_MyNotEvalutedItem_Top1_New] '"&FRectUserID&"' , '"&FRectItemID&"'"
		rsget.CursorLocation = adUseClient
        rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		...
		IF not rsget.eof THEN
			...
			FEvalItem.FOrderIDX		   = rsget("orderidx")			'//오프샵 주문디테일 고유번호
		END IF
	...
	Public Sub NotEvalutedItemListNew()
		...
		sqlStr = "EXEC [db_board].[dbo].[usp_WWW_My10x10_ItemNotEvaluted_MobileTotalCount_Get] " & Cstr(FPageSize) & ", '" & Cstr(FRectUserID) & "'"
		rsget.CursorLocation = adUseClient
        rsget.Open sqlStr, dbget, adOpenForwardOnly, adLockReadOnly
		IF not rsget.eof THEN
			FTotalCount = rsget("TotalCnt")
		...
		'// ====================================================================
		sqlStr = "EXEC [db_board].[dbo].[usp_WWW_My10x10_ItemNotEvaluted_MobileList_Get] " & Cstr(FPageSize) & ", " &  Cstr(FCurrPage) & ", '"  & Cstr(FRectUserID) & "'"
'Response.write sqlStr
		rsget.pagesize = FPageSize
	...
	'// 최근 3개월 이내 구매 & 후기 안쓰인 상품 리스트(2013-09-26 신규 카테고리 대응)
	Public Sub NotEvalutedItemOrderList()

		dim sqlStr, addSql, i
        if (FRectUserID="") then Exit Sub ''2015/04/17 추가
            
		'// ====================================================================
		sqlStr = " EXEC [db_board].[dbo].[usp_WWW_My10x10_ItemNotEvaluted_MobileOrderNo_Get] " & Cstr(FPageSize) & ", " &  Cstr(FCurrPage) & ", '"  & Cstr(FRectUserID) & "'"
'Response.write sqlStr
		rsget.pagesize = FPageSize
		rsget.cursorlocation=3
		rsget.open sqlStr ,dbget,1
