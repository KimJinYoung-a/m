...
<script type="text/javascript" src="/apps/appCom/wish/web2014/lib/js/customapp.js?bgnv=5"></script>
<%
	...
	for i=LBound(bagarr) to UBound(bagarr)
	    if Trim(bagarr(i))<>"" then
			...
			if UBound(tmparr)>1 then
				if (tmparr(0)<>"") and (tmparr(1)<>"") and (tmparr(2)<>"") and (tmparr(0)<>"undefined") then
...
elseif (mode="DO1") then  ''바로주문  (장바구니) ''2014 나눔
   ''바로 주문.(선택 초기화 > 해당상품 체크 > 주문페이지로 이동)
    ...
    if (oshoppingbag.CheckOutOneItem(itemid,itemoption,itemea)) then
        Call AppendLog_shoppingBagProc() ''2016/05/18 추가
        response.redirect M_SSLUrl & "/apps/appCom/wish/web2014/inipay/UserInfo.asp"
        dbget.close() : response.end
    else
        response.write "<script>alert('죄송합니다. 유효하지 않은 상품이거나 품절된 상품입니다.');</script>"
        response.write "<script>history.back();</script>"
...
elseif (mode="DO2") then ''바로주문 (상품 페이지내 - 배열)
    ...
    if vChkOutItem then
        Call AppendLog_shoppingBagProc() ''2016/05/18 추가
        ''response.redirect M_SSLUrl & "/apps/appCom/wish/web2014/inipay/UserInfo.asp"
        response.write "<script>fnAPPpopupBaguniUserinfo();</script>"		''주문정보입력 팝업(APP)
        dbget.close() : response.end
    else
        response.write "<script>alert('죄송합니다. 유효하지 않은 상품이거나 품절된 상품입니다.');</script>"
        response.write "<script>history.back();</script>"
...
elseif (mode="DO3") then ''바로주문 (상품페이지 APP전용)
   ''바로 주문.(선택 초기화 > 해당상품 체크 > 주문페이지로 이동)
    if (itemid<>"") and (itemoption<>"") and (itemea<>"") then
		'선택 초기화
		call oshoppingbag.OrderCheckOutDefault
    ...
    if (oshoppingbag.CheckOutOneItem(itemid,itemoption,itemea)) then
        Call AppendLog_shoppingBagProc() ''2016/05/18 추가
        response.write "<script>fnAPPpopupBaguniUserinfo();</script>"
        dbget.close() : response.end
    else
        response.write "<script>alert('죄송합니다. 유효하지 않은 상품이거나 품절된 상품입니다.');</script>"
        response.write "<script>history.back();</script>"
...
elseif (mode="DO4") Then	''장바구니내 바로 주문
    ...
    if (oshoppingbag.CheckOutOneItem(itemid,itemoption,itemea)) then
        Call AppendLog_shoppingBagProc() ''2016/05/18 추가
        response.redirect M_SSLUrl & "/apps/appCom/wish/web2014/inipay/UserInfo.asp" & vParam
    else
        response.write "<script>alert('죄송합니다. 유효하지 않은 상품이거나 품절된 상품입니다.');</script>"
        response.write "<script>history.back();</script>"
...
elseif (mode="DLARR") then
    ''장바구니에서 삭제
    chk_item   = split(request.Form("chk_item"),",")
    itemid     = split(request.Form("itemid"),",")
    itemoption = split(request.Form("itemoption"),",")
    ...
    if IsArray(chk_item) then
        retBool = True
        for i=LBound(chk_item) to UBound(chk_item)
            ckdidx = Cint(Trim(chk_item(i)))
    	    if (ckdidx<>"") and (UBound(itemid)>=ckdidx) and (UBound(itemoption)>=ckdidx) then
    			if (Trim(itemid(ckdidx))<>"") and (Trim(itemoption(ckdidx))<>"") then
...
%>
        <form name="frmCk" method="post" action="/apps/appCom/wish/web2014/inipay/ShoppingBag.asp">
...
// 장바구니 상품수 Updqte
fnAPPsetCartNum(<%=getDBCartCount%>);
...
//		//top.location.href="/apps/appCom/wish/web2014/inipay/ShoppingBag.asp";
//		fnAPPpopupBaguni();
//	}
//},100);
</script>
