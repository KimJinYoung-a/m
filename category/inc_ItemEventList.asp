...
'db_Cache Ver.
	...
	If isArray(arrEvt) and not(isnull(arrEvt)) Then
	    ...
	    '// arrEvt(6,i)="I" 즉 이벤트링크타입이 I이고 링크값이 빈 값이 아닐때 링크 구하는 함수
	    Function getThisEventLink(page)
            If LEFT(page,5) = "https" Then
                getThisEventLink = page
            ElseIf LEFT(page,4) = "http" Then
                getThisEventLink = "https" & RIGHT(page, LEN(page)-4)
            ELse
                getThisEventLink = M_SSLUrl & page
            End If
        End Function
			...
			SELECT CASE cStr(arrEvt(0,i))
				...
				case "1", "29", "31" '//쇼핑 찬스, 헤이썸띵
					IF arrEvt(6,i)="I" and arrEvt(7,i)<>"" Then
						strLink = "<a href="""& getThisEventLink(arrEvt(7,i) & chkIIF(instr(arrEvt(7,i),"?")>0,"&","?") & strEvtRc) & """ onclick=""fnAmplitudeEventMultiPropertiesAction('click_itemprd_planevents','eventcode|index','"&arrEvt(4,i)&"|"&i&"');"">"
					Else
						strLink = "<a href=""" & M_SSLUrl & "/event/eventmain.asp?eventid=" & arrEvt(4,i) & "&"&strEvtRc & """ onclick=""fnAmplitudeEventMultiPropertiesAction('click_itemprd_planevents','eventcode|index','"&arrEvt(4,i)&"|"&i&"');"">"
					End If
