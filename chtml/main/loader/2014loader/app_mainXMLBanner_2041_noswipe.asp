...
If fileCont2 <> "" Then
	...
	For each tplNodes2 in cTmpl2
		...
		If CDate(CtrlDate2) >= CDate(startdate2) AND CDate(CtrlDate2) <= CDate(enddate2) Then
			...
			issueYN = "issue"
			issueHtml = "<div class=""swiper-slide""><img src="""& image2 &""" alt="""& altname2 &""" onclick=""fnAPPpopupEvent_URL('http://m.10x10.co.kr/apps/appCom/wish/web2014"& link2 &"');""/></div>"
...
<%
	...
	For each tplNodes in cTmpl
		...
		If CDate(CtrlDate) >= CDate(startdate) AND CDate(CtrlDate) <= CDate(enddate) Then
			If issueYN <> "" And i = i2 Then
				Exit for
			End If 
%>
			<div class="swiper-slide">
				<img src="<%= image %>" alt="<%= altname %>" onclick="fnAPPpopupEvent_URL('http://m.10x10.co.kr/apps/appCom/wish/web2014<%= link %>');"/>
			</div>
<%
			i = i + 1
		End If
