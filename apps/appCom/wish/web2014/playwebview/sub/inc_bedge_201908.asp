...
eventStartDate = Cdate("2019-08-05")
evnetEndDate = Cdate("2019-08-11")
presentDay = Cdate("2019-08-12")

title = "가장 좋아하는<br>여름 간식은 무엇인가요?"
thingMonth = 8
themeColor = "#3b9ad6"
mainCopy = "<p>요즘 1일 1아이스크림 실천 중인 저는<br>이 재미에 여름이 기다려지기도 해요.<br>특히 가장 좋아하는 건<br>삐이삐이 꼬여있는 하드 바 !<br>여러분은 어떤 여름 간식을<br>제일 좋아하나요?</p>"
badgeItemCode = "2443361"
introText = "좋아하는 여름 간식을 추천해 주세요.<br>적어주신 분 중 30분에게<br>8월의 THING배지.아이스크림을 드립니다.<br>(12자 이내로 적어주세요!)"
CommentPlaceHolder = "예) 팥빙수"
badgeImg1 = "img_badge_201908_1.jpg"
badgeImg2 = "img_badge_201908_2.jpg"
badgeImg3 = "img_badge_201908_3.jpg"
...
<article class="playDetailV16 thingthing">
	...
	<div class="badge-detail">
		...
		<% if date() < presentDay then %>
			<%=mainCopy%>
		<% else %>
			<p>좋아하는 여름 간식을<br>추천해 주셔서 감사합니다.<br>맛있는 간식과 함께<br>시원한 여름 되세요 : )</p>
		<% end if %>
			<div class="btnGet">
		...
		<% else %>
			<div class="other">
				<h3>여러분의 추천 여름 간식</h3>
				<ul>
					<li><span style="border-color:<%=themeColor%>; color:<%=themeColor%>;">선풍기틀고 먹는 쭈쭈바</span></li>
					<li><span style="border-color:<%=themeColor%>; color:<%=themeColor%>;">여름하면 옥식이와 슈박</span></li>
					<li><span style="border-color:<%=themeColor%>; color:<%=themeColor%>;">토마토무침이</span></li>
					<li><span style="border-color:<%=themeColor%>; color:<%=themeColor%>;">셀렉션 아이스크림</span></li>
					<li><span style="border-color:<%=themeColor%>; color:<%=themeColor%>;">시원한~맥주 한 캔</span></li>
					<li><span style="border-color:<%=themeColor%>; color:<%=themeColor%>;">우롱차 냉침</span></li>
				</ul>
			</div>
		<% end if %>
		</div>
