...
<%
	...
	dim result, oJson, mktTest, imgSrc, cnt
...
if mode = "add" then
    sqlstr = "SELECT COUNT(*) as cnt FROM [db_event].[dbo].[tbl_event_subscript]  WHERE userid= '"&LoginUserid&"' and evt_code="& eCode & " and sub_opt1='try'"
    ...
    If cnt < 1 Then
        ...
        sqlstr = sqlstr & " VALUES ("& eCode &", '"& LoginUserid &"', '" & device & "', 'try','" & imgSrc & "')"
...
elseif mode="alarm" then
    ...
    If cnt < 1 Then
        ...
        oJson("response") = "ok"
        oJson("message") = "알림 신청이 완료되었습니다. 3월 2일 당첨일을 기다려주세요!"
        oJson.flush
        Set oJson = Nothing
        dbget.close() : Response.End
    Else
        oJson("response") = "retry"
        oJson("message") = "이미 신청이 완료되었습니다. 3월 2일 당첨일을 기다려주세요!"
        oJson.flush
        Set oJson = Nothing
        dbget.close() : Response.End
    End If
