...
<%
	...
	if mode = "add" Then
		...
		if cartTotalAmount < amountLimit Then
			oJson("response") = "err"
			oJson("message") = "장바구니에 상품 300,000원 이상을 담은 후 이벤트 참여가 가능합니다."
			oJson.flush
			Set oJson = Nothing
			dbget.close() : Response.End	
		end if
		...
		Else				
			oJson("response") = "err"
			oJson("message") = "응모가 완료되었습니다. 당첨일 7월 14일을 기다려주세요!"
			oJson.flush
			Set oJson = Nothing
			dbget.close() : Response.End
		End If
