...
{
	$.ajax({
		url: "/play/popComment_ajax.asp?view=l&iCC="+page+"&contentsidx=<%=vContentsIdx%>",
		cache: false,
		async: false,
		...
		<div class="content bgGry" id="layerScroll">
			<div id="scrollarea">
					...
					<div class="tab01 noMove" id="writediv" style="display:<%=CHKIIF(vView="w","block","none")%>;">
						...
						<div class="tabContainer box1">
							<div id="cmtWrite" class="tabContent">
								...
								<% Else %>
								<span class="button btB1 btRed cWh1 w100p"><input type="button" onClick="alert('로그인을 하셔야 합니다.'); return false;" value="등록" /></span>
								<% End If %>
								</form>
							</div>
						</div>
				...
				%>
					...
					<form name="frmactNew" method="post" action="/play/lib/dozfcomment.asp" target="iframeDB" style="margin:0px;">
