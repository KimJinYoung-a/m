Vue.component('Biz-Modal-Filter',{
    template : `
        <!-- 꼼꼼하게 찾기 - 상품 필터 -->
    ...
    methods : {
        get_kkomkkom_count() { // 꼼꼼하게찾기 상품 갯수 조회 API 호출
            const _this = this;
            const kkomkkom_count_apiurl = apiurl + '/b2b/mobile/product/count' + this.create_product_parameter(false);
            console.log("kkomkkom_count_apiurl", kkomkkom_count_apiurl);
        ...
        create_product_parameter(is_do_search) { // 파라미터 생성
            ...
            if( this.is_category_search ) {
                ...
                param += (is_do_search ? '?keyword=' : '?keywords=') + this.parameter.search_keyword;
            }
        ...
        do_kkomkkom_search() { // 꼼꼼하게 찾기 검색 실행
            ...
            if( !this.is_category_search ) {
                ...
                //this.create_product_parameter(true);
            } else {
        ...
        brand_search(e) { // 브랜드 검색
            ...
            if( this.is_category_search ) { // 카테고리탐색 꼼꼼하게찾기
                kkomkkom_brand_apiurl = apiurl + '/search/kkomkkom/brands'
                    ...
                    + '&brand_name_keyword=' + e.target.value.toLowerCase();
            } else { // 상품검색 꼼꼼하게 찾기
                kkomkkom_brand_apiurl = apiurl + '/search/kkomkkom/brands'
