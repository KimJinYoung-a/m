...
'// 디스트로 도조 관련 텐텐배송 배너 노출 처리
Dim DistroDojoUserChk
DistroDojoUserChk = 0
If isUserLoginOK Then
	sqlStr = "select count(*) from [db_temp].[dbo].[tbl_tempDistroDojoUserDataYes] Where userid='"&getLoginUserId&"' "
	'Response.write sqlStr
	rsget.Open sqlStr,dbget,1
		DistroDojoUserChk = rsget(0)
	rsget.Close
End If
'// 디스트로 도조 관련 텐텐배송 배너 노출 처리
...
Else
	limitcnt = 5 '배열이라 -1개 총 5개 (M 전용 배너)
End If  
...
sqlStr = "select top "& topcnt &" imageurl , linkurl , startdate ,  enddate , altname from [db_sitemaster].[dbo].tbl_mobile_mainCont"
	...
	Else
		sqlStr = sqlStr & " where poscode in (2063,2072) "
	End If 
...
%>			
			<div class="swiper-slide">
				...
				<a href="<%=alink%>">
				<% End If %>
				<img src="<%=getThumbImgFromURL(img,640,"","","")%>" alt="<%=altname%>" /></a>
...
<%
	...
	Next
%>
			<% '// DistroDojo 텐텐배송 해당 인원에게만 보여줌 %>
			<% If DistroDojoUserChk > 0 Then %>
				<div class="swiper-slide">
					<% If isapp = "1" Then %>
						<a href="" onclick="fnAPPpopupSearch('텐바이텐배송');return false;">
					<% Else %>
						<a href="/search/search_item.asp?cpg=1&rect=%ED%85%90%EB%B0%94%EC%9D%B4%ED%85%90%EB%B0%B0%EC%86%A1&srm=ne&gaparam=today_mainroll_dojo">
					<% End If %>
					<img src="http://webimage.10x10.co.kr/eventIMG/2017/79350/m/bnr_main.jpg" alt="텐텐배송" /></a>
				</div>
			<% End If %>
			<% '// DistroDojo 텐텐배송 해당 인원에게만 보여줌 %>
		</div>
...
setTimeout(function(){toprollingslide();},0);
</script>
<%
End If 
on Error Goto 0
