...
if mode="iteminsert" then
	If left(currenttime,10)="2014-10-11" or left(currenttime,10)="2014-10-12" or left(currenttime,10)="2014-10-18" or left(currenttime,10)="2014-10-19" Then
		Response.Write "<script type='text/javascript'>alert('주말에는 참여 하실수 없습니다.'); parent.top.location.href='"& appUrlPath &"/event/eventmain.asp?eventid="&getevt_codeprint&"'</script>"
		dbget.close() : Response.End
	End IF
	...
	if subscriptcount>0 or bonuscouponcount>0 then
		Response.Write "<script type='text/javascript'>alert('한 개의 아이디당 하루 한 번만 응모하실 수 있습니다.'); parent.top.location.href='"& appUrlPath &"/event/eventmain.asp?eventid="&getevt_codeprint&"'</script>"
		dbget.close() : Response.End
	end if
	...
	if totalsubscriptcount>=limitbounscoupon or totalbonuscouponcount>=limitbounscoupon then
		Response.Write "<script type='text/javascript'>alert('오늘의 상품이 전부 소진되었습니다.'); parent.top.location.href='"& appUrlPath &"/event/eventmain.asp?eventid="&getevt_codeprint&"'</script>"
		dbget.close() : Response.End
	end if

	if dateopenyn(currenttime)<>"Y" then
		Response.Write "<script type='text/javascript'>alert('오늘의 앱쇼는 아직 오픈 전입니다.\n9시~12시 사이에 랜덤으로 오픈됩니다.\n앱쇼를 기다려주세요!'); parent.top.location.href='"& appUrlPath &"/event/eventmain.asp?eventid="&getevt_codeprint&"'</script>"
		dbget.close() : Response.End
	end if
