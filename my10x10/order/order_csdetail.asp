...
if (CsAsID = "") then
	response.redirect "/my10x10/order/order_cslist.asp"
	dbget.close()
	Response.End
end if
...
If mycslist.FResultCount = 0 Then
	...
	Response.Write "<script>alert('처리된 서비스 또는 잘못된 접근입니다.'); window.location.href = '/my10x10/order/order_cslist.asp';</script>"
	dbget.close()
	Response.End
End If
								...
								<% else %>
									<% if (Not IsNULL(mycsdetail.FoneItem.FsongjangNo)) and (mycsdetail.FoneItem.FsongjangNo<>"") then %>
										...
										<% if (CsDeliverDivCd2Nm(mycsdetail.FoneItem.FsongjangDiv) <> "") and (CsDeliverDivTrace(mycsdetail.FoneItem.FsongjangDiv) <> "") then %>
											<a href="<%= CsDeliverDivTrace(mycsdetail.FoneItem.FsongjangDiv) %><%= mycsdetail.FoneItem.FsongjangNo %>" target="_blank" class="btn btn-line-red btn-small" title="배송 조회하기">조회하기</a>
										<% end if %>
									<% else %>
										등록된 운송장 정보가 없습니다.
									<% end if %>
