...
dim userid,orderserial,itemid,ItemOption,EvaluatedYN,userlevel,referVal, orderidx
...
function SubmitForm(frm) {
	...
	if (!chktext(FrmGoodusing.usedcontents.value,'10')) {
	...
	if (confirm("입력사항이 정확합니까?") == true){
		$("#regbtn").hide();
		$("#lyLoading").show();
		fnAmplitudeEventMultiPropertiesAction('review_submitted','itemid|product_name|price|category_code|category_name|brand_id|brand_name|review_rating|review_body','<%=itemID%>|<%=Server.URLEncode(replace(EvList.FEvalItem.FItemName,"'",""))%>|<%=EvList.FEvalItem.FItemCost%>|<%=vDisp%>|<%=Trim(fnCateCodeToCategory1DepthName(vDisp))%>|<%=vMakerid%>|<%=Server.URLEncode(replace(EvList.FEvalItem.FMakerName,"'",""))%>|'+FrmGoodusing.totPnt.value+'|'+<%=chkIIF(Eval_excludeyn="N","encodeURI(FrmGoodusing.usedcontents.value.replace(/(\r\n\t|\n|\r\t)/gm,""""))","''")%>,'');
        ...
        setTimeout(function(){fnAppierLogEventProperties("review_submitted", appier_review_submitted_data);}, 50);
			...
			<div class="reviewFoamV17">			
				<fieldset>
					<form name="FrmGoodusing" method="post" action="<%=staticImgUpUrl%>/linkweb/doevaluatewithimage_mobile_new.asp?gaparam=<%=gaparam%>">
						...
						<input type="hidden" name="apptype" value="<%=vProcess%>" />
