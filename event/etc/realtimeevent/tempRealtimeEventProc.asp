...
' 1: 마샬 스피커
	...
	if userPrizeInfo > 0 then
		'줍줍 당첨자는 한번만 당첨 시킴
		result = "B03"
		fnReWinnerLogSave eCode,LoginUserid
	else
	...
	if result = "C01" Then
		...
		'당첨일 경우 장바구니에 상품 담기
		sqlstr = "insert into [db_my10x10].[dbo].[tbl_my_baguni] (userKey,isLoginUser,itemid,itemoption,itemea,regdate,chkOrder)" & vbcrlf
		sqlstr = sqlstr & " values('" & LoginUserid & "','Y'," & winItemid & ",'0000',1,getdate(),'N')" & vbcrlf
...
elseif mode = "evtobj" then
	...
	dim iteminfo, itemOption, mainimg, applink, DetailitemName, mainimg2
	set eventobj = new RealtimeEventCls
	...
	if isArray(itemList) then
		for i = 0 to ubound(itemList,2)
			...
			itemName = replace(itemList(2, i),"|","")	'option1(아이템명)
			...
			DetailitemName = itemList(11, i) 		'앱 링크
			mainimg2 = itemList(12, i) 		'모웹 메인 이미지
			...
			oJson("data")(null)("DetailitemName") = DetailitemName
			oJson("data")(null)("mainIMG2") = mainimg2
		next
	end if
...
elseif mode = "pushadd" then
	...
	'// 다음날 푸쉬 신청을 했는지 확인한다.
	vQuery = "SELECT count(*) FROM [db_temp].[dbo].[tbl_pickUpEvent_Push] WITH (NOLOCK) WHERE userid='"&LoginUserid&"' And convert(varchar(10), SendDate, 120) = '"&Left(pushDate, 10)&"' and evt_code=" & eCode
	...
	vQuery = " INSERT INTO [db_temp].[dbo].[tbl_pickUpEvent_Push](userid, SendDate, Sendstatus, RegDate, evt_code) VALUES('" & LoginUserid & "', '"&Left(pushDate, 10)&"', 'N', getdate()," & eCode & ")"
