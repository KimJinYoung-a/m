...
'	nowdate = "2015-08-14"		'''''''''''''''''''''''''''''''''''''''''''''''''''''
	...
	if mode="add" then
		...
		If not(nowdate>="2015-08-14" and nowdate<"2015-08-17") Then
			response.write "33"
			dbget.close()
			response.end
		End IF	
		...
		vQuery = "insert into [db_user].dbo.tbl_user_coupon(masteridx,userid,coupontype,couponvalue,couponname,minbuyprice,startdate,expiredate,targetitemlist,couponmeaipprice,reguserid,validsitename) " & _
				 "values('"& couponid &"', '" & userid & "', '2','10000','신데렐라쿠폰(1만원)','50000','2015-08-14 09:00:00','2015-08-16 23:59:59','',0,'system','app')"
		dbget.execute vQuery
