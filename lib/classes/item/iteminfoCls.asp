...
CLASS CCategoryPrdItem
	...
	Dim Ftestercnt '테스터 카운트
	Dim FEvalOffCnt '오프샵 카운트
	dim FQnaCnt 
	dim FOptionCnt
	dim FAvgDlvDate
	...
	public FItemOptionCnt '딜상품 할인율 대체

	Public FDeliverFixDay '// 직구 상품 플래그 - G
	...
	public FDeliveryCode
	public FDeliveryName

	public Ftotalpoint '// 후기 평점
	...
	'// 상품 쿠폰 내용  '!
	public function GetWishCouponDiscountStr()

		Select Case Fitemcoupontype
			Case "1"
				GetWishCouponDiscountStr =CStr(Fitemcouponvalue) + "%"
			Case "2"
				GetWishCouponDiscountStr = formatNumber(Fitemcouponvalue,0) + "원 "
			Case "3"
				GetWishCouponDiscountStr ="무료배송"
			Case Else
				GetWishCouponDiscountStr = Fitemcoupontype
		End Select
	...
	public Function getFreeBeasongLimitByUserLevel()
		...
		if (FDeliverytype="9") then
			...
			if now() > #01/24/2022 00:00:10# AND now() < #02/06/2022 00:03:00# then
    ...
    ''주문제작 상품 여부
    public function IsManufactureItem()
        IsManufactureItem = (Fitemdiv="06")
    end function
