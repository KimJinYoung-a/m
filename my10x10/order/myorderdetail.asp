	...
	elseif (myorder.FOneItem.FIpkumDiv="2") or (myorder.FOneItem.FIpkumDiv="3") then'결제 대기 중
		...
		for i=0 to myorderdetail.FResultCount-1
			...
			elseif myorderdetail.FItemList(i).Fcurrstate="7" and (myorderdetail.FItemList(i).Fcancelyn="N" or myorderdetail.FItemList(i).Fcancelyn="A") and IsNull(myorderdetail.FItemList(i).Fdlvfinishdt) then'배송 시작
				...
				CurrStateHtml4=CurrStateHtml4 & "	<a href='/category/category_itemprd.asp?itemid=" & myorderdetail.FItemList(i).Fitemid & "'>" & vbcrlf
			...
			elseif myorderdetail.FItemList(i).Fcurrstate="7" and (myorderdetail.FItemList(i).Fcancelyn="N" or myorderdetail.FItemList(i).Fcancelyn="A") and not IsNull(myorderdetail.FItemList(i).Fdlvfinishdt) then'배송 완료
				...
				CurrStateHtml5=CurrStateHtml5 & "	<a href='/category/category_itemprd.asp?itemid=" & myorderdetail.FItemList(i).Fitemid & "'>" & vbcrlf
...
function popEditOrderInfo(orderserial,etype){
	location.replace('/my10x10/orderPopup/popEditOrderInfo.asp?orderserial=' + orderserial + '&etype=' + etype);
}

function popEditHandMadeReq(orderserial,idx){
	location.replace('/my10x10/orderPopup/popEditHandMadeReq.asp?orderserial=' + orderserial + '&idx=' + idx);
}

<%'// 해외 직구 %>
function popEditDirectPurchaseNumber(orderserial){
	location.replace('/my10x10/orderPopup/popEditCustomsID.asp?orderserial=' + orderserial);
}
	...
	<div id="content" class="content">
		<div class="myOrderView">
			...
			<div class="nav nav-stripe nav-stripe-default nav-stripe-red"<% if orderState = "E" then %> style="border-top:0"<% end if %>>
				<ul class="tabNav">
				<% If vIsPacked = "Y" Then %>
					<li style="width:20%;"><a href="" onclick="viewOrderDetailDiv('1'); return false;" class="on">주문상품</a></li>
					<li style="width:20%;"><a href="" onclick="viewOrderDetailDiv('2'); return false;">구매자</a></li>
					<li style="width:20%;"><a href="" onclick="viewOrderDetailDiv('3'); return false;">결제정보</a></li>
					<li style="width:20%;"><a href="" onclick="viewOrderDetailDiv('4'); return false;">배송지</a></li>
					<li style="width:20%;"><a href="" onclick="viewOrderDetailDiv('5'); return false;">선물포장</a></li>
				<% Else %>
					<% If vIsDeliveItemExist = False Then %>
						<li style="width:34%;"><a href="" onclick="viewOrderDetailDiv('1'); return false;"  class="on">주문상품</a></li>
						<li style="width:33%;"><a href="" onclick="viewOrderDetailDiv('2'); return false;">구매자</a></li>
						<li style="width:33%;"><a href="" onclick="viewOrderDetailDiv('3'); return false;">결제정보</a></li>
					<% Else %>
						<li><a href="" onclick="viewOrderDetailDiv('1'); return false;" class="on">주문상품</a></li>
						<li><a href="" onclick="viewOrderDetailDiv('2'); return false;">구매자</a></li>
						<li><a href="" onclick="viewOrderDetailDiv('3'); return false;">결제정보</a></li>
						<li><a href="" onclick="viewOrderDetailDiv('4'); return false;">배송지</a></li>
					<% End If %>
				<% End If %>
				</ul>
			</div>
			...
			<div class="inner10">
				...
				<div id="myorderTab1">
							...
							%>
								<li>
									<div class="box3">
										<div class="pdtWrap">
											<div class="pPhoto">
												<a href="/category/category_itemprd.asp?itemid=<%= myorderdetail.FItemList(i).Fitemid %>">
													...
													<% else %>
														<% if DateDiff("d",myorder.FOneItem.FIpkumDate,now()) > 4 then %>
															<% if IsNull(myorderdetail.FItemList(i).Fcurrstate) or myorderdetail.FItemList(i).Fcurrstate=0 or myorderdetail.FItemList(i).Fcurrstate="2" or myorderdetail.FItemList(i).Fcurrstate="3" then %>
																<span class="button btS1 btBckBdr tMar0-5r"><a href="/my10x10/qna/myqnawrite.asp?qadiv=04&orderserial=<%=orderserial%>&itemid=<%=myorderdetail.FItemList(i).FItemid%>">취소 요청</a></span>
															<% end if %>
														<% end if %>
													<% end if %>
												<% end if %>
										...
										<div class="pdtInfo">
											...
											<dl class="pPrice">
												...
												%>
													...
													<span class="button btS1 btRed  cWh1 btnCancel"><a href="/my10x10/order/order_cancel_detail.asp?mode=so&idx=<%= myorderdetail.FItemList(i).FOrderSerial %>">주문취소</a></span>
												<% else %>
					...
					<!-- <div class="bnr-diary-order">
						<a href="/diarystory2021/">2021년 다이어리 준비하셨나요?</a>
					</div> -->
					...
					<% IF IsValidOrder Then %>
						<% if (MyOrdActType = "E") then %>
							<% if (myorder.FOneItem.IsWebOrderInfoEditEnable) then %>
								<span class="button btM1 btRed cWh1 w100p tMar10"><a href="" onclick="popEditOrderInfo('<%= orderserial %>','ordr'); return false;">구매자 정보변경</a></span>
							<% end if %>
						<% end if %>
					<% end if %>
				...
				<div id="myorderTab3" style="display:none">
					<div class="groupTotal box3 tMar15">
						...
						<% if myorder.FOneItem.FIpkumdiv>3 then %>
							...
							<% Else %>
								<dl class="pPrice">
									<dt>결제 금액</dt>
									<dd><%= FormatNumber(myorder.FOneItem.TotalMajorPaymentPrice,0) %>원</dd>
								</dl>
							<% End IF %>
						<% else %>
							<dl class="pPrice tMar05">
					...
					function fnMyRefundInfoSet(){
						var popwin = window.open("/my10x10/order/myorder_refund_info_edit.asp");
						popwin.focus();
					}
					</script>
					<!-- 품절 시 처리 방법 -->
				...
				<div id="myorderTab4" style="display:none">
					...
					<% IF IsValidOrder Then %>
						<span class="button btM1 btRed cWh1 w100p tMar10"><a href="" onclick="popEditOrderInfo('<%= orderserial %>','recv'); return false;">배송지 정보변경</a></span>
					<% end if %>
				</div>
				<!--// 배송지 -->
				<%
				...
				%>
					...
					<form name="frmpack" action="/inipay/pack/pack_message_edit.asp" method="post" style="margin:0px;">
