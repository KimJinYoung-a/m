...
'// 상품 쿠폰 내용  '!
	...
	snpLink = Server.URLEncode("http://m.10x10.co.kr/deal/deal.asp?itemid=" & itemid)
...
<script type="application/x-javascript" src="/apps/appCom/wish/web2014/deal/itemPrdDetail.js"></script>
...
function TnAddFavoriteDeal(iitemid){
	<% If IsUserLoginOK() Then %>
...
function fnDealItemEvalView(itemid){
	$.ajax({
		url: "/apps/appCom/wish/web2014/deal/act_itemEvaluate.asp?itemid="+itemid+"&tcnt=<%=formatNumber(DealItemEvalTotalCNT,0)%>",
		cache: false,
		async: false,
		success: function(message) {
			//alert(message);
...
$(function(){
	...
	<div class="itemFloatingV16a itemFloatingV17"><!-- opening16a -->
		...
		<div class="btnAreaV16a">
			<% If IsUserLoginOK() Then %>
			<p><button type="button" class="btnV16a btnWishV16a<%=chkIIF(isMyFavItem," actWish","")%>" onclick="TnAddFavoriteDeal('<% = oDeal.Prd.FMasterItemCode %>');return false;"><%=FormatNumber(ofavItem.Prd.FfavCount,0)%></button></p>
			<% Else %>
