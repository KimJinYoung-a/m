				...
				<% if subscriptcount=0 and bonuscouponcount=0 then %>
					...
					<% else %>
						<% if Hour(currenttime) < 09 then %>
							...
							return;
						<% else %>
							frm.action="/apps/appcom/wish/web2014/event/etc/doEventSubscript56093.asp";
...
<div class="evtCont">
	...
	<div class="mEvt56092">
		...
		<% If IsUserLoginOK() Then %>
			<p><a href="" onclick="alert('이미 회원가입이 되어 있습니다.'); return false;"><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/txt_join_10x10_app.png" alt="어머! 텐바이텐에 처음오셨나요? 가입하고 첫 구매 하러가기" /></a></p>
		<% else %>						
			<p><a href="" onclick="fnAPPpopupBrowserURL('회원가입','<%=wwwUrl%>/apps/appCom/wish/web2014/member/join.asp');"><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/txt_join_10x10_app.png" alt="어머! 텐바이텐에 처음오셨나요? 가입하고 첫 구매 하러가기" /></a></p>					
		<% End If %>
		<dl class="evtNoti">
			<dt>이벤트 유의사항</dt>
			<dd>
				...
				<p><img src="http://webimage.10x10.co.kr/eventIMG/2014/<%=folderimgselect%>/img_coupon_ex.png" alt="텐바이텐 APP설치하고 쿠폰쓰러가자!" /></p>
			</dd>
		</dl>
	</div>
	<!--// 결정해줘! App쿠폰(M) -->
