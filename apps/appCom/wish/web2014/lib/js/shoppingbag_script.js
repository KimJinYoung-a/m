...
function TnAddShoppingBag(bool){
    ...
    if (bool==true){
        frm.mode.value = "add"; //2014 추가
        frm.action = "/apps/appCom/wish/web2014/inipay/shoppingbag_process.asp?tp=pop";
        frm.target = "iiBagWin";
        frm.submit();
    }else{
		frm.mode.value = "DO3";  //2014 분기 (APP용 상품내 호출)
        frm.target = "iiBagWin"; //2014 변경
	...
	callNativeFunction('getDeviceInfo', {"callback": function(deviceInfo) {
		if (isMobile.iOS())
		{
			...
			{
				fnAPPsetTrackLog("cart","['"+frm.itemid.value+"']",document.sbagfrm.itemPrice.value);
			}
		}
		if (isMobile.Android())
		{
...
//    jsOpenModal('/apps/appcom/wish/web2014/my10x10/ajax_MyFavorite.asp?ispop=ajax&mode=add&itemid=' + iitemid + '&backurl=%2Fapps%2FappCom%2Fwish%2Fweb2014%2Finipay%2FShoppingBag%2Easp');
//    return;
//}
...
function TnAddFavoriteList(){
	...
	if (ArrayFavItemID.length < 1){
		...
		if (confirm('관심품목에 추가하시겠습니까?')){
			jsOpenModal('/apps/appcom/wish/web2014/my10x10/ajax_MyFavorite.asp?ispop=ajax&mode=AddFavItems&bagarray=' + ArrayFavItemID + '&backurl=%2Fapps%2FappCom%2Fwish%2Fweb2014%2Finipay%2FShoppingBag%2Easp');
			return;
		}
	}
}
...
function TnEditItemRequire(iid,ocd){
    ...
    jsOpenModal('/apps/appCom/wish/web2014/inipay/Pop_EditItemRequire.asp?itemid=' + iid + '&itemoption=' + ocd);
    return;
}
*/
...
function FnAddShoppingBag(bool) {
	...
	if (bool==true){
		if(sAddBagArr=="") {
			...
			aFrm.action = "/apps/appCom/wish/web2014/inipay/shoppingbag_process.asp?tp=pop";
		...
		if(sAddBagArr=="") {
			...
			aFrm.mode.value = "DO2";
			aFrm.bagarr.value = sAddBagArr;
	    	aFrm.action="/apps/appCom/wish/web2014/inipay/shoppingbag_process.asp";
			aFrm.target = "iiBagWin"; //2014 변경
			aFrm.submit();
		}
	}
