...
var app = new Vue({
    ...
    template: `
        <div id="content" class="content search_detail">
            ...
            <!-- 검색결과 존재 -->
            <template v-if="is_searched && event_count > 0">
                <header class="head_type2">
                    <h2 class="ttl">{{number_format(event_count)}}건의 이벤트를<br>찾아냈어요!</h2>
                </header>
                
                <!-- 기획전 검색 결과 -->
                <Event-List-Marketing
                    v-if="events.length > 0"
                    @click_event="send_click_search_result_item_amplitude"
                    :events="events"
                ></Event-List-Marketing>
            ...
            <!-- 검색결과 없음 -->
            <template v-else-if="is_searched && event_count == 0">
    ...
    created() {
        ...
        this.$store.dispatch('READ_SEARCH_EVENTS'); // READ 검색결과
    },

    mounted() {
        this.scroll(this.loading_page); // 스크롤 시 페이지 로딩

        this.send_search_amplitude(this.parameter.keyword, 'mktevent'
    ...
    computed : {
        ...
        event_count() { // 이벤트 검색결과 수
            return this.$store.getters.event_count;
        },
        events() { // 이벤트 리스트
    ...
    methods : {
        ...
        send_click_search_result_item_amplitude(index, event) {
