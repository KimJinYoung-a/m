...
If userid = "" Then
	Response.Write "<script type='text/javascript'>alert('로그인을 해주세요'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
	dbget.close() : Response.End
End IF
If not(left(currenttime,10)>="2014-07-08" and left(currenttime,10)<"2014-07-11") Then
	Response.Write "<script type='text/javascript'>alert('이벤트 응모 기간이 아닙니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
	dbget.close() : Response.End
End IF
...
if mode="addsms" then
	...
	if smssubscriptcount > 3 then
		Response.Write "<script type='text/javascript'>alert('메세지는 3회까지 발송 가능 합니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
		dbget.close() : Response.End
	end if
	...
	Response.Write "<script type='text/javascript'>alert('메세지가 발송 되었습니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
	dbget.close() : Response.End	

elseif mode="couponreg" then
	if not(geteventisusingyn) then
		Response.Write "<script type='text/javascript'>alert('종료 되었습니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
		dbget.close() : Response.End
	end if
	...
	if subscriptcount <> 0 or bonuscouponcount <> 0 then
		Response.Write "<script type='text/javascript'>alert('쿠폰은 한 개의 아이디당 한 번만 다운 받으실 수 있습니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
		dbget.close() : Response.End
	end if
	...
	if Hour(currenttime) < 09 then
		Response.Write "<script type='text/javascript'>alert('쿠폰은 오전 9시부터 다운 받으실수 있습니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
		dbget.close() : Response.End
	end if	
	...
	Response.Write "<script type='text/javascript'>alert('지금부터 24시간 이내에 app에서 사용하세요!'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
	dbget.close() : Response.End
else
	Response.Write "<script type='text/javascript'>alert('정상적인 경로가 아닙니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&elinkCode&"'</script>"
	dbget.close() : Response.End
end if
%>
