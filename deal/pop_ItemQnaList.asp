...
function fnGoQnaPage(page) {
	$.ajax({
		url: "pop_ItemQnaList.asp?itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&cpg="+page,
		cache: false,
		async: false,
...
function fnGoQnaTab(mode) {
	...
	<%
		If Not(IsUserLoginOK) Then
			...
			Response.Write "jsChklogin_mobile('','" & Server.URLencode("/deal/pop_itemqnalist.asp?itemid="&itemid&"&dealitemid="&dealitemid&"&mode=wr") & "');" & vbCrLf
...
function modiItemQna(idx) {
	location.href="/deal/pop_itemQnAList.asp?itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&mode=wr&id="+idx;
}
// 상품 문의 삭제
function delItemQna(idx){
	if(confirm("상품문의를 삭제 하시겠습니까?")){
		location.href="/deal/doitemqna.asp?id="+idx+"&itemid=<%=itemid%>&dealitemid=<%=dealitemid%>&mode=del";
	}
}
...
<header class="tenten-header header-popup">
	<div class="title-wrap">
		<h1>상품문의</h1>
		<button type="button" class="btn-close" onclick="location.replace('<%=wwwUrl%>/deal/deal.asp?itemid=<%=dealitemid%>'); return false;">닫기</button>
	</div>
</header>
<!-- content area -->
