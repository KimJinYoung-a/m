...
'	currenttime = "2019-10-19 오전 10:03:35"  '// 테스트 코드 변경해야됨
	...
	Select Case Trim(mode)
		Case "add"
			...
			'// 마일리지 로그 테이블에 넣는다.
			vQuery = " insert into db_user.dbo.tbl_mileagelog(userid , mileage , jukyocd , jukyo , deleteyn) values ('"&userid&"', '+"&sumMileage&"','"&eCode&"', '텐바이텐 18주년맞이 매일리지 이벤트 마일리지 지급','N') "
			dbget.Execute vQuery
		...
		Case "pushadd"
			...
			if left(currenttime, 10) >= "2019-10-19" then
				pushDate = "2019-10-21"
			else
			...
			'// 다음날 푸쉬 신청을 했는지 확인한다.
			vQuery = "SELECT count(*) FROM db_temp.[dbo].[tbl_maeliagePush18th] WITH (NOLOCK) WHERE userid='"&userid&"' And convert(varchar(10), SendDate, 120) = '"&Left(pushDate, 10)&"' "
			rsget.CursorLocation = adUseClient
			rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
			IF Not rsget.Eof Then
				If rsget(0) > 0 Then
			...
			vQuery = " INSERT INTO db_temp.[dbo].[tbl_maeliagePush18th](userid, SendDate, Sendstatus, RegDate) VALUES('" & userid & "', '"&Left(pushDate, 10)&"', 'N', getdate())"
			dbget.Execute vQuery
