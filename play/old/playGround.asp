...
'###########################################################
	Dim idx, i, contentsidx, playcode, arrlist
		...
		If idx = "" Then
			If getNumeric(requestCheckVar(request("gidx"),10)) <> "" Then
				idx = getNumeric(requestCheckVar(request("gidx"),10))
			End If
		End If
		contentsidx = getNumeric(requestCheckVar(request("contentsidx"),10))
		If contentsidx = "" Then
			If getNumeric(requestCheckVar(request("gcidx"),10)) <> "" Then
				contentsidx = getNumeric(requestCheckVar(request("gcidx"),10))
			End If
		End If
