		...
		<% end if %>
				...
				<div class="templateF">
					...
					<dl onclick="location.href='/category/category_itemPrd.asp?itemid=<%=subNodes.getElementsByTagName("itemid").item(0).text%>';">
						...
						<dd class="saleRate"><% if datediff("d",date,selFullTime)<=0 then %><p><%=subNodes.getElementsByTagName("text2").item(0).text%></p><% end if %></dd>
					</dl>
					<p onclick="location.href='/category/category_itemPrd.asp?itemid=<%=subNodes.getElementsByTagName("itemid").item(0).text%>';"><img src="<%=subNodes.getElementsByTagName("itemImg400").item(0).text%>" alt="<%=replace(subNodes.getElementsByTagName("itemname").item(0).text,"""","")%>" style="width:120px;" /></p>
					<%
							i=i+1
							end if
