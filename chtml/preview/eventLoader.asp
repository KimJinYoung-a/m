...
'// 변수 선언
	...
	Dim cSub, subNodes, i, chkDt, startDt
	...
	'출력 기준일 지정
	startDt = dateAdd("d",chkDt,startDt)
		...
		<div class="swiper-container swiper">
			...
			<%
				for each subNodes in cSub
					'출력 날짜 및 이미지 확인
					if cDate(startDt)>=cDate(subNodes.getElementsByTagName("startdate").item(0).text) and cDate(startDt)<=cDate(subNodes.getElementsByTagName("enddate").item(0).text) and subNodes.getElementsByTagName("image").item(0).text<>"" then
			%>
