...
<body class="default-font body-main play-renewal">
	...
	<div id="content" class="content">
		...
		<% if (flgDevice="I") then %>
			<% if (vAdrVer >= 2.35) then %>
				<!--<div class="app-update"><button type="button" class="btn-update" onclick=fnAPPselectGNBMenu("PLAY"); return false;><span>PLAY 구경하러 가기</span></button></div>-->
			<% else %>
		...
		<% elseif (flgDevice="A") then %>
			<div class="app-update"><div class="txt-standby"><span>Android 8월 중 오픈 예정</span></div></div>
		<% else %>
			<div class="app-update"><button type="button" class="btn-update" onclick=gotoDownload(); return false;><span>텐바이텐 APP 업데이트</span></button></div>
		<% end if %>
	</div>	
