...
<!-- #include virtual="/lib/classes/main/main_onairCls.asp"-->
<%
'' 관리자 확정시 Data 작성. 
...
for j=1 to cInt(vTerm2)
	...
	sqlStr = "select * "
	sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_mobile_main_onair_list as l "
	sqlStr = sqlStr + " inner join [db_sitemaster].[dbo].tbl_mobile_main_onair_item as s "
...
For j=1 to cInt(vTerm2)
	...
	'// 메인 데이터 접수
	sqlStr = "Select s.subidx , s.listidx , s.itemid , s.isusing as itemusing , s.sortnum , i.itemname, i.basicimage , l.startdate , l.enddate , l.ctitle , l.cper , l.cnum , l.cgubun , i.sellCash , i.orgPrice , i.sailyn , i.itemcouponYn, i.itemcouponvalue ,i.limitYN "
	...
	sqlStr = sqlStr & "	left join [db_sitemaster].dbo.tbl_mobile_main_onair_list as l "
	sqlStr = sqlStr & "	on s.listidx = l.idx  "
	sqlStr = sqlStr & " Where s.isusing = 'Y' and (" & sqlDate & ") and l.gubun = "& i &" "
	sqlStr = sqlStr & " order by l.gubun asc , s.sortnum asc "
...
'	response.end
	...
	if vTotalCount>0 then
		...
		Do Until rsget.EOF
			...
			objXMLv.appendChild(objXML.createElement("ctitle"))
			objXMLv.appendChild(objXML.createElement("cper"))
			objXMLv.appendChild(objXML.createElement("cnum"))
			objXMLv.appendChild(objXML.createElement("cgubun"))
			...
			objXMLv.childNodes(5).text = db2Html(rsget("ctitle"))
			objXMLv.childNodes(6).text = db2Html(rsget("cper"))
			objXMLv.childNodes(7).text = db2Html(rsget("cnum"))
			objXMLv.childNodes(8).text = db2Html(rsget("cgubun"))

			objXMLv.childNodes(9).text = db2Html(rsget("sellCash"))
			objXMLv.childNodes(10).text = db2Html(rsget("orgPrice"))
			objXMLv.childNodes(11).text = db2Html(rsget("sailYN"))
			objXMLv.childNodes(12).text = db2Html(rsget("itemcouponYn"))
			objXMLv.childNodes(13).text = db2Html(rsget("itemcouponvalue"))
			objXMLv.childNodes(14).text = db2Html(rsget("LimitYn"))
	...
	sqlStr = "Update [db_sitemaster].[dbo].tbl_mobile_main_onair_list " &_
				...
				" Where  isusing = 'Y' and (('" & lprevDate & "' between convert(varchar(10),startdate,120) and convert(varchar(10),startdate,120))) and gubun = "& i &"   "
	'response.write sqlStr
