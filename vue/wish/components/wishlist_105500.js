    ...
    template : '\
                <li>\
                    <a @click="itemUrl(isApp , item.itemid)">\
                        ...
                        <div class="desc">\
                            <div class="price">\
                                <b class="sum">{{item.totalprice}}<span class="won">원</span></b>\
                                <b class="discount color-red" v-if="item.saleyn == \'Y\'">{{item.totalsaleper}}</b>\
                            </div>\
                            <div class="name">[{{item.brandname}}]  {{item.itemname}}</div>\
                        </div>\
                    </a>\
                    <button @click.stop="addCart(isApp, item.itemid , item.itemoptioncount)"><img src="//webimage.10x10.co.kr/fixevent/event/2020/105500/m/btn_cart.png" alt="장바구니 담기"></button>\
                </li>\
                ',
    methods : {
