...
Else
    mktTest = False
End If
eventStartDate  = cdate("2021-10-15")		'이벤트 시작일
eventEndDate 	= cdate("2021-10-21")		'이벤트 종료일

if mktTest then
currentDate = cdate("2021-10-15")
else
currentDate = date()
end if
...
function eventTry(){
	...
	<% else %>
		...
		$.ajax({
			type:"POST",
			url:"/event/etc/doEventSubscript114763.asp",
			data: data,
			dataType: "JSON",
			success : function(res){
					if(res!="") {
						// console.log(res)
						if(res.response == "ok"){
							alert('신청이 완료되었습니다.\n10월 22일에 마일리지가 지급되면 알림톡이 발송됩니다.');
							return false;
						}else{
							alert(res.faildesc);
							return false;
	...
	<% End If %>
			...
			<div class="mEvt88837 double-mileage">
				<h2><img src="//webimage.10x10.co.kr/fixevent/event/2021/114763/m/img_main.jpg" alt="더블 마일리지"></h2>
				<div class="my-mileage">
					...
					<div class="btn-group">
						<% If IsUserLoginOK Then %>
						<a href="/my10x10/goodsusing.asp" target="_blank" class="mWeb"><img src="//webimage.10x10.co.kr/fixevent/event/2021/114763/m/btn_goprd.png" alt="상품후기 쓰러 가기"></a>
                ...
                <div class="sec-aram">
                    <img src="//webimage.10x10.co.kr/fixevent/event/2021/114763/m/img_aram.jpg" alt="이벤트 안내">
                    <button type="button" onclick="eventTry();"></button>
                </div>
				<p><img src="//webimage.10x10.co.kr/fixevent/event/2021/114763/m/img_noti.jpg" alt="이벤트 유의사항"></p>
			</div>
