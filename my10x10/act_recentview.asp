...
'####################################################
	...
	If Trim(vMode)="list" Then
		...
		If Trim(vTypeItem) = "false" And Trim(vTypeEvt) = "false" And Trim(vTypeMkt) = "false" And Trim(vTypeBrand) = "false" And Trim(vTypeRect) = "false" Then
			...
			response.write "		<a href='/' class='btnV16a btnRed2V16a'>쇼핑하러 가기</a>"
			response.write "</div>"
			Response.End
		end if
...
%>
		...
		<% If myRecentView.FResultCount>0 Then %>
			...
			For lp=0 To myRecentView.FResultCount-1 
				...
				linkUrl = "/category/category_itemPrd.asp?itemid="& myRecentView.FItemList(lp).FItemId & "&gaparam=recentview_product_1"
				...
				<% Select Case Trim(myRecentView.FItemList(lp).Ftype) %>
							...
							<div class="inner">
								...
								<% Else %>
									<% if adultChkFlag then %>										
										<a href="javascript:confirmAdultAuth('<%=linkUrl%>');" target="_blank">
									<% else %>
										<a href="/category/category_itemprd.asp?itemid=<%=myRecentView.FItemList(lp).FItemId%>&gaparam=recentview_product_1" target="_blank">
									<% end if %>									
								<% End If %>
						...
						<div class="section brand" id="div<%=myRecentView.FItemList(lp).FIdx%>">
							<div class="inner">
								<% If myRecentView.FItemList(lp).FBrandUsing="N" Then %>
									<a href="" onclick="return false;">
								<% Else %>
								    <a href="/brand/brand_detail2020.asp?brandid=<%=myRecentView.FItemList(lp).Fmakerid%>&gaparam=recentview_brand_1" target="_blank">
								<% End If %>
						...
						<div class="section exhibition" id="div<%=myRecentView.FItemList(lp).FIdx%>">
							<div class="inner">
								<% If Datediff("d", Now(), myRecentView.FItemList(lp).Fevtenddate) < 0 Or myRecentView.FItemList(lp).Fevtstate="9" Then%>
									<a href="" onclick="return false;">
								<% Else %>
									<% If Trim(myRecentView.FItemList(lp).FevtUsingMw)="True" Then %>
										<a href="/event/eventmain.asp?eventid=<%=myRecentView.FItemList(lp).Fevtcode%>&gaparam=recentview_planevt_1" target="_blank">
									<% Else %>
										<% If myRecentView.FItemList(lp).FevtUsingPc="True" And myRecentView.FItemList(lp).FevtUsingApp="True" Then %>
											<a href="" onclick="alert('텐바이텐 PC나 텐바이텐 앱에서만 볼 수 있는 기획전 입니다.');return false;">
										<% ElseIf myRecentView.FItemList(lp).FevtUsingPc="False" And myRecentView.FItemList(lp).FevtUsingApp="True" Then %>
											<a href="" onclick="alert('텐바이텐 앱에서만 볼 수 있는 기획전 입니다.');return false;">
										<% ElseIf myRecentView.FItemList(lp).FevtUsingPc="True" And myRecentView.FItemList(lp).FevtUsingApp="False" Then %>
					...
					<% Case "mktevt" %>
						<div class="section event" id="div<%=myRecentView.FItemList(lp).FIdx%>">
							<div class="inner">
								...
								<% Else %>
									<% If Trim(myRecentView.FItemList(lp).FevtUsingMw)="True" Then %>
										<a href="/event/eventmain.asp?eventid=<%=myRecentView.FItemList(lp).Fevtcode%>&gaparam=recentview_mktevt_1" target="_blank">
									<% Else %>
										<% If myRecentView.FItemList(lp).FevtUsingPc="True" And myRecentView.FItemList(lp).FevtUsingApp="True" Then %>
											<a href="" onclick="alert('텐바이텐 PC나 텐바이텐 앱에서만 볼 수 있는 이벤트 입니다.');return false;">
										<% ElseIf myRecentView.FItemList(lp).FevtUsingPc="False" And myRecentView.FItemList(lp).FevtUsingApp="True" Then %>
											<a href="" onclick="alert('텐바이텐 앱에서만 볼 수 있는 이벤트 입니다.');return false;">
										<% ElseIf myRecentView.FItemList(lp).FevtUsingPc="True" And myRecentView.FItemList(lp).FevtUsingApp="False" Then %>
					...
					<% Case "rect" %>
						<div class="section searchword" id="div<%=myRecentView.FItemList(lp).FIdx%>">
							<div class="inner">
								<a href="/search/search_result2020.asp?keyword=<%=myRecentView.FItemList(lp).Frect%>" target="_blank">
