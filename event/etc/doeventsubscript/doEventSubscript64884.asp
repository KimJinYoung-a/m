...
<%
dim nowdate, refer, device, selectnum
dim eCode, userid, sqlstr, mode , vTotalCount
Dim vQuery

	userid = GetLoginUserID
	mode = requestcheckvar(request("mode"),32)
	selectnum = requestcheckvar(request("selectnum"),32)
...
'	nowdate = "2015-07-20"		'''''''''''''''''''''''''''''''''''''''''''''''''''''
	...
	if mode="add" then
		...
		sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', '"& selectnum &"', '"& device &"')" + vbcrlf
		dbget.execute sqlstr
	
		Call fnCautionEventLog(eCode,userid,selectnum,"","",device)
