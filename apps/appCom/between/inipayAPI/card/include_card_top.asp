...
if (CLng(iorderParams.Fmiletotalprice)>CLng(availtotalMile)) then
	...
	'response.write "<script type='text/javascript'>location.replace('" & wwwUrl & "/inipayAPI/shoppingbag.asp')</script>"
...
if (iErrStr<>"") then
	...
	vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "결제는 이루어 지지 않았습니다. \n\n: 오류 -" & replace(iErrStr,"'",""))
	response.redirect fnGetUserInfo("FAILURL")
	response.end
end if
%>