...
if (GetLoginUserLevel="7") or (GetLoginUserID="leecong2") or (GetLoginUserID="zisooo") or (GetLoginUserID="crazykkt") or (GetLoginUserID="urami10004") or (GetLoginUserID="thensi7") or (GetLoginUserID="10x10blue") or (GetLoginUserID="10x10green") or (GetLoginUserID="10x10yellow") or (GetLoginUserID="saintyun7873") then ''임시 테스트
    G_PG_PAYCO_ENABLE = TRUE
end If
...
<div class="heightGrid">
	<div class="mainSection">
		<div class="container">
				...
				<div class="cartV16a">
					...
					<div class="orderListV16a showHideV16a">
						<div class="bxLGy2V16a grpTitV16a tglBtnV16a showToggle">
							<h2 class="hasArrow">주문 리스트 <span class="fs1-2r lPad0-5r">( <strong class="cRd1V16a" id="itemcntt"></strong>개 ㅣ <strong class="cRd1V16a"><%= FormatNumber(oshoppingbag.GetTotalItemOrgPrice-oshoppingbag.GetMileageShopItemPrice,0) %></strong>원 )</span></h2>
						</div>
										...
										<div class="pdtInfoV16a">
											...
											if G_IsPojangok AND oshoppingbag.FItemList(i).FPojangOk="Y" then
												if oshoppingbag.FItemList(i).FPojangVaild then
													Response.Write "<p class=""pdtFlagV16a""><i class=""icoPV16a"">선물포장 가능 상품 - 포장서비스 신청상품</i></p>"
												else
													Response.Write "<p class=""pdtFlagV16a""><i class=""icoPNoV16a"">선물포장 가능 상품 - 포장서비스 비신청상품</i></p>"
												end if
					...
					<div class="orderInfoV16a">
						<div class="cartGrpV16a">
							...
							<div class="bxWt1V16a infoUnitV16a">
								<dl class="infoArrayV16a">
									<dt>주문자</dt>
									<dd><input type="text" style="width:100%;" name="buyname" value="<%= doubleQuote(oUserInfo.FOneItem.FUserName) %>" autocomplete="off" /></dd>
								</dl>
								<dl class="infoArrayV16a">
									<dt>이메일</dt>
						...
						<% If vIsDeliveItemExist Then %>
							...
							<div class="cartGrpV16a">
								...
								<% if (IsForeignDlv) then %>
									...
									<dl class="infoArrayV16a">
										<dt>Zip Code</dt>
										<dd><input type="number" name="emsZipCode" class="lPad0-6r" style="width:100%;" maxlength="20" value="" autocomplete="off" /></dd>
								...
								<% else %>
									...
									<dl class="infoArrayV16a">
										<dt class="vTop">주소</dt>
										<dd>
											<p><input type="text" class="ct" style="width:8.5rem;" name="txZip" value="" ReadOnly /> <input type="button" class="btnV16a btnLGryV16a lMar0-5r" style="width:10.25rem;" onclick="fnOpenModal('/lib/pop_searchzipNew.asp?target=frmorder&gb=5'); return false;" value="우편번호 찾기" /></p>
						...
						<div class="cartGrpV16a">
							...
							<div class="bxWt1V16a infoUnitV16a">
								<div class="bxWt1V16a" style="padding-top:0.05rem; padding-bottom:0.9rem">
									...
									<dl class="infoArrayV16a">
										<dt>
											<input type="radio" id="moCp" disabled="disabled" /><label for="moCp" class="lMar0-5r">모바일쿠폰</label>
										</dt>
										<dd>
											<select style="width:100%;">
												<option>모바일쿠폰 선택하세요</option>
											</select>
										</dd>
									</dl>
									//-->
								...
								<div>
									<dl class="infoArrayV16a">
										...
										<dd class="cMGy1V16a">
											<input type="text" id="mileage" name="spendmileage" value="<%= oshoppingbag.GetMileageShopItemPrice %>" style="width:7rem" class="rt rPad0-6r readOnly" readonly /><span class="fs1-2r cBk1V16a lMar0-4r">Point</span>
							...
							<%=evtDesc%>
								...
								%>
									<dl class="showHideV16a">
										<dt class="tglBtnV16a"><p class="hasArrow"><%=oOpenGift.FItemList(i).Fgift_range1%>원 이상 구매 사은품</p></dt>
								...
								<%	End If %>
												...
												<li <%=chkIIF(oOpenGift.FItemList(i).IsGiftItemSoldOut or (optAllsoldOut),"class=""soldoutV16a""","")%>>
													...
													%>
													<div><input type="radio" name="rRange" id="<%= oOpenGift.FItemList(i).Fgift_range1+1000000  %>" value="<%= oOpenGift.FItemList(i).Fgiftkind_code %>" disabled OnClick="giftOptEnable(this);" /></div>
													<% else
													...
													<div>
														...
														<p class="tMar0-4r cMGy1V16a fs1-1r"><%=vGiftNotice%></p>
														<p class="tMar0-6r">
						...
						<div class="cartGrpV16a freebiesV16a">
							<div class="bxLGy2V16a grpTitV16a">
								<h2>다이어리 구매 사은품 선택</h2>
							</div>
							<div class="bxWt1V16a freebieSltV16a">
								<% if Not (isNULL(Diary_evtStDT) or isNULL(Diary_evtEdDT)) then %>
								<div class="bxWt1V16a">이벤트 기간 : <%= replace(Diary_evtStDT,"-",".") %> ~ <%= replace(replace(Diary_evtEdDT,"-","."),Left(Diary_evtEdDT,4)&".","") %></div>
								<% end if %>
								<dl>
								...
								<ul class="cartInfoV16a tPad0-9r">
									<li>DIARY STORY 다이어리 포함 텐바이텐 배송상품<br />1/2/4만원 이상 구매 시 증정 (쿠폰, 할인카드 등 사용 후 구매확정금액 기준)</li>
									<li>환불 및 교환으로 기준 금액 미만이 될 경우 사은품 반품</li>
									<li>사은품 디자인은 랜덤 증정</li>
								</ul>
							</div>
						</div>
						<%
							...
							<div class="bxWt1V16a payMethodV16a">
								<div id="i_paymethod">
								    ...
								    <% if (oshoppingbag.IsBuyOrderItemExists) then %>
										<ul class="btnBarV16b fstFloor">
											<li class="" style="width:50%;" onclick="CheckPayMethod('100');"><div>신용카드<%= ChkIIF(IsKBRdSite," (KB카드)","") %></div></li>
											<li class="" style="width:50%;" onclick="CheckPayMethod('400');"><div>휴대폰</div></li>
										</ul>
										<ul class="btnBarV16b upstairs">
											<li class="kakaoP" style="width:33%;" onclick="CheckPayMethod('800');"><div>카카오페이</div></li>
											<% if G_PG_NAVERPAY_ENABLE then %>
												<li class="naverP" style="width:34%;" onclick="CheckPayMethod('900');"><div>네이버페이</div></li>
											<% end if %>
											<% if G_PG_PAYCO_ENABLE then %>
												<li class="payco" style="width:33%;" onclick="CheckPayMethod('950');"><div>PAYCO간편결제</div></li>
											<% End If %>
										</ul>
								    <% else %>
										<ul class="btnBarV16b fstFloor">
											<li class="" style="width:33%;" onclick="CheckPayMethod('100');"><div>신용카드<%= ChkIIF(IsKBRdSite," (KB카드)","") %></div></li>
											<li class="" style="width:34%;" onclick="CheckPayMethod('400');"><div>휴대폰</div></li>
											<li class="" style="width:33%;" onclick="CheckPayMethod('7');"><div>무통장<%= ChkIIF(IsCyberAccountEnable,"","") %></div></li>
										</ul>
										<ul class="btnBarV16b upstairs">
											<li class="kakaoP" style="width:33%;" onclick="CheckPayMethod('800');"><div>카카오페이</div></li>
											<% if G_PG_NAVERPAY_ENABLE then %>
			...
			<form name="allAtfrm" method="post" action="" style="margin:0px;">
