...
<%
Dim vSaleFreeDeliv
Dim vListGubun : vListGubun = requestCheckVar(request("LstGun2"),12)
...
Dim vCateCode : vCateCode = requestCheckVar(request("catecode2"),12)
		...
		<li>
			<div class="wishPdtWrapV15a">
				<div class="wishPicV15a">
					<a href="/category/category_itemprd.asp?itemid=<%=oDoc.FWishPrdList(i).FItemID%>"  id="Hlink">
						<% if imgmod = 0 then %>
				...
				<div class="wishContV15a">
					...
					<% end if %>
					<p class="wisher" onclick="location.href='/my10x10/myWish/myWish.asp?ucid=<%=Server.UrlEncode(tenEnc(oDoc.FWishPrdList(i).Fuserid))%>'" id="Hlink3"><%=printUserId(oDoc.FWishPrdList(i).Fuserid,2,"*")%></p><!-- 아이디는 **포함 10자까지만 노출됩니다 -->
				</div>
			</div>
		</li>
	<% End IF %>
...
<% If fCnt = 0 And Trim(vListGubun)="FollowList" Then %>
	<script>
		$(function() {
			fnOpenModal('popFollowList.asp'); return false;
		});
	</script>
<% response.End %>
<% End If %>
...
<% If mCnt = 0 And Trim(vListGubun)="MateList" Then %>
	<script>
		$(function() {
			fnOpenModal('popMateList.asp'); return false;
		});
	</script>
<% response.End %>
<% End If %>
