...
<!-- #include file="./UserInfo_javascript2.asp" -->
					...
					<div class="orderInfoV16a">
						...
						<div class="cartGrpV16a" id="i_paymethod">
							...
							<% If Not(vlsOnlyHanaTenPayExist) Then %>
								<% If G_PG_TOSSPAYNEW_ENABLE And currentDate >= #09/01/2020 00:00:00# and currentDate <= #09/30/2020 23:59:59# Then %>
									<div class="bnr-toss" id="bnr-toss" style="padding:1.8rem 1.28rem 1.2rem; letter-spacing:-1px;"><strong style="font-weight:bold;">토스로 7만 원 이상 결제 시 5,000원 즉시 할인!</strong><span>5,000</span></div>
									<script>
									$(function() {
										//Positioning
								...
								<% If currentDate >= #10/05/2020 00:00:00# and currentDate <= #10/07/2020 23:59:59# Then %>
									<div style="margin-bottom:-2vw; background:#fff" id="bnr-shCard"><img src="//webimage.10x10.co.kr/fixevent/event/2020/104988/m/bnr_shinhan.png" alt="신한카드"></div>
									<script>
									$(function() {
										$(".btnBarV16b li").on("click",function(){
								...
								<% If currentDate >= #10/12/2020 00:00:00# and currentDate <= #10/18/2020 23:59:59# Then %>
									<div class="bnr-pay kakao" id="bnr-kakao">카카오페이로 5만 원 이상 결제 시<br>3,000원 즉시 할인!</div>
									<script>
									$(function() {
										//Positioning
								...
								<% If currentDate >= #10/19/2020 00:00:00# and currentDate <= #10/25/2020 23:59:59# Then %>
									<div class="bnr-pay chai" id="bnr-chai">차이로 4만 원 이상 결제 시 3,000원 즉시 할인!</div>
									<script>
									$(function() {
										//Positioning
								...
								<% End If %>
								<% If currentDate >= #10/26/2020 00:00:00# and currentDate <= #10/28/2020 23:59:59# Then %>
									<div class="bnr-pay bc" id="bnr-BcCard">BC카드로 5만 원 이상 결제 시<br>5,000원 즉시 할인!</div>
									<script>
									$(function() {
										$(".btnBarV16b li").on("click",function(){
											if($("input[name=Tn_paymethod]").val()=="100") {
						...
						<div class="cartGrpV16a">
							...
							<div class="bxWt1V16a infoUnitV16a">
								<p class="fs1-2r">빠른 주문 처리를 위해 품절 발생 시 별도의 연락을 하지 않고 선택하신 결제 방법으로 안전하게 환불해 드립니다.</p>
								<div class="infoUnitV16a fs1-2r">
									<input type="checkbox" id="soldoutAgree" checked="checked" /><label for="soldoutAgree" class="lMar0-5r">네, 동의합니다.</label>
								</div>
								<%
								...
								%>
								<div class="intlNotiV16a" id="refundInfo" style="display:none">
									<dl class="infoArrayV16a">
