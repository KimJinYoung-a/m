...
%>
<script>
function TnGotoEventMain_cate(v,t){
	if (v==""){
	document.location = '/shoppingtoday/shoppingchance_allevent.asp';
	}else{
	document.location = '/event/eventmain.asp?eventid=' + v + t;
	}
}
</script>
	...
	<ul class="evtListV15">
		...
		For intLoop =0 To UBound(arrList,2)
			IF arrList(6,intLoop)="I" and arrList(7,intLoop)<>"" THEN '링크타입 체크
				vLink = "location.href='" & arrList(7,intLoop) & gaParam & "';"
			ELSE
				vLink = "javascript:TnGotoEventMain_cate('" & arrList(0,intLoop) & "','"& gaParam &"');"
			END IF
		...
		%>
			<li onclick="<%=vLink%>" class="<%=chkiif(datediff("D",arrList(3,intLoop),Date()+4) > 0,"hurryUpV15","")%>">
				<div class="evtPhotoV15"><img src="<%=arrList(24,intLoop)%>" alt="<%=db2html(arrList(10,intLoop))%>" /></div>
				<dl>
					<dt>											
						<%	'//이벤트 명 할인이나 쿠폰시
...
<% End If %>
<div class="btnMoreV15a"><a href="/shoppingtoday/shoppingchance_allevent.asp"><%=gnbname%> 기획전 더보기</a></div>
<div class="btnEvtAllV15a"><a href="/shoppingtoday/shoppingchance_allevent.asp"><i>기획전 전체보기</i></a></div>