...
<%
'####################################################
' Description : 2019 18주년 텐텐절 매일리지 1차
' History : 2019-10-04 원승현 생성
' 주의사항
'   - 이벤트 기간 : 2019-10-11 ~ 2019-10-19
'   - 오픈시간 : 24시간
'####################################################
%>
...
<%
	...
	IF application("Svr_Info") = "Dev" THEN
		eCode  = 90406 '// 1차 텐텐절 매일리지 코드
        mobileEventCode = 90405 '// 모바일 이벤트 코드
	Else
		eCode  = 97541 '// 1차 텐텐절 매일리지 코드
        mobileEventCode = 97540 '// 모바일 이벤트 코드
	End If
	...
	'// 현재시간
	currenttime = now()
'	currenttime = "2019-10-19 오전 10:03:35" '// 테스트 코드 변경해야됨

	vEventStartDate = "2019-10-11"
	vEventEndDate = "2019-10-19"

	If isapp <> "1" Then 
		Response.redirect "/event/eventmain.asp?eventid="&mobileEventCode&"&gaparam="&gaparamChkVal
		Response.End
	Else
		if currenttime >= "2019-10-20" then
			Response.redirect "/apps/appCom/wish/web2014/event/eventmain.asp?eventid=97567&gaparam="&gaparamChkVal
			Response.End
		end if
	End If
...
%>
<style type="text/css">
.maeileage {background:#f0007e;}
.maeileage .top {position:relative; background:url(//webimage.10x10.co.kr/fixevent/event/2019/97541/m/bg_top.png) no-repeat center / 100% 100%;}
...
.daily-board {overflow:hidden; width:100%; background-color:#402059;}
.daily-board li {position:relative; float:left; width:33.3%; padding-bottom:24%; border:1px solid #492c60;}
.daily-board li:nth-child(1), .daily-board li:nth-child(2), .daily-board li:nth-child(3) {border-top:2px solid #492c60;}
...
.daily-board li .daily-box p {color:#d4a8f6; font-size:1.32rem; letter-spacing:-.13rem;}
...
.daily-board li.current:before {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-top:3px solid #37f5a7; z-index:10; transform-origin:100% 0;}
.daily-board li.current:after {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-right:3px solid #37f5a7; z-index:10; transform-origin:0 100%;}
...
.daily-board li.current .current-fr:before {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-bottom:3px solid #37f5a7; z-index:10; transform-origin:0 100%;}
.daily-board li.current .current-fr:after {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-left:3px solid #37f5a7; z-index:10; transform-origin:0 0;}
...
.daily-board li.previous .daily-box p {color:#8b6da2;}
.daily-board li.previous .daily-box strong {color:#8b6da2;}
...
<script type="text/javascript">
	$(function() {
		...
		fnAmplitudeEventMultiPropertiesAction('view_maeliage18th','eventcode|chasu','<%=ecode%>|1','');
	});
	function jsMaeilageSubmit(){

		fnAmplitudeEventMultiPropertiesAction('click_maeliage18th_submit','eventcode|chasu','<%=ecode%>|1','');
		...
		$.ajax({
			type:"GET",
			url:"/apps/appCom/wish/web2014/event/18th/doEventSubscript97541.asp?mode=add",
			dataType: "text",
			async:false,
			cache:true,
	...
	function jsMaeilagePushSubmit(){
		...
		fnAmplitudeEventMultiPropertiesAction('click_maeliage18th_push','eventcode|chasu','<%=ecode%>|1','');
		...
		$.ajax({
			type:"GET",
			url:"/apps/appCom/wish/web2014/event/18th/doEventSubscript97541.asp?mode=pushadd",
			dataType: "text",
			async:false,
			cache:true,
			success : function(Data, textStatus, jqXHR){
...
<%' 매일리지 1차 %>
<div class="mEvt97541 maeileage">
    <div class="top">
        <h2><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/tit_maeileage.png" alt="매일리지" /></h2>
        <div class="attendance">
            ...
            <div class="get-mileage" onclick="fnAmplitudeEventMultiPropertiesAction('click_maeliage18th_my10x10','eventcode|chasu','<%=ecode%>|1', function(bool){if(bool) {fnAPPpopupBrowserURL('마일리지 내역','<%=wwwUrl%>/apps/appCom/wish/web2014/offshop/point/mileagelist.asp','right','','sc');}});return false;">
                ...
                <img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/bg_now_mileage.png" alt="현재까지 받은 마일리지" />
            </div>
            <button type="button" id="chkAttendance" class="chk-attendance" onclick="jsMaeilageSubmit();return false;" <% If TodayDateCheck > 0 Then %>style="display:none"<% Else %><% End If %>><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/btn_attendance.png" alt="출석 체크하기" /></button>
            <span class="chk-attendance-ok" id="attendancecheckok" <% If TodayDateCheck > 0 Then %><% Else %>style="display:none"<% End If %>><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/btn_attendance_ok.png" alt="출석체크 완료" /></span>
            <div class="today-noti">
                <img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/txt_replay_noti.png" alt="하루라도 놓치면 100p 부터 다시 시작" />
				<span><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/txt_noti.png" alt="본 마일리지는 스페셜 마일리지로 11월 1일에 소멸됩니다." /></span>
            </div>
        </div>
        <div class="dc-group">
    ...
    <div id="lyrDailyChk" class="lyr-daily-chk">
        <% If Not(Left(currenttime, 10) >= vEventEndDate) Then %>
            <div>
                ...
                <p class="tip"><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/txt_tip.png" alt="푸시 신청하고 매일 불어나는 마일리지를 놓치지 마세요! " /></p>
                ...
                <img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/lyr_bg.png" alt="" />
            </div>
        <% Else %>
            <%' 10/19 마지막날 노출 레이어 %>
            <div class="lyr-finish">
                ...
                <button type="button" id="btnClose" class="btnClose">닫기</button>
                <img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/lyr_bg_finish.png" alt="" />
            </div>
        <% End If %>
    </div>
    <div id="mask"></div>
    ...
    <div class="step-process">
        <% If IsUserLoginOK() Then %>
            ...
            <%
                ...
                vQuery = vQuery & "	From db_temp.[dbo].[tbl_maeliageDate18th_First] m "
                rsget.CursorLocation = adUseClient
                rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
                If Not(rsget.bof Or rsget.eof) Then
                    Do Until rsget.eof
    ...
    <% If Left(currenttime, 10) < Left(vEventEndDate, 10) Then %>
        <div class="push-request">
            <button type="button" onClick="jsMaeilagePushSubmit();">
                <img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/btn_tomorrow_push.png" alt="TIP! 푸시 신청하고 매일 불어나는 마일리지를 놓치지 마세요! 내일의 매일리지 알림 신청" />
            </button>
        </div>
    <% End If %>        
    <div class="push-way">
        <h3><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/tit_push_check.png" alt="푸시 수신 확인 방법" /></h3>
        <div class="rolling">
            <div class="swiper-container">
                <div class="swiper-wrapper">
                    <div class="swiper-slide"><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/img_slide_1.png" alt="" /></div>
                    <div class="swiper-slide"><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/img_slide_2.png" alt="" /></div>
                    <div class="swiper-slide"><img src="//webimage.10x10.co.kr/fixevent/event/2019/97541/m/img_slide_3.png" alt="" /></div>
                </div>
            </div>
            <div class="btn-area">
