...
Dim vQuery, vTotalCount , vTotalSum, eLinkCode
	...
	If Now() > #04/01/2015 00:00:00# Then
	...
	elseif mode="add" then
		'//응모
		vQuery = " EXEC [db_order].[dbo].[sp_Ten_MyOrderList_SUM] '" & userid & "', '', '', '2015-03-01', '2015-04-01', '10x10', '', 'issue' "
		rsget.CursorLocation = adUseClient
		rsget.CursorType = adOpenStatic
		rsget.LockType = adLockOptimistic
		...
		If vTotalCount < 1 Then
			response.write "<script language='javascript'>alert('3월 구매내역이 없습니다.\n구매 후, 다시 응모하러 와주세요!'); parent.location.reload();</script>"
			dbget.close()
			response.end
		End If
