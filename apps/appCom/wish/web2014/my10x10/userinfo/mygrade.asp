...
Function besturl(v)
	Select Case v
		...
		Case Else
			besturl = "/apps/appcom/wish/web2014/award/awarditem.asp?userlevel=" 'STAFF
	End select
End Function
...
<body class="default-font body-sub">
	...
	<div id="content" class="content membershipV18">
		<div class="nav nav-stripe nav-stripe-default nav-stripe-red">
			<ul class="grid2">
				<li><a href="" onclick="jsChgurl('/apps/appcom/wish/web2014/my10x10/userinfo/mygrade.asp');return false;" class="on">나의 등급</a></li>
				<li><a href="" onclick="jsChgurl('/apps/appcom/wish/web2014/my10x10/userinfo/mybadge.asp');return false;">나의 뱃지</a></li>
			</ul>
		</div>
		<div class="my-grade g-<%=GetUserStr(GetLoginUserLevel)%>">
			...
			<% If stroke1 = 100 Then %>
			<div class="next-grade">등급 <%=chkiif(GetLoginUserLevel = 6 Or GetLoginUserLevel = 7,"유지","상승")%>조건 달성!<br><em><%= month(dateadd("m",1,date)) %></em>월의 예상등급은 <em><%= GetUserLevelStr(NextuserLevel)%></em> 입니다.</div>
			<% Else %>
			<div class="next-grade"><% If GetLoginUserLevel <> 6 Or GetLoginUserLevel <> 7 Then %><em><%= getRequireLevelUpBuyCountVer2(NextuserLevel2,BuyCount) %></em>번의 주문 또는 <% End If %><em><%= FormatNumber(getRequireLevelUpBuySumVer2(NextuserLevel2,BuySum),0) %></em>원 이상 결제 시<br><em><%= GetUserLevelStr(NextuserLevel2)%></em>로 등급 <%=chkiif(GetLoginUserLevel = 6 Or GetLoginUserLevel = 7,"유지","상승")%>!</div>
			<% End If %>
			<a href="" onclick="fnAPPpopupBrowserURL('등급별 혜택','<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/userinfo/grade_guide.asp');return false;" class="btn-guide btn btn-xsmall btn-radius color-black btn-icon">등급별 혜택<span class="icon icon-arrow"></span></a>
		</div>
		<% If GetLoginUserLevel <> 7 then %>
