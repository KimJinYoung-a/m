...
class cdiary_list
		...
		if  not rsget.EOF  then
			...
			do until rsget.eof
				set FItemList(i) = new cdiary_oneitem
					...
					FItemList(i).FDiaryBasicImg		= webImgUrl & "/diary_collection/2012/basic/" & rsget("BasicImg")
					FItemList(i).FDiaryBasicImg2	= webImgUrl & "/diary_collection/2012/basic2/" & rsget("BasicImg2")
					FItemList(i).FDiaryBasicImg3	= webImgUrl & "/diary_collection/2012/basic3/" & rsget("BasicImg3")
					FItemList(i).FStoryImg		= webImgUrl & "/diary_collection/2012/story/" & rsget("StoryImg")
	...
	Public Sub getOneDailynot()
		...
		sqlStr = "SELECT top 1"
		sqlStr = sqlStr & " t.itemid, t.linkpath"
		...
		sqlStr = sqlStr & " from ("
		sqlStr = sqlStr & " 	select top 1 pi.evt_code as itemid, pi.linkpath"
	...
	Public Sub getOneplusOneDaily()
		...
		sqlStr = sqlStr & " , i.limitsold, i.itemname, i.SailYn as SaleYn, i.orgprice, i.sailprice,i.basicimage, i.icon1image"
		sqlStr = sqlStr & " , i.icon2image, i.sellcash, i.itemcouponyn ,i.sellyn , i.itemCouponValue , i.itemCouponType"
		sqlStr = sqlStr & " , o.itemid, o.image1, o.image2, o.imageend, o.endlink, o.explain, o.plustype, c.socname " +vbcrlf
			...
			if  not rsget.EOF  then
				...
				do until rsget.eof
					set FItemList(i) = new cdiary_oneitem
						...
						FItemList(i).FImageicon2		= webImgUrl & "/image/icon2/" & GetImageSubFolderByItemid(rsget("Itemid")) & "/" & rsget("icon2image")
						FItemList(i).Fsocname		= rsget("socname")
						FItemList(i).FpreviewImg = rsget("diary_idx")
						If fuserid <> "" then
							FItemList(i).Fuserid			= rsget("userid")
						End If
					i=i+1
		...
		If FTotalCount > 0 Then
			...
			if  not rsget.EOF  then
				...
				do until rsget.eof
					...
					FItemList(i).fiscoupon		= rsget("iscoupon")										
					FItemList(i).fiscomment		= rsget("iscomment")					
					FItemList(i).fisbbs		= rsget("isbbs")					
					FItemList(i).fisapply		= rsget("isapply")
					FItemList(i).fisOnlyTen		= rsget("isOnlyTen")
					FItemList(i).fisoneplusone		= rsget("isoneplusone")
					FItemList(i).fisfreedelivery		= rsget("isfreedelivery")
					FItemList(i).fisbookingsell		= rsget("isbookingsell")
					FItemList(i).FEvt_name  	  = db2html(rsget("evt_name"))
					FItemList(i).FEvt_subcopyK    = db2html(rsget("evt_subcopyK")) 'pc용 이벤트 서브카피
					FItemList(i).FEvt_subname    = db2html(rsget("evt_subname")) 'm,app용 이벤트 서브카피
					FItemList(i).FEvt_bannerimg   = db2html(rsget("evt_bannerimg"))
					FItemList(i).fevt_mo_listbanner   = db2html(rsget("evt_mo_listbanner"))
					...
					FItemList(i).FIsusing 		= rsget("evt_using")
