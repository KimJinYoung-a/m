...
<% IF oclearancelist.FResultCount > 0 THEN %>
	<% 
		For i=0 To oclearancelist.FResultCount-1 
			...
			classStr = ""		
			linkUrl = replace(Request.ServerVariables("PATH_INFO"), "act_index.asp","index.asp") & "?" & GetParam("adtprdid=" & oclearancelist.FItemList(i).FItemID & "&cpg=")								
	...
	<li class="<%=classStr%>" <%=chkIIF(adultChkFlag, "onclick=""confirmAdultAuth('"& linkUrl &"', "& chkiif(IsUserLoginOK, "true", "false") &");""","")%>>				
		<a href="" onclick="fnAPPpopupProduct('<%= oclearancelist.FItemList(i).FItemID %>');return false;">
			...
			<% End If %>
			<div class="thumbnail"><img src="<%= getThumbImgFromURL(oclearancelist.FItemList(i).FImageBasic,286,286,"true","false") %>" alt="" /><%=CHKIIF(oclearancelist.FItemList(i).isSoldOut,"<b class='soldout'>일시 품절</b>","")%></div>
			<div class="desc">
