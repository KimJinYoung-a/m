...
<%
	...
	IF (oitem.Prd.FOptionCnt>0) then
		if (IsReceiveSiteItem) or (IsPresentItem) or (IsTicketItem) then
		    ...
		    ioptionLayerBoxHtml = GetBetweenLayerBoxHTML_Test(itemid, oitem.Prd.IsSoldOut)
		end if
	End If
...
<script type="application/x-javascript" src="/apps/appCom/between/lib/js/shoppingbag_script.js"></script>
<script>
				...
				<div class="pdtInfo boxMdl">
					...
					<div class="price">
						<% IF oItem.Prd.IsSaleItem THEN %>						
							<del><%= FormatNumber(oItem.Prd.getOrgPrice,0) %><% if oItem.Prd.IsMileShopitem then %> Point<% else %> 원<% end  if %></del> <span class="txtSaleRed"><%= FormatNumber(oItem.Prd.getRealPrice,0) %> 원</span> 
				...
				<div class="pdtContWrap">
						...
						<dl class="detailShort" >
							...
							<dd class="detailInfo" >
								<div class="detailView">
									<div id="lyLoading" style="position:relative;text-align:center; margin-left:-8px;"><img src="http://fiximage.10x10.co.kr/icons/loading16.gif" style="height:16px;" /></div>										
									<div>
					...
					<div class="detailCont" id="detail03">
						<div class="tenCs">
							...
							<p class="csMail"><strong><a href="mailto:customer@10x10.co.kr">customer@10x10.co.kr</a></strong></p>
				...
				<table class="tbl ctgyList">
					...
					<tbody>
						<tr>
							...
							<td <% If btvDisp="104" Then %> class="on"<% End If %>><a href="category_list.asp?disp=104&cv=1">푸드</a></td>
						</tr>
						<tr>
		...
		<div class="floatingBar boxMdl cartIn <% If oItem.Prd.IsSoldOut Then %><% Else %>optIs<% End If %>">
			<div class="inner">
				<div class="btnWrap">
					...
					<% Else %>
						<p class="btn01 btnCart"><a href="javascript:TnAddShoppingBag(true);" class="btw">장바구니<em class="newIco tenRed"><%=GetBetweenCartCount()%></em></a></p>
						<p class="btn01 btnOrder"><a href="javascript:TnAddShoppingBag(false);" class="edwPk">구매하기</a></p>
					<% End If %>
				</div>
				<% IF oItem.Prd.IsSoldOut Then %>
				<% Else %>
