...
if mode="vacationreg" then
	...
	if subscriptcount > 0 then
		Response.Write "<script type='text/javascript'>alert('참여는 1일 1회 가능 합니다.'); parent.top.location.href='/event/eventmain.asp?eventid="&eCode&"'</script>"
		dbget.close() : Response.End
	end if
	
	If vacationgubun = "" Then
		Response.Write "<script type='text/javascript'>alert('휴가 계획을 선택해 주세요.'); parent.top.location.href='/event/eventmain.asp?eventid="&eCode&"'</script>"
		dbget.close() : Response.End
	End IF

	sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript](evt_code, userid, sub_opt1, sub_opt2, sub_opt3, device)" + vbcrlf
	sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', '"& getnowdate &"', "& vacationgubun &", 'N', 'M')" + vbcrlf
