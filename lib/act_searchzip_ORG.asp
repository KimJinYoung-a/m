...
%>
	<li><p class="addr" onclick="DetailPost(this,'<%=rsget("ADDR_ZIP1")%>','<%=rsget("ADDR_ZIP2")%>','<%=useraddr01%>', '<%=useraddr02%>');"><span><%=rsget("ADDR_zip1")%>-<%=rsget("ADDR_zip2")%></span><%=strAddress%></p></li>
<%
			  rsget.MoveNext
			loop
		else
