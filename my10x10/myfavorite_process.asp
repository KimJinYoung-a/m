...
'	End If
		    ...
		    if IsArray(bufBagArr) then
    		    for intloop = 0 to ubound(bufBagArr)
    		        if (bufBagArr(intLoop)<>"") then
    		            Call fnUserLogCheck("fav", userid, bufBagArr(intLoop), "", "arr", "mw")
    		        end if
    		    next
    		end if
		end if
	...
	elseif (mode= "add") then
		...
		''2017/05/23 :: 위시 로그 추가==================================
		if (userid<>"") then
		    Call fnUserLogCheck("fav", userid, itemid, "", "", "mw")
		end if
		''==============================================================
...
'		dbget.close()
'		response.end
		response.write "<script>top.location.href='/my10x10/myWish/myWish.asp?ucid="&Server.UrlEncode(tenEnc(userid))&"'</script>"
		response.End
	End If
end if
...
%>
	<%
		'// 위시액션 쿠폰발급
		If Trim(fnWishActionCoupon(getEncLoginUserID))="2" Then
	%>
<% '임시 중지 2015-09-18 이종화 %>
<!-- 	<div class="heightGrid bgGry"> -->
<!-- 		<div class="container popWin"> -->
<!-- 			<div class="header"> -->
<!-- 				<h1>위시폴더</h1> -->
<!-- 				<p class="btnPopClose"><button class="pButton" onclick="<%=wishActionLinkUrl%>;">닫기</button></p> -->
<!-- 			</div> -->
<!-- 			<!-- content area --> -->
<!-- 			<div class="content" id="contentArea"> -->
<!-- 				<div class="wishAction"> -->
<!-- 					<p class="box1"><em>지금 텐바이텐 APP에서만 사용할 수 있는<br /> <strong>3천원 할인쿠폰</strong>을 지급해드렸습니다.</em> (1만원이상구매시, 24시간 이내사용)</p> -->
<!-- 					<div class="btnwrap"><span class="button btB1 btRed cWh1"><button type="button" onclick="<%=wishActionLinkUrl%>;">확인</button></span></div> -->
<!-- 					<ul> -->
