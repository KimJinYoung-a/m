...
<%
Dim vDate, vRankingCount, vCateType, sqlStr, vDateDisplayValue, vDaccuType, i
...
function fnChgDaccuRankingDate(v) {
    location.href = '<%=appUrlPath%>/diaryStory2019/daccu_ranking.asp?date='+v+'&daccutype=<%=vDaccuType%>';
}
...
function fnDiaryRankingBasketCheck(g, itemid) {
    ...
    if (g=="basket") {
        ...
        frm.action = "<%=appUrlPath%>/inipay/shoppingbag_process.asp?tp=pop";
        frm.target = "iiBagWin";
        frm.submit();
    }
    if (g=="option") {
