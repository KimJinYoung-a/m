			...
			For intAi = 0 To UBound(arrAddbanner,2)
				If arrAddbanner(1,intAi) <> "" Then
					If arrAddbanner(0,intAi) = "1" And (CStr(Date()) >= CStr(arrAddbanner(4,intAi)) and CStr(Date()) <= CStr(arrAddbanner(5,intAi))) Then
						tArea = tArea & "<a href='"& chkiif(arrAddbanner(3,intAi) <> "",arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode),"#") &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
					ElseIf arrAddbanner(0,intAi) = "2" And (CStr(Date()) >= CStr(arrAddbanner(4,intAi)) and CStr(Date()) <= CStr(arrAddbanner(5,intAi))) Then
						mArea = mArea & "<a href='"& chkiif(arrAddbanner(3,intAi) <> "",arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode),"#") &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
					ElseIf arrAddbanner(0,intAi) = "3" And (CStr(Date()) >= CStr(arrAddbanner(4,intAi)) and CStr(Date()) <= CStr(arrAddbanner(5,intAi))) Then
						bArea = bArea & "<a href='"& chkiif(arrAddbanner(3,intAi) <> "",arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode),"#") &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
					End If
				End If
			Next
		End If
...
$(function(){
	...
	<%' amplitude 이벤트 로깅 %>
		<% if trim(gaparamChkVal) <> "" then %>
			//tagScriptSend('', '<%=gaparamChkVal%>', '', 'amplitude');
		<% end if %>
		<% If ekind="28" Then %>
			//setTimeout("tagScriptSend('mktEvent', '<%=eCode%>', '', 'amplitudeProperties')", 100);
		<% Else %>
			//setTimeout("tagScriptSend('planEvent', '<%=eCode%>', '', 'amplitudeProperties')", 100);
		<% End If %>
	<%'// amplitude 이벤트 로깅 %>
});
...
<%
	...
	snpLink = "http://10x10.co.kr/event/eventmain.asp?eventid="&CStr(eCode)
	snpPre = "10x10 이벤트"
%>
<body class="default-font body-sub bg-grey"><%'!-- for dev msg : GNB메뉴에서 들어갈때만 body-main클래스가 붙습니다. 그 이외의 페이지에서는 body-sub가 붙습니다. --%>
	...
	<div id="content" class="content">
		...
		<!-- 2/19~ 20일 7억데이 배너 -->
		<div class="bnr">
			<div class="item-bnr-event"><a href="/event/eventmain.asp?eventid=84594"><img src="http://webimage.10x10.co.kr/eventIMG/2018/84594/m/bnr_detail_m.jpg" alt="미리 준비하는 올 봄 트렌드 키워드 기획전 바로가기"></a></div>
		</div>
		<% end if %>
		...
		<div class="event-article event-text-type"><!-- type 1 : text 타입 -->
			<section class="section-event hgroup-event">
				...
				<!-- 브랜드 링크 -->
				<% If ebrand<>"" Then %><div class="btnGroup"><a href="/street/street_brand.asp?makerid=<%=ebrand%>" class="btnV16a"><%=SocName_Kor%></a></div><% end if %>
			</section>
		</div>
