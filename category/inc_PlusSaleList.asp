		...
		<div class="swiper-container">
			<ul class="swiper-wrapper">
				...
				<li class="swiper-slide">
					<a href="/category/category_itemPrd.asp?itemid=<%= oPlusSaleItem.FItemList(bi).Fitemid %>" onclick="fnAmplitudeEventMultiPropertiesAction('click_plus_sale_in_product','indexnumber|itemid|categoryname|brand_id','<%=bi%>|<%= oPlusSaleItem.FItemList(bi).Fitemid %>|<%=fnItemIdToCategory1DepthName(oPlusSaleItem.FItemList(bi).Fitemid)%>|<%=fnItemIdToBrandName(oPlusSaleItem.FItemList(bi).Fitemid)%>');">
