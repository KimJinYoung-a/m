...
<body class="default-font body-main">
	...
	<div id="content" class="content">
		<main>
            ...
            <a href="/diarystory2022/index.asp?gaparam=today_banner" class="bnr_main_dr mWeb"></a>
            <% else %>
            <a href="https://m.10x10.co.kr/common/news/news_view.asp?type=&idx=19266&page=1" class="bnr_main_dr mWeb"></a>
            <% end if %>

			<%'// 마케팅 배너 %>
			<div id="mktbanner" class="marketing-bnr" v-cloak>
				<main-banner v-for="(item,index) in items" v-if="item.type == 'marketingRolling'" :item="item" :index="index"/>
			</div>
			<%'// 공지사항 %>
				...
				<div class="items type-multi-row swiper-container">
					<ul class="swiper-wrapper">
					    <li v-for="(items,index) in mixItems" class="swiper-slide">
					        <main-mdpick v-for="(item,order) in items" :sub="item" :key="item.itemId" :index="(index*2)+order+1"></main-mdpick>
					        <a v-if="items.length === 1" href="<%=mdpickUrl%>?gaparam=today_mdpick_plus" title="md&#39;s pick 더 보러가기" class="btn-more">+45</a>
                        </li>
						<li v-if="isEvenItemsLength" class="swiper-slide">
							<a href="<%=mdpickUrl%>?gaparam=today_mdpick_plus" title="md&#39;s pick 더 보러가기" class="btn-more">+45</a>
						</li>
					</ul>
				</div>
			</section>
			...
			<div id="imgbanC" class="gif-bnr" v-cloak>
				<main-banner v-for="item in items" :item="item" v-if="item.type == 'imageBannerC'"/>
			</div>
				...
				<div class="list-card type-align-left">
					<ul>
						<main-eventban v-for="(item, index) in items" :key="item.title1"
						    :item="item" :index="index+1" :start="0"/>
					</ul>
				</div>
			</section>
			...
			<section id="imgbanA" class="text-bnr" v-cloak>
				<img-banner v-for="item in items" :item="item" :key="item.mainCopy" v-if="item.type == 'imageBannerA'"/>
			</section>

			<%'//twinitems %>
			<div id="twinitems" class="items-single-bnr" v-cloak>
				<twin-item v-for="twinItem in twinItems" :items="twinItem.items"/>
			</div>
			...
			<section id="saleSwiper" class="item-carousel on-sale-items" v-cloak>
				...
				<ul class="swiper-wrapper">
					<li v-for="(items,index) in mixItems" class="swiper-slide">
						<main-sale v-for="(item,order) in items" :sub="item" :key="item.itemId" :index="(index*2)+order+1"/>
					</li>
				</ul>
			</div>
			</section>
			...
			<section id="enjoyevent2" class="exhibition-list" v-cloak>
				...
				<div class="list-card type-align-left">
					<ul>
						<main-eventban v-for="(item, index) in items" :key="item.title1"
						    :item="item" :index="index+1" :start="3"/>
					</ul>
				</div>
			</section>
			...
			<div id="mainkeyword" v-cloak>
				<section class="hot-keyword" v-for="item in items" :item="item" :style="{background:item.backgroundColor}">
					<h2 class="headline">
						...
						<small><a :href="item.link">#{{item.keyword}}</a></small>
					</h2>
					<ul>
			...
			<div id="enjoySwiper" v-cloak>
				<section class="item-carousel enjoy-items" v-for="item in items" :item="item">
					...
					<div class="swiper-container items type-box-grey">
						<ul class="swiper-wrapper">
							<main-itemlist v-for="(sub,index) in item.exhibition1" :sub="sub" :index="index+1" :key="sub.id"></main-itemlist>
						</ul>
					</div>
				</section>
			</div>
			...
			<section id="mainbrand" class="brand-bnr" v-cloak>
				<brand-banner v-for="item in items" :item="item"/>
			</section>
			...
			<section id="imgbanB" class="text-bnr" v-cloak>
				<img-banner v-for="item in items" :item="item" :key="item.mainCopy" v-if="item.type == 'imageBannerB'"/>
			</section>
			...
			<section id="enjoyevent3" class="exhibition-list" v-cloak>
				...
				<div class="list-card type-align-left">
					<ul>
						<main-eventban v-for="(item, index) in items" :key="item.title1"
						    :item="item" :index="index+1" :start="6"/>
					</ul>
				</div>
			</section>
			...
			<section id="newSwiper" class="item-carousel new-items" v-cloak>
				...
				<div class="swiper-container items type-box-grey">
					<ul class="swiper-wrapper">
						<main-itemlist v-for="(sub,index) in items" :sub="sub" :index="index+1" :key="sub.itemId"/>
					</ul>
				</div>
			</section>
			...
			<section id="enjoyevent4" class="exhibition-list" v-cloak>
				...
				<div class="list-card type-align-left">
					<ul>
						<main-eventban v-for="(item, index) in items" :key="item.title1"
                            :item="item" :index="index+1" :start="9"/>
					</ul>
				</div>
			</section>
<%'//가이드배너 추가 위치 최종원 20180807%>
			...
			<div id="guideList" class="marketing-bnr" v-cloak>
				<main-banner v-for="item in items" :item="item" v-if="item.type == 'guideBanner'"/>
			</div>
<%'//가이드배너 추가 위치 최종원 20180807%>
			...
			<div id="HCPlist" class="contents-bnr" v-cloak>
				<div v-for="item in items" :item="item" :key="item.mainCopy" v-if="item.type == 'HCP'">
					...
					<section class="culture-bnr" v-else-if="item.contentsType == 'cultureStation'">
						<hcp-list :item="item"/>
						<div class="btn-group">
						    <a href="/culturestation/index.asp?gaparam=today_HCP_c0" class="btn-plus color-blue">
						        <span class="icon icon-plus icon-plus-blue"></span> 더 많은 컬쳐스테이션
                            </a>
                        </div>
					</section>
				</div>
			...
			<%'// Today 더보기 카테고리%>
			<div class="category-item-content bg-grey" id="todaymore" v-if="show && sliced" v-cloak>
					...
					<div class="items">
						<ul>
							<cateitem-more v-for="(sub, index) in item.items" :key="sub.itemId" :item="sub"
							    :code="item.categoryCode" :index="index+1"/>
						</ul>
					</div>
            ...
            <div class="category_banner">
                <a href="/diarystory2022/index.asp?gaparam=today_banner">
                    <img src="http://fiximage.10x10.co.kr/m/2021/diary/bnr_big_diary2022_01.png" alt="2022 다이어리 준비하셨나요?">
                </a>
            </div>
		</main>
	</div>
...
<script src="/vue/contents.js?v=1.75"></script>
