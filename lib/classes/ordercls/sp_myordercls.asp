...
class CMyOrderDetailItem
    ...
    '' 마스터 현재상태를 같이 넘겨야함.
    public function GetItemDeliverStateNameNew(CurrMasterIpkumDiv, CurrMasterCancelyn, CurrMasterBaljuDate, TenbeasongCnt, CurrMasterIpkumDate, CurrMasterjumundiv)
        if ((CurrMasterCancelyn="Y") or (CurrMasterCancelyn="D") or (Fcancelyn="Y")) then
            GetItemDeliverStateNameNew = "취소"
        elseif CurrMasterjumundiv="6" then
            GetItemDeliverStateNameNew = "교환 완료"
		elseif CurrMasterjumundiv="9" then
            GetItemDeliverStateNameNew = "반품 완료"
		else
            if (CurrMasterIpkumDiv="0") then
            ...
            elseif (CurrMasterIpkumDiv="9") then
                ...
                if (IsNull(Fcurrstate) or (Fcurrstate=0)) then
					if TenbeasongCnt<1 then
						...
						if (datediff("n",CurrMasterIpkumDate,now()) >= 30) then
    ...
    public function GetSongjangURL_app()
		...
		if (FDeliveryURL="" or isnull(FDeliveryURL)) or (FSongjangNO="" or isnull(FSongjangNO)) then
			GetSongjangURL_app = "<span onclick=""alert('▷▷▷▷▷ 화물추적 불능안내 ◁◁◁◁◁\n\n고객님께서 주문하신 상품의 배송조회는\n배송업체 사정상 조회가 불가능 합니다.\n이 점 널리 양해해주시기 바라며,\n보다 빠른 배송처리가 이뤄질수 있도록 최선의 노력을 다하겠습니다.');"" style=""cursor:pointer;"">" & FSongjangNO & "</span>"
		else
...
class CMyOrderMasterItem
    ...
    public function IsForeignDeliver()
        IsForeignDeliver = (Not IsNULL(FDlvcountryCode)) and (FDlvcountryCode<>"") and (FDlvcountryCode<>"KR") and (FDlvcountryCode<>"ZZ") and (FDlvcountryCode<>"Z4") and (FDlvcountryCode<>"QQ")  ''2018/06/21 QQ(퀵배송 추가)
    end function
...
'    end function
    ...
    '' 이니렌탈 취소 Type
    public function IsInirentalCancelRequire(IsAllCancell)
        IsInirentalCancelRequire = false

        if (Not IsPayed) then Exit function

        '' 이니렌탈 or All@ And 전체취소인경우
        if ((Faccountdiv="150")) and (IsAllCancell) then IsInirentalCancelRequire=true
    end function
	...
	function GetIpkumDivNameNew()
        ...
        select case oipkumdiv
            ...
            case "4"
                : if (datediff("n",Fipkumdate,now()) >= 30) then
					GetIpkumDivNameNew = "상품 확인 중"
				else
					GetIpkumDivNameNew = "결제 완료"
				end if
           case "5"
                : if FTenbeasongCnt < 1 then	'업체배송만 있을때
					GetIpkumDivNameNew = "상품 확인 중"
				else
...
Class COrderAddSongjangItem
    ...
    public function GetDeliveryName()
        GetDeliveryName = Fsongjangdivname
    end function

    public function GetSongjangURL()
		if (Ffindurl="" or isnull(Ffindurl)) or (FSongjangNO="" or isnull(FSongjangNO)) then
			GetSongjangURL = "<span onclick=""alert('▷▷▷▷▷ 화물추적 불능안내 ◁◁◁◁◁\n\n고객님께서 주문하신 상품의 배송조회는\n배송업체 사정상 조회가 불가능 합니다.\n이 점 널리 양해해주시기 바라며,\n보다 빠른 배송처리가 이뤄질수 있도록 최선의 노력을 다하겠습니다.');"" style=""cursor:pointer;"">" & FSongjangNO & "</span>"
		else
			GetSongjangURL = "<a class=""detailItem"" href=" & db2html(Ffindurl) & FSongjangNO & " target=""_blank"">" & FSongjangNO & "</a>"
		end if
    end function
    ...
    public function GetSongjangURL_app()
		if (Ffindurl="" or isnull(Ffindurl)) or (FSongjangNO="" or isnull(FSongjangNO)) then
			...
			GetSongjangURL_app = "<a class=""detailItem"" href="""" onclick=""fnAPPpopupExternalBrowser('"  & db2html(Ffindurl) & FSongjangNO & "'); return false;"">" & FSongjangNO & "</a>"
		end if
    end function
...
Class CMyOrder
    ...
    public Sub GetAddSongjangList()
        ...
        sqlStr = sqlStr + " from "
        sqlStr = sqlStr + " 	[db_order].[dbo].[tbl_order_add_songjang] a "
        sqlStr = sqlStr + " 	left join [db_order].[dbo].tbl_songjang_div s on a.songjangdiv=s.divcd "
	...
	public Sub getMainPaymentInfo(byval paymethod, byref orgpayment, byref cardcancelok, byref cardcancelerrormsg, byref cardcancelcount, byref cardcancelsum, byref cardcode)
		...
		if (paymethod <> "100") then
			...
			elseif (paymethod = "20") and (pggubun="TS") then                              ''2019/10/22 추가
				...
				cardcancelcount = 0
				cardcode = payetcresult
			elseif (paymethod = "20") and (pggubun="KK") then                              ''2020/12/07 추가
			    cardcancelok = "Y"
				cardcancelcount = 0
				cardcode = payetcresult
			elseif (paymethod = "20") and (pggubun="CH") then                              ''2020/12/07 추가
			    cardcancelok = "Y"
				cardcancelcount = 0
				cardcode = payetcresult
			elseif (paymethod = "20") and (pggubun="PY") then                              ''2021/01/28 추가
			    cardcancelok = "Y"
				cardcancelcount = 0
				cardcode = payetcresult
			else
...
'			end if
	...
	public Sub GetOrderDetail()
		...
		'/출고지연 상품		'/2016.03.31 한용민 추가
		'arrmibeadelay = fnmibeadelay(FRectOrderserial)

		'/택배파업 상품		'/2022.01.11 한용민 추가
	...
	public Sub GetOrderResultDetail()
        ...
        if Not rsget.Eof then
			do until rsget.Eof
				...
				'/품절출고불가 상품		'/2016.03.31 한용민 추가
				if rsget("cancelyn")="N" And rsget("currstate") < "7" and rsget("itemlackno") > 0 then
		            if InStr(arrmibeasoldout, rsget("idx")) > 0 then
		            	FItemList(i).Fmibeasoldoutyn = "Y"
		        	end if
					'/출고지연 상품		'/2016.03.31 한용민 추가
    ...
    public Sub GetMyOrderList()
		...
		if FrectSiteName<>"" then
            if (FrectSiteName = "10x10") then
                sqlStr = sqlStr + " and m.sitename in ('10x10', '10x10_cs') "
            else
...
'// 무통장 입금의 입금 마감일 가져오기
Function fnGetCyberAccountEndDate(orderserial)
	if (orderserial="") then Exit function
	Dim vQuery, vTemp
	vQuery = "SELECT TOP 1 CLOSEDATE FROM [db_order].[dbo].[tbl_order_CyberAccountLog] WITH(NOLOCK) "
	vQuery = vQuery & " WHERE orderserial = '" & orderserial & "' AND isDelete='N'"
	vQuery = vQuery & " ORDER BY regdate DESC"
	rsget.CursorLocation = adUseClient
	rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
	if not rsget.eof then
		vTemp = rsget(0)
	...
	rsget.close
	fnGetCyberAccountEndDate = vTemp
End Function
%>
