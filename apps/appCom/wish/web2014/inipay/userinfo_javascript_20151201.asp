	...
	$(function() {
		...
		$("input[name='rRange']:checked").parent().parent().parent().parent().parent().children('dd').show();
		$("input[name='rRange']:checked").parent().parent().parent().parent().parent().addClass('open');

        $("input[name='dRange']:checked").parent().parent().parent().parent().parent().children('dd').show();
		$("input[name='dRange']:checked").parent().parent().parent().parent().parent().addClass('open');
	...
	function copyDefaultinfo(comp){
		...
		$("#addressOption4").removeClass("active");
				...
				$.ajax({
					...
					success: function(rst) {
						...
						if($(rst).find("item").length>0) {
							vRtn = '<select name="chgmyaddr" class="chgmyaddr form full-size" title="저장된 나의 주소록">';
							...
							vRtn = '<span>등록된 나의 주소록이 없습니다.</span>';
						}
						$("#myaddressSelArea").html(vRtn);
						FnSetChgMyAddr();
					}
					,error: function(err) {
			...
			if($("#recentOrderSelArea").html()=="") {
				$.ajax({
					...
					success: function(rst) {
						var vRtn="", vLp=1;
						if($(rst).find("item").length>0) {
							vRtn = '<select name="chgmyaddr" class="chgmyaddr form full-size" title="과거 배송지">';
							...
							vRtn = '<span>최근 주문배송 내역이 없습니다.</span>';
						}
						$("#recentOrderSelArea").html(vRtn);
						FnSetChgMyAddr();
					}
					,error: function(err) {
						alert(err.responseText);
