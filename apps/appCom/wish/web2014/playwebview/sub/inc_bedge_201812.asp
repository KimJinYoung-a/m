...
dim presentDay
presentDay = #12/16/2018 23:59:59#
...
function jsSubmitComment(frm){		
	<% If IsUserLoginOK() Then %>	
		<% if date() >="2018-12-08" and date() < "2018-12-16" then %>
...
<article class="playDetailV16 thingthing">
	...
	<% if now() <= presentDay then %>
		<div>
			...
			<h2>나의 <b style="color:#633023;">이름</b>을<br>지어주세요!</h2>
		</div>
	<% else %>	
		<div>
			<em class="month"><span>12월 THING</span></em>
			<p class="id">텐바이텐 X yui**** 고객님</p>
			<h2>내 이름은 <b style="color:#633023;">웜어</b></h2>
		</div>
	<% end if %>	
	</div>				
	<div class="badge-detail">
		<div id="thingRolling" class="swiperFull">
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<div class="swiper-slide"><img src="http://webimage.10x10.co.kr/fixevent/play/badge/img_badge_201812_1.jpg" alt=""></div>
					<div class="swiper-slide"><img src="http://webimage.10x10.co.kr/fixevent/play/badge/img_badge_201812_2.jpg" alt=""></div>
					<div class="swiper-slide"><img src="http://webimage.10x10.co.kr/fixevent/play/badge/img_badge_201812_3.jpg" alt=""></div>
				</div> 
				<div class="paginationDot"></div>
			</div>
		</div>
		<div class="itemDesc" style="background-color:#633023;">
		<% if now() <= presentDay then %>
			<p>이렇게 맛있는 손난로를 보셨나요?<br/>
				한입 베어 물면 김이 모락모락 ~<br/>
				나의 이름을 지어주세요!
			</p>
		<% else %>
		...
		<% end if %>
			<div class="btnGet">
				<a href="/category/category_itemPrd.asp?itemid=2160246" onclick="TnGotoProduct('2160246');return false;">배지 구매하러가기</a>
			</div>
		</div>
		...
		<% if now() <= presentDay then %>					
			<div class="desc">
				<p class="msg" style="color:#633023;">
					<span>텐바이텐과 당신이 함께 만드는 친구!</span>
					지어주신 이름 중 하나를 뽑아 <br />
					12월의 THING 배지가 한정 출시됩니다.<br />
					위트있는 이름을 지어주신 <br />
					작명가에게는 본 배지를 드립니다.<br />
					( 1명:5개 / 15명:1개 증정 )
				</p>
				<p class="date">응모기간 : 12.10 ~ 12.16 <span>|</span> 발표 : 12.17</p>
			</div>
			<div class="form">
				...
				<input type="hidden" name="spoint"/>						
					...
					<legend class="hidden">어울리는 이름 짓기</legend>
						<input type="text" maxlength="12" id="txtcomm" onClick="jsCheckLimit();" name="txtcomm" title="이름 입력" placeholder="내 이름은" style="border-color:#633023;" />
						<input type="button" onclick="jsSubmitComment(document.frmcom);return false;" value="이름 지어주기" style="background-color:#633023; color:#fff; margin-top:0.75rem;" />
					</fieldset>								
				</form>		
		...
		<% else %>	
			<div class="other">
				<h3>그 외의 작명센스!</h3>
				<ul>
					<li><span style="border-color:#633023; color:#633023;">팥꼬기</span></li>
					<li><span style="border-color:#633023; color:#633023;">붕어빵긋</span></li>
					<li><span style="border-color:#633023; color:#633023;">따따뿡</span></li>
					<li><span style="border-color:#633023; color:#633023;">:) 33 =</span></li>
					<li><span style="border-color:#633023; color:#633023;">잼아저씨의야심작</span></li>
					<li><span style="border-color:#633023; color:#633023;">팥찌</span></li>
				</ul>
			</div>							
		<% end if %>						
		</div>
		...
		<div class="listComment" id="comment">
			...
			<ul>
				...
				<li>
					<span>내이름은 <em style="color:#633023;"><%=ReplaceBracket(db2html(arrCList(1,intCLoop)))%></em>
