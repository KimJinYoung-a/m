	...
	$(window).scroll(function() {
		if ($(window).scrollTop() >= ($(document).height()-$(window).height())*0.85){
			if(vScrl) {
				...
				vPg++;
				$.ajax({
					url: "act_awardSteady.asp?atype=<%=trim(atype)%>&disp=<%=vDisp%>&cpg="+vPg,
					cache: false,
	...
	<div class="container" style="background-color:#e7eaea;">
		...
		<div class="content" id="contentArea">
			<ul class="commonTabV16a">
				...
				<li class="current" name="tab04" style="width:25%;" onclick="location.replace('awardSteady.asp');return false;">STEADY</li>
			</ul>
			<!-- MAN BEST -->
