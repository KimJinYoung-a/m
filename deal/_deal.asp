...
<script type="application/x-javascript" src="/deal/itemPrdDetail.js"></script>
...
$(function() {
	...
	/* dropdown */
	$(".dropdwon-box .btnDrop").on("click", function(e){
		e.stopPropagation();
	...
	$(".dropdwon-box .dropBox ul li a").on("click", function(e){
	...
	$(document).on("click", function(e){
		$(".dropdwon-box .dropBox + .dropBox").hide();
		$(".dropdwon-box .btnDrop").removeClass("on");
		$(".dropdwon-box .dropBox").removeClass("on");
	});
	...
	<div id="content" class="content">
		...
		<section class="items item-detail">
			<div class="desc">
				...
				<span class="brand" id="brandshow" style="display:none"><a href="/street/street_brand.asp?makerid=<%=oItem.Prd.Fmakerid%>"><%=oItem.Prd.FBrandName%></a></span>
...
function fnDealOtherItemView(itemid,viewnum){
	var popwin = window.open("/deal/deal_view.asp?itemid=" + itemid + "&viewnum="+viewnum+"&dealitemid=<%=itemid%>",'popDealView');
	popwin.focus();
}
//-->
</script>
...
function fnDealItemBasicInfoView(itemid){
	$.ajax({
		url: "act_itembasicinfo.asp?itemid="+itemid,
		cache: false,
		async: false,
		success: function(message) {
			//alert(message);
...
<%
Dim oDealEval, ArrDealItemEval, ArrDealItemQNA
...
Set  oDealEval = Nothing
%>
					<div id="tab03" class="postTxtV16a" style="display:none;">
						<div class="dropdwon-box">
							<div class="dropDown">
								<button type="button" class="btnDrop">[상품1] <%=ArrDealItemEval(1,0)%></button>
								<div class="dropBox multi">
									<% If isArray(ArrDealItemEval) Then %>
									<ul>
										<% For intLoop = 0 To UBound(ArrDealItemEval,2) %>
										<% DealItemEvalTotalCNT = DealItemEvalTotalCNT + ArrDealItemEval(2,intLoop) %>
										<li><a href="" onclick="fnDealItemEvalView(<%=ArrDealItemEval(0,intLoop)%>);return false;"><div class="option">[상품<%=intLoop+1%>] <%=ArrDealItemEval(1,intLoop)%> <em class="value"><%=FormatNumber(ArrDealItemEval(2,intLoop),0)%>건</em></div></a></li>
										<% Next %>
									</ul>
									<% End If %>
								</div>
...
<script>
function fnDealItemEvalView(itemid){
	$.ajax({
		url: "act_itemEvaluate.asp?itemid="+itemid+"&tcnt=<%=formatNumber(DealItemEvalTotalCNT,0)%>",
		cache: false,
		async: false,
		success: function(message) {
			//alert(message);
...
$(function(){
	fnDealItemEvalView(<%=ArrDealItemEval(0,0)%>);
	$("#evalTotal").empty().append("(<%=formatNumber(DealItemEvalTotalCNT,0)%>)");
});
</script>
...
$(function(){
	fnDealItemQnaView(<%=ArrDealItemEval(0,0)%>);
	...
	<div class="itemFloatingV16a itemFloatingV17"><!-- opening16a -->
		...
		<div class="btnAreaV16a">
			<% If IsUserLoginOK() Then %>
			<p><button type="button" class="btnV16a btnWishV16a<%=chkIIF(isMyFavItem," actWish","")%>" onclick="TnAddFavoritePrd('<% = oDeal.Prd.FMasterItemCode %>');return false;"><%=FormatNumber(ofavItem.Prd.FfavCount,0)%></button></p>
			<% Else %>
			<p><button type="button" class="btnV16a btnWishV16a<%=chkIIF(isMyFavItem," actWish","")%>" onclick="top.location.href = '/login/login.asp?backpath=<%=Server.URLencode(CurrURLQ())%>';return false;"><%=FormatNumber(ofavItem.Prd.FfavCount,0)%></button></p>
			<% End If %>
			<% If oItem.Prd.isDealSoldout or oItem.Prd.isTempSoldOut Then	'### 일반, 포토북, 티켓 품절일 경우 %>
