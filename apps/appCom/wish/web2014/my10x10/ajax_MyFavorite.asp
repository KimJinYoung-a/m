...
if instr(backurl,"/apps/appCom/wish/web2014/my10x10/ajax_MyFavorite.asp") > 0 then
	backurl=""
end if	
...
<script type="text/javascript">
	...
	function jseditFolder(){	
		...
		var rstStr = $.ajax({
			type: "POST",
			url: "/apps/appCom/wish/web2014/my10x10/myfavorite_folderProc.asp",
			data: "sFN="+sFN+"&viewisusing="+tmpviewisusing+"&hidM=I&ispop=<%=vIsPop%>&bagarray=<%=bagarray%>mode=<%=mode%>&itemid=<%=itemid%>&backurl=/apps/appCom/wish/web2014/category/category_itemPrd.asp?itemid=<%= itemid %>",
			dataType: "text",
			async: false
		}).responseText;

		if (rstStr > 0){
			jsOpenModal('/apps/appCom/wish/web2014/my10x10/ajax_MyFavorite.asp?ispop=ajax&mode=add&itemid=<%=itemid%>&backurl=<%=Replace(CurrURLQ(),"&","^")%>')
			return;
		}else if(rstStr ==-1){
...
<form name="frmW" method="post" action="/apps/appCom/wish/web2014/my10x10/myfavorite_process.asp" style="margin:0px;">
    ...
    <div class="box">
        ...
        <div class="modal-body">
        	<div class="iscroll-area">
	        	...
	        	<form name="frmF" method="post" action="/apps/appCom/wish/web2014/my10x10/myfavorite_folderProc.asp" style="margin:0px;" onsubmit="return false;">
	        	<input type="hidden" name="hidM" value="I">
	        	<input type="hidden" name="ispop" value="<%=vIsPop%>">
	        	<input type="hidden" name="bagarray" value="<%=bagarray%>">
	    		<input type="hidden" name="mode" value="<%=mode%>">
	    		<input type="hidden" name="itemid" value="<%=itemid%>">
	        	<input type="hidden" name="backurl" value="/apps/appCom/wish/web2014/category/category_itemPrd.asp?itemid=<%= itemid %>">
