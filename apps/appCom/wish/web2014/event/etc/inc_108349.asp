...
<%
	...
	IF application("Svr_Info") = "Dev" THEN
		eCode  = 103230 '// 매일리지 코드
        mobileEventCode = 103229 '// 모바일 이벤트 코드
	Else
		eCode  = 108349 '// 매일리지 코드
        mobileEventCode = 108348 '// 모바일 이벤트 코드
	End If
	...
	if mktTest then
		if request("checkday")="1" then
			currenttime=#12/21/2020 00:00:00#
		elseif request("checkday")="2" then
			currenttime=#12/22/2020 00:00:00#
		elseif request("checkday")="3" then
			currenttime=#12/23/2020 00:00:00#
		elseif request("checkday")="4" then
			currenttime=#12/24/2020 00:00:00#
		elseif request("checkday")="5" then
			currenttime=#12/25/2020 00:00:00#
		elseif request("checkday")="6" then
			currenttime=#12/26/2020 00:00:00#
		elseif request("checkday")="7" then
			currenttime=#12/27/2020 00:00:00#
		elseif request("checkday")="8" then
			currenttime=#12/28/2020 00:00:00#
		elseif request("checkday")="9" then
			currenttime=#12/29/2020 00:00:00#
		end if
	end if

	vEventStartDate = "2020-12-21"
	vEventEndDate = "2020-12-29"
...
.maeileage .topic {position:relative; background:#171b3a;}
.maeileage h2, .attendance, .today-noti {position:relative; z-index:5;}
.chk-attendance {width:100%;}
.today-noti span {position:absolute; left:0; top:-1rem; width:100%; animation:bounce 1s 20;}
...
.dc-group .dc1 {top:31.6%; margin-left:-55%; width:10rem; height:6.23rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2020/106014/m/img_dc_1.png); animation-name:move4; animation-duration:25s; animation-direction:normal;}
.dc-group .dc2 {top:37.7%; margin-left:-18.7%; width:1.2rem; height:1.2rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2020/106014/m/img_dc_2.png); animation-name:move2; animation-duration:10s; animation-direction:alternate-reverse;}
.dc-group .dc3 {top:-1.7%; margin-left:23.7%; width:3rem; height:3rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2020/106014/m/img_dc_3.png);animation-name:move1; animation-duration:13s;}
.dc-group .dc4 {top:78.7%; margin-left:-45.3%; width:2.7rem; height:2.7rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2020/106014/m/img_dc_4.png); animation-name:move1; animation-duration:13s;}
.dc-group .dc5 {top:23%; margin-left:40.7%; width:1.5rem; height:3rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2020/106014/m/img_dc_5.png); animation-name:move3; animation-duration:10s;}
.dc-group .dc6 {top:88.2%; margin-left:31.7%; width:3.4rem; height:3.4rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2020/106014/m/img_dc_6.png); animation-name:move2; animation-duration:15s;}
@keyframes move1 {
...
.lyr-daily-chk .tomorrow-chk {position:absolute; left:0; top:58%; width:100%; text-align:center; color:#444; font-size:1.58rem; line-height:1.3; font-family:'CoreSansCBold', 'AppleSDGothicNeo-Bold', 'NotoSansKRBold';}
.lyr-daily-chk .tomorrow-chk strong {font-weight:700; font-family:'Roboto'; color:#9e0202;}
...
.lyr-daily-chk .lyr-finish .point {top:34%;}
.daily-board {overflow:hidden; width:100%; background:#0a4a37;}
...
.daily-board li .daily-box p {color:#fff; font-size:1.32rem; letter-spacing:-.13rem;}
...
.daily-board li.current:before {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-top:3px solid #9e0202; z-index:10; transform-origin:100% 0;}
.daily-board li.current:after {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-right:3px solid #9e0202; z-index:10; transform-origin:0 100%;}
...
.daily-board li.current .current-fr:before {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-bottom:3px solid #9e0202; z-index:10; transform-origin:0 100%;}
.daily-board li.current .current-fr:after {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-left:3px solid #9e0202; z-index:10; transform-origin:0 0;}
...
.daily-board li.previous .daily-box strong {color:#a3b3ae;} /* 지난회차 포인트 */
.daily-board li.done .daily-box p {color:#a3b3ae;}
...
$(function() {
	...
	function jsMaeilageSubmit(){
		...
		$.ajax({
			type:"GET",
			<% if mktTest then %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript108349.asp?mode=add&checkday=<%=request("checkday")%>",
			<% else %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript108349.asp?mode=add",
			<% end if %>
			dataType: "text",
			async:false,
			cache:true,
	...
	function jsMaeilagePushSubmit(){
		...
		$.ajax({
			type:"GET",
			<% if mktTest then %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript108349.asp?mode=pushadd&checkday=<%=request("checkday")%>",
			<% else %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript108349.asp?mode=pushadd",
			<% end if %>
			dataType: "text",
			async:false,
			cache:true,
...
<% '<!-- MKT 매일리지 108349 --> %>
<div class="mEvt108349 maeileage">
    <div class="topic">
        <% '<!-- for dev msg : 타이틀 이미지 파일명 첫째날만 tit_maeileage_1 둘째날부터 tit_maeileage_2 --> %>
        <% If Left(currenttime, 10) > Left(vEventStartDate, 10) Then %>
            <h2><img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/tit_maeileage_2.png" alt="매일리지"></h2>
        <% Else %>
            <h2><img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/tit_maeileage_1.png" alt="매일리지"></h2>
        <% End If %>
        ...
        <div class="attendance">
            ...
            <div class="get-mileage" onclick="fnAmplitudeEventMultiPropertiesAction('click_event_my10x10','','', function(bool){if(bool) {fnAPPpopupBrowserURL('마일리지 내역','<%=wwwUrl%>/apps/appCom/wish/web2014/offshop/point/mileagelist.asp','right','','sc');}});return false;">
                ...
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/bg_now_mileage.png" alt="현재까지 받은 마일리지">
            </div>
            <button type="button" id="chkAttendance" class="chk-attendance" onclick="jsMaeilageSubmit();return false;" <% If TodayDateCheck > 0 Then %>style="display:none"<% Else %><% End If %>><img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/btn_attendance.png" alt="출석 체크하기"></button>
            <span class="chk-attendance-ok" id="attendancecheckok" <% If TodayDateCheck > 0 Then %><% Else %>style="display:none"<% End If %>><img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/btn_attendance_ok.png" alt="출석 완료"></span>
        </div>

        <div class="today-noti">
            <span><img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/txt_restart.png" alt="하루라도 놓치면 100p부터 시작"></span>
            <img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/txt_noti.png" alt="소멸됩니다">
        </div>
        <div class="dc-group">
    ...
    <div id="lyrDailyChk" class="lyr-daily-chk">
        <% If Not(Left(currenttime, 10) >= vEventEndDate) Then %>
            <div>
                ...
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/lyr_bg.png" alt="">
            </div>
        <% Else %>
            <% '<!-- 10/6 마지막날 노출 레이어 --> %>
            <div class="lyr-finish">
                ...
                <strong class="point"><em id="today-mileage-value"></em>p</strong>
                <button type="button" id="btnClose" class="btn-close">닫기</button>
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/lyr_bg_finish.png" alt="">
            </div>
        <% End If %>
    </div>
    <div id="mask"></div>
    ...
    <div class="step-process">
        <% If IsUserLoginOK() Then %>
            ...
            <%
                ...
                vQuery = vQuery & "	From db_temp.[dbo].[tbl_maeliageDate108349] m "
                rsget.CursorLocation = adUseClient
                rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
                If Not(rsget.bof Or rsget.eof) Then
                    Do Until rsget.eof
    ...
    <% '<!-- for dev msg : 이벤트 마지막날 신청,설정 영역 사라짐 --> %>
    <% If Left(currenttime, 10) < Left(vEventEndDate, 10) Then %>
        <div class="push-request">
            <button type="button" onclick="jsMaeilagePushSubmit();return false;">
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/btn_push_alarm.png" alt="내일도 매일리지 알림 받기">
            </button>
        </div>
        <!--<div class="push-way">
            <button type="button" onclick="fnAPPpopupSetting();return false;">
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/btn_push_setting.png" alt="PUSH 알림 설정하러 가기">
            </button>
        </div>-->
    <% End If %>
    <% '<!-- 알림신청 팝업 --> %>
    <div id="lyrPush" class="lyr-push">
        ...
        <img src="//webimage.10x10.co.kr/fixevent/event/2020/108349/m/lyr_push.png" alt="">
    </div>
    <div id="mask2"></div>
    ...
    <div class="noti">
        ...
        <ul>
			...
			<li>- 지급된 마일리지는 스페셜 마일리지로, 2021년 1월 1일 00:00까지 사용 가능합니다. 미사용 시 자동으로 소멸됩니다.</li>
        </ul>
    </div>
</div>
<% '<!-- // MKT 매일리지 --> %>
