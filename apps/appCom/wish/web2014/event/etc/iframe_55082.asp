...
<!-- #include virtual="/apps/appcom/wish/web2014/event/etc/event55082Cls.asp" -->
...
.appshow .scheduleList {position:relative; z-index:50; padding:0 10px 20px 10px;}
...
function pagereload(itemid){
	...
	<% if isApp="1" then %>
		parent.fnAPPpopupProduct(itemid);
		return false;
	<% else %>
		parent.location.href='<%=appUrlPath%>/category/category_itemprd.asp?itemid='+itemid;
		return false;
	<% end if %>
}
		...
		<% else %>
			...
			<% else %>
				<% if subscriptcount>0 or bonuscouponcount>0 then %>
					alert("이미 쿠폰이 발행되었습니다.\n상품페이지로 이동합니다.");

					<% if isApp="1" then %>
						parent.fnAPPpopupProduct("<%= dateitemval(left(currenttime,10)) %>");
						return false;
					<% else %>
						parent.location.href='<%=appUrlPath%>/category/category_itemprd.asp?itemid=<%= dateitemval(left(currenttime,10)) %>';
						return false;
					<% end if %>
				<% else %>
					<% if totalsubscriptcount>=limitbounscoupon or totalbonuscouponcount>=limitbounscoupon then %>
						alert("오늘의 상품은 전부 소진되었습니다.\n내일 또 다른 앱쇼가 열립니다.");
						return;
					<% else %>
						...
						<% else %>
							<% if staffconfirm and  request.cookies("uinfo")("muserlevel")=7 then		'request.cookies("uinfo")("muserlevel")		'/M , A		'response.cookies("uinfo")("userlevel")		'/WWW %>
								...
								return;
							<% else %>
								frm.action="doEventSubscript55082.asp";
	...
	<div class="anniversary13th appshow">
					...
					<div class="swiper-container">
						<div class="swiper-wrapper">
							<div class="swiper-slide">
								...
								<img src="http://webimage.10x10.co.kr/eventIMG/2014/55083/img_slide_<%= datenumber(left(currenttime,10)) %>_01.jpg" alt="<%= dateitemval(left(currenttime,10)) %>" />
								<!--</a>-->
							</div>
							<div class="swiper-slide">
								<!--<a href="" onclick="goitempage(<%= dateitemval(left(currenttime,10)) %>); return false;">-->
								<img src="http://webimage.10x10.co.kr/eventIMG/2014/55083/img_slide_<%= datenumber(left(currenttime,10)) %>_02.jpg" alt="<%= dateitemval(left(currenttime,10)) %>" />
								<!--</a>-->
							</div>
							<div class="swiper-slide">
								<!--<a href="" onclick="goitempage(<%= dateitemval(left(currenttime,10)) %>); return false;">-->
								<img src="http://webimage.10x10.co.kr/eventIMG/2014/55083/img_slide_<%= datenumber(left(currenttime,10)) %>_03.jpg" alt="<%= dateitemval(left(currenttime,10)) %>" />
								<!--</a>-->
							</div>
						</div>
		...
		<div class="evtInfo">
			...
			<ul>
				...
				<li>게릴라 앱쇼 상품 결제 시, <반값 할인 쿠폰>을 사용해주세요.(쿠폰은 선착순 100장 한정이며, 모두 소진 시 자동 품절처리됩니다.)</li>
				<li>다운 받은 <반값 할인 쿠폰>은 자정 기준으로 자동 소멸됩니다. 다운 받은 당일 꼭 사용해주세요.</li>
			</ul>
		</div>
