	...
	function EditRequireDetail(iitemid,iitemoption){
		//var popwin = window.open('/apps/appCom/wish/webview/inipay/Pop_EditItemRequire.asp?itemid=' + iitemid + '&itemoption=' + iitemoption,'edititemrequire','width=300,height=300,scrollbars=yes,resizable=yes');
		//popwin.focus();
	}

	function PopMileageItemView(iitemid){
		var popwin = window.open('/apps/appCom/wish/webview/my10x10/Pop_mileageshop_itemview.asp?itemid=' + iitemid,'Pop_mileageshop_itemview','width=464,height=600,scrollbars=yes,resizable=yes');
		popwin.focus();
	}
	...
	function goSaveDvlPay(){
		location.href="/apps/appCom/wish/webview/event/eventmain.asp?eventid=14067";
	}
	...
	function addWishSelected(){
		...
		if (confirm('선택 상품을 위시리스트에 추가하시겠습니까?')){
			var FavWin = window.open('/apps/appCom/wish/webview/my10x10/popMyFavorite.asp?mode=AddFavItems&bagarray=' + ArrayFavItemID ,'FavWin','width=380,height=300,scrollbars=no,resizable=no');
			FavWin.focus();
		}
	...
	$(function(){
		...
		'if (CStr(GetCartCount)<>CStr(sBagCount)) then
			...
			response.write "cartnum('"&sBagCount&"');"
		'end if
		%>
	});
	</script>