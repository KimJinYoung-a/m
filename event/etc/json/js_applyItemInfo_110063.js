...
function fnApplyItemInfoList(opts) {
	...
	$.ajax({
		type: "post",
		url: "/event/etc/json/act_getItemInfo3.asp",
					...
					$(message.items).each(function(){
						...
						if(isPrc){
							if(isSale){
								...
								if (this.itemType == "deal") {
									$("."+opts.target+this.itemid+" .price").html(this.sellprice+"~ " + additionalHtml(this.saleper, '', opts.saleBracket));
								} else {
									$("."+opts.target+this.itemid+" .price").html("<s>"+this.orgprice+"</s> "+ this.sellprice + additionalHtml(this.saleString, this.couponString, opts.saleBracket));
								}
							}else{
								// 판매가 표시
								$("."+opts.target+this.itemid+" .price").html(this.sellprice);
						...
						if(isLmt){
							if(this.limityn=="Y") {
								$("."+opts.target+this.itemid+" .limit span").html(this.limitRemain);
							} else {
								$("."+opts.target+this.itemid+" .limit").hide();
							}
						}
