...
Function fnDisplayPaging_New_OLD(strCurrentPage, intTotalRecord, intRecordPerPage, intBlockPerPage, strJsFuncName)
	...
	If intStartBlock > 1 Then
		vPageBody = vPageBody & "			<span class=""arrow prevBtn""><a href="""" onclick=""" & strJsFuncName & "(" & intStartBlock -1 & ");return false;"" >prev</a></span>" & vbCrLf
	Else
		vPageBody = vPageBody & "			<span class=""arrow prevBtn""><a href="""" onclick=""return false;"" >prev</a></span>" & vbCrLf
	End If
	...
	If intTotalPage > 1 Then
		For intLoop = intStartBlock To intEndBlock
			...
			If Int(intLoop) = Int(intCurrentPage) Then
				vPageBody = vPageBody & "			<span class=""current""><a href="""" onclick=""" & strJsFuncName & "(" & intLoop & ");return false;"" >" & intLoop & "</a></span>" & vbCrLf
			Else
				vPageBody = vPageBody & "			<span><a href="""" onclick=""" & strJsFuncName & "(" & intLoop & ");return false;"">" & intLoop & "</a></span>" & vbCrLf
			End If
		Next
	Else
		vPageBody = vPageBody & "			<span class=""current""><a href="""" onclick=""" & strJsFuncName & "(1);return false;"" >1</a></span>" & vbCrLf
	End If
	
	'## 다음 페이지
	If Int(intEndBlock) < Int(intTotalPage) Then	'####### 다음페이지
		vPageBody = vPageBody & "			<span class=""arrow nextBtn""><a href="""" onclick=""" & strJsFuncName & "(" & intEndBlock+1 & ");return false;"" >next</a></span>" & vbCrLf
	Else
		vPageBody = vPageBody & "			<span class=""arrow nextBtn""><a href="""" onclick=""return false;"" >next</a></span>" & vbCrLf
	End If
...
Function fnDisplayPaging_New(strCurrentPage, intTotalRecord, intRecordPerPage, intBlockPerPage, strJsFuncName)
	...
	If intTotalPage > 5 and intCurrentPage > 3 Then
		vPageBody = vPageBody & "			<span class=""arrow prevBtn""><a href="""" onclick=""" & strJsFuncName & "(" & prePgNum & ");return false;"" >prev</a></span>" & vbCrLf
	End If
