...
<%
	...
	if PageSize > 300 then PageSize=300
	...
	Select Case Trim(sGubun)
		...
		Case "jibun" '// 지번 주소로 검색했을때
			...
			if oDoc.FTotalCount>0 Then
				...
				IF oDoc.FResultCount >0 then
				    ...
				    Next
					'tmppagingVal = fnDisplayPaging_New(CurrPage,oDoc.FTotalCount,PageSize,4,"jsPageGo")
			    end If
				Response.write "OK|"&tmpconfirmVal&"|"& chkiif(clng(oDoc.FTotalCount)>clng(PageSize),PageSize,oDoc.FTotalCount) &"|"&tmppagingVal
				Response.End
			Else
		...
		Case "RoadBnumber" '// 도로명 주소에 도로명 + 건물번호로 검색했을때
			...
			strsql = " Select top 300 zipcode, sido, gungu, dong, eupmyun, ri, road "
			...
			If Not(rsEVTget.bof Or rsEVTget.eof) Then
				...
				rsEVTget.movenext
				Loop
				Response.write "OK|"&tmpconfirmVal&"|"& chkiif(clng(tmpsReturnCnt)>clng(PageSize),PageSize,tmpsReturnCnt)
				Response.End
			Else
				Response.write "OK|<li class='nodata'>검색된 주소가 없습니다.</li>"
				Response.End
