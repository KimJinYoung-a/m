...
if IsPresentOrder then
	if oshoppingbag.isPresentItemOrderLimitOver(userSn,2) then
		...
		response.write "<script type='text/javascript'>alert('고객님께서는 PRESENT 상품을 이미 2회 주문하셨습니다.\n(최대 2회까지 주문가능)');</script>"
		vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "고객님께서는 PRESENT 상품을 이미 2회 주문하셨습니다.(최대 2회까지 주문가능)")
...
if IsEventOrderItem then
	if oshoppingbag.isEventOrderItemLimitOver(userSn,1) then
		...
		response.write "<script type='text/javascript'>alert('고객님께서는 이벤트 상품을 이미 주문하셨습니다.\n(최대 1회 주문가능)');</script>"
		vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "고객님께서는 이벤트 상품을 이미 주문하셨습니다.(최대 1회 주문가능)")
		response.redirect fnGetUserInfo("FAILURL")
		dbget.Close: response.End
	end if
end if
...
<!-- #include virtual="/apps/appcom/between/inipayAPI/userinfo_javascript.asp" -->
<script type="text/javascript">

function jsOpenPopup1(sUrl) {
	setTimeout("jsOpenPopup('"+ sUrl +"')",100);
	return false;
}
...
<div class="wrapper" id="btwMypage">
	<% if oshoppingbag.IsShoppingBagVoid then %>
		<%
	    'response.redirect "/apps/appcom/between/inipayAPI/shoppingbag.asp"
		response.write "<script type='text/javascript'>alert('장바구니에 상품이 없습니다.');</script>"
		vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "장바구니에 상품이 없습니다.")
		response.redirect fnGetUserInfo("FAILURL")
	    dbget.close()	:	response.end
		%>
	<% else %>
						...
						<tr>
							...
							<td>
								<div class="row emailField">
									<div id="lyrEmail" class="cell">
										<% ' <!-- for dev msg : 직접입력을 선택할 경우 direct 클래스명 추가해주세요. <div class="cell direct">... </div> --> %>
		...
		<form name="errReport" method="post" action="/apps/appCom/between/inipayAPI/card/errReport.asp" target="ifrProc">
		...
		<% if getIsTenLogin then %>
		<form name="frmC" method="get" action="/apps/appCom/between/inipayAPI/couponshop_process.asp" style="margin:0px;" target="ifrProc">
		...
		if (oshoppingbag.IsFixNnormalSangpumExists) then
			response.write "<script type='text/javascript'> ChkErrMsg = '지정일 배송상품(꽃배달)과 일반택배 상품은 같이 배송되지 않으니 양해하시기 바랍니다.';</script>"
		elseif oshoppingbag.Is09NnormalSangpumExists then
			response.write "<script type='text/javascript'> ChkErrMsg = '단독구매 상품과 일반상품은 같이 구매하실 수 없습니다.';</script>"
		elseif (oshoppingbag.GetMileshopItemCount>0) and (oshoppingbag.GetTenBeasongCount<1) then
		response.write "<script type='text/javascript'> ChkErrMsg = '마일리지샾 상품은 텐바이텐 배송상품과 함께 하셔야 배송 가능 합니다.';</script>"
		elseif (oshoppingbag.GetMileageShopItemPrice>availtotalMile) then
			response.write "<script type='text/javascript'> ChkErrMsg = '사용 가능한 마일리지는 " & availtotalMile & " 입니다. - 마일리지 상품 합계가 현재 마일리지보다 많습니다.';</script>"
		elseif (oshoppingbag.IsBuyOrderItemExists) then
			response.write "<script type='text/javascript'> ChkErrMsg = '선착순 구매상품은 무통장으로 주문 하실 수 없으니 양해해 주시기 바랍니다.';</script>"
		end if
	
		if (NotWriteRequireDetailExists) then
			response.write "<script type='text/javascript'>ChkErrMsg = '주문 제작 문구를 작성하지 않은 상품이 존재합니다. - 주문 제작문구를 작성해주세요.';</script>"
		end if
	
		if (IsTicketLimitOver) then
			response.write "<script type='text/javascript'>ChkErrMsg = '티켓 상품은 기주문 수량 포함 총 "& MaxTicketNo &"장 까지 구매 가능하십니다. 기 구매하신 수량 ("&PreBuyedTicketNo &") 장';</script>"
		end if
		if (IsPresentLimitOver) then
			response.write "<script type='text/javascript'>ChkErrMsg = 'Present상품은 한 주문에 "& MaxPresentItemNo &"개 구매 가능하십니다.';</script>"
		end if
		if (IsEventLimitOver) then
			response.write "<script type='text/javascript'>ChkErrMsg = 'Event상품은 한 주문에 "& MaxEventItemNo &"개 구매 가능하십니다.';</script>"
		end if
		%>
