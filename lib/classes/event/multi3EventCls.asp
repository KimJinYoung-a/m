...
Class Multi3
    ...
    public Sub getUnitItemsList()
		...
		if FRectUnitIdx <> "" then
			sqlwhere = sqlwhere + " and unit_idx = " & FRectUnitIdx
		end if
		sqlwhere = sqlwhere + " AND B.SELLYN = 'Y' " 

		sqlStr = " select count(idx) as cnt from db_event.dbo.[tbl_multi3_items] AS A "
        ...
        sqlStr = "select * "
        sqlStr = sqlStr + " from db_event.dbo.[tbl_multi3_items] AS A "
		sqlStr = sqlStr + " INNER JOIN DB_ITEM.DBO.TBL_ITEM B ON A.ITEMID = B.itemid "
        sqlStr = sqlStr + " where 1=1 "
		sqlStr = sqlStr + sqlWhere        
	...
	public Function getSalePer(vItemId)
        dim strSql
        ...
        strSql = "SELECT TOP 1 CONVERT(INT, (ORGPRICE - SELLCASH) / orgprice * 100) AS SALEPER "
        strSql = strSql + " from DB_ITEM.DBO.TBL_ITEM "
        strSql = strSql + " WHERE ITEMID ='" + CStr(vItemId) + "'"				
'getSalePer = "3"
		rsget.Open strSql, dbget, 1
		if Not(rsget.EOF or rsget.BOF) then
			getSalePer = rsget("SALEPER")
		end if
