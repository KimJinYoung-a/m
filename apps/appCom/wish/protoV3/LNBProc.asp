...
IF (FALSE) then
    oJson("recentordercnt") =0
    oJson("newnotiexists") = 0
ELSE
	oJson("recentordercnt")     = getRecentOrderCount(userid) ''최근 주문 수량.
    ''Set oJson("personalnoti")   = getPersonalNotiList(userid)  ''V3 삭제.
    if (sDeviceId<>"") then
    ...
    if ((sAppKey="5") and (sVerCd>="1.98")) then 
        if oJson("recentordercnt") > 1 THEN oJson("recentordercnt") = 1
        if oJson("newnotiexists") > 1 THEN  oJson("newnotiexists") = 1
    end if 
END IF    
    if ((sAppKey="6") and (sVerCd>"83")) or ((sAppKey="5") and (sVerCd>"1.984")) then   'lnb 개편
        SET oJson("lnbcate") = getLnbCatewithIconJson
        SET oJson("lnbsquare") = getLnbSquareJson
        SET oJson("lnbbottom") = getLnbBottomJson
    else
        oJson("lnbbgtype") = getLnbBgColorType(userid)
        SET oJson("lnbcate") = getLnbCateJson
        SET oJson("lnbnavi") = getLnbNaviJson
    end if 
