...
<%
	...
	Else
		eCode  = 106014 '// 매일리지 코드
        mobileEventCode = 106012 '// 모바일 이벤트 코드
	End If

	Response.redirect "/apps/appCom/wish/web2014/event/eventmain.asp?eventid=108349"
	Response.End
	...
	if mktTest then
		if request("checkday")="1" then
			currenttime=#09/28/2020 00:00:00#
		elseif request("checkday")="2" then
			currenttime=#09/29/2020 00:00:00#
		elseif request("checkday")="3" then
			currenttime=#09/30/2020 00:00:00#
		elseif request("checkday")="4" then
			currenttime=#10/01/2020 00:00:00#
		elseif request("checkday")="5" then
			currenttime=#10/02/2020 00:00:00#
		elseif request("checkday")="6" then
			currenttime=#10/03/2020 00:00:00#
		elseif request("checkday")="7" then
			currenttime=#10/04/2020 00:00:00#
		elseif request("checkday")="8" then
			currenttime=#10/05/2020 00:00:00#
		elseif request("checkday")="9" then
			currenttime=#10/06/2020 00:00:00#
		else
			currenttime=#09/28/2020 00:00:00#
		end if
	end if

	vEventStartDate = "2020-09-28"
	vEventEndDate = "2020-10-06"
...
.maeileage .topic {position:relative; background:#5946d3;}
...
$(function() {
	...
	function jsMaeilageSubmit(){
		...
		$.ajax({
			type:"GET",
			<% if mktTest then %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript106014.asp?mode=add&checkday=<%=request("checkday")%>",
			<% else %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript106014.asp?mode=add",
			<% end if %>
			dataType: "text",
			async:false,
			cache:true,
	...
	function jsMaeilagePushSubmit(){
		...
		$.ajax({
			type:"GET",
			<% if mktTest then %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript106014.asp?mode=pushadd&checkday=<%=request("checkday")%>",
			<% else %>
			url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript106014.asp?mode=pushadd",
			<% end if %>
			dataType: "text",
			async:false,
			cache:true,
...
<% '<!-- MKT 매일리지 106014 --> %>
<div class="mEvt106014 maeileage">
    <div class="topic">
        <% '<!-- for dev msg : 타이틀 이미지 파일명 첫째날만 tit_maeileage_1 둘째날부터 tit_maeileage_2 --> %>
        <% If Left(currenttime, 10) > Left(vEventStartDate, 10) Then %>
            <h2><img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/tit_maeileage_2.png" alt="매일리지"></h2>
        <% Else %>
            <h2><img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/tit_maeileage_1.png" alt="매일리지"></h2>
        <% End If %>
        ...
        <div class="today-noti">
            ...
            <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/txt_noti.png" alt="소멸됩니다">
        </div>
        <div class="dc-group">
    ...
    <div id="lyrDailyChk" class="lyr-daily-chk">
        <% If Not(Left(currenttime, 10) >= vEventEndDate) Then %>
            <div>
                ...
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/lyr_bg.png" alt="">
            </div>
        <% Else %>
            <% '<!-- 10/6 마지막날 노출 레이어 --> %>
            <div class="lyr-finish">
                ...
                <strong class="point"><em id="today-mileage-value"></em>p</strong>
                <button type="button" id="btnClose" class="btn-close">닫기</button>
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/lyr_bg_finish.png" alt="">
            </div>
        <% End If %>
    </div>
    <div id="mask"></div>
    ...
    <div class="step-process">
        <% If IsUserLoginOK() Then %>
            ...
            <%
                ...
                vQuery = vQuery & "	From db_temp.[dbo].[tbl_maeliageDate106014] m "
                rsget.CursorLocation = adUseClient
                rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
                If Not(rsget.bof Or rsget.eof) Then
                    Do Until rsget.eof
    ...
    <% If Left(currenttime, 10) < Left(vEventEndDate, 10) Then %>
        <div class="push-request">
            <button type="button" onclick="jsMaeilagePushSubmit();return false;">
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/btn_push_alarm.jpg" alt="내일도 매일리지 알림 받기">
            </button>
        </div>
        <div class="push-way">
            <button type="button" onclick="fnAPPpopupSetting();return false;">
                <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/btn_push_setting.jpg" alt="PUSH 알림 설정하러 가기">
            </button>
        </div>
    <% End If %>
    <% '<!-- 알림신청 팝업 --> %>
    <div id="lyrPush" class="lyr-push">
        ...
        <img src="//webimage.10x10.co.kr/fixevent/event/2020/106014/m/lyr_push.png" alt="">
    </div>
    <div id="mask2"></div>
    ...
    <div class="noti">
        ...
        <ul>
            ...
            <li>- 지급된 마일리지는 스페셜 마일리지로, 2020년 10월 19일 23:59까지 사용 가능합니다.</li>
        </ul>
    </div>
</div>
<% '<!-- // MKT 매일리지 --> %>
