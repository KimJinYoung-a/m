...
Dim urlchg
	...
	if Not(Now() > #06/19/2017 00:00:00# And Now() < #06/23/2017 23:59:59#) then											'''''''''''''''''''''''''''날짜 체크
	...
	if mode="inst" Then
		If zipcode = ""  or isNull(zipcode) Then
			Response.Write "Err|주소를 다시 확인해 주세요"
			dbget.close() : Response.End
		End If
		...
		if amode = "edit" then
			if myaddridx <> "" then
				...
				sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', '" & myaddridx & "', '" & amode & "' , '" & vDeviceGn & "' )" + vbcrlf	
	...
	'			response.write "Err|"& sqlstr
				...
				sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', '" & vDeviceGn & "' )" + vbcrlf	
	'			response.write "Err|"& sqlstr
				dbget.execute sqlstr
