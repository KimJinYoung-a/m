    ...
    %>
        <article class="prd_item">
            ...
            <div class="prd_info">
                ...
                <!-- 뱃지 수정 -->
                <div class="prd_badge<% if fnGetDeliveryFreeiCon(bestList(i).Fdeliverytype,bestList(i).Fsellcash,bestList(i).FdefaultFreeBeasongLimit) and (fnGetGiftiCon(bestList(i).Fdeliverytype,bestList(i).Forgprice,bestList(i).Fitemid) and giftCheck) then %> badge_two<% end if %>">
            ...
            <% if isapp=1 then %>
            <a href="javascript:TnGotoAPPDiaryProduct(<%=bestList(i).Fitemid%>);" class="prd_link" onclick="fnAmplitudeEventAction('click_diarystory_mdpick','item_id','<%=bestList(i).Fitemid%>');">
            <% else %>
            <a href="javascript:TnGotoDiaryProduct(<%=bestList(i).Fitemid%>);" class="prd_link" onclick="fnAmplitudeEventAction('click_diarystory_mdpick','item_id','<%=bestList(i).Fitemid%>');">
            <% end if %>
            <span class="blind">상품 바로가기</span></a>
        </article>
    <% next %>
    </div>
</section>
