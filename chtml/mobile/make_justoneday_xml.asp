...
<%
'#######################################################
' PageName : make_justoneday_xml.asp
...
for j=1 to cInt(vTerm2)
	...
	sqlStr = "select * "
	sqlStr = sqlStr + " from db_sitemaster.dbo.tbl_mobile_main_justoneday_list as l "
	sqlStr = sqlStr + " inner join [db_sitemaster].[dbo].tbl_mobile_main_justoneday_item as s "
...
For j=1 to cInt(vTerm2)
	...
	'// 메인 데이터 접수
	sqlStr = "Select top 1 s.subidx , s.listidx , s.itemid , s.isusing as itemusing , s.sortnum , i.itemname, i.basicimage , l.startdate , l.enddate , s.extraimg , s.extraurl , l.saleper , l.onairtitle , l.saleprice "
	sqlStr = sqlStr & " From [db_sitemaster].[dbo].tbl_mobile_main_justoneday_item as s "
	sqlStr = sqlStr & "	left join db_item.dbo.tbl_item as i "
	sqlStr = sqlStr & "		on s.itemid=i.itemid "
	sqlStr = sqlStr & "			and i.itemid<>0 "
	sqlStr = sqlStr & "	left join [db_sitemaster].dbo.tbl_mobile_main_justoneday_list as l "
	sqlStr = sqlStr & "	on s.listidx = l.idx  "
	sqlStr = sqlStr & " Where l.isusing='Y' and s.isusing = 'Y' and (" & sqlDate & ") "
	...
	if vTotalCount>0 then
		...
		Do Until rsget.EOF
			...
			objXMLv.appendChild(objXML.createElement("extraimg"))
			objXMLv.appendChild(objXML.createElement("extraurl"))
			...
			objXMLv.appendChild(objXML.createElement("onairtitle"))
			objXMLv.appendChild(objXML.createElement("saleprice"))
			...
			objXMLv.childNodes(3).text = staticImgUrl & "/mobile/justoneday" + db2Html(rsget("extraimg"))
			objXMLv.childNodes(4).text = db2Html(rsget("extraurl"))
			objXMLv.childNodes(5).text = db2Html(rsget("saleper"))
			objXMLv.childNodes(6).text = Replace(Left(rsget("startdate"),10),"-",",")
			objXMLv.childNodes(7).text = Replace(Left(rsget("enddate"),10),"-",",")
			objXMLv.childNodes(8).text = db2Html(rsget("onairtitle"))
			objXMLv.childNodes(9).text = db2Html(rsget("saleprice"))
