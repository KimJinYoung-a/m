...
if (LCase(Session("InfoConfirmFlag"))<>LCase(userid)) or (LCase(EcChk)<>LCase(userid)) then
	...
	if GetLoginUserDiv="05" then
		'간편로그인 가입자 확인
	    sqlStr = "select userid, IsNULL(userdiv,'02') as userdiv from [db_user].[dbo].tbl_logindata where userid='" & userid & "'"
	    ...
	    if GetLoginUserDiv<>userdiv then
...
dim iandOrIos, iappVer, snsViewchk, snsViewchknew, snsViewchk3
snsViewchknew = FALSE
snsViewchk3 = FALSE
iappVer = getAppVerByAgent(iandOrIos)
if (iandOrIos="a") then	'안드
    if (iappVer>="1.91") then	'안드 최신버전이면(실섭적용시 해야할것)
'    if (iappVer>="1.48") then	'안드 최신버전이면
        snsViewchk = TRUE

	    if (iappVer>="1.92") then snsViewchknew = TRUE
	    If (iappVer>="2.527") then snsViewchk3 = TRUE
    else							'안드 구버전이면 
        snsViewchk = FALSE
    end if
else	'ios
    if (iappVer>="1.997") then	'ios 최신버전이면(실섭적용시 해야할것)
'    if (iappVer>="1.48") then	'ios 최신버전이면
        snsViewchk = TRUE

	    if (iappVer>="1.998") then snsViewchknew = TRUE
		If (iappVer>="3.006") then snsViewchk3 = TRUE
...
function popKakaoAuth() {
	<% if Not(chkKakao) then %>
		var kurl;
		kurl = "<%= C_WEBVIEWURL %>/my10x10/kakaotalk/layer_step1.asp"
		fnAPPpopupBrowserURL('카카오톡 인증',kurl);
		//fnOpenModal("/apps/appCom/wish/web2014/my10x10/kakaotalk/layer_step1.asp")
		return;
	<% else %>
		var kurl;
		kurl = "<%= C_WEBVIEWURL %>/my10x10/kakaotalk/layer_clear.asp"
		fnAPPpopupBrowserURL('카카오톡 해제',kurl);
		//fnOpenModal("/apps/appCom/wish/web2014/my10x10/kakaotalk/layer_clear.asp")
		return;
	<% end if %>
}
...
function sendCnfEmail(frm) {
	...
	var eurl;
	eurl = "<%= C_WEBVIEWURL %>/my10x10/userinfo/layer_sendModiemail.asp?id=<%=userid%>&mail="+sEm
	if(confirm("입력하신 이메일 '"+sEm+"'(으)로 인증을 받으시겠습니까?\n\n※인증메일에서 링크를 클릭하시면 인증이 완료되며 이메일정보가 수정됩니다.")) {
		fnAPPpopupBrowserURL('이메일 인증',eurl);
		return ;
		//fnOpenModal("/apps/appCom/wish/web2014/my10x10/userinfo/layer_sendModiemail.asp?id=<%=userid%>&mail="+sEm)
	}
}
...
function sendCnfSMS(frm) {
	...
	var surl;
	surl = "<%= C_WEBVIEWURL %>/my10x10/userinfo/layer_sendModiSMS.asp?id=<%=userid%>&phone="+sHp
	if(confirm("입력하신 휴대폰 '"+sHp+"'(으)로 인증을 받으시겠습니까?\n\n※전송된 인증번호를 입력창에 넣으시면 인증이 완료되며 휴대폰정보가 수정됩니다.")) {
		fnAPPpopupBrowserURL('SMS 인증',surl);
		return ;
		//fnOpenModal("/apps/appCom/wish/web2014/my10x10/userinfo/layer_sendModiSMS.asp?id=<%=userid%>&phone="+sHp)
	}
}
...
function pagereload(){
	location.replace('/apps/appCom/wish/web2014/my10x10/userinfo/membermodify.asp');
	return false;
}
function passwordpagereload(){
	location.replace('/apps/appCom/wish/web2014/my10x10/userinfo/memberUserPass.asp');
	return false;
}
			...
			async: false,
	       	success: function (responseText, statusText) {
				if(responseText.response=="Disc") {
					fnAPPChgSNSLoginConn(snsgb,'disc')
//					window.location.reload(true);
...
<div class="heightGrid">
	<div class="container popWin bgGry">
		...
		<div class="content myInfoMod" id="contentArea">
			<div class="inner5 bgGry">
				<div class="tab01 tMar15">
					<ul class="tabNav tNum2 noMove">
						<li class="current"><a href="" onclick="pagereload(); return false;">개인정보관리<span></span></a></li>
					<%
						''간편로그인수정;허진원 2018.04.24
						if Not(isSNSMember) then
					%>
						<li><a href="" onclick="passwordpagereload(); return false;">비밀번호 변경<span></span></a></li>
					<%	end if %>
					</ul>
					<div class="tabContainer">
						...
						<div class="tabContent">
							<div class="box1">
							<form name="frminfo" method="post" action="<%=M_SSLUrl%>/apps/appcom/wish/web2014/my10x10/userinfo/membermodify_process.asp" style="margin:0px;" onsubmit="return false;">
							...
							<input type="hidden" name="isMobileChk" value="<%=chkIIF(myUserInfo.FOneItem.FisMobileChk="Y","Y","N")%>">
									...
									<table class="writeTbl01 addEss">
										...
										<tbody>
											...
											<tr>
												...
												<td>
													<p>
														<input type="text" name="txZip" value="<%= myUserInfo.FOneItem.Fzipcode %>" maxlength="7"  class="w25p" />
														<span class="button btB2 btGry cBk1 lMar05"><a href="" onclick="TnFindZipNew('frminfo',''); return false;">우편번호 검색</a></span>
													</p>
													<p class="tPad05">
														<input type="text" name="txAddr1" value="<%= myUserInfo.FOneItem.FAddress1 %>" autocomplete="off" readonly class="w100p" />
													</p>
													<p class="tPad05">
														<input type="text" name="txAddr2" value="<%= myUserInfo.FOneItem.FAddress2 %>" autocomplete="off" class="w100p" />
													</p>
											...
											<tr>
												<th>전화</th>
												<td>
													<input type="tel" name="userphone1" pattern="[0-9]*" value="<%= SplitValue(myUserInfo.FOneItem.Fuserphone,"-",0) %>" onkeyup="TnTabNumber('userphone1','userphone2',3);" maxlength="4" autocomplete="off" class="w30p" />
													<input type="tel" name="userphone2" pattern="[0-9]*" value="<%= SplitValue(myUserInfo.FOneItem.Fuserphone,"-",1) %>" onkeyup="TnTabNumber('userphone2','userphone3',4);" maxlength="4" autocomplete="off" class="w30p lMar05" />
													<input type="tel" name="userphone3" pattern="[0-9]*" value="<%= SplitValue(myUserInfo.FOneItem.Fuserphone,"-",2) %>" maxlength="4" autocomplete="off" class="w30p lMar05" />
												</td>
											</tr>
										</tbody>
									</table>
										...
										<tbody>
											<tr>
												...
												<td>
													...
													<div class="emailField">
														...
														<input type="email" name="txEmail1" value="<%= E1 %>" onkeyup="chkChangeAuth(this.form,'E');" maxlength="32" title="이메일 계정" autocomplete="off" class="emailAccount" />
														<span class="symbol">@</span>
														<% call DrawEamilBoxHTML_App("document.frminfo","selfemail","txEmail2",E2,"emailService",""," onkeyup=""chkChangeAuth(this.form,'E');"" title='이메일 서비스'"," onchange=""jsShowMailBox('document.frminfo','txEmail2','selfemail'); chkChangeAuth(this.form,'E');"" title='이메일 서비스 선택'") %>
													</div>
											...
											<tr>
												...
												<td>
													...
													<input type="tel" name="usercell1" value="<%= SplitValue(myUserInfo.FOneItem.Fusercell,"-",0) %>" onkeyup="TnTabNumber('usercell1','usercell2',3); chkChangeAuth(this.form,'P');" pattern="[0-9]*" maxlength="4" maxlength="3" autocomplete="off" class="w30p" />
													<input type="tel" name="usercell2" value="<%= SplitValue(myUserInfo.FOneItem.Fusercell,"-",1) %>" onkeyup="TnTabNumber('usercell2','usercell3',4); chkChangeAuth(this.form,'P');" pattern="[0-9]*" maxlength="4" maxlength="4" autocomplete="off" class="w30p lMar05" />
													<input type="tel" name="usercell3" value="<%= SplitValue(myUserInfo.FOneItem.Fusercell,"-",2) %>" onkeyup="chkChangeAuth(this.form,'P');" pattern="[0-9]*" maxlength="4" maxlength="4" autocomplete="off" class="w30p lMar05" />
								...
								%>
									<%
									dim mynvsnsgubun, mynvsnsregdate, myfbsnsgubun, myfbsnsregdate, mykasnsgubun, mykasnsregdate, myglsnsgubun, myglsnsregdate, myapsnsgubun, myapsnsregdate
									...
									sqlstr = sqlstr & " , 'ap' as apsnsgubun, (select top 1 regdate From [db_user].[dbo].[tbl_user_sns] Where tenbytenid='"& userid &"' And snsgubun='ap' And isusing='Y') as apregdate " + vbcrlf
									...
									if Not rsget.Eof then
										...
										myapsnsgubun = rsget("apsnsgubun")
										myapsnsregdate = rsget("apregdate")
									end if
									rsget.close
									%>
									<% if snsViewchk then %>
										...
										<div class="connectSns">
											<ul>
												<% if snsViewchknew = TRUE then %>											
													<li>
														<div class="sns-accountV20 kakao"><span class="text">카카오톡</span></div>
														<div class="switch">
															...
															<% else %>
																<input type="checkbox" id="connect03" onclick="fnAPPChgSNSLoginConn('ka','conn'); return false;" class="toggle toggle-red"><label for="connect03"><span>끔</span></label>
															<% end if %>
														</div>
													</li>
												<% End If %>
												...
												<li>
													<div class="sns-accountV20 naver"><span class="text">네이버</span></div>
													<div class="switch">
														...
														<% else %>
															<input type="checkbox" id="connect02" onclick="fnAPPChgSNSLoginConn('nv','conn'); return false;" class="toggle toggle-red"><label for="connect02"><span>끔</span></label>
														<% end if %>
													</div>
												</li>
												...
												<% if snsViewchknew = TRUE then %>
													<li>
														<div class="sns-accountV20 facebook"><span class="text">페이스북</span></div>
														<div class="switch">
															...
															<% else %>
																<input type="checkbox" id="connect01" onclick="fnAPPChgSNSLoginConn('fb','conn'); return false;" class="toggle toggle-red"><label for="connect01"><span>끔</span></label>
															<% end if %>
														</div>
													</li>

													<li>
														<div class="sns-accountV20 google"><span class="text">구글</span></div>
														<div class="switch">
															...
															<% else %>
																<input type="checkbox" id="connect04" onclick="fnAPPChgSNSLoginConn('gl','conn'); return false;" class="toggle toggle-red"><label for="connect04"><span>끔</span></label>
															<% end if %>
														</div>
													</li>
												<% end if %>
												<% if snsViewchk3 then %>
													<li>
														<div class="sns-accountV20 apple"><span class="text">애플</span></div>
														<div class="switch">
															<% if myapsnsgubun = "ap" and myapsnsregdate <> "" then %>
																<input type="checkbox" id="connect05" onclick="fnSNSdisconnect('ap');return false;" class="toggle toggle-red" checked="checked"><label for="connect05"><span>켬</span></label>
															<% else %>
																<input type="checkbox" id="connect05" onclick="fnAPPChgSNSLoginConn('ap','conn'); return false;" class="toggle toggle-red"><label for="connect05"><span>끔</span></label>
															<% end if %>
														</div>
													</li>
												<% end if %>
								...
								<% end if %>
									...
									<div class="btnWrap tMar25">
										<div class="ftLt w50p"><span class="button btB1 btRedBdr cRd1 w100p"><a href="" onclick="fnAPPclosePopup(); return false;">취소</a></span></div>
				...
				<div class="btn-member-leave">
					<a href="" onclick="fnAPPpopupBrowserURL('회원탈퇴','<%=M_SSLUrl%>/apps/appCom/wish/web2014/my10x10/userinfo/withdrawal.asp','right','','sc'); return false;"><span>회원 탈퇴</span></a>
				</div>
			</div>
		</div>
		<!-- //content area -->
