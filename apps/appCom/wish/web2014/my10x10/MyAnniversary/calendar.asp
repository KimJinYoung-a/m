...
<%
	strPageTitle = "생활감성채널, 텐바이텐 > 나의 기념일 등록:날짜 선택"
%>
...
<script type="text/javascript">
	...
	{
		fnAPPopenerJsCallClose("calendarDataSelect('"+a+"')");
	}
</script>
</head>
<body>
							...
							<%
								...
								For i=1 to vDayCount
									...
									If WeekDay(vNow) = 1 Then
										vBody = vBody & "<td class=""sun""><a href='' onclick=""selectdate('" & vYear & "-" & TwoNumber(vMonth) & "-" & TwoNumber(i) & "');return false;"">" & i & "</a></td>" & vbCrLf
									Else 
										vBody = vBody & "<td><a href='' onclick=""javascript:selectdate('" & vYear & "-" & TwoNumber(vMonth) & "-" & TwoNumber(i) & "');return false;"">" & i & "</a></td>" & vbCrLf
									End If
