...
dim smssubscriptcount
	...
	Dim vQuery, vTotalCount
		...
		vQuery = "SELECT count(*) FROM db_temp.dbo.tbl_temp_Send_UserMail WHERE userid = '" & userid & "' AND yyyymmdd = '20140916'"
		...
		If vTotalCount < 1 Then
			response.write "<script language='javascript'>alert('이벤트 대상이 아닙니다.'); parent.location.reload();</script>"
			dbget.close()
		    response.end
		End IF
