...
if mode="couponreg" then
	...
	if Hour(currenttime) < 11 then
		Response.Write "<script type='text/javascript'>alert('쿠폰은 오전 11시부터 다운 받으실수 있습니다.'); parent.top.location.href='"&appUrlPath&"/event/eventmain.asp?eventid="&eCode&"'</script>"
		dbget.close() : Response.End
	end if
	...
	sqlstr = sqlstr & " 	SELECT idx, '"& userid &"',coupontype,couponvalue,couponname,minbuyprice,targetitemlist,getdate(),'2015-06-15 23:59:59',couponmeaipprice,validsitename" + vbcrlf
	...
	Response.Write "<script type='text/javascript'>alert('쿠폰이 발급 되었습니다.\n오늘하루 app에서 사용하세요!!'); parent.top.location.href='"&appUrlPath&"/event/eventmain.asp?eventid="&eCode&"'</script>"
	dbget.close() : Response.End
else
