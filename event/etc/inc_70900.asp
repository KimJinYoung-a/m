...
<%
'####################################################
' Description : 5월 up! 등업
' History : 2016.05.24 유태욱 생성
'####################################################
%>
...
.gradeUp button {background-color:transparent;}

.gradeUp .topic {overflow:hidden; position:relative;}
.gradeUp .topic h2 {position:absolute; top:0; left:0; width:100%;}
.gradeUp .topic h2 {animation-name:bounce; animation-duration:2.5s; animation-iteration-count:3; animation-fill-mode:both; -webkit-animation-name:bounce; -webkit-animation-duration:2.5s; -webkit-animation-iteration-count:3; -webkit-animation-fill-mode:both;}
@keyframes bounce {
...
.eventBox .check .btncheck {position:absolute; bottom:11%; left:50%; width:80.625%; margin-left:-40.3125%; z-index:5;}
...
.presentBox .btnEnter {position:absolute; bottom:11.4%; left:50%; width:80.625%; margin-left:-40.3125%;}

.noti {padding:7% 4.68% 8%; background-color:#f1f1f1;}
.noti h3 {color:#222; font-size:1.4rem; font-weight:bold;}
.noti h3 strong {display:inline-block; padding-bottom:1px; border-bottom:2px solid #222;}
.noti ul {margin-top:1.3rem;}
.noti ul li {position:relative; margin-top:0.3rem; padding-left:0.7rem; color:#666; font-size:1.1rem; line-height:1.5em;}
...
$(function(){
	window.$('html,body').animate({scrollTop:$(".mEvt70900").offset().top}, 0);
});

function jsorder(){
	<% If IsUserLoginOK() Then %>
		<% If not( left(currenttime,10)>="2016-05-24" and left(currenttime,10)<"2016-06-01" ) Then %>
			alert("이벤트 응모 기간이 아닙니다.");
			return;
		<% else %>
			var str = $.ajax({
				type: "POST",
				url: "/event/etc/doeventsubscript/doEventSubscript70900.asp",
				data: "mode=order",
				dataType: "text",
				async: false
			}).responseText;
			...
			if (str1[0] == "04"){
				...
				$("#innermember").addClass("inner <%= userlevelname %>");
				$(".btncheck").hide();
				$("#grval").show();
		...
		<% else %>
			...
			<% else %>
				if (evtFrm1.clickvar.value!="ON"){
					alert('5월 고객님의 구매내역을 먼저 확인해 주세요.');
					return false;
				}
				if (evtFrm1.totalcnt.value==0 && evtFrm1.totalsum.value==0){
					alert('5월 구매내역이 있어야 응모할수 있습니다.');
					return false;
				}
				var str = $.ajax({
					type: "POST",
...
<input type="hidden" name="totalsum">
	...
	<div class="mEvt70900 gradeUp">
		<article>
			<div class="topic">
				<h2><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/tit_up.png" alt ="업 Up" /></h2>
				<p><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/txt_up.jpg" alt ="여러분의 구매내역을 확인하세요! 새롭게 등급up되는 여러분을 응원합니다" /></p>
			</div>

			<%''// 구매내역 확인 %>
			<div class="eventBox">
				<div class="check">
					<h3><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/tit_check.jpg" alt="5월 고객님의 구매내역을 확인하세요!" /></h3>

					<%''// for dev msg : 확인하기 후 확인하기 버튼은 숨겨주세요 %>
					<button type="button" onclick="jsorder(); return false;" class="btncheck"><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/btn_check.png" alt="확인하기" /></button>
					<div class="inner" id="innermember">
						<ul>
							<li>
								<span class="ftLt"><strong>구매횟수 :</strong></span>
								<span class="ftRt"><strong><em id="totalcnt">*</em> 회</strong></span><!-- 확인하기 이후 em 태그 안에 해당 횟수과 금액 숫자 들어가면 됩니다. -->
							</li>
							<li>
						...
						<div class="nextGrade" id="grval" style="display:none;">
							<div>6월 예상등급 : <em class="grTxt"><%= NextuserLevelname %></em></div>
							<p><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/txt_today.png" alt="오늘 기준, 구매에 따라 변동 될 수 있음" /></p>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="presentBox">
				<h3><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/tit_apply.png" alt="6월 예상등급 확인하고 응모하세요! 다음달 등급up되신 분들 중 500명을 추첨하여 2,000마일리지 증정" /></h3>
				<button type="button" onclick="jsgift(); return false;" class="btnEnter"><img src="http://webimage.10x10.co.kr/eventIMG/2016/70900/btn_enter.png" alt="응모하기" /></button>
			</div>
			...
			<div class="noti">
				...
				<ul>
					...
					<li>환불이나 교환으로 인해 5월 구매횟수나 구매금액이 충족되지 않을 경우 응모는 자동 취소 됩니다.</li>
