...
DIM addText1 , addText2 , optionCount , sellCash, issort, evalPoint
...
listType = NullFillWith(requestCheckVar(request("listtype"),1),"B")
isPick = requestCheckvar(request("ispick"),1)
issort = requestCheckvar(request("sorted"),1)

IF isPick <> "1" THEN isPick = ""
if issort = "" then issort = "8"
...
SET oExhibition = new ExhibitionCls
    ...
    oExhibition.FrectSortMet = issort
...
ELSE
    ...
    IF oExhibition.FTotalCount > 0 THEN
        FOR i = 0 TO oExhibition.FResultCount-1
            ...
            evalPoint = fnEvalTotalPointAVG(oExhibition.FItemList(i).FtotalPoint,"search")
            ...
            SET oJson("itemlist")(NULL) = jsObject()
                ...
                oJson("itemlist")(NULL)("evalPoint")          = evalPoint
        NEXT
    END IF
END IF
	'Json 출력(JSON)
