...
$(function(){
	fnAPPchangPopCaption('<%=chkIIF(ntype="E","이벤트 당첨자","공지사항")%>');
});

function goPage(page){
	location.href = "/apps/appCom/wish/web2014/common/news.asp?page="+page+"&type=<%=ntype%>";
}
</script>
</head>
		...
		<div class="">
			<div class="tab01">
				<div class="nav nav-stripe nav-stripe-default nav-stripe-red">
					<ul class="grid2">
						<li><a href="/apps/appCom/wish/web2014/common/news.asp?type=A" <% if ntype="A" then response.write " class='on'" %>>공지사항<span></span></a></li>
						<li><a href="/apps/appCom/wish/web2014/common/news.asp?type=E" <% if ntype="E" then response.write " class='on'" %>>이벤트 당첨자<span></span></a></li>
					</ul>
				</div>
				...
				<div class="">
					<div class="categoryListup bMar20">
						<ul class="noticeList">
							...
							<li>
								<a href="/apps/appCom/wish/web2014/common/news_view.asp?type=<%=ntype%>&idx=<%= oBoardNotice.FItemList(ibb).Fid %>"><%= chrbyte(oBoardNotice.FItemList(ibb).Ftitle,60,"Y") %>
