...
function app_mainchk(){
	var str = $.ajax({
		...
		data: "mode=app_main&ecode=70684",
		dataType: "text",
		async: false
	}).responseText;
	if (str == "OK"){
		fnAPPpopupEvent_URL('<%=wwwUrl%>/apps/appcom/wish/web2014/event/eventmain.asp?eventid=70684');
		return false;
	}else{
		alert('오류가 발생했습니다.');
		return false;
...
<body>
	<div class="appFrontBanner">
		<h2><img src="http://webimage.10x10.co.kr/eventIMG/2016/70684/m/img_front_banner.jpg" alt="텐바이텐 오벤져스 - 앱에서 처음 로그인한 분께 드리는 5천원의 행복" /></h2>
