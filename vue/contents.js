	...
	template : `
        <a @click="clickItem">
            <div class="thumbnail">
                ...
                <img v-lazy="decodeBase64(sub.image)" :alt="sub.itemName"/>
            </div>
            <div class="desc">
...
Vue.component('main-itemlist',{
	...
	template : `
		<li class="swiper-slide" v-else>
			<a @click="clickItem">
				<span class="label label-triangle" v-if="sub.bestProduct"><em>BEST</em></span>
				<span class="label label-triangle" v-else-if="sub.newProduct"><em>NEW</em></span>
				<div class="thumbnail"><img v-lazy="decodeBase64(sub.image)" :alt="sub.itemName"/></div>
				<div class="desc">
					<p class="name">{{sub.itemName}}</p>
					<div class="price">
						<b class="discount color-red" v-if="sub.discount">{{sub.discountText}}</b>
		...
		<li>
			<a @click="clickItem(index)">
				<div class="thumbnail">
					...
					<img v-lazy="decodeBase64(item.image)" style="display:block;width:100%;"/>
				</div>
				<div class="desc">
					<p>
						<b class="headline">
...
Vue.component('brand-banner',{
	...
	template : `
		<div>
			<a @click="clickTitle">
				<div class="thumbnail type-b"><img v-lazy="decodeBase64(item.titleImage)" alt=""/></div>
				<div class="desc">
			...
			<div class="brand-items">
				<ul>
					<li v-for="(product, index) in item.items">
						<a @click="clickItem(index, product.itemId)">
							<div class="thumbnail">
								<img v-lazy="decodeBase64(product.image)" alt=""/>
							</div>
						</a>
					</li>
				</ul>
				<div class="btn-group">
					<a @click="clickMore">
						<div class="thumbnail">
							<img v-lazy="decodeBase64(item.moreImage)" style="filter:Alpha(Opacity=50);Opacity:0.5;" alt="" />
						</div>
			...
			<li v-for="(item, index) in items">
				<a @click="clickItem(item.itemId, index)">
					...
					<div class="desc">
						...
						<span class="price">
							<b v-if="item.discountText" class="discount color-red">{{item.discountText}}</b>
							<b class="sum">{{numberFormat(item.price)}}<span class="won">원</span></b>
						</span>
					</div>
				</a>
			</li>
...
const categoryMoreMixin = Vue.mixin({
	methods : {
		 ...
		 * GA Parameter 생성
		 */
		createGaParameter(categoryCode, isEvent, index) {
			let keyword;
			switch (categoryCode) {
				case "102":
			...
			return `&gaparam=today_${keyword}_${isEvent ? 'event' : ''}${index}`;
		},
	}
});
...
Vue.component('cateitem-more',{
	...
	methods : {
		 ...
		 */
		clickItem() {
			const link = '/category/category_itemPrd.asp?itemid=' + this.item.itemId
				+ this.createGaParameter(this.code, false, this.index);
			if( this.isapp )
				fnAPPpopupAutoUrl(link);
			else
				location.href = link;
...
Vue.component('event-more',{
	...
	methods : {
		 ...
		 */
		clickItem() {
			const link = '/event/eventmain.asp?eventid=' + this.item.eventCode
				+ this.createGaParameter(this.code, true, this.index);
			if( this.isapp )
				fnAPPpopupAutoUrl(link);
			else
				location.href = link;
...
Vue.component('best-renewal',{
	...
	template : `
		<div class="best_2021">
			<div @click="go_bestpage" class="today_best">
				<div class="best_ment">
...
new Vue({
	...
	methods : {
		...
		clickMore(categoryCode) {
			if( this.isApp )
				fnAPPpopupCategory(categoryCode);
			else
				location.href = '/category/category_main2020.asp?disp=' + categoryCode + this.createGaParameter(categoryCode, false, 0);
		},
	},
	mounted : function(){
		this.scroll();
