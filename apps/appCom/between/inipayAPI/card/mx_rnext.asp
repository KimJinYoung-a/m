...
<%
	...
	If P_STATUS = "01" Then '인증결과가 실패일 경우
		Response.write "<script type='text/javascript'>alert('01. 이니시스에 인증결과 실패가 발생하였습니다. 고객센터로 문의해 주세요.');location.replace('/apps/appcom/between/inipayAPI/shoppingbag.asp');</script>"
		dbget.close()
		Response.End
	End If
...
'''-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	Dim vUserID, vUserSn, vGuestSeKey, vCountryCode, vEmsPrice, vRdsite, vSailcoupon, vCouponmoney, vPacktype, vSpendmileage, vSpendtencash, vSpendgiftmoney, vPrice, vCheckitemcouponlist
	...
	IF Not rsget.EOF THEN
		...
		vGuestSeKey		= rsget("rduserid")
		vUserSn			= rsget("rduserid")
   ...
   '''장바구니 금액 후Check===================================================================================================
    ...
    if (CLng(oshoppingbag.getTotalCouponAssignPrice(vPacktype)-vSpendmileage-vCouponmoney-vSpendtencash-vSpendgiftmoney) <> CLng(vPrice)) then
        'sqlStr = "Insert into [db_sms].[ismsuser].em_tran(tran_phone, tran_callback, tran_status, tran_date, tran_msg )"
		'sqlStr = sqlStr + " values( '010-6324-9110', '1644-6030', '1', getdate(), "
		'sqlStr = sqlStr + " convert(varchar(250),'장바구니 금액오류 moRnext :"&CStr(vIdx)&":"&oshoppingbag.getTotalCouponAssignPrice(vPacktype)-vSpendmileage-vCouponmoney-vSpendtencash-vSpendgiftmoney&"::"&vPrice&"'))"
		...
		sqlStr = sqlStr & " 'rnext','" & vIdx & "','" & vUserID & "','" & vGuestSeKey & "','" & oshoppingbag.getTotalCouponAssignPrice(vPacktype) & "','" & vSpendmileage & "','" & vCouponmoney & "','" & vSpendtencash & "', "
...
'''-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    xmlHttp.setTimeouts 5000,90000,90000,90000 ''2013/03/14 추가
	xmlHttp.Send postdata	'post data send

	IF Err.Number <> 0 then
		Response.write "<script type='text/javascript'>alert('02. 이니시스에 승인요청 중 오류가 발생하였습니다. 고객센터로 문의해 주세요.');</script>"
		vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "02. 이니시스에 승인요청 중 오류가 발생하였습니다. 고객센터로 문의해 주세요.")
		response.redirect fnGetUserInfo("FAILURL")
...
'	dbget.execute vQuery
	...
	If CStr(Trim(Split(arr(0),"=")(1))) <> CStr("00") Then
		Response.write "<script type='text/javascript'>alert('이니시스에서 승인요청 결과 오류가 발생하였습니다. 고객센터로 문의해 주세요.');</script>"
		vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "이니시스에서 승인요청 결과 오류가 발생하였습니다. 고객센터로 문의해 주세요.")
		response.redirect fnGetUserInfo("FAILURL")
		dbget.close()	:	response.end
	End IF
	...
	''2013/01/28 추가
	if (vResult<>"ok") then
	    Response.write "<script type='text/javascript'>alert('주문 처리 과정중 오류가 발생하였습니다. 고객센터로 문의해 주세요.');</script>"
	    vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "주문 처리 과정중 오류가 발생하였습니다. 고객센터로 문의해 주세요.")
		response.redirect fnGetUserInfo("FAILURL")
		dbget.close()	:	response.end
	end if
...
'response.write vBetweenPostReport &"/" & ref_Status&"/" & ref_result_str & "<br>신용카드"
'response.end
%>
...
<head></head>
<body>
<form name="frm" method="post" action="order_real_save.asp">
<input type="hidden" name="idx" value="<%=vIdx%>">
</form>
<script language="javascript">
	//document.location.href = "<%=vAppLink%>/inipayAPI/DisplayOrder.asp?dumi=<%=FormatDate(now(),"00000000000000")%>";  //캐시 먹음. post로 때릴것
	location.href='<%= fnGetUserInfo("RETURNURL") %>';
</script>
</body>
</html>
