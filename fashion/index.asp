...
<body class="default-font body-main">
	...
	<div id="content" class="content fashion-main">
		...
		<div class="fashion-main-bnr">
			<div class="swiper-container">
				<div class="swiper-wrapper" id="rollingbanner">
					<item-rolling v-for="item in items" :item="item" :key="item.gubun" v-if="item.gubun == 1 || item.gubun == 2 "></item-rolling>
				</div>
			</div>
		</div>
		...
		<div class="fashion-brand">
			...
			<div class="swiper-container">
				<div class="swiper-wrapper" id="hotbrand">
					<brand-list v-for="(item , index) in items" :item="item" :index="index" :key="item.id"></brand-list>
				</div>
			</div>
		</div>
		...
		<div class="exhibition-list">
			...
			<div class="list-card type-align-left">
				<ul id="exhibition1">
					<item-list v-for="(item , index) in items" :item="item" :index="index" :key="item.id" v-if="item.gubun == 3 && index < 11"></item-list>
				</ul>
			</div>
		</div>
		...
		<div class="exhibition-list">
			...
			<div class="list-card type-align-left">
				<ul id="exhibition2">
					<item-list v-for="(item , index) in items" :item="item" :index="index" :key="item.id" v-if="item.gubun == 4 && index < 15"></item-list>
				</ul>
			</div>
		</div>
			...
			<div id="fashionCtgy" class="fashion-category search-filter">
				<div id="fashionType1" class="panel current">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							...
							<% Call fnPrntDispCateNavi_2Depth("117","E","fnChgDisp") %>
						</ul>
					</div>
				</div>
				...
				<div id="fashionType2" class="panel">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							<%'정렬상자 호출; sDisp:전시카테고리, sType:확장여부, sCallback:콜백함수명 (via functions.asp)%>
							<% Call fnPrntDispCateNavi_2Depth("116","E","fnChgDisp") %>
						</ul>
					</div>
				</div>
				...
				<div id="fashionType3" class="panel">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							<%'정렬상자 호출; sDisp:전시카테고리, sType:확장여부, sCallback:콜백함수명 (via functions.asp)%>
							<% Call fnPrntDispCateNavi_2Depth("118","E","fnChgDisp") %>
						</ul>
					</div>
				</div>
				<div id="fashionType4" class="panel">
					...
					<div class="panelcont">
						<ul class="depth1 one-select">
							<%'정렬상자 호출; sDisp:전시카테고리, sType:확장여부, sCallback:콜백함수명 (via functions.asp)%>
							<% Call fnPrntDispCateNavi_2Depth("125","E","fnChgDisp") %>
						</ul>
					</div>
				</div>
			</div>
	...
	function fnChgDisp(disp) {
		if ((disp+"").length>3){
			location.href = "/category/category_detail2020.asp?disp="+disp;
		}
		else{
			location.href = "/category/category_main2020.asp?disp="+disp;
		}
	}
	</script>
