...
<%
    ...
    Dim checkDateDiff, mileage105235Regdate, mktTest
    ...
    sqlstr = "SELECT top 1 sub_opt2, regdate FROM [db_event].[dbo].[tbl_event_subscript] WITH (NOLOCK) WHERE userid= '"&LoginUserid&"' and evt_code in (101083,101392) "
    ...
    If Not(rsget.bof or rsget.eof) Then
        ...
        mileage105235Regdate = rsget("regdate")
    Else
        sub_opt2 = ""
    End If
    rsget.close
    ...
    Select Case Trim(alramAction)
        ...
        Case "request"
            ...
            If sub_opt2 = "1" Then
                Response.write "ERR|이미 신청하셨습니다.>?n신청일 : "&mid(mileage105235Regdate, 6, 2)&"월 "&mid(mileage105235Regdate, 9, 2)&"일 (ID당 1회만 참여 가능)"
                dbget.close()	:	response.End
        ...
        Case "action"
            ...
            Else
                ...
                If issueCheck = "N" Then
                    ...
                    '// 마일리지 로그 테이블에 넣는다.
                    sqlstr = " insert into db_user.dbo.tbl_mileagelog(userid , mileage , jukyocd , jukyo , deleteyn) values ('"&LoginUserid&"', '+200','"&eCode&"', '마일리지 이벤트 200원 지급','N') "
                    dbget.Execute sqlstr

                    '// 마일리지 테이블에 넣는다.
                    sqlstr = " update [db_user].[dbo].[tbl_user_current_mileage] set bonusmileage = bonusmileage + 200, lastupdate=getdate() Where userid='"&LoginUserid&"' "
                    dbget.Execute sqlstr
