...
<!-- #include virtual="/lib/classes/main/main_cate_contents_managecls.asp" -->
...
if (ocontentsCode.FResultCount<1) then
    response.write "<script >alert('not poscode');self.close();</script>"
	response.end
end if
...
If ocontentsCode.FOneItem.Ffixtype = "D" Then	'### 일별등록
	sqlStr = "select * from [db_sitemaster].[dbo].tbl_mobile_cate_mainCont"
	sqlStr = sqlStr & " where poscode = " & poscode & " and gnbcode = '"& gcode &"' and isusing = 'Y' and isnull(imageurl,'') <> '' "
...
Else
	If ocontentsCode.FOneItem.Ffixtype = "R" Then '###실시간
		...
		sqlStr = sqlStr & " order by orderidx asc , idx desc "
	Else
		sqlStr = "select top " & ocontentsCode.FOneItem.FuseSet & " * from [db_sitemaster].[dbo].tbl_mobile_cate_mainCont"
		sqlStr = sqlStr & " where poscode = " & poscode & " and gnbcode = '"& gcode &"' and isusing = 'Y' and isnull(imageurl,'') <> '' "
...
if vTotalCount>0 then
	...
	Do Until rsget.EOF
		...
		objXMLv.appendChild(objXML.createElement("texttitle1"))
		objXMLv.appendChild(objXML.createElement("texttitle2"))
		objXMLv.appendChild(objXML.createElement("saleflag"))
		objXMLv.appendChild(objXML.createElement("saletext"))
		...
		objXMLv.childNodes(6).text = db2Html(rsget("texttitle1"))
		objXMLv.childNodes(7).text = db2Html(rsget("texttitle2"))
		objXMLv.childNodes(8).text = db2Html(rsget("saleflag"))
		objXMLv.childNodes(9).text = db2Html(rsget("saletext"))
