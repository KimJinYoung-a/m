...
$(function() {
	...
	fnAPPchangPopCaption("CLEARANCE");
...
$(function(){
	...
	$(window).scroll(function() {
		if ($(window).scrollTop() >= ($(document).height()-$(window).height())*0.80){
			if(vScrl) {
				vScrl = false;
				vPg++;
				$.ajax({
...
function jsGoUrl(catecode, price){
      location.href = "/apps/appcom/wish/web2014/clearancesale/index.asp?disp="+catecode+"&price="+price+"&srm=<%=SortMet%>&flo1=<%=flo1%>&flo2=<%=flo2%>&flo3=<%=flo3%>";
}
...
function chkfree(flo1,flo2,flo3){
	...
	if(document.all.chksearchfree.checked==false){
		location.href = "/apps/appcom/wish/web2014/clearancesale/index.asp?disp=<%=catecode%>&price=<%=price%>&srm=<%=SortMet%>&flo1=&flo2="+flo2+"&flo3="+flo3;
	}
}
...
function chktenbae(flo1,flo2,flo3){
	...
	if(document.all.chksearchtenbae.checked==false){
		location.href = "/apps/appcom/wish/web2014/clearancesale/index.asp?disp=<%=catecode%>&price=<%=price%>&srm=<%=SortMet%>&flo1="+flo1+"&flo2=&flo3="+flo3;
	}
}
...
function chkpojangok(flo1,flo2,flo3){
	...
	if(document.all.chksearchpojangok.checked==false){
		location.href = "/apps/appcom/wish/web2014/clearancesale/index.asp?disp=<%=catecode%>&price=<%=price%>&srm=<%=SortMet%>&flo1="+flo1+"&flo2="+flo2+"&flo3=";
	}
}
</script>
<style>
		...
		<div class="titwrap bg-orange">
			<div class="tit-area">
				<div class="nav nav-stripe">
					<ul class="grid2">
						<li><a href="/apps/appcom/wish/web2014/sale/saleitem.asp">세일</a></li>
						<li><a href="/apps/appcom/wish/web2014/clearancesale/" class="on">클리어런스</a></li>
					</ul>
				</div>
				<h2>CLEARLANCE</h2>
			</div>
			...
			<ul id="lyrBestList">
				<% 
					For i=0 To oclearancelist.FResultCount-1 
						classStr = ""					
						linkUrl = Request.ServerVariables("PATH_INFO") & "?" & GetParam("adtprdid="&oclearancelist.FItemList(i).FItemID)							
				...
				<li class="<%=classStr%>" <%=chkIIF(adultChkFlag, "onclick=""confirmAdultAuth('"& linkUrl &"', "& chkiif(IsUserLoginOK, "true", "false") &");""","")%>>				
					<a href="" onclick="fnAPPpopupProduct('<%= oclearancelist.FItemList(i).FItemID %>');return false;">
						<%'// 해외직구배송작업추가 %>
