...
if IsUserLoginOK then
	'isMyFavBrand = getIsMyFavBrand(LoginUserid,oItem.Prd.FMakerid)	'## 에러납니다.
...
Dim vImgUrl
vImgUrl = b64encode(getThumbImgFromURL(oItem.Prd.FImageBasic,300,300,"true","false"))
%>
<!-- #include virtual="/apps/appCom/wish/web2014/lib/head.asp" -->
<script>
function jsAddCompareApp(){
	var itemid = <%=itemid%>;
	var img = "<%=vImgUrl%>";
	callNativeFunction('addCompare', {"itemid": itemid, "img": img});
	return false;
}

function jsSearchLayerClose(){
	callNativeFunction('quickviewlayerClose');
	return false;
}
</script>
</head>
<body class="default-font">
<div id="quickview" class="quickview" style="display:block;">
	<div class="inner">
		...
		<div class="btn-group">
			<div class="btn-half"><a href="" onClick="jsAddCompareApp(); return false;">비교 담기</a></div>
			<div class="btn-half"><a href="" onClick="fnAPPpopupProduct('<%=itemid%>'); return false;"><%=CHKIIF(oItem.Prd.FSellYn="Y","구매하기","상세보기")%></a></div>
		</div>
		<button type="button" class="btn-close" onClick="jsSearchLayerClose(); return false;">닫기</button>
	</div>
</div>
</body>
</html>
