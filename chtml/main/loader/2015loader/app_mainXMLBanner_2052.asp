...
Dim gaParam : gaParam = "&gaparam=todaymain_A_1"
...
If fileCont2 <> "" Then
	...
	For each tplNodes2 in cTmpl2
		...
		If CDate(CtrlDate2) >= CDate(startdate2) AND CDate(CtrlDate2) <= CDate(enddate2) Then
			...
			issueYN = "issue"
			issueHtml = "<div class=""swiper-slide""><img src="""& image2 &""" alt="""& altname2 &""" onclick=""fnAPPpopupEvent_URL('http://m.10x10.co.kr/apps/appCom/wish/web2014"& link2 & gaParam &"');""/></div>"
...
%>
			<div class="swiper-slide">
				<img src="<%= image %>" alt="<%= altname %>" onclick="fnAPPpopupEvent_URL('http://m.10x10.co.kr/apps/appCom/wish/web2014<%= link %><%=gaParam%>');"/>
			</div>
<%
			i = i + 1
		End If
