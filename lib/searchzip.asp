...
<title>10x10: 우편번호 찾기</title>
	...
	function DetailPost(elm,post1,post2,add,dong) {
		...
		$(".addDetail").remove();
		$(elm).after('<li class="addDetail bgGry4"><span>상세주소</span><input type="text" name="detail" style="width:200px;" /><button class="btn gryB" style="padding:8px 5px; font-size:11px; font-weight:normal;" onclick="CopyZip(); return false;">완료</button></li>');
	}
	...
	function chgTab(dv) {
		if(dv=="a") {
			$("#kTab1").addClass("on");
			$("#kTab2").removeClass("on");
			$("#dRowEx").html("찾고자 하는 주소의 동, 읍, 면 이름을 입력하세요.<br /><span class='fsSmall c999'>(예: 대치동,곡성읍,오곡면)</span>");
			$("#stype").val("addr");
		} else {
			$("#kTab1").removeClass("on");
			$("#kTab2").addClass("on");
			$("#dRowEx").html("찾고자 하는 주소의 도로명을 입력하세요.<br /><span class='fsSmall c999'>(예: 동숭1길, 세종대로)</span>");
			$("#stype").val("road");
		}
	}
	</SCRIPT>
...
<input type="hidden" name="stype"	id="stype" value="<%=stype%>" >
	<div class="popup">
		...
		<ul class="tabItem tMar20">
			<li id="kTab1" class="<%=chkIIF(stype="addr","on","")%> w50"><a href="javascript:chgTab('a');">지번검색<span class="elmBg"></span></a></li>
			<li id="kTab2" class="<%=chkIIF(stype="road","on","")%> w50"><a href="javascript:chgTab('r');">도로명검색<span class="elmBg"></span></a></li>
		</ul>

		<% If stype="addr" then %>
		<p id="dRowEx" class="innerH15W10 ct ftMidSm lh12">찾고자 하는 주소의 동, 읍, 면 이름을 입력하세요.<br /><span class="fsSmall c999">(예: 대치동,곡성읍,오곡면)</span></p>
		<% Else %>
		<p id="dRowEx" class="innerH15W10 ct ftMidSm lh12">찾고자 하는 주소의 도로명을 입력하세요.<br /><span class="fsSmall c999">(예: 동숭1길, 세종대로)</span></p>
		<% End If %>

		<div class="innerH15W10 bgGry2 ct">
			<input name="query" type="text" style="width:65%;" title="주소를 입력하세요." />
			<span class="btn btn3 redB w70B"><a href="javascript:SubmitForm(document.gil);">검색</a></span>
		</div>

		<% if (FRecultCount>0) then %>
		<ul class="zipCode ftMidSm2 c999">
			...
			%>
			<li onclick="DetailPost(this,'<%=rsget("ADDR_ZIP1")%>','<%=rsget("ADDR_ZIP2")%>','<% = useraddr01 %>', '<% = useraddr02 %>')"><span><%=rsget("ADDR_zip1")%>-<%=rsget("ADDR_zip2")%></span> <%=strAddress%></li>
			<%
	              rsget.MoveNext
	    		loop
	    	    end if
		...
		%>
		<ul class="zipCode ftMidSm2 c999">
			<li align="center"><% if (strQuery="") then %><% else %>검색 결과가 없습니다.<% end if %></li>
		</ul>
		<% end if %>
	</div>
</form>
