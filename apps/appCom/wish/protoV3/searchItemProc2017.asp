...
dim oResult, sDeviceId, sVerCd, sOS, sAppKey
...
elseif sType<>"searchlist" then
	...
	if SortMet="" then
		If dispCate="117" or dispCate="101" Then
			'//패션의류, 디자인문구
			SortMet="ne"
		else
			SortMet="be"		'베스트:be, 신상:ne
		end If
	end if
	...
	If vListOption = "all" or vListOption = "item" Then	'### 전체검색, 상품검색 만
		...
		If vListOption = "all" and (SearchText<>"") Then
			...
			If oGrEvt.FResultCount > 0 Then
				...
				If oGrEvt.FItemList(0).Fetc_itemimg = "" Then
					If oGrEvt.FItemList(0).Fetc_itemid <> "" AND oGrEvt.FItemList(0).Fetc_itemid <> "0" Then
...
'    end if 
	...
	oFilList("rstxt") = cStr(vRealReSearchText)
