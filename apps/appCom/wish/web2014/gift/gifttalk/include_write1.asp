<%
'###########################################################
' Description :  기프트톡 상품1개 추천받기(O,X)
	...
	<div class="container popWin bgGry">
		<div class="content bgGry" id="contentArea">
			<div class="writeTalk">
				...
				<div id="field">
					<form name="talkfrm" action="/apps/appCom/wish/web2014/gift/gifttalk/save_giftTalk.asp" method="post" style="margin:0px;" target="iframeproc">
					...
					<input type="hidden" name="itemcount" value="<%=vItemCount%>">
						<fieldset>
							<div class="add caseB">
								...
								<div class="item">
									<% If vTalkIdx = "" Then %>
										<a href="" onclick="if(confirm('비교상품을 찾아보시겠습니까?\n현재 창은 임시저장된 상태로 사라집니다.\n상품화면에서 TALK을 이용해서 상품을 등록해주세요.')){ fnAPPclosePopup(); }; return false;"><strong><span></span>비교상품<br /> 직접 찾기</strong></a>
									<% end if %>
								</div>
								<% If vTalkIdx = "" Then %>
									<div class="btnQuick"><span class="button btM2 cWh1 w100p"><a href="" onclick="fnAPPopenerJsCallClose('goitemReturn()'); return false;">빠른 상품추가</a></span></div>
								<% end if %>
							</div>
						...
						<div class="floatingBar">
							<div class="btnWrap bNum2">
								<div class="ftBtn"><span class="button btB1 btGry2 cWh1 w100p"><button type="button" onclick="<%If vTalkIdx="" Then %>if(confirm('취소버튼 선택시 내용이 모두 지워 집니다.')){ closeTalkDiv();}<%else%>fnAPPclosePopup()<% End If %>; return false;">취소</button></span></div>
