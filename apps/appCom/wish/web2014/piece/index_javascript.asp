...
$(function(){
	...
	$(window).scroll(function() {
		var zidx = $(".tag-and-items .swiper-container").css("zIndex");
		if (zidx > 200){
			zidx = 1
		}
		$(".tag-and-items .swiper-container").css("z-index",parseInt(zidx)+1);
				...
				$.ajax({
					...
					success: function(message) {
						if(message!="") {
							...
							vScrl=true;
							$(".tag-and-items .swiper-container").css("zIndex",1);
						}
					}
					,error: function(err) {
	...
	$("#piecemore").ajaxStop(function(){
		<% if flgDevice="A" then %>
		arrCancelP = new Array();
...
function jsPieceSearch(v){
	...
	$.ajax({
		...
		data: "rect="+v+"&adminid=<%=vAdminID%>&piecepop=<%=piecepop%>&tagSearchYN=Y&RvSelPiece=<%=RvSelPiece%>",
		cache: false,
		success: function(message) {
			if(message!="") {
