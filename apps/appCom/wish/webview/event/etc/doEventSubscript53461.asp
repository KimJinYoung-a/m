...
if mode="bingsureg" then
	If bingsugubun = "" Then
		Response.Write "<script type='text/javascript'>alert('미션을 선택해 주세요.'); parent.top.location.href='/apps/appCom/wish/webview/event/eventmain.asp?eventid="&getevt_codepage&"'</script>"
		dbget.close() : Response.End
	End IF
	...
	if bingsugubun="1" then
		...
		if subscriptcount1 > 0 then
			Response.Write "<script type='text/javascript'>alert('이미 참여 하셨습니다.'); parent.top.location.href='/apps/appCom/wish/webview/event/eventmain.asp?eventid="&getevt_codepage&"'</script>"
			dbget.close() : Response.End
		end if
		
		onlineordercount = get10x10onlineordercount(userid, "2014-07-10", "2014-07-21", "10x10", "app_wish", "", "N")
		if onlineordercount < 1 then
			Response.Write "<script type='text/javascript'>alert('텐바이텐APP 쇼핑 후, 응모하실 수 있습니다.'); parent.top.location.href='/apps/appCom/wish/webview/event/eventmain.asp?eventid="&getevt_codepage&"'</script>"
			dbget.close() : Response.End
		end if

		sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript](evt_code, userid, sub_opt1, sub_opt2, sub_opt3, device)" + vbcrlf
		sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', 'BINGSU', 1, '', 'A')" + vbcrlf
	...
	elseif bingsugubun="2" then
		...
		if onlineemailyn ="N" then
			Response.Write "<script type='text/javascript'>alert('텐바이텐 E-mail 수신동의후, 응모하실 수 있습니다.'); parent.top.location.href='/apps/appCom/wish/webview/event/eventmain.asp?eventid="&getevt_codepage&"'</script>"
			dbget.close() : Response.End
		end if

		sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript](evt_code, userid, sub_opt1, sub_opt2, sub_opt3, device)" + vbcrlf
		sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', 'BINGSU', 2, '', 'A')" + vbcrlf
	...
	elseif bingsugubun="3" then
		...
		sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript](evt_code, userid, sub_opt1, sub_opt2, sub_opt3, device)" + vbcrlf
		sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', 'BINGSU', 3, '', 'A')" + vbcrlf
