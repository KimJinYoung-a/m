...
if (GetLoginUserLevel()="7") or (GetLoginUserID="thensi7") or (GetLoginUserID="skyer9") or (GetLoginUserID="10x10vip") then
...
'앱 2.509 미만 버전 사용자들은 차이 결제 사용 불가
If (iappVer<"2.509") Then
...
If userid="cjstmdgh70" or userid="dudcjf9999" or userid="roqhfo11" Then
	IsEGiftMoneyEnable = False
End If
...
If isRentalCheck Then
	If oshoppingbag.FShoppingBagItemCount = 1 Then
		response.redirect "/apps/appCom/wish/web2014/inipay/userinfoRental.asp"
		response.end
	Else
		If oshoppingbag.FShoppingBagItemCount > 1 Then
			response.write "<script>alert('렌탈 상품은 단독으로 1개 상품만 구매 가능합니다.');location.href='/apps/appCom/wish/web2014/inipay/shoppingbag.asp';</script>"
			response.End
		End If
	End If
End If
...
'// 결제 정보 입력 페이지 상품 리스트 amplitude, cdp 전송
Dim revenueInfoEventValuesJson
revenueInfoEventValuesJson = "{"
revenueInfoEventValuesJson = revenueInfoEventValuesJson & "'order_total_price' : '"&subtotalPrice&"' "
revenueInfoEventValuesJson = revenueInfoEventValuesJson & ", 'order_total_count' : '"&oshoppingbag.FShoppingBagItemCount&"'"
revenueInfoEventValuesJson = revenueInfoEventValuesJson & ", 'products' : ["
If oshoppingbag.FShoppingBagItemCount > 0 Then
	For i = 0 to oshoppingbag.FShoppingBagItemCount - 1
		revenueInfoEventValuesJson = revenueInfoEventValuesJson & "{'itemid' : '"&oshoppingbag.FItemList(i).FItemID&"', 'price' : '"&oshoppingbag.FItemList(i).GetCouponAssignPrice*oshoppingbag.FItemList(i).FItemEa&"', 'quantity' : '"&oshoppingbag.FItemList(i).FItemEa&"','product_name':'"&Server.URLEncode(replace(oshoppingbag.FItemList(i).FItemName,"'",""))&"'},"
	Next
Else
	revenueInfoEventValuesJson = revenueInfoEventValuesJson & ""
End If
revenueInfoEventValuesJson = left(revenueInfoEventValuesJson, Len(revenueInfoEventValuesJson)-1)	
revenueInfoEventValuesJson = revenueInfoEventValuesJson & "]}"
revenueInfoEventValuesJson = Replace(revenueInfoEventValuesJson, "'", "\""")
'response.write revenueInfoEventValuesJson
%>
<%
	'토스 즉시 할인 이벤트 정태훈 2020.02.05 -> 2020.03.24
	dim currentDate
	currentDate = date()
%>
...
$(function() {
	setTimeout(function(){
		fnAmplitudeEventActionJsonData('view_userinfo','<%=revenueInfoEventValuesJson%>');
	},100);
});	
	...
	<div class="container">
		...
		<div class="content" id="contentArea" style="padding-bottom:0;">
			<div class="cartV16a">
				...
				<input type="hidden" name="pdiv" value="userinfo" />
				</form>
				...
				<div class="orderInfoV16a">
					...
					<div class="cartGrpV16a personal-index">
						...
						<div class="bxWt1V16a infoUnitV16a">
							...
							<div class="tMar1r cartNotiV21">
                                <ul>
                                    <li class="cRd1V16a txtArrow">개인통관고유번호의 발급자명과 수령자명이 일치해야 합니다.</li>
                                    <li class="cMGy1V16a txtArrow">통관 시 전체 주문/결제금액이 $150을(를) 초과할 경우 관/부가세가 발생할 수 있습니다.</li>
                                </ul>
                            </div>
						</div>
					</div>
							...
							<div>
								<dl class="infoArrayV16a">
									...
									<dd class="cMGy1V16a">
										...
										<p class="tMar0-3r"><%= MileageDisabledString %></p>
									</dd>
									<% else %>
					...
					<div class="cartGrpV16a freebiesV16a diarystoryGift">
						<style>
							.freebieSltV16a dl dt.tglBtnV16a p.hasArrow:after {display:none;}
						</style>
						<div class="bxLGy2V16a grpTitV16a">
							<h2>2022 다이어리 구매 사은품 선택</h2>
						</div>
						<div class="bxWt1V16a freebieSltV16a">
							<div class="bxWt1V16a">2021.09.01 AM 11:00 ~ 12.31</div>
							<%
							for i=0 to oDiaryOpenGift.FResultCount-1
							...
							%>
							<dl class="showHideV16a">
								<dt class="tglBtnV16a"><p class="hasArrow"><%=formatnumber(oDiaryOpenGift.FItemList(i).Fgift_range1,0)%>원 이상 구매 시</p></dt>
								<dd class="tglContV16a">
									<ul class="freebieListV16a">
							<%	
									End If 
							%>
										<li <%=chkIIF(oDiaryOpenGift.FItemList(i).IsGiftItemSoldOut or (optAllsoldOut),"class=""soldoutV16a""","")%>>
											...
											<div>
												...
												<% IF oDiaryOpenGift.FItemList(i).Fgift_delivery = "N" THEN %>
												<p class="tMar0-4r cMGy1V16a fs1-1r">랜덤 증정</p>
												<% END IF %>
					...
					<div class="cartGrpV16a" id="i_paymethod">
						...
						<% If Not(vlsOnlyHanaTenPayExist) Then %>
							<%
							...
							bannerTitle = oCardDisInfo.FOneItem.FbannerTitle
							set oCardDisInfo = nothing
							%>
							<% If cardCode=40 and salePrice>0 Then %>
								<div class="bnr-kakao" id="bnr-kakao"><strong style="font-weight:bold;"><%=bannerTitle%></strong><span><%=FormatNumber(salePrice,0)%></span></div>
								<script>
								$(function() {
									//Positioning
									$("#tabKakao").insertBefore("#tabCard");
									$("input[name='Tn_paymethod']").val('800');

									//Description
									$("#paymethod_desc1_100").css("display","none");
									$("#paymethod_desc1_800").css("display","table-row-group");

									//Banner
									$(".btnBarV16b li").on("click",function(){
										$(this).addClass("current");
										if($("input[name=Tn_paymethod]").val()=="800") {
											$("#bnr-kakao").show();
										} else {
											$("#bnr-kakao").hide();
										}
							...
							<% elseIf cardCode=41 and salePrice>0 Then %>
								<div class="bnr-naver" id="bnr-naver"><strong style="font-weight:bold;"><%=bannerTitle%></strong><span><%=FormatNumber(salePrice,0)%></span></div>
								<script>
								$(function() {
									//Positioning
									$("#tabNPay").insertBefore("#tabCard");
									$("input[name='Tn_paymethod']").val('900');
									...
									$(".btnBarV16b li").on("click",function(){
										$(this).addClass("current");
										if($("input[name=Tn_paymethod]").val()=="900") {
											$("#bnr-naver").show();
										} else {
											$("#bnr-naver").hide();
										}
							...
							<% elseIf cardCode=42 and salePrice>0 Then %>
								<div class="bnr-payco" id="bnr-payco"><strong style="font-weight:bold;"><%=bannerTitle%></strong><span><%=FormatNumber(salePrice,0)%></span></div>
								<script>
								$(function() {
									//Positioning
									$("#tabPayco").insertBefore("#tabCard");
									$("input[name='Tn_paymethod']").val('950');
									...
									$(".btnBarV16b li").on("click",function(){
										$(this).addClass("current");
										if($("input[name=Tn_paymethod]").val()=="950") {
											$("#bnr-payco").show();
										} else {
											$("#bnr-payco").hide();
										}
							...
							<% elseIf cardCode=43 and salePrice>0 Then %>
								<div class="bnr-toss" id="bnr-toss"><strong style="font-weight:bold;"><%=bannerTitle%></strong><span><%=FormatNumber(salePrice,0)%></span></div>
								<script>
								$(function() {
									//Positioning
									$("#tabToss").insertBefore("#tabCard");
									$("input[name='Tn_paymethod']").val('980');
									...
									$(".btnBarV16b li").on("click",function(){
										$(this).addClass("current");
										if($("input[name=Tn_paymethod]").val()=="980") {
											$("#bnr-toss").show();
										} else {
											$("#bnr-toss").hide();
										}
							...
							<% elseIf cardCode=44 and salePrice>0 Then %>
								<div class="bnr-chai" id="bnr-chai"><strong style="font-weight:bold;"><%=bannerTitle%></strong><span><%=FormatNumber(salePrice,0)%></span></div>
								<script>
								$(function() {
									//Positioning
									$("#tabChai").insertBefore("#tabCard");
									$("input[name='Tn_paymethod']").val('990');
									...
									$(".btnBarV16b li").on("click",function(){
										$(this).addClass("current");
										if($("input[name=Tn_paymethod]").val()=="990") {
											$("#bnr-chai").show();
										} else {
											$("#bnr-chai").hide();
										}
							...
							<% elseIf salePrice>0 then %>
								<div class="bnr-pay" id="bnr-pay"><strong style="font-weight:bold;"><%=bannerTitle%></strong><span><%=FormatNumber(salePrice,0)%></span></div>
								<script>
								$(function() {
									$(".btnBarV16b li").on("click",function(){
										if($("input[name=Tn_paymethod]").val()=="100") {
											$("#bnr-pay").show();
										} else {
											$("#bnr-pay").hide();
										}
						...
						<div class="bxWt1V16a payMethodV16a">
							<div>
								<div id="paymethodtab">
									<% If G_PG_HANATEN_ENABLE and vlsOnlyHanaTenPayExist Then '텐텐하나체크카드 전용결제 상품 %>
									<ul class="btnBarV16b">
										<li class="hanaten" onclick="CheckPayMethod('190');"><div>텐바이텐 체크카드</div></li>
									</ul>
									<% Else '일반 상품%>
									<ul class="btnBarV16b">
										<li class="" id="tabCard" onclick="CheckPayMethod('100');"><div>신용카드<%= ChkIIF(IsKBRdSite," (KB카드)","") %></div></li>
										<% if G_PG_HANATEN_ENABLE then %>
											<li class="hanaten" id="tabTenHana" onclick="CheckPayMethod('190');"><div>텐바이텐 체크카드</div></li>
										<% end if %>
										<li class="mobileP" id="tabHPP" onclick="CheckPayMethod('400');"><div>휴대폰</div></li>
										<% if Not(oshoppingbag.IsBuyOrderItemExists) then %>
											<li class="accP" id="tabBank" onclick="CheckPayMethod('7');"><div>무통장<%= ChkIIF(IsCyberAccountEnable,"","") %></div></li>
										<% end if %>
										<% If G_PG_KAKAOPAY_ENABLE Then %>
											<li class="kakaoP" id="tabKakao" onclick="CheckPayMethod('800');"><div>카카오페이</div></li>
										<% End If %>
										<% if G_PG_NAVERPAY_ENABLE then %>
											<li class="naverP" id="tabNPay" onclick="CheckPayMethod('900');"><div>네이버페이</div></li>
										<% end if %>
										<% if G_PG_PAYCO_ENABLE then %>
											<li class="payco" id="tabPayco" onclick="CheckPayMethod('950');"><div>PAYCO간편결제</div></li>
										<% End If %>
										<% if G_PG_TOSSPAYNEW_ENABLE then %>
											<li class="toss" id="tabToss" onclick="CheckPayMethod('980');"><div>토스</div></li>
										<% end if %>
										<% if G_PG_CHAIPAYNEW_ENABLE then %>
											<li class="chai" id="tabChai" onclick="CheckPayMethod('990');"><div>CHAI</div></li>
										<% end if %>																								
									</ul>
									<% End If %>
								</div>
								...
								<% '차이 관련 안내문구 추가 %>
								<div id="paymethod_desc1_990" class="bxLGy1V16a cartNotiV16a" style="display:none;">
									<ul>
										<li>차이는 은행 계좌만 등록하면 간편하게 결제할 수 있는 결제 서비스 입니다.</li>
										<li>은행 점검시간인 23:30~00:30까지는 이용이 불가 합니다.</li>
										<li>차이 결제 문의, 차이 고객센터 1544-7839</li>
									</ul>
								</div>
								<% '// 차이 관련 안내문구 추가 %>																	
								<% '토스 관련 안내문구 추가 %>
									...
									<div id="payDescSub3_7" class="bxLGy1V16a cartNotiV16a">
										<ul>
											...
											<% If now() >= #2021-11-24 10:00:00# Then %>
												<li>무통장주문 후 3일이 지날 때까지 입금이 안되면 주문은 자동으로 취소됩니다. 한정상품 주문 시 유의하여 주시기 바랍니다.</li>
											<% Else %>
					...
					<script>
					$(function(){
						$("input[type='checkbox']:not('#orderAgree')").click(function(){
							if($(this).attr("id")=="policyY"){
								$("#orderAgree,#agreechk01,#agreechk02").attr("checked",$(this).is(":checked"));
							} else {
								$("#policyY").attr("checked",$("#orderAgree").is(":checked")&&$("#agreechk01").is(":checked")&&$("#agreechk02").is(":checked"));
							}
						});
					...
					<div class="bxWt1V16a payMethodV16a cartNotiV21">
                        <div class="bMar20 agreeWrap">
                            <div class="txtArrow">품절 발생 시 별도의 연락을 하지 않고 선택하신 결제 방법으로 안전하게 환불해 드립니다.</div>

							<div class="bxLGy infoUnitV16a tMar10" id="refundInfo" style="display:none">
								<dl class="infoArrayV16a">
									<dt>환불 은행 선택</dt>
									<dd>
								...
								<dl class="infoArrayV16a">
									<dt>환불 계좌번호</dt>
									<dd><input type="text" name="encaccount" style="width:100%;"></dd>
								</dl>
								<dl class="infoArrayV16a">
									<dt>예금주</dt>
									<dd><input type="text" name="rebankownername" style="width:100%;"></dd>
								</dl>
								<p class="caution">주문 취소일 기준, 3-5일(주말제외) 후 환불금액이 입금됩니다.</p>
							</div>

							<div class="ismscheck">
							<input type="checkbox" id="orderAgree">
							<label for="orderAgree">
								모든 내용을 확인하였으며 구매조건에 동의합니다.                                         
							</label>
							</div>
							<div class="ismscheck">
							<input type="checkbox" id="agreechk01">
							<label for="agreechk01">
								주문 진행을 위해 관련된 개인정보를 수집합니다.                                           
								<button class="btnMore" onclick="showDetail(this, 'detailCollect')" type="button" style="display:block;margin-left:0; ">수집 내용 자세히보기</button>
							</label>
							</div>
							...
							<label for="agreechk02">
								다음의 판매자(제3자)에게 개인정보를 제공하는 것에 동의합니다.
                        ...
                        <div class="chkAgreeV21">
                            <input type="checkbox" id="policyY" <% If (vlsOnlyHanaTenPayExist) Then : Response.Write "onclick=""CheckPayMethod('190');""" : End If %>>
                            <label for="policyY">모두 동의합니다.</label>
                        </div>
						<div class="btnAreaV16a tMar1r">
                            <button type="button" class="btnV16a btnRed2V16a" onclick="PayNext(document.frmorder,'<%= iErrMsg %>'); return false;">결제하기</button>
                        </div>
					</div>
					<% else %>
					<div class="bxWt1V16a payMethodV16a cartNotiV21">
                        ...
                        <div class="chkAgreeV21">
                            <input type="checkbox" id="orderAgree" <% If (vlsOnlyHanaTenPayExist) Then : Response.Write "onclick=""CheckPayMethod('190');""" : End If %>>
                            <label for="orderAgree">모든 내용을 확인하였으며 구매조건에 동의합니다.</label>
                        </div>
						<div class="btnAreaV16a tMar1r">
                            <button type="button" class="btnV16a btnRed2V16a" onclick="PayNext(document.frmorder,'<%= iErrMsg %>'); return false;">결제하기</button>
                        </div>
