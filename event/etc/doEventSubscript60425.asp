...
userid = GetLoginUserID
	...
	If Left(Now(), 10)>="2015-03-18" And Left(Now(), 10) < "2015-03-19" Then
	Else
...
elseif mode="coupon" then
	...
	If Now() < #03/18/2014 08:59:59# Then
	...
	If rsget(0) > 30000 Then
		response.write "<script>alert('오늘의 쿠폰이 모두 소진되었습니다.'); parent.location.reload();</script>"
		response.End
	End If
	rsget.close()


	vQuery = "select count(*) from [db_user].dbo.tbl_user_coupon where masteridx = '713' and userid = '" & userid & "'"
	rsget.Open vQuery,dbget,1
	If rsget(0) > 0 Then
	...
	vQuery = "insert into [db_user].dbo.tbl_user_coupon(masteridx,userid,coupontype,couponvalue,couponname,minbuyprice,startdate,expiredate,targetitemlist,couponmeaipprice,reguserid,validsitename) " & _
			 "values('713', '" & userid & "', '2','10000','[app쿠폰] 만원의 기적','30000','2015-03-18 00:00:00','2015-03-18 23:59:59','',0,'system','app')"
	dbget.execute vQuery

		response.write "<script language='javascript'>alert('발급완료!\n쿠폰은 금일 03/18(수) 23시59분 종료됩니다!'); parent.top.location.reload();</script>"
		dbget.close()
	    response.end
end if
%>
