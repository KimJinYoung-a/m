...
function paging_ajax(){
	...
	var str = $.ajax({
			type: "GET",
	        url: "/apps/appCom/between/my10x10/order/myorderlist_ajax.asp",
			...
			<div class="orderDelivery">
				...
				<% if myorder.FResultCount > 0 then %>
					...
					<ul class="myOdrList" id="lySearchResult">
						...
						<li>
							<div class="odrInfo">
								<a href="/apps/appCom/between/my10x10/order/myorderdetail.asp?idx=<%= myorder.FItemList(i).FOrderSerial %>&pflag=<%= pflag %>&cflag=<%= cflag %>">
							...
							<div class="btnCont">
		                    	<% if (myorder.FItemList(i).IsWebOrderCancelEnable) then %>
		                    		<span class="btn02 cnclGry"><a href="/apps/appCom/between/my10x10/order/myorderallcancel.asp?idx=<%= myorder.FItemList(i).FOrderSerial %>">주문취소</a></span>
		                    	<% end if %>
                            	<% if (myorder.FItemList(i).IsWebOrderInfoEditEnable) then %>
                            		<span class="btn02 cnclGry"><a href="/apps/appCom/between/my10x10/order/myorderchangeaddr.asp?orderserial=<%= myorder.FItemList(i).FOrderSerial %>">배송지변경</a></span>
		                    	<% end if %>
