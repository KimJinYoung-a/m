...
{
	$.ajax({
		url: "/culturestation/event_commajax.asp?view=l&iCC="+page+"&eventid=<%=vculidx%>",
		cache: false,
		async: false,
...
function cmtreg(cmtidx){
	<%IF not(IsUserLoginOK) THEN%>
		if(confirm("로그인을 하셔야 글을 남길 수 있습니다.\n로그인 하시겠습니까?") == true) {
			parent.location.href = "<%=M_SSLUrl%>/login/login.asp?backpath=/culturestation/culturestation_event.asp?view=l&eventid=<%=vculidx%>'";
			return true;
		} else {
			return false;
		}
	...
	if (str.length=='2'){
		if (str=='i1'){
			fnOpenModal('/culturestation/event_comment.asp?view=l&eventid=<%=vculidx%>');
			return;
		}else if (str=='99'){
			alert('로그인을 해주세요.');
			return;
		...
		<div class="content bgGry" id="layerScroll">
			<div id="scrollarea">
				<div class="inner5 tMar15 cmtCont">
					<div class="tab01 noMove" id="writediv" style="display:<%=CHKIIF(vView="w","block","none")%>;">
						...
						<div class="tabContainer box1">
							<div id="cmtWrite" class="tabContent">
								<form method="post" action="/culturestation/doEventComment.asp" name="upcomment" id="upcomment" target="iframeDB" style="margin:0px;">
					...
					<form name="frmactNew" id="frmactNew" method="post" action="/culturestation/doEventComment.asp" target="iframeDB" style="margin:0px;">
