...
iorderParams.Fbuyname           = Trim(LeftB((request.Form("buyname")),30))
...
If GetLoginUserID = "dlwjseh" or GetLoginUserID = "corpse2" Then
    Dim validCoupon : validCoupon = Split(oshoppingbag.validationCoupon(iorderParams.Fcouponmoney, mayBCpnDiscountPrc), "/")
...
if (CLng(oshoppingbag.getTotalCouponAssignPrice(packtype) + iorderParams.fpojangcash -iorderParams.Fmiletotalprice-iorderParams.Fcouponmoney-iorderParams.Fspendtencash-iorderParams.Fspendgiftmoney) <> CLng(subtotalprice)) then
	...
	'sqlStr = sqlStr + " convert(varchar(250),'장바구니 금액 오류 mo_acc ::"&vAppLink&":"&iorderParams.Freferip&"::"&oshoppingbag.getTotalCouponAssignPrice(packtype) + iorderParams.fpojangcash &"::"&iorderParams.Fmiletotalprice&"::"&iorderParams.Fcouponmoney&"::"&iorderParams.Fspendtencash&"::"&iorderParams.Fspendgiftmoney&"::"&subtotalprice&"'))"
...
IF application("Svr_Info")="Dev" THEN LGD_CASNOTEURL = "http://61.252.133.2:8888/admin/apps/DC_CA_noteurl.asp"
    ...
    if xpay.TX() then
        ...
        ' sqlStr = "Insert into [db_sms].[ismsuser].em_tran(tran_phone, tran_callback, tran_status, tran_date, tran_msg )"
    	' sqlStr = sqlStr + " values( '010-6324-9110', '1644-6030', '1', getdate(), "
    	' sqlStr = sqlStr + " '가상M-"&application("Svr_Info")&" [" + xpay.resCode + "] " & Replace(Left(xpay.resMsg,60),"'","") & "')"

        sqlStr = " exec [db_log].[dbo].[usp_ErrorNoti_Input_With_SMS] '"&iorderserial&"','xPayVacct-M','[" + xpay.resCode + "]:"&replace(Left(xpay.resMsg,60),"'","")&"'"
    	dbget.Execute sqlStr
    	
    	'response.write "죄송합니다. 가상계좌 발급시 오류가 발생하였습니다."
        'response.end
    end if
...
'' ================ 보증보험 끝 ================================
    ...
    '' ================ 품절 시 자동 취소 처리 환불 계좌 정보 추가 2020.11.19 정태훈 ================================
    if userid <> "" then
...
IF (InsureErrorMsg<>"") then
    ...
    ''response.write "<script language='javascript'>location.replace('" & wwwUrl & vAppLink & "/inipay/displayorder.asp?dumi="&dumi&"');</script>"
ELSE
