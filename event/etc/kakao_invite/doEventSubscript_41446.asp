...
'	'response.end
	...
	If resultGo = "B" Then '배송 시작
		...
		If cnt > 0 then
			sqlStr = "update db_temp.dbo.kakao_result set okcode = 'Y' , address = '"& addr &"' , zipcode = '"& zipcode &"' where uname='"& uname &"' and uphone = '"& uphone &"' and inviteidx = "&przidx
	...
	ElseIf resultGo = "G" Then '즉석당첨 시작
		...
		" From db_temp.dbo.kakao_invite " &_
		" WHERE evt_code='" & evt_code & "'" &_
		" and uname='"& uname &"' and uphone = '"& uphone &"' and isusing = 'N'  and invitecnt = 3 "
		rsget.Open sqlStr,dbget,1
		if rsget.EOF or rsget.BOF Then
		idx = 0
		Else 
		...
		Else
			...
			randomize
			rndPt=int(Rnd*7000)+1 '확률 1/7000

			if rndPt>0 and rndPt<=5 then
				'#11inch 사첼백(5명) 
				przNo = 1
				lmtCnt = 5
			elseif rndPt>5 and rndPt<=100 then
				...
				przNo = 2
				lmtCnt = 100
			elseif rndPt>100 and rndPt<=1000 then
				'#바나나맛 우유 2개 기프티콘 (1,000명)
				przNo = 3
				lmtCnt = 1000
			else
				'꽝
...
'			response.write sqlStr
	...
	Else '응모하기가 아닌 일반
		If returnIdx <> "" And returnPhone <> "" Then '초대에응하기 일때
			...
			If idx = "" Then '내역에 없을때
				Response.Write	"<script language='javascript'>" &_
							"if (confirm('당신을 초대한 친구가 목표 초대수(3명)를 이미 달성하였습니다.\n 이제 직접 이벤트에 참여해 사첼백에 도전하고 싶으면 아래의 확인 버튼을 누르세요.')){ top.document.location.href = 'http://m.10x10.co.kr/event/eventmain.asp?eventid="& evt_code &"'}else{ top.document.location.href = 'http://m.10x10.co.kr' 	} " &_
							"</script>"
				response.end	
			Else
				If uip = Request.ServerVariables("REMOTE_ADDR") Then '동일 ip접속시 튕김
					...
					response.end	
				End If 
				If invitecnt < 3 Then '3명이하일때 update
					sqlStr = "Select count(idx) " &_
						" From db_temp.dbo.kakao_invite " &_
						" WHERE evt_code='" & evt_code & "'" &_
