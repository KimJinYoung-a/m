...
isCrossPlatform = TRUE
	...
	' */
	LGD_RETURNURL				= httpOrSSLTenURL&"/inipayAPI/xpay/returnurl.asp"


    '/*
    ' * 가상계좌(무통장) 결제 연동을 하시는 경우 아래 LGD_CASNOTEURL 을 설정하여 주시기 바랍니다.
    ' */
	'''LGD_CASNOTEURL             = httpOrSSLTenURL&"/inipayAPI/xpay/cas_noteurl.asp"
	'/*
	' * ISP 카드결제 연동중 모바일ISP방식(고객세션을 유지하지않는 비동기방식)의 경우, LGD_KVPMISPNOTEURL/LGD_KVPMISPWAPURL/LGD_KVPMISPCANCELURL를 설정하여 주시기 바랍니다.
	' */
	'LGD_KVPMISPNOTEURL       = httpOrSSLTenURL&"/inipayAPI/xpay/note_url.asp"
	'LGD_KVPMISPWAPURL		 = httpOrSSLTenURL&"/inipayAPI/xpay/mispwapurl.asp?LGD_OID=" + LGD_OID    'ISP 카드 결제시, URL 대신 앱명 입력시, 앱호출함
	'LGD_KVPMISPCANCELURL     = httpOrSSLTenURL&"/inipayAPI/xpay/cancel_url.asp"
...
<!--
	...
	function setLGDResult(){
		...
		if(document.getElementById('LGD_RESPCODE').value == '0000' ){
			...
			fdo.action = "/inipayAPI/MobileResult_Dacom.asp";
			fdo.submit();
		} else {
            setEnableComp();
...
function doPay_ActiveX(){
    ...
    if (ret=="00"){     //ActiveX 로딩 성공
        ...
        if( "0000" == LGD_RESPCODE ) { //인증성공
			...
			parent.document.frmorder.action = "/inipayAPI/MobileResult_Dacom.asp";
