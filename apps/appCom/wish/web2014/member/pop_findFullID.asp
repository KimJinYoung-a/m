...
<%
	...
	if sName="" or (sCell="" and sEmail="") then
		Response.Write "<script>fnAPPopenerJsCallClose(""jsPopMsg('E11')"");</script>"
	...
	if sCell<>"" then
		if ubound(split(sCell,"-"))<2 then
			Response.Write "<script>fnAPPopenerJsCallClose(""jsPopMsg('E12')"");</script>"
			dbget.Close: Response.End
		end if
	end if
	...
	if sEmail<>"" then
		if ubound(split(sEmail,"@"))<1 then
			Response.Write "<script>fnAPPopenerJsCallClose(""jsPopMsg('E13')"");</script>"
			dbget.Close: Response.End
		end if
	end if
...
function fnSendFullId(mtd) {
	...
	var rstStr = $.ajax({
		type: "POST",
		url: "/member/pop_findFullId_proc.asp",
	...
	switch(rstStr) {
		case "E1":
			// 유효기간 만료
			fnAPPopenerJsCallClose("jsPopMsg('E11')");
			break;
		case "E2":
			// 파라메터 없음
		...
		case "10":
			// 휴대폰 전송 성공
			fnAPPopenerJsCallClose("jsPopMsg('10')");
			break;
		case "20":
			// 이메일 전송 성공
			fnAPPopenerJsCallClose("jsPopMsg('20')");
			break;
		default:
				...
				<div class="findResult tPad0">
					<div class="history">
						...
						<%
								...
								Response.Write "<script>fnAPPopenerJsCallClose(""jsPopMsg('E31')"");</script>"
