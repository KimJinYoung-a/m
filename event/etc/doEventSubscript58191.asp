...
elseif mode="coupon" then
	...
	If Now() < #12/30/2014 08:59:59# Then
		response.write "<script language='javascript'>alert('아직 이벤트 시간이 되지 않았습니다.'); parent.location.reload();</script>"
		dbget.close()
	    response.end
	End IF

	vQuery = "select count(*) from [db_user].dbo.tbl_user_coupon where masteridx = '680' and userid = '" & userid & "'"
	...
	vQuery = "insert into [db_user].dbo.tbl_user_coupon(masteridx,userid,coupontype,couponvalue,couponname,minbuyprice,startdate,expiredate,targetitemlist,couponmeaipprice,reguserid,validsitename) " & _
			 "values('680', '" & userid & "', '2','10000','[app쿠폰]10,000원-3만원이상','30000','2014-12-30 00:00:00','2014-12-30 23:59:59','',0,'system','app')"
	dbget.execute vQuery

		response.write "<script language='javascript'>alert('발급완료!\n쿠폰은 금일 12/30(화) 23시59분 종료됩니다!'); parent.top.location.reload();</script>"
		dbget.close()
	    response.end
end if
%>
