...
<%
	...
	dim dispCate : dispCate = getNumeric(requestCheckVar(request("dispCate"),18))
		if dispCate="" then dispCate = getNumeric(requestCheckVar(request("disp"),18))
	...
	if dispCate<>"" then
		vCateDepth = len(dispCate)\3+1			'하위 뎁스
	else
		vCateDepth = 1
	end if
	...
	if vCateDepth=1 then
		Response.Write "<li " & chkIIF(dispCate="","class=""selected""","") & "><a href=""#"" onclick=""goCategoryList('');return false;""><span>전체<span></a></li>" & vbCrLf
	end if
	...
	If oGrCat.FResultCount>0 Then
		FOR lp = 0 to oGrCat.FResultCount-1
			Response.Write "<li " & chkIIF(dispCate=left(oGrCat.FItemList(lp).FCateCode,vCateDepth*3),"class=""selected""","") & "><a href=""#"" onclick=""goCategoryList('" & left(oGrCat.FItemList(lp).FCateCode,vCateDepth*3) & "');return false;""><span>" & splitValue(oGrCat.FItemList(lp).FCateName,"^^",(vCateDepth-1)) & "<span></a></li>" & vbCrLf
		Next
	End If
