...
Dim fnGetList
    ...
    If CInt(vImageCount) >= CInt(vTotalCount) Then
		...
		For i = 1 to vTotalCount
    		...
    		BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<a href=javascript:fnAPPpopupBrand('"& Trim(fnGetList(7,i-1)) &"');><div class='brandInfoV15a'>" + DoubleQuat + VbCrlf
		...
		End If
			...
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<img src='" & db2Html(fnGetList(0,i-1)) &"' alt='브랜드Pick"&i&"' /></a>" + DoubleQuat + VbCrlf
			Else
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "	<img src='" & staticImgUrl & "/brandstreet/main/" & db2Html(fnGetList(0,i-1)) &"' alt='브랜드Pick"&i&"' /></a>" + DoubleQuat + VbCrlf
			end if
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "</div>" + DoubleQuat + VbCrlf
		Next
			BufStr = BufStr + VarName + "+=" + DoubleQuat + "</div>" + DoubleQuat + VbCrlf
