	...
	If vListOption = "all" or vListOption = "item" Then	'### 전체검색, 상품검색 만
		...
		If vListOption = "all" and (SearchText<>"") Then
			...
			If oGrEvt.FResultCount > 0 Then
				...
				oMktEvt("evtcode") = cStr(oGrEvt.FItemList(0).Fevt_code)
				oMktEvt("evtname") = cStr(vEnm)
			...
			If oGrEvt.FResultCount > 0 Then
				...
				oPlanEvt("evtcode") = cStr(oGrEvt.FItemList(0).Fevt_code)
				oPlanEvt("evtname") = cStr(vEnm)
				oPlanEvt("evtsubcopy") = cStr(db2html(oGrEvt.FItemList(0).Fevt_subcopyK))
				
				If oGrEvt.FItemList(0).Fetc_itemimg = "" Then
					oPlanEvt("evtimg") = b64encode("http://webimage.10x10.co.kr/image/icon1/" & GetImageSubFolderByItemid(itemid) & "/" & oGrEvt.FItemList(0).Ficon1image & "")
				Else
			...
			If oDocPl.FResultCount > 0 Then
				...
				oPlaying("linkurl") = b64encode(mDomain &"/apps/appCom/wish/web2014/playing/view.asp?didx="&oDocPl.FItemList(0).Fdidx&"")
		...
		End If
	ElseIf vListOption = "event" and (SearchText<>"") Then	'### 이벤트 만
	ElseIf vListOption = "playing" and (SearchText<>"") Then	'### 플레잉 만
	End If
...
''필요시 이걸 사용
		...
		If vQuickCnt > 0 Then
			If isArray(vArrQ) Then
				...
				If vQLinkGubun = "etc" or vQLinkGubun = "prd" Then
					oQuick("quickurl") = b64encode(vArrQ(6,0))
					oQuick("quickcode") = ""
					oQuick("quicktitle") = ""
				Else
				...
				If vQBLinkGubun = "etc" or vQBLinkGubun = "prd" Then
					oQuick("btnurl") = b64encode(vArrQ(12,0))
					oQuick("btncode") = ""
					oQuick("btntitle") = ""
				Else
					'### category, street, search, event
	...
	if (offset="" or offset="0") and (SearchText<>"") and (vListOption = "all") then
		...
		If cCu.FResultCount > 0 Then
			...
			ElseIf cCu.FItemList(0).Fbgclass = "brown" Then
			...
			For i = 0 To cCu.FResultCount - 1
				Set oCuItem = jsObject()
					...
					oCuItem("itemcode") = cStr(cCu.FItemList(i).FItemID)
					oCuItem("itemname") = cStr(cCu.FItemList(i).FItemName)
...
'    end if 
    ...
    'if (GetLoginUserLevel="7") or (application("Svr_Info")="090") then
        Call addIIsLog(SearchText,ReSearchText,makerid,dispCate,CLNG(offset/sSize)+1,SortMet)
    'end if
end if
...
function addIIsLog(SearchText,rstxt,makerid,dispCate,ipage,isortMtd)
