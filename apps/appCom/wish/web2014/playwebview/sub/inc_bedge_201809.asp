...
<%
'####################################################
' Description : 띵 배지 9월 (수작업)
' History : 2018-09-07 최종원
'####################################################
...
dim presentDay
presentDay = #09/16/2018 23:59:59#
...
function jsSubmitComment(frm){		
	<% If IsUserLoginOK() Then %>	
		<% if date() >="2018-09-06" and date() < "2018-09-16" then %>
...
<article class="playDetailV16 thingthing">
	...
	<% if now() <= presentDay then %>
		<div>
			<em class="month"><span>9월 THING</span></em>
			<h2>나에게 <b style="color:#5bbfb6;">이름</b>을<br> 지어주세요!</h2>
		</div>
	<% else %>	
		<div>
			<em class="month"><span>9월 THING</span></em>
			<p class="id">텐바이텐 X jm**** 고객님</p>
			<h2>내 이름은 <b style="color:#5bbfb6;">빙그레미</b></h2>
		</div>					
	<% end if %>	
	</div>				
	<div class="badge-detail">
		<div id="thingRolling" class="swiperFull">
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<div class="swiper-slide"><img src="http://webimage.10x10.co.kr/fixevent/play/badge/img_badge_201809_1.jpg" alt=""></div>
					<div class="swiper-slide"><img src="http://webimage.10x10.co.kr/fixevent/play/badge/img_badge_201809_2.jpg" alt=""></div>
					<div class="swiper-slide"><img src="http://webimage.10x10.co.kr/fixevent/play/badge/img_badge_201809_3.jpg" alt=""></div>
				</div>
				<div class="paginationDot"></div>
			</div>
		</div>
		<div class="itemDesc" style="background-color:#5bbfb6;">
		<% if now() <= presentDay then %>
			<p>어디선가 흘러나오는 피아노 소리에<br>귀 기울여 본 적 있나요?<br>도레미 소리를 내며<br>마음을 따뜻하게 해주는<br>나의 이름을 지어주세요!</p>
		<% else %>
			<p>텐바이텐에서 디자인한 THING 배지는<br>매달 이름짓기 이벤트를 통해<br>한정수량 제작됩니다.<br>응모 기간 동안 재치있는 이름을<br>지어주신 여러분, 감사합니다 :)</p>
		<% end if %>
			<div class="btnGet">
				<a href="/category/category_itemPrd.asp?itemid=2066446" onclick="TnGotoProduct('2066446');return false;">배지 구매하러가기</a>
			</div>
		</div>
		...
		<% if now() <= presentDay then %>					
			<div class="desc">
				<p class="msg" style="color:#5bbfb6;">
					<span>텐바이텐과 당신이 함께 만드는 친구!</span>지어주신 이름 중 하나를 뽑아<br>9월의 THING 배지가 한정 출시됩니다.<br>위트있는 이름을 지어주신 <br>작명가에게는 본 배지를 드립니다.<br>( 1명:5개 / 15명:1개 증정 )
				</p>							
				<p class="date">응모기간 : 09.10 ~ 09.16 <span>|</span> 발표 : 09.17</p>
			</div>
			<div class="form">
				...
				<input type="hidden" name="spoint"/>						
					...
					<legend class="hidden">어울리는 이름 짓기</legend>
						<input type="text" maxlength="12" id="txtcomm" onClick="jsCheckLimit();" name="txtcomm" title="이름 입력" placeholder="내 이름은" style="border-color:#5bbfb6;" />
						<input type="button" onclick="jsSubmitComment(document.frmcom);return false;" value="이름 지어주기" style="background-color:#5bbfb6; color:#fff; margin-top:0.75rem;" />
					</fieldset>								
				</form>		
		...
		<% else %>	
			<div class="other">
				...
				<ul>
					...
					<li><span style="border-color:#5bbfb6; color:#5bbfb6;">도레미파솔로탈출</span></li>
					<li><span style="border-color:#5bbfb6; color:#5bbfb6;">귀또레미</span></li>
					<li><span style="border-color:#5bbfb6; color:#5bbfb6;">멜로디ON</span></li>
					<li><span style="border-color:#5bbfb6; color:#5bbfb6;">핑키보드</span></li>
					<li><span style="border-color:#5bbfb6; color:#5bbfb6;">파솔솔건반탁</span></li>
					<li><span style="border-color:#5bbfb6; color:#5bbfb6;">풍금으로들었소</span></li>
				</ul>
			</div>											
		<% end if %>						
		</div>
		...
		<div class="listComment" id="comment">
			...
			<ul>
				...
				<li>
					<span>내이름은 <em style="color:#5bbfb6;"><%=ReplaceBracket(db2html(arrCList(1,intCLoop)))%></em>
