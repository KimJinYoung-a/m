...
<footer class="category-footer">
	<div class="footer-content">
		<div class="footer-nav">
			<a href="<%=M_SSLUrl%>/common/news.asp?type=A">공지사항</a>
			<a href="<%=M_SSLUrl%>/cscenter/">고객행복센터</a>
			<a href="<%=M_SSLUrl%>/offshop/">매장안내</a>
		...
		<ul class="tenten-sns">		
			...
			<li><a href="<%=M_SSLUrl%>/my10x10/qna/myqnalist.asp" class="counsel">1:1 상담 신청</a></li>
		</ul>
		<p class="copyright">&copy; Tenbyten inc.</p>
	</div>	
	<div class="bot-nav">
		<a href="<%=M_SSLUrl%>/member/join.asp">회원가입</a>
		<%
	...
	<div class="footer-bnr">
		<a href="<%=M_SSLUrl%>/event/benefit/">신규회원 혜택 확인하러 가기</a>
	</div>	
	<% end if %>
	<!-- for dev msg : 다음 탭으로 이동하는 부분 -->
	<% If InStr(Request.ServerVariables("url"),"/trend/") > 0 Then %>
	<div class="btn-next-tab">
		<a href="<%=M_SSLUrl%>/award/awarditem.asp?gaparam=before_footer_trend">여기를 터치하면<br /><b>'베스트'</b>로 이동합니다</a>
	</div>
	<% End If %>
	<% If InStr(Request.ServerVariables("url"),"/award/awarditem.asp") > 0 Then %>
	<div class="btn-next-tab">
		<a href="<%=M_SSLUrl%>/gnbevent/shoppingchance_allevent.asp?gaparam=before_footer_best">여기를 터치하면<br /><b>'기획전'</b>로 이동합니다</a>
	</div>
	<% End If %>
</footer>
<% End If %>
