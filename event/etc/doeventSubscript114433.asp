...
<%
    dim mode, referer,refip, apgubun, phoneNumber, mktTest, currentTime, currentDate
    ...
    if mktTest then
        ...
        if request("testdate")<>"" then
            ...
            currentDate = CDate("2021-10-11 15:00:00")
        end if
    ...
    If (left(now(),10)<"2021-10-11") and not mktTest Then
        Response.Write "Err|알림 신청기간이 아닙니다."
        dbget.close() : Response.End
    elseIf (left(now(),10)>"2021-10-12") and not mktTest Then
        Response.Write "Err|알림 신청기간이 아닙니다."
        dbget.close() : Response.End
    End IF
    ...
    If mode="kamsg" Then
        ...
        if mktTest then
            ...
            If currentDate >= #10/11/2021 00:00:00# and currentDate < #10/12/2021 00:00:00# Then
                requestDate = formatdate(DateAdd("n",-40,#10/12/2021 09:00:00#),"0000.00.00 00:00:00")
                episode2=1
            else
                requestDate = formatdate(DateAdd("n",-40,#10/12/2021 09:00:00#),"0000.00.00 00:00:00")
                episode2=1
