...
elseif sType<>"searchlist" then
	...
	if (keyword<>"") then
	    keyword = RepWord(keyword,"[^ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z0-9.&%+\-\_\s]","")
	    if (Trim(rstxt)<>"") then
	        rstxt = RepWord(rstxt,"[^ㄱ-ㅎㅏ-ㅣ가-힣a-zA-Z0-9.&%+\-\_\s]","")
...
''필요시 이걸 사용
    ...
    'if (GetLoginUserLevel="7") or (application("Svr_Info")="090") then
        Call addIIsLog(keyword,rstxt,makerid,dispCate,CLNG(sOffset/sSize)+1,sortMtd,p_numofproduct)
    'end if
end if
...
function addIIsLog(keyword,rstxt,makerid,dispCate,ipage,isortMtd,rectcnt)
    ...
    iAddLogs = ""
    if (keyword<>"") then 
        iAddLogs = iAddLogs&"rect="&server.UrlEncode(keyword)  ''검색어
    end if
