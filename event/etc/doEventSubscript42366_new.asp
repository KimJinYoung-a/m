...
<%
	...
	elseif date<rsget("evt_startdate") or date>rsget("evt_enddate") then
		Response.Write	"<script language='javascript'>" &_
						...
						"top.location.href='/event/eventmain.asp?eventid=20903';" &_
						"</script>"
		dbget.close()	:	response.End
	end if
	rsget.Close
	...
	ElseIf renloop > 10 And renloop < 56 Then
	...
	response.write "<script type='text/javascript'>" &_
		...
		If evt_option2 ="1" or evt_option2="2"Then				'3000 or 5000 쿠폰일 경우
			...
			sqlStr = " declare @tidx as int " & vbCrLf &_
					" select top 1 @tidx=idx from db_temp.dbo.tbl_innisfree_coupon where isusing='N' and gubun='" & evt_option2 & "' " & vbCrLf &_
					" if (@tidx is Null) " & vbCrLf &_
					" 	select 'end' " & vbCrLf &_
					" else " & vbCrLf &_
					" 	begin " & vbCrLf &_
					" 		Insert into db_event.dbo.tbl_event_subscript (evt_code, userid, sub_opt1 ,sub_opt2) " & vbCrLf &_
					" 		values " & vbCrLf &_
					" 		(" & evt_code & ",'" & loginid & "',@tidx,'" & evt_option2 & "'); " & vbCrLf &_
					" 		update db_temp.dbo.tbl_innisfree_coupon set userid='" & loginid & "', isusing='Y' " & vbCrLf &_
					" 		WHERE idx=@tidx and gubun = '" & evt_option2 & "' and isusing ='N'; " & vbCrLf &_
					" 		select 'ok'; " & vbCrLf &_
					" 	end " & vbCrLf
			rsget.Open sqlStr,dbget,1
...
function SendMailCoupon(usermail,couponkey)
        ...
        fileName = dirPath&"\innisfree_mail42366.html"
