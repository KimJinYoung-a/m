...
function DelComments(cmtidx){
	<% IF not(IsUserLoginOK) THEN %>
		if(confirm("로그인을 하셔야 삭제할 수 있습니다.\n로그인 하시겠습니까?") == true) {
			parent.location.href = "<%=M_SSLUrl%>/login/login.asp?backpath=/_culturestation/culturestation_event.asp?evt_code=<%=evt_code%>";
			return true;
		} else {
			return false;
		}
	...
	if(confirm("선택한 댓글을 삭제하시겠습니까?") == true) {
		...
		if (str.length=='2'){
			if (str=='d1'){
				location.href = "/_culturestation/culturestation_event.asp?evt_code=<%=evt_code%>&pagereload=ON";
			}else if (str=='99'){
				alert('로그인을 해주세요.');
				return;
			}
...
<body class="default-font body-sub bg-grey">
	...
	<div id="content" class="content">
			...
			<div class="cult-station">
                <div class="other-cult">
                    <section id="other-cult-evt" class="swiperCarousel newArrivalV16">
                        <div class="hgroup">
                            <h2>다른 컬쳐스테이션</h2>
                            <a href="/_culturestation/" class="btn-more btn-arrow">더보기</a>
                        </div>
        ...
        <input type="hidden" name="returnurl" value="/_culturestation/culturestation_event.asp?evt_code=<%= eCode %>&comm=o">
