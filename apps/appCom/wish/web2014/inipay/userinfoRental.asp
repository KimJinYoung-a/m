...
Dim G_PG_KAKAOPAY_ENABLE : G_PG_KAKAOPAY_ENABLE = FALSE   ''카카오페이 사용여부
...
for i=0 to oshoppingbag.FShoppingBagItemCount - 1
    ...
    If Not(oshoppingbag.FItemList(i).Fitemdiv = "30") Then
        response.redirect wwwURL&"/apps/appCom/wish/web2014/inipay/userinfo.asp"
        response.end            
    End If
Next
...
<% if (isTenLocalUser) then %>
	...
	function fnTenLocalUserConfirm(){
	    ...
	    fnAPPpopupBrowserURL('직원인증','<%=C_WEBVIEWURL%>/inipay/popLocalUserConfirm.asp');
	}
...
function PayNext(frm, iErrMsg){
    ...
    if ((paymethod=="150")) {
        ...
        <% if (G_USE_BAGUNITEMP) then %>
            frm.action = "<%=M_SSLUrl%>/inipay/card/ordertemp_ini.asp";    
        <% else %>
...
function RecalcuSubTotal(comp){ <% '### 작업시 하단에 RecalcuSubTotalOnly 스크립트와 alert빼고 반드시 맞춰야함. %>
	var frm = document.frmorder;
	var spendmileage = 0;
	var spendtencash = 0;
	...
	if ((comp.name=="sailcoupon")||((comp.name=="barobtn")&&(frm.itemcouponOrsailcoupon[0].checked))){
		...
		if (coupontype=="0"){
			...
			if ((couponmoney>0)&&(isquickdlv)) {couponmoney=0;}
			<% if (IsForeignDlv) then %>
			if (tenbeasongpay==0){
				setTimeout(function(){
        ...
        if(coupontype=="2"){
            ...
            if (couponmoney*1<1){
				...
				fnAlertNativeDisplay(altMsg, "alert");
            }
	...
	if (comp.name=="appcoupon"){
		...
		if (coupontype=="0"){
			...
			// % 보너스쿠폰
			//couponmoney = parseInt(duplicateSailAvailItemTotal*1 * (couponvalue / 100)*1);
			couponmoney = parseInt(getPCpnDiscountPrice(couponvalue,couponmxdis,frm.appcoupon[frm.appcoupon.selectedIndex].value));
        ...
        if(coupontype=="2"){
            ...
            couponmoney = AssignBCBonusCoupon(coupontype,couponvalue,frm.appcoupon[frm.appcoupon.selectedIndex].value);
            if (couponmoney*1<1){
                alert('추가 할인되는 상품이 없습니다.\n\n보너스 쿠폰의 경우 기존 할인 상품, 일부 추가할인 불가상품은 추가할인이 제외됩니다.');
                frm.appcoupon.value=""
                couponmoney = 0;
...
function RecalcuSubTotalOnly(comp){ <% '### 작업시 상단에 RecalcuSubTotal 스크립트와 alert빼고 반드시 맞춰야함. %>
	var frm = document.frmorder;
	var spendmileage = 0;
	var spendtencash = 0;
	var spendgiftmoney = 0;
...
function CheckGift(isFirst,isSilent){
	var frm = document.frmorder;
...
function chgRSVSel(chk){
	...
	if(chk=="R") {
		...
		$("#addressOption4").addClass("active");
...
function packreg(reload){
	...
	if (reload==''){
		if (confirm('이용중 팝업을 강제로 종료할 경우,\n설정된 포장 내용은 저장되지 않습니다.')){
			fnAPPpopupBrowserURL('선물포장','<%= M_SSLUrl %>/apps/appCom/wish/web2014/inipay/pack/pack_step_intro.asp');
			return false;
		}
	}else{
		fnAPPpopupBrowserURL('선물포장','<%= M_SSLUrl %>/apps/appCom/wish/web2014/inipay/pack/pack_step1.asp');
		return false;
	}
}
...
function chpojangdel(midx){
	...
	pojangfrm.action = "<%= M_SSLUrl %>/apps/appCom/wish/web2014/inipay/pack/pack_process.asp";
	pojangfrm.submit();
	return;
}
...
function fnOpenZipAddrr(){
	//fnAPPpopupBrowserURL("우편번호 찾기","<%=wwwUrl%>/apps/appCom/wish/web2014/lib/pop_searchzipNew.asp?target=frmorder&gb=zip2016","","zip");
	fnAPPpopupBrowserURL("우편번호 찾기","<%=wwwUrl%>/apps/appCom/wish/web2014/lib/pop_searchzip_ka.asp?target=frmorder&gb=zip2016","","zip");
}

function fnOpenZipAddrrNew(){
	//fnAPPpopupBrowserURL("우편번호 찾기","<%=wwwUrl%>/apps/appCom/wish/web2014/lib/pop_searchzipNew.asp?target=frmorder&gb=zip2016","","zip");
	fnAPPpopupBrowserURL("우편번호 찾기","<%=wwwUrl%>/apps/appCom/wish/web2014/lib/pop_searchzip_ka.asp?target=frmorder&gb=zip2016","","zip");
}

function fnGiftItemImageView(gkc,evid,gnoti){
	fnAPPpopupBrowserURL("사은품 이미지","<%=wwwUrl%>/apps/appCom/wish/web2014/inipay/freebieView.asp?gkc="+gkc+"&evid="+evid+"&gnoti="+encodeURIComponent(gnoti)+"","","giftimagee");
}
...
$( document ).ready(function() {
	...
	<% if reload = "ON" then %>
		setTimeout("fnAPPhideLeftBtns()",500);
	<% end if %>
});
</script>
	...
	<div class="container">
		...
		<div class="content" id="contentArea" style="padding-bottom:0;">
			<div class="cartV16a">
				...
				<input type=hidden name=gopaymethod value="RTPAY" /> <!-- or onlydbank -->
				...
				<input name="appcoupon" id="appcoupon" value="" type="hidden" />				
							...
							<div class="pay-rental-container" id="i_paymethod">
								<div class="border-area">
									...
									<div class="pay-section02">
										<p class="sub-txt"><span class="month" id="monthlyPrice"></span>개월 간 월<span class="price memberCountCon"></span>원씩 납부됩니다</p><!-- for dev msg : memberCountCon 에서 선택 value값 호출 -->
										<a href="" onclick="fnAPPpopupBrowserURL('이니렌탈 안내','<%=wwwUrl%>/apps/appcom/wish/web2014/category/pop_inirental_guide.asp'); return false;" class="btn-view">자세히 알아보기</a>
									</div>
								</div>
