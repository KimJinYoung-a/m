...
Class CWishItem
	...
	public Function IsFreeBeasong()
		if (FsellPrice>=getFreeBeasongLimit()) then
			IsFreeBeasong = true
		else
			IsFreeBeasong = false
		end if
...
Class CWish
	...
	public Sub getWisMatchUser()
		...
		sqlStr = sqlStr & "	from db_appWish.dbo.tbl_app_wish_matchInfo "
		sqlStr = sqlStr & "	where userid='" & FRectUserID & "'"
		rsget.Open sqlStr,dbAppWishget,1
		...
		if chkMatch then
			...
			sqlStr = sqlStr & "		from db_appWish.dbo.tbl_myfavorite_folder "
	...
	public Sub getFollowUser()
		...
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_app_wish_followInfo "
		sqlStr = sqlStr & " where userid='" & FRectUserID & "'"
		rsget.Open sqlStr,dbAppWishget,1
		if rsget(0)>0 then
		...
		if chkRst then
			...
			sqlStr = sqlStr & "		from db_appWish.dbo.tbl_myfavorite_folder as a "
			sqlStr = sqlStr & "			join db_appWish.dbo.tbl_app_wish_followInfo as b "
			sqlStr = sqlStr & "				on a.userid=b.followUid and b.userid='" & FRectUserID & "' "
			sqlStr = sqlStr & "			left join db_appWish.dbo.tbl_app_wish_matchInfo as m "
	...
	public Sub getFollowUserList()
		...
		Select Case FRectKind
			...
			sqlStr = sqlStr & "		from db_appWish.dbo.tbl_app_wish_followInfo as b "
			sqlStr = sqlStr & "			left join db_appWish.dbo.tbl_app_wish_matchInfo as m "
	...
	public Sub getCategoyWishUser()
		...
		sqlStr = sqlStr & "		from db_appWish.dbo.tbl_myfavorite as f "
'		sqlStr = sqlStr & "			join db_appWish.dbo.tbl_display_cate_item as d "
'		sqlStr = sqlStr & "				on f.itemid=d.itemid "
'		sqlStr = sqlStr & "				and d.catecode like '" & FRectCateCd & "%' "
		sqlStr = sqlStr & "			join db_appWish.dbo.tbl_item as i "
	...
	public Sub getWishTrendUser()
		...
		sqlStr = sqlStr & "		from db_appWish.dbo.tbl_favorite_popular as p "
		sqlStr = sqlStr & "			join db_appWish.dbo.tbl_myfavorite as f "
		...
		if FTotalCount>FRectLimitCnt then
			...
			sqlStr = sqlStr & "					and p.userid=f.userid "
			sqlStr = sqlStr & "					and f.viewisUsing='Y' "
			sqlStr = sqlStr & "			left join db_appWish.dbo.tbl_app_wish_matchInfo as w "
	...
	public Sub getWishUserFromItem()
		...
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_myfavorite as wi "
		sqlStr = sqlStr & " 	join db_appWish.dbo.tbl_myfavorite_folder as wf "
	...
	public Sub getWishCollectFromItem()
		...
		'# 새벽 리빌딩 시간동안은 결과 반환 안함 (2015.02.11; 허진원)
		IF (formatdatetime(now(),4)>"02:50:00") and (formatdatetime(now(),4)<"04:00:00") then
			FResultCount = 0
			Exit Sub
		end if
...
'		sqlStr = sqlStr & "			from db_appWish.dbo.tbl_myfavorite "
...
'		sqlStr = sqlStr & "	) as A "
'		sqlStr = sqlStr & "	join ( "
'		sqlStr = sqlStr & "		select top 3 wi.userid "
...
'		sqlStr = sqlStr & " order by B.updt desc, A.userid asc, A.rowno asc"
		...
		sqlStr = "exec [db_AppWish].[dbo].[sp_Ten_WishCollection_byItem] "& FPageSize &","& itemid &", '"& FRectUserID &"'"

        rsAppWishget.CursorLocation = adUseClient
		rsAppWishget.CursorType = adOpenStatic
		rsAppWishget.LockType = adLockOptimistic
        rsAppWishget.pagesize = FPageSize
		rsAppWishget.Open sqlStr, dbAppWishget

		FResultCount = rsAppWishget.RecordCount
		redim preserve FItemList(FResultCount)
		...
		if Not(rsAppWishget.EOF or rsAppWishget.BOF) then
			...
			Do until rsAppWishget.EOF
				set FItemList(i) = new CWishItem
				FItemList(i).Fuserid	= rsAppWishget("userid")
				FItemList(i).FfavCount	= rsAppWishget("cnt")
				FItemList(i).Fitemid	= rsAppWishget("itemid")
				FItemList(i).Fitemname	= rsAppWishget("itemname")
				FItemList(i).FimageOrg		= "http://webimage.10x10.co.kr/image/basic/" + GetImageSubFolderByItemid(rsAppWishget("itemid")) + "/" + rsAppWishget("basicimage")
				FItemList(i).FimageUrl		= "http://thumbnail.10x10.co.kr/webimage/image/basic/" + GetImageSubFolderByItemid(rsAppWishget("itemid")) + "/" + rsAppWishget("basicimage") &"?cmd=thumb&width=400&height=400"
	...
	public Function getWishItemListJson()
		...
		if FRectColorCd<>"" then
			...
			itemAdd = itemAdd & "	from db_appWish.dbo.tbl_item_colorOption "
		...
		if FRectCateCd<>"" then
			if len(FRectCateCd)>3 then
				itemAdd = itemAdd & " and i.itemid in ("
				itemAdd = itemAdd & "	select distinct itemid "
				itemAdd = itemAdd & "	from db_appWish.dbo.tbl_display_cate_item "
		...
		if FRectKeyword<>"" then
			addSql = addSql & " and i.itemid in (select itemid from db_appWish.dbo.tbl_item_SearchBase where contains(searchKey,'" & FRectKeyword & "'))"
		end if
		...
		if FRectMyUID<>"" then
			if FRectIsInclude="A" then
				...
				sqlStr = sqlStr & " 		from db_appWish.dbo.tbl_myfavorite where userid='" & FRectMyUID & "' "
		...
		sqlStr = sqlStr & " 	,isnull(i.favcount,0) as favcount, i.evalcnt, i.limitYn, i.limitNo, i.limitSold, i.sellYn, i.sailYn, i.itemcouponYn, i.SpecialUserItem, i.tenOnlyYn, i.regdate "
		if FRectMyUID<>"" then
			sqlStr = sqlStr & " 	, Case When ex.itemid is null then '0' else '1' end as isMy "
		else
			sqlStr = sqlStr & "		,'0' as isMy "
		end if
		sqlStr = sqlStr & " 	,i.basicimage600, i.basicimage, i.deliverytype, c.defaultFreeBeasongLimit "
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_myfavorite as wi "
		sqlStr = sqlStr & " 	join db_appWish.dbo.tbl_item as i "
		sqlStr = sqlStr & " 		on wi.itemid=i.itemid "
		sqlStr = sqlStr & " 			and i.isusing='Y' " & itemAdd
		sqlStr = sqlStr & "		join db_appWish.dbo.tbl_user_c as c "
	...
	public Function getPopularWishListJson()
		...
		sqlStr = sqlStr & " 	,i.basicimage600, i.basicimage, i.deliverytype, c.defaultFreeBeasongLimit "
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_favorite_popular as wi "
		sqlStr = sqlStr & " 	join db_appWish.dbo.tbl_item as i "
		sqlStr = sqlStr & " 		on wi.itemid=i.itemid "
		sqlStr = sqlStr & " 			and i.sellyn in ('Y','S') "
		sqlStr = sqlStr & " 			and i.isusing='Y' " & itemAdd
		sqlStr = sqlStr & "		join db_appWish.dbo.tbl_user_c as c "
		sqlStr = sqlStr & "			on i.makerid=c.userid "
		if FRectMyUID<>"" then
		...
		if Not(rsget.EOF or rsget.BOF) then
			...
			Do until rsget.EOF
				...
				'	FItemList(i).FimageUrl		= "http://webimage.10x10.co.kr/image/basic600/" + GetImageSubFolderByItemid(rsget("itemid")) + "/" + rsget("basicimage600")
				'end if
				FItemList(i).FimageUrl		= "http://thumbnail.10x10.co.kr/webimage/image/basic/" + GetImageSubFolderByItemid(rsget("itemid")) + "/" + rsget("basicimage") &"?cmd=thumb&width=300&height=300"
				FItemList(i).FwebItemUrl	= cstItemPrdUrl & "?itemid=" & rsget("itemid")
				FItemList(i).FEvalcnt		= rsget("evalcnt")
	...
	public Function getBestSellerListJson()
		...
		sqlStr = sqlStr & " 	,i.basicimage600, i.basicimage, i.deliverytype, c.defaultFreeBeasongLimit "
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_const_award as wi "
		sqlStr = sqlStr & " 	join db_appWish.dbo.tbl_item as i "
		sqlStr = sqlStr & " 		on wi.itemid=i.itemid "
		sqlStr = sqlStr & " 			and i.sellyn in ('Y','S') "
		sqlStr = sqlStr & " 			and i.isusing='Y' " & itemAdd
	...
	public Function getUserBadgeListJson()
		...
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_badge_userObtain as b "
		sqlStr = sqlStr & " 	join db_appWish.dbo.tbl_badge_info as i "
	...
	public Function getUserWishFolderListJson()
		...
		if FRectUserID=FRectMyUID then
			sqlStr = "select "
			sqlStr = sqlStr & "	(select count(*) from db_appWish.dbo.tbl_myfavorite "
			...
			sqlStr = sqlStr & "		from db_appWish.dbo.tbl_myfavorite as w "
			sqlStr = sqlStr & "		join db_appWish.dbo.tbl_item as i on w.itemid=i.itemid "
		...
		sqlStr = sqlStr & " from db_appWish.dbo.tbl_myfavorite_folder as f "
	...
	public Function getMyBrandSimpleListJson()
		...
		if FRectIsInclude="A" and FRectUserID<>"" then
			sqlStr = "Select count(*) cnt "
			sqlStr = sqlStr & "	from db_appWish.dbo.tbl_mybrand as z "
			sqlStr = sqlStr & "		join db_appWish.dbo.tbl_user_c as c "
		...
		sqlStr = sqlStr & "		from db_appWish.dbo.tbl_user_c as b "


		if FRectUserID<>"" then
			sqlStr = sqlStr & "			left join db_appWish.dbo.tbl_mybrand as z "
			...
			sqlStr = sqlStr & "				from db_appWish.dbo.tbl_item as mi "
			sqlStr = sqlStr & "					join db_appWish.dbo.tbl_myfavorite as mf "
		...
		sqlStr = sqlStr & "			left join db_appWish.dbo.tbl_street_manager as h "
	...
	public Function getMyZzimBrandListJson()
		...
		'Tag검색 시
		if FRectKeyword<>"" then
			sqlStr = sqlStr & "			join db_appWish.dbo.tbl_street_Hello as bh "
		...
		sqlStr = sqlStr & "				from db_appWish.dbo.tbl_item "
	...
	public Sub getBrandInfo()
		...
		sqlStr = sqlStr & " 	left join db_appWish.dbo.tbl_street_Hello as h "
