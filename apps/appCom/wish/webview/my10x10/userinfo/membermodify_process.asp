...
'	end if
	...
	response.write "    location.replace('"& wwwUrl &"/apps/appcom/wish/webview/my10x10/userinfo/modiUserPass.asp');"
...
elseif (mode="infomodi") then
	...
	if (Enc_userpass64<>Enc_Old_userpass64)  then   '' 암호화 사용(SHA256)
		...
		response.write "	location.replace('"& wwwUrl &"/apps/appcom/wish/webview/my10x10/userinfo/modiUserInfo.asp');"
		response.write "</script>"
		dbget.close()	:	response.end
	end if
