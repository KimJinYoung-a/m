...
Function fnGubunToastPopup(vItemid , evalyn)
	...
	sqlStr = "SELECT gubun FROM db_temp.dbo.tbl_toastpopup_itemid with(nolock) WHERE ItemID ='"& CStr(vItemid) &"'"
...
If IsArray(arrList) Then
	If (request.Cookies("catePrdLowBannerMA") <> "done" or request.Cookies("catePrdLowBannerMA")="") Then
				...
				<% If Not(checkCouponIssue) Then '// 쿠폰 이벤트일 경우 쿠폰을 발급 받았으면 표시하지 않음%>
					<script>
						...
						function bannerCloseToSevenDay(){	//오늘 하루 보지 않기
							setPopupCookie("catePrdLowBannerMA", "done", 1)
