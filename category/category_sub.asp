					...
					<div class="tMar10 box1 w100">
						<ul class="categoryListup">
							...
							<% FOR lp = 0 to oGrCat.FResultCount-1 %>				
								<li><a href="" onclick="moveCategoryList('<%= oGrCat.FItemList(lp).Fcatecode %>','1','',''); return false;"><span><%= oGrCat.FItemList(lp).Fcatename %></span> <% if oGrCat.FItemList(lp).fishot ="o" then %><em class="icoHot">HOT</em><% End If %></a>
								</li><% Next %>
							<% end if %>
						</ul>
					</div>
