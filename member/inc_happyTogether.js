...
function getRecoListCheck(vCHkHT, vRuid)
{
	...
	if (vCHkHT=="N")
	{
		$.getJSON("https://api.recopick.com/v1/recommendations/most/131/"+vRuid+"?limit=9&channel=recopick_member&type=view2&callback=?",
...
function getRecoListCheckView(vCHkHT, vRuid)
{
	...
	if (vCHkHT=="N")
	{
		$.getJSON("https://api.recopick.com/v1/recommendations/most/131/"+vRuid+"?limit=9&channel=recopick_member&type=view&callback=?",
			function(data, status)
			{
				if (status=="success")
				{
...
function FnGoProdItem(vi,oi,mt,dv,uid,lnk,chn){
	...
	//} else {
		if(lnk!="") {
			...
			location.href = "https://api.recopick.com/1/banner/131/pick?source=" + oi + "&pick=" + vi + "&uid=" + uid + "&method=" + mt + "&channel=recopick_member&reco_type=item-item";
