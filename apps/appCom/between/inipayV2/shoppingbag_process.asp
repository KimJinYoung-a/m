    ...
    if (itemid<>"") and (itemoption<>"") and (itemea<>"") then
        ...
        elseif (ValidRet=2) then
            ...
            if (fc<>"") then
                ...
                response.write "<script>if (confirm('이미 장바구니에 담긴 상품입니다.\n수량 변경은 장바구니에서 가능합니다.\n\n장바구니로 이동하시겠습니까?')){top.location.replace('/apps/appCom/between/inipayV2/ShoppingBag.asp');}</script>"
                dbget.close() : response.end
            end if
        else
            oshoppingbag.AddshoppingBagDB itemid,itemoption,itemea,requiredetail
...
%>
        <form name="frmCk" method="post" action="/apps/appCom/between/inipayV2/ShoppingBag.asp">
...
if(confirm("선택한 상품이 장바구니에 담겼습니다.\n\n장바구니로 이동하시겠습니까?")) {
	top.location.href="/apps/appCom/between/inipayV2/ShoppingBag.asp";
} else {
