...
<ul class="pdtDetailListV16a">
	...
	<!-- 배송/교환/환불 -------------------------->
	<!-- #include virtual="/category/inc_DeliveryDescription.asp" -->
</ul>
...
<script type="text/javascript">
	function chk_myeval(v){
		$.ajax({
			type: "POST",
			url:"/apps/appcom/wish/web2014/category/act_myEval.asp?itemid="+v,
			dataType: "text",
			async: false,
	        success: function (str) {
				...
				if(reStr[0]=="01"){
					...
					alert(reStr[1]);
					AddEval2(reStr[2],reStr[3],reStr[4]);
					return false;
				}else if (reStr[0]=="05"){
					AddEval2(reStr[2],reStr[3],reStr[4]);
					return false;
	...
	function AddEval2(OrdSr,itID,OptCd){
		fnAPPpopupBrowser(OpenType.FROM_BOTTOM, [], '상품후기', [], '<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/goodsUsingWrite.asp?orderserial=' + OrdSr + '&itemid=' + itID + '&optionCD=' + OptCd + '&referVal=C');
		return false;
	}
	...
	function goPageEval(v) {
		...
		$.ajax({
			type: "get",
			url: "/apps/appcom/wish/web2014/category/category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg="+v+"&tabno=<%=tabno%>&tnm=<%=tabname%>",
			cache: false,
			success: function(message) 
			{			            	
				$(".itemDetailContV16a .tabCont div[id|='<%=tabname%>']").empty().append(message);
...
<div class="bxWt1V16a">
	<ul class="btnBarV16a">
		<% If oItem.Prd.Ftestercnt > 0 Then %>
			<li <%=CHKIIF(tabno="3","class='current'","")%> name="cmtTab01" style="width:33%;"><div><a href="" onclick="tabchange('/apps/appcom/wish/web2014/category/category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=3&tnm=<%=tabname%>');return false;"><%=chkIIF(Not(IsTicketItem),"상품후기","후기")%><span>(<%=oItem.Prd.FEvalCnt%>)</span></a></div></li>
			<li <%=CHKIIF(tabno="31","class='current'","")%> name="cmtTab02" style="width:33%;"><div><a href="" onclick="tabchange('/apps/appcom/wish/web2014/category/category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=31&tnm=<%=tabname%>');return false;">포토후기<span>(<%=oItem.Prd.FEvalCnt_photo%>)</span></a></div></li>
			<% If oItem.Prd.Ftestercnt > 0 Then %>
			<li <%=CHKIIF(tabno="32","class='current'","")%> name="cmtTab03" style="width:34%;"><div><a href="" onclick="tabchange('/apps/appcom/wish/web2014/category/category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=32&tnm=<%=tabname%>');return false;">테스터후기<span>(<%=oItem.Prd.Ftestercnt%>)</span></a></div></li>
			<% End If %>
		<% Else %>
			<li <%=CHKIIF(tabno="3","class='current'","")%> name="cmtTab01" style="width:50%;"><div><a href="" onclick="tabchange('/apps/appcom/wish/web2014/category/category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=3&tnm=<%=tabname%>');return false;"><%=chkIIF(Not(IsTicketItem),"상품후기","후기")%><span>(<%=oItem.Prd.FEvalCnt%>)</span></a></div></li>
			<li <%=CHKIIF(tabno="31","class='current'","")%> name="cmtTab02" style="width:50%;"><div><a href="" onclick="tabchange('/apps/appcom/wish/web2014/category/category_itemprd_ajax.asp?itemid=<%=itemid%>&cpg=1&tabno=31&tnm=<%=tabname%>');return false;">포토후기<span>(<%=oItem.Prd.FEvalCnt_photo%>)</span></a></div></li>
		<% End If %>
	</ul>
</div>
...
<% IF oItem.Prd.FEvalCnt>0 Or oItem.Prd.Ftestercnt>0 then %>
	...
	<div class="btnAreaV16a">
		<p><button type="button" class="btnV16a btnRed1V16a" onclick="fnAPPpopupBrowserURL('<%=chkIIF(Not(IsTicketItem),"상품후기","후기")%>','<%=wwwUrl%>/apps/appcom/wish/web2014/category/pop_ItemEvalList.asp?itemid=<%=itemid%>&sortMtd=<% If tabno="21" Then Response.write "ph" End If %><% If tabno="22" Then Response.write "ne" End If %>','right','','sc'); return false;">전체보기</button></p>
	</div>
	<% end if %>
<% End If %>
...
<script type="text/javascript">
	...
	function modiItemQna(idx) {
		fnAPPpopupBrowserURL("상품문의 하기","<%=wwwUrl%>/apps/appcom/wish/web2014/category/itemQnA_Write.asp?id="+idx+"&itemid=<%=itemid%>");
	}
	// 상품 문의 삭제
	function delItemQna(idx){
		if(confirm("상품문의를 삭제 하시겠습니까?")){
			$.ajax({
				type: "get",
				url: "/apps/appcom/wish/web2014/my10x10/doitemqna.asp?id="+idx+"&itemid=<%=itemid%>&mode=del",
				cache: false,
				success: function(message) {
					goPageQna(1);
				}
...
<div class="btnAreaV16a">
	<p><button type="button" class="btnV16a btnRed2V16a" onclick="fnAPPpopupBrowserURL('상품문의 하기','<%=wwwUrl%>/apps/appcom/wish/web2014/category/pop_ItemQnaList.asp?itemid=<%=itemid%>&mode=wr'); return false;">상품문의 하기</button></p>
</div>
...
<div class="btnAreaV16a">
	<p><button type="button" class="btnV16a btnRed1V16a" onclick="fnAPPpopupBrowserURL('전체보기','<%=wwwUrl%>/apps/appcom/wish/web2014/category/pop_ItemQnaList.asp?itemid=<%=itemid%>','right','','sc'); return false;">전체보기</button></p>
</div>
<% end if %>
