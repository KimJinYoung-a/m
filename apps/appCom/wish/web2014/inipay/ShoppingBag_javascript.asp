	...
	function EditRequireDetail(iitemid,iitemoption){
		//var popwin = window.open('/apps/appCom/wish/web2014/inipay/Pop_EditItemRequire.asp?itemid=' + iitemid + '&itemoption=' + iitemoption,'edititemrequire','width=300,height=300,scrollbars=yes,resizable=yes');
		//popwin.focus();
	}

	function PopMileageItemView(iitemid){
		var popwin = window.open('/apps/appCom/wish/web2014/my10x10/Pop_mileageshop_itemview.asp?itemid=' + iitemid,'Pop_mileageshop_itemview','width=464,height=600,scrollbars=yes,resizable=yes');
		popwin.focus();
	}
	...
	function hidePopupLayer(){
		iPopupLayer.style.visibility = "hidden"
	}

	function goSaveDvlPay(){
		location.href="/apps/appCom/wish/web2014/event/eventmain.asp?eventid=14067";
	}
	...
	function addWishSelected(){
		...
		if (confirm('선택 상품을 위시리스트에 추가하시겠습니까?')){
			var FavWin = window.open('/apps/appCom/wish/web2014/my10x10/popMyFavorite.asp?mode=AddFavItems&bagarray=' + ArrayFavItemID ,'FavWin','width=380,height=300,scrollbars=no,resizable=no');
			FavWin.focus();
		}
	...
	$(function(){
		<%
		''쇼핑백 갯수 처리
		'if (CStr(GetCartCount)<>CStr(sBagCount)) then
			Call setCartCount(sBagCount)
			response.write "setTimeout(function(){fnAPPsetCartNum("&sBagCount&");},100);"
		'end if
		%>
	});
	</script>