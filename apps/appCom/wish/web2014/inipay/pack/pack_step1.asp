...
elseif vShoppingBag_pojang_checkValidItem=2 then
	response.write "<script type='text/javascript'>alert('장바구니에 담긴 상품이 없습니다.'); fnAPPclosePopup();</script>"
	dbget.close()	:	response.end
...
$(function(){
	...
	//아이폰에서 팝업시 왼쪽 상단에 백버튼 삭제
	setTimeout("fnAPPhideLeftBtns()",200);
});
...
function chpojangdel(midx){
	...
	if(confirm("선물포장을 삭제 하시겠습니까?")){	
		...
		pojangfrm.action = "/apps/appCom/wish/web2014/inipay/pack/pack_process.asp";
		pojangfrm.submit();
	}
	return;
}
...
function gostep2edit(midx){
	...
	pojangfrm.midx.value=midx;
	pojangfrm.action = "/apps/appCom/wish/web2014/inipay/pack/pack_step2.asp";
	pojangfrm.submit();
	return;
}
