...
<%
	Dim vKeyword , vSort , vKey1, vCurrPage, vGnbFlag
...
function getList(){
	var str = $.ajax({
			type: "GET",
	        url: "/apps/appCom/wish/web2014/gift/gifttalk/index_act.asp",
...
function getcommentlist_act(talkidx){
	var str = $.ajax({
			type: "GET",
	        url: "/apps/appCom/wish/web2014/gift/gifttalk/index_comment_act.asp",
...
function DelComments(talkidx,cmtidx){
	...
	if(confirm("선택한 글을 삭제하시겠습니까?") == true) {
		...
		if (str.length=='2'){
			if (str=='d1'){
				location.href = "/apps/appcom/wish/web2014/gift/gifttalk/index.asp?gnbflag=<%=vGnbFlag%>";
			}else if (str=='99'){
				alert('로그인을 해주세요.');
				return;
			}
...
function jsMyTalkList(){
	fnAPPpopupBrowserURL("내가 쓴 글","<%=wwwUrl%>/apps/appcom/wish/web2014/gift/gifttalk/mytalk.asp");
}
</script>
</head>
