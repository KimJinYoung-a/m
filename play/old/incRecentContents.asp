...
%>
				<% if nowViewPage="/play/playPicDiary.asp" then %>
					<div class="inner5">
				<% elseif nowViewPage="/play/playStylePlus.asp" then %>
				...
				<% end if %>
					...
					<div class="rctContent box1">
						...
						<% For iRC = 0 To UBound(vArrRC,2) %>
							<li>
								<a href="<%=fnPlayLinkMoWeb(vArrRC(4,iRC))%>?idx=<%=vArrRC(0,iRC)%>&contentsidx=<%=vArrRC(6,iRC)%>&type=<%=vArrRC(4,iRC)%>">
