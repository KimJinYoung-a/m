...
'session.codepage=65001
	...
	If redirect_url <> "" Then
		dbget.close()
		Response.redirect redirect_url & "/shoppingtoday/shoppingchance_allevent.asp"
		Response.end
	End If
...
$(function() {
	if ($("body").hasClass("body-sub")){
		$(window).scroll(sticky_sortingbar('#exhibitionList'));
	}
});
				...
				$.ajax({
					...
					success: function(message) {
						if(message!="") {
							...
							var ctbnr = "<li class='culture-bnr' id='ctbnr'><a href='/culturestation/index.asp'><span class='icon icon-culture'></span><b>컬쳐 스테이션</b> <span class='subcopy'>감성을 채우는 문화정거장</span></a></li>"
							$("#lyrEvtList").append(ctbnr);
							<% if sCategory<>"" then  %>
							var anotherDisp = "<%=DrawDispCategoryList(sCategory,"1","0") %>"
...
<body class="default-font body-sub"><!-- for dev msg : GNB메뉴에서 들어갈때만 body-main클래스가 붙습니다. 그 이외의 페이지에서는 body-sub가 붙습니다. -->
	...
	<div id="content" class="content">
		...
		<section id="exhibitionList" class="exhibition-list">
			...
			'24	~ 29 : , A.evt_subname, B.evt_mo_listbanner , B.evt_todaybanner, b.isitemps, b.saleCper, b.evt_tag'
					...
					if arrList(29,intLoop) <> "" or not(isnull(arrList(29,intLoop))) then
						if ubound(Split(arrList(29,intLoop),",")) > 0 Then
							...
							for ip = 0 to ubound(Split(arrList(29,intLoop),","))
								...
								addTag = addTag & "<li><a href='/search/search_result.asp?rect="& Split(arrList(29,intLoop),",")(ip) &"'>#"& Split(arrList(29,intLoop),",")(ip) &"</a></li>"
							next 
							addTag = addTag & "</ul>"
						end if 
					end if
				%>
					<li<% If arrList(28,intLoop)<>"" Then %> class="has-vod"<% End If %>>
						<a href="" onclick="<%=vLink%> return false;">
							...
							<div class="thumbnail"><img src="<%=chkiif(arrList(24,intLoop)="",arrList(25,intLoop),arrList(24,intLoop))%>" alt="" /></div><%'=getThumbImgFromURL(arrList(24,intLoop),640,"","","")%>
							<div class="desc">
								<p>
									<b class="headline"><span class="ellipsis" <% IF arrList(11,intLoop) THEN %>style="width:80%;"<% else %>style="width:100%;"<% end if %>><%=Replace(stripHTML(db2html(eName)),"[☆2015 다이어리]","")%></span> <% IF arrList(11,intLoop) THEN %><b class="discount color-red"><%=eNameredsale%></b><% end if %></b>
