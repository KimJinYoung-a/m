...
.mEvt72443 button {background-color:transparent;}

.topic {position:relative;}
.topic .close, .topic .soldout {position:absolute; top:52.67%; left:16.4%; width:20.93%;}
.topic .close {
...
.couponArea .btnDownloadAll, .couponArea .soldout {position:absolute; bottom:8%; left:50%; width:84.53%; margin-left:-42.265%;}

.noti {padding-top:8%; background-color:#f8f8f8;}
.noti .inner {padding:0 7.81%;}
.noti h3 {color:#9ea3be; font-size:1.5rem; font-weight:bold;}
...
.noti ul li:after {content:' '; display:block; position:absolute; top:0.5rem; left:0; width:0.4rem; height:0.1rem; background-color:#9ea3be;}
.noti p {margin-top:7%;}
</style>
...
<div class="mEvt72443">
	<div class="topic">
		<h2><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/txt_coupon.jpg" alt="수요일 밤 12시 까지 무더운 여름, 차원이 다른 두 가지 할인쿠폰을 만나보세요!" /></h2>
		<% if couponcnt >= 25000 and couponcnt < 30000 then %>
			<strong class="close"><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/ico_close.png" alt="마감임박" /></strong>
		<% end if %>
		
		<% if couponcnt >= 30000 then %>
			<strong class="soldout"><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/txt_soldout_01.png" alt="솔드아웃" /></strong>
		<% end if %>
	</div>

	<div class="couponArea">
		<p><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/img_coupon.jpg" alt="삼만원 이상 구매시 오천원 할인 쿠폰, 육만원 이상 구매시 만원 할인 쿠폰 사용기간은 8월 16일부터 8월 17일 2일간 입니다." /></p>
		<% if couponcnt >= 30000 then %>
			<%''  for dev msg : 쿠폰 소진 시 보여주세요  %>
			<p class="soldout"><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/txt_soldout_02.png" alt="쿠폰이 모두 소진되었습니다" /></p>
		<% else %>
			<%''  for dev msg : 쿠폰 소진 시 숨겨주세요  %>
			<button type="button" onclick="jsevtDownCoupon('evtsel,evtsel','<%= getbonuscoupon1 %>,<%= getbonuscoupon2 %>'); return false;" class="btnDownloadAll"><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/btn_download_all.png" alt="쿠폰 한번에 다운받기" /></button>
		<% end if %>
	</div>
	...
	<div class="appdownJoin">
		...
		<% if isApp=1 then %>
		<% else %>
			<p><a href="/event/appdown/"><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/btn_app.png" alt="텐바이텐 앱 설치 아직이신가요? 텐바이텐 앱 다운" /></a></p>
		<% end if %>
		<%'' for dev msg : 로그인시 숨겨주세요 (비로그인시에만 보여주세요) %>
		<% if Not(IsUserLoginOK) then %>
			<p><a href="/member/join.asp"><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/btn_join.png" alt="텐바이텐에 처음오셨나요? 회원가입하고 구매하러 가기!" /></a></p>
		<% end if %>
	</div>
	...
	<div class="noti">
		<div class="inner">
			...
			<ul>
				...
				<li>쿠폰은 8/17(수) 23시 59분 59초에 종료됩니다.</li>
				<li>주문한 상품에 따라, 배송비용은 추가로 발생 할 수 있습니다.</li>
				<li>이벤트는 조기 마감 될 수 있습니다.</li>
			</ul>
		</div>
		<p><img src="http://webimage.10x10.co.kr/eventIMG/2016/72443/m/img_ex.png" alt="주문결제 화면에서 할인정보의 보너스 쿠폰 선택 상자에서 ￦쿠폰를 선택해주세요" /></p>
	</div>
	<%
