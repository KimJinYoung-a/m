...
ELSE
     ...
     G_2NDSCH_ADDR = "192.168.0.107"        ''192.168.0.107  :: mobile                 6시48분 재시작
     G_3RDSCH_ADDR = "192.168.0.109"        ''192.168.0.109  :: GiftPlus , scn_dt_itemDispColor :: 확인.
     G_4THSCH_ADDR = "192.168.0.107"        ''192.168.0.108  :: 카테고리, 상품, 브랜드 6시20분 재시작 6시 40분 이전 벌크카피완료
...
Class SearchItemCls
	...
	PUBLIC SUB getSearchList()
		...
		IF (FLogsAccept) and (FRectSearchTxt<>"") THEN
	...
	PUBLIC SUB getGroupbyCategoryList()
		...
		FOR iRows = 0 to FResultcount -1
			...
			SET FItemList(iRows) = new SearchGroupByItems
				...
				FItemList(iRows).FCateCd1	= arrData(38)
				FItemList(iRows).FCateCd2	= arrData(39)
				FItemList(iRows).FCateCd3	= arrData(40)
				FItemList(iRows).FCateDepth	= arrData(37)

				FItemList(iRows).FImageSmall = getItemImageUrl & "/small/" & GetImageSubFolderByItemid(arrData(4)) & "/" &db2html( arrData(17))
	...
	PUBLIC SUB getGroupbyBrandList()
		...
		FOR iRows = 0 to FResultcount -1
			...
			SET FItemList(iRows) = new CCategoryPrdItem
				FItemList(iRows).FMakerID		= arrData(9)
				FItemList(iRows).FBrandName		= arrData(10)
				FItemList(iRows).FImageSmall	= getItemImageUrl & "/small/" & GetImageSubFolderByItemid(arrData(4)) & "/" &db2html( arrData(17))
				FItemList(iRows).FisBestBrand	= arrData(42)
				FItemList(iRows).FItemScore 	= Scores(iRows)
				FItemList(iRows).FCurrRank		= arrData(43)
	...
	PUBLIC SUB getTotalCount()
        ...
        ''칼라칩검색/브랜드검색 3차
        if (Scn= "scn_dt_itemDispColor") or (FRectMakerid<>"") then SvrAddr = getTimeChkAddr(G_4THSCH_ADDR)
        ''---------------------------------------------------------------------------------------------------------
