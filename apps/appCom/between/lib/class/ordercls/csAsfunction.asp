...
'Gift카드환원
    ...
    if (userid<>"") and (Not IsAllCancel) and (refundgiftcardsum <> 0) then
        ...
        sqlStr = sqlStr + " (userid, useCash, jukyocd, jukyo, orderserial, deleteyn) " + VbCrlf
        sqlStr = sqlStr + " values ("
        sqlStr = sqlStr + " '" + userid + "'"
...
'	sqlStr = sqlStr + " set [db_user].[dbo].tbl_user_current_mileage.spendmileage=IsNull(T.totmile,0) " + VbCrlf
'	sqlStr = sqlStr + " from " + VbCrlf
'	sqlStr = sqlStr + " (select sum(Abs(mileage)) as totmile " + VbCrlf
'	sqlStr = sqlStr + "     from [db_user].[dbo].tbl_mileagelog " + VbCrlf
'	sqlStr = sqlStr + "     where userid='" + CStr(userid) + "' " + VbCrlf
'	sqlStr = sqlStr + "     and deleteyn='N' " + VbCrlf
'	sqlStr = sqlStr + "     and mileage<0 " + VbCrlf
...
function CheckNRegRefund(id, orderserial, reguserid)
    ...
    if (returnmethod="R000") or (returnmethod="") or (trim(returnmethod)="") then
        Exit function
    elseif (returnmethod="R020") or (returnmethod="R080") or (returnmethod="R100") or (returnmethod="R400") then
        RegDivCd = "A007"
