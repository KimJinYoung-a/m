<%
	'// [E Type] 베스트 어워드
	'# 파일이 없거나 존재하더라도 1시간에 한번 신규 XML 생성!!(로드밸런싱 1차 서버에서만, 오늘날짜일때만 생성)
	''Application("chk_main_award_update")="2008-12-12 05:00:00"
	if (application("Svr_Info")="137" or application("Svr_Info")="082" or application("Svr_Info")="Dev") then
		...
		if not(fso.FileExists(server.MapPath(sFolder & "sub_award.xml"))) or dateDiff("h",Application("chk_main_award_update"),now())>0 then
			if selDate=replace(date,"-","") then
				Server.Execute("/chtml/make_main_awardXML.asp")
				...
				<div class="templateD">
					<table>
						...
						<tr onclick="location.href='/category/category_itemPrd.asp?itemid=<%=subNodes.getElementsByTagName("itemid").item(0).text%>';">
							...
							<td class="lt">
								<p class="ftMidSm"><%=subNodes.getElementsByTagName("itemname").item(0).text%></p>
								<p class="ftMidSm2 cAaa tPad05 lh14"><%=subNodes.getElementsByTagName("cateLarge").item(0).text%> &gt; <br /><%=subNodes.getElementsByTagName("cateMid").item(0).text%></p>
							</td>
							<td>
							<%
