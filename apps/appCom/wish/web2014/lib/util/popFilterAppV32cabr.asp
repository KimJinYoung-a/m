...
dim offsetttpg
offsetttpg = -(Int(-((vTotalCount/PageSize))))
				...
				<div class="panel quickmove">
					...
					<div class="form">
						...
						<legend class="hidden">결과 내 빠른이동</legend>
							<input type="hidden" id="maxoffset" name="maxoffset" value="<%= offsetttpg %>" />
							<input type="number" id="offset" name="offset" oninput="maxLengthCheck(this)" maxlength="4" title="페이지 넘버 입력" placeholder="<%= offset+1 %>" /> <span class="page-no"><span class="slash">/</span> <b><%= FormatNumber(offsetttpg,0) %> </b>페이지</span>
							<button type="button" onclick="jsGoSearchh();" class="btn-submit">이동</button>
						</fieldset>
					</div>
				</div>
				-->
...
<%
	...
	'//스타일 검색결과
	dim oGrStl, vStyleTitle, vStyleList, vTitleCnt, vDispTitle
	vTitleCnt = 0
...
%>
	...
	<button type="button" class="btn-close-down" onClick="jsSearchLayerClose();">닫기</button>
</div>
...
function jsGoSearchh(){
	...
	var maxoffset = parseInt($("#maxoffset").val());
//빠른 페이지 이동 일단 보류
//		if(offset < 1){
//			$("#offset").val("0");
//			offset = "0"
...
//		}
	...
	var ifilter = {"keyword":keyWd,"sscp":sscp,"delitp":delitp,"sflag":sflag,"pojangok":pojok,"mode":mde,"displaytype":srtmt,"categoryid":catCd,"brandid":mkrid,"pricelimitlow":prcMin,"pricelimithigh":prcMax,"rstxt":rstxt,"stylecd":styCd,"colorcd":ColrCd,"offset":offset,"listoption":listOption};
...
function jsGoDefaultSetting(){
	...
	return false;
//	location.href = "http://m.10x10.co.kr/apps/appCom/wish/web2014/lib/util/popFilterAppV32cabr.asp?dispCate=<%=dispCate%>&mkr=<%=makerid%>";
}

function maxLengthCheck(object){
	var maxpglen = maxoffset.value.length;
