...
''4월 18,19일 확율 : 요기요 10, 바나나10, 몬스터감귤15, 아이패드0.2

''구분1 : 요기요쿠폰	renloop > 499 and renloop < 551		5%		SUCCESS1
''구분2 : 바나나맛우유	renloop > 299 and renloop < 501		20%		SUCCESS2
''구분3 : 몬스터감귤	renloop > 99 and renloop < 151		5%		SUCCESS3
''구분4 : 아이패드		renloop > 6 and renloop < 8			0.1%	SUCCESS4
''꽝	: 무배쿠폰 COUPON
	...
	'// 이벤트 응모기간 확인
	If not(nowdate>="2015-04-17" and nowdate <"2015-04-21") Then
	...
	If mode = "add" Then '//응모하기 버튼 클릭
		...
		If result1 = "" And result2="" Then '//응모 내역이 없음
			If renloop > 499 and renloop < 551 Then '요기요쿠폰 난수 당첨시
				''요기요쿠폰 하루 발급량 카운팅
				...
				if cnt >= 500 then	''요기요쿠폰 하루 발급량이 500 넘으면 무배쿠폰 발급
				...
				if cnt < 500 then	''요기요쿠폰 하루 발급량이 500 개 미만이면 요기요쿠폰쿠폰 발급
					''당첨된 이력 있는지 체크
					...
					if userid <> "" then
						''요기요쿠폰 하나에 userid넣음
						sqlstr = "update [db_temp].[dbo].[tbl_3comdalcom_coupon_2015_yogiyo] set userid='"& userid &"',isusing='Y',regdate='" & nowdate & "' " + vbcrlf
				...
				End if
			Elseif renloop > 299 and renloop < 501 Then '바나나맛우유
				''바나나맛우유 하루 발급량 카운팅
				sqlStr = "Select count(idx) " &_
						" From db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo " &_
				...
				if cnt >= 579 then	''바나나맛우유 하루 발급량이 579 개 넘으면 무배쿠폰 발급
					sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript] (evt_code , userid , sub_opt1 , sub_opt2, device, regdate)" + vbcrlf
					sqlstr = sqlstr & " VALUES("& eCode &", '"& userid &"', '1', '0', 'A','"& nowdate &"' )" + vbcrlf
					dbget.execute sqlstr
				...
				if cnt < 579 then	''바나나맛우유 하루 발급량이 579 개 미만이면 바나나맛우유 발급
					''당첨된 이력 있는지 체크
					sqlstr = "select count(userid) as cnt "
					sqlstr = sqlstr & " from db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo"
					sqlstr = sqlstr & " where userid='"& userid &"'"
				...
				End if
			Elseif renloop > 99 and renloop < 151 Then '몬스터감귤
				''몬스터감귤 하루 발급량 카운팅
				sqlStr = "Select count(idx) " &_
						" From db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo " &_
				...
				if cnt >= 375 then	''몬스터감귤 하루 발급량이 375 개 넘으면 무배쿠폰 발급
					sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript] (evt_code , userid , sub_opt1 , sub_opt2, device, regdate)" + vbcrlf
					sqlstr = sqlstr & " VALUES("& eCode &", '"& userid &"', '1', '0', 'A','"& nowdate &"' )" + vbcrlf
					dbget.execute sqlstr
				...
				if cnt < 375 then	''몬스터감귤 하루 발급량이 375 미만이면 몬스터감귤 발급
					''당첨된 이력 있는지 체크
					sqlstr = "select count(userid) as cnt "
					sqlstr = sqlstr & " from db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo"
					sqlstr = sqlstr & " where userid='"& userid &"'"
				...
				End if
			Elseif renloop > 6 and renloop < 8 Then '아이패드
				''아이패드 하루 발급량 카운팅
				sqlStr = "Select count(idx) " &_
						" From db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo " &_
				...
				if cnt > 0 then	''아이패드 하루 발급량이 1개 넘으면 무배쿠폰 발급
					sqlstr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript] (evt_code , userid , sub_opt1 , sub_opt2, device, regdate)" + vbcrlf
					sqlstr = sqlstr & " VALUES("& eCode &", '"& userid &"', '1', '0', 'A','"& nowdate &"' )" + vbcrlf
					dbget.execute sqlstr
				...
				if cnt < 1 then	''아이패드 하루 발급량이 1 미만이면 아이패드 발급
					''당첨된 이력 있는지 체크
					sqlstr = "select count(userid) as cnt "
					sqlstr = sqlstr & " from db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo"
					sqlstr = sqlstr & " where userid='"& userid &"'"
		...
		ElseIf result1 = "1" Then '// 두번째 응모시
			If result2 = "0" Then '//두번쨰 응모지만 첫번째응모가 미당첨일 경우
				...
				Else '//카카오 초대 누르고 응모 누름
					If renloop > 499 and renloop < 551 Then '요기요쿠폰쿠폰\\난수 당첨시
						''요기요쿠폰 하루 발급량 카운팅
						sqlStr = "Select count(idx) " &_
								" From db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo " &_
								" WHERE gubun='1' and convert(varchar(10),regdate,120) = '" & nowdate & "' and userid is not null"
						...
						if cnt >= 500 then	''요기요쿠폰 하루 발급량이 500 개 넘으면 무배쿠폰 발급
						...
						if cnt < 500 then	''요기요쿠폰 하루 발급량이 500 개 미만이면 요기요쿠폰 발급
							''당첨된 이력 있는지 체크
							sqlstr = "select count(userid) as cnt "
							sqlstr = sqlstr & " from db_temp.dbo.tbl_3comdalcom_coupon_2015_yogiyo"
							sqlstr = sqlstr & " where userid='"& userid &"'"
