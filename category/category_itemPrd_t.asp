...
<%
	...
	if (oItem.Prd.FItemDiv = "21") Then
		'딜상품 딜 페이지로 이동
		Response.redirect("/deal/deal.asp?itemid="+Cstr(itemid))
	End If
...
//			}, 300);
			...
			<%' amplitude 이벤트 로깅 %>
				tagScriptSend('', 'itemprd', '', 'amplitude');
...
<%' #INCLUDE Virtual="/member/actnvshopPayLayerCont.asp" %>
<% ElseIf isDaumOpen AND Left(request.Cookies("rdsite"), 15) = "mobile_daumshop" Then %>
...
<div id="content" class="content">
	...
	<% End If %>
	<% If (now() >  #11/24/2017 00:00:00# And now() <= #11/27/2017 23:59:59#) then %>
	<!-- 쇼킹 원데이 2017.11.27 -->
	<div class="bnr">
		<a href="/event/eventmain.asp?eventid=82448&gaparam=item_banner_shorkingone" title="쇼킹 원데이 페이지로 이동"><img src="http://webimage.10x10.co.kr/eventIMG/2017/82448/m/bnr_detail.jpg" alt="쇼킹 원데이" /></a>
	</div>
	<% end if %>
	<% If clsDiaryPrdCheck.FResultCount > 0 then %>
		<% If Left(Now(), 10) > "2017-10-15" and Left(Now(), 10) < "2018-01-01" Then %>
			<a href="/diarystory2018/?gnbflag=1"><img src="http://fiximage.10x10.co.kr/web2017/diary2018/m/bnr_detail_diary.jpg" class="vTop" alt="DIARY STORY 2018" /></a>
		<% end if %>
	<% end if %>
