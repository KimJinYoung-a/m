...
<%
	...
	dim result, oJson, md5userid, winItemid, winPercent, winItemea, winItemOption
	...
	eventEndDate 	= cdate("2020-11-17")		'이벤트 종료일+1
	currentDate 	= date()
...
'=========이벤트 시작일======================================================
		Case "2020-11-09"
            if selectedPdt = 2 then
                chkWinTime "11:41", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 3 then
				chkWinTime "08:45", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 6 then
				chkWinTime "13:00", 30, winStartTimeLine, winEndTimeLine
				chkWinTime "00:45", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 7 then
				chkWinTime "22:36", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-11-10"
			if selectedPdt = 3 then
				chkWinTime "13:54", 30, winStartTimeLine, winEndTimeLine
			end if
		...
		Case "2020-11-11"
			...
			if selectedPdt = 7 then
				chkWinTime "13:44", 30, winStartTimeLine, winEndTimeLine
				chkWinTime "18:41", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 8 then
				chkWinTime "15:11", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-11-12"
			if selectedPdt = 3 then
				chkWinTime "21:38", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 4 then
				chkWinTime "19:18", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 6 then
				chkWinTime "14:12", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-11-13"
			if selectedPdt = 1 then
				chkWinTime "22:28", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 3 then
				chkWinTime "17:29", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 5 then
		...
		Case "2020-11-14"
			if selectedPdt = 3 then
				chkWinTime "13:59", 30, winStartTimeLine, winEndTimeLine
			end if
			if selectedPdt = 8 then
				chkWinTime "09:39", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 9 then
		...
		Case "2020-11-15"
            ...
            if selectedPdt = 6 then
				chkWinTime "14:31", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 7 then
				chkWinTime "08:32", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-11-16"
			if selectedPdt = 2 then
				chkWinTime "15:34", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 3 then
			...
			if selectedPdt = 7 then
				chkWinTime "20:52", 30, winStartTimeLine, winEndTimeLine
			end if
	End select
	...
	if result = "C01" Then
		...
		if selectedPdt="2" or selectedPdt="7" then
			winItemea="1"
		else
			winItemea="2"
		end if
		...
		elseif selectedPdt="6" then
			winItemOption="0011"
		elseif selectedPdt="8" then
			winItemOption="Z122"
		elseif selectedPdt="9" then
			winItemOption="0011"
		else
			winItemOption="0000"
		end if

		sqlstr = "insert into [db_my10x10].[dbo].[tbl_my_baguni](userKey, itemid, itemoption, itemea) values (" & vbcrlf
		sqlstr = sqlstr & " '" & LoginUserid & "', " & winItemid & ", '" & winItemOption & "', "& winItemea & ")"
