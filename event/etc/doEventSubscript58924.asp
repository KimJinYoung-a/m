...
Dim vQuery, vTotalCount , vTotalSum
	...
	If Now() > #02/01/2015 00:00:00# Then
	...
	if mode="myorder" then
		'내 주문건 조회
		sqlStr = " EXEC [db_order].[dbo].[sp_Ten_MyOrderList_SUM] '" & userid & "', '', '', '2015-01-01', '2015-02-01', '10x10', '', 'issue' "
	...
	elseif mode="add" then
		'//응모
		vQuery = " EXEC [db_order].[dbo].[sp_Ten_MyOrderList_SUM] '" & userid & "', '', '', '2015-01-01', '2015-02-01', '10x10', '', 'issue' "
		rsget.CursorLocation = adUseClient
		rsget.CursorType = adOpenStatic
		rsget.LockType = adLockOptimistic
		...
		If vTotalCount < 1 Then
			response.write "<script language='javascript'>alert('1월 구매내역이 없습니다.\n구매 후, 다시 응모하러 와주세요!'); parent.location.reload();</script>"
			dbget.close()
			response.end
		End If
		...
		Response.Write "<script type='text/javascript'>alert('정상적인 경로가 아닙니다.'); parent.top.location.href='/event/eventmain.asp?eventid=58924'</script>"
		dbget.close() : Response.End
	end if
%>
