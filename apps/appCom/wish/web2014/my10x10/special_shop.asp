...
<script>
	...
	<% If IsUserLoginOK() Then %>
		fnAPPpopupBrowserURL("위시폴더","<%=wwwUrl%>/apps/appcom/wish/web2014/common/popWishFolder.asp?itemid="+iitemid+"&ErBValue=12");
	<% Else %>
		calllogin();
	<% End If %>
	}
...
<div class="heightGrid bgGry">
	<div class="container popWin bgGry">
		<div class="content vipShop" id="contentArea">
			<div class="inner10">
				...
				<div class="pdtListWrap">
					...
					%>
						<div class="comingSoon">
							...
							<% If vReservDate <> "" Then %><p class="cRd1 tPad15">오픈예정일 : <%=Replace(vReservDate,"-",".")%></p><% End If %><p>&nbsp;</p>
						</div>
						<% else %>
							<% for j=0 to iRows-1 %>
								<li <% IF ospecialshop.FItemList(j).isSoldOut Then %>class="soldOut"<% End If %>>
									<div class="pPhoto" onclick="fnAPPpopupProduct('<%= ospecialshop.FItemList(j).FItemID %>'); return false;"><% IF ospecialshop.FItemList(j).isSoldOut Then %><p><span><em>품절</em></span></p><% End If %><img src="<%= ospecialshop.FItemList(j).FImageBasic %>" alt="<%= ospecialshop.FItemList(j).FItemName %>" /></div>
									<div class="pdtCont">
