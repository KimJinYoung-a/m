...
<!-- #include virtual="/apps/appCom/between/inipay/card/order_real_save_function.asp" -->
<%
	...
	If P_STATUS = "01" Then '인증결과가 실패일 경우
		Response.write "<script>alert('01. 이니시스에 인증결과 실패가 발생하였습니다. 고객센터로 문의해 주세요.');location.replace('/apps/appcom/between/inipay/shoppingbag.asp');</script>"
		dbget.close()
		Response.End
	End If
...
'''-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	IF Err.Number <> 0 then
		Response.write "<script language='javascript'>alert('02. 이니시스에 승인요청 중 오류가 발생하였습니다. 고객센터로 문의해 주세요.');</script>"
		dbget.close()
		Response.End
	End If
...
//document.frm.submit();
document.location.href = "<%=vAppLink%>/inipay/DisplayOrder.asp?dumi=<%=FormatDate(now(),"00000000000000")%>";  //캐시 먹음. post로 때릴것
</script>
</body>
</html>
