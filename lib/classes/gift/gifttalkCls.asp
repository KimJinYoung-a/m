...
Class CGiftTalk
	...
	public Function fnGiftTalkKeywordList
		...
		strSql = "EXECUTE [db_board].[dbo].[sp_Ten_GiftTalk_Keyword_List] '" & FRectUseYN & "'"
		'response.write strSql
		rsget.CursorLocation = adUseClient
		rsget.CursorType = adOpenStatic
		rsget.LockType = adLockOptimistic
		...
		if not rsget.EOF then
			fnGiftTalkKeywordList = rsget.getRows()
		end if
		rsget.close
	End Function
...
'		End IF
		...
		strSql = "EXECUTE [db_board].[dbo].[sp_Ten_GiftTalk_PreNext] '" & FRectTalkIdx & "', '" & FRectUserId & "', '" & FRectItemId & "', '" & FRectTheme & "', '" & FRectKeyword & "', '" & FRectUseYN & "', '" & FRectDiv & "', '" & FRectSort & "'"
		'response.write strSql
		rsget.CursorLocation = adUseClient
		rsget.CursorType = adOpenStatic
		rsget.LockType = adLockOptimistic
	...
	public Function fnGiftTalkBestTalk
		...
		strSql = "EXECUTE [db_board].[dbo].[sp_Ten_GiftTalk_BestTalk]"
		'response.write strSql
		rsget.CursorLocation = adUseClient
		rsget.CursorType = adOpenStatic
		rsget.LockType = adLockOptimistic
		...
		if not rsget.EOF then
			fnGiftTalkBestTalk = rsget.getRows()
		end if
		rsget.close
	End Function
	...
	public Function fnGiftStoryList
		...
		strSql = "EXECUTE [db_board].[dbo].[sp_Ten_GiftStory_List] '" & FRectItemId & "', '" & FRectUseYN & "'"
		'response.write strSql
		rsget.CursorLocation = adUseClient
		rsget.CursorType = adOpenStatic
		rsget.LockType = adLockOptimistic
...
Function fnConFirmMyTalk(talkidx, userID)
	Dim vQuery
	vQuery = "EXECUTE [db_board].[dbo].[sp_Ten_GiftTalk_MyTalkConfirm] '" & talkidx & "', '" & userID & "'"
