			...
			<div class="list-card type-align-left">
				...
				<%
                    ...
                    if fixBanner(4,0) = "13" Then '//상품이벤트 일경우
						vLink = "fnAPPpopupProduct('" & fixBanner(8,0) & "');"
					Else '// 그외에
						IF fixBanner(6,0)="I" and fixBanner(7,0)<>"" THEN '링크타입 체크
							vLink = "fnAPPpopupEvent_URL('" & fixBanner(7,0) & "');"
						ELSE
							vLink = "fnAPPpopupEvent('" & fixBanner(0,0) & "'); return false;"
						END IF
					End If
					...
					if fixBanner(29,0) <> "" or not(isnull(fixBanner(29,0))) then
						if ubound(Split(fixBanner(29,0),",")) > 0 Then
							...
							for ip = 0 to ubound(Split(fixBanner(29,0),","))
								...
								addTag = addTag & "<li><a href='' onclick='fnAPPpopupSearchOnNormal(""" & Split(fixBanner(29,0),",")(ip) & """);return false;'>#"& Split(fixBanner(29,0),",")(ip) &"</a></li>"
							next 
							addTag = addTag & "</ul>"
						end if 
					end if
				%>
					<li<% If fixBanner(28,0)<>"" Then %> class="has-vod"<% End If %>>
						<a href="" onclick="<%=vLink%> return false;">
							<!-- for dev msg : 썸네일 alt값 alt=""으로 처리해주세요. -->
							<div class="thumbnail"><img src="<%=chkiif(fixBanner(24,0)="",fixBanner(25,0),fixBanner(24,0))%>" alt="<%=db2html(fixBanner(10,0))%>"></div>
							<div class="desc">
								<p>
									<b class="headline"><span class="ellipsis" <% IF fixBanner(11,0) and eNameredsale <> "" THEN %>style="width:80%;"<% else %>style="width:100%;"<% end if %>><%=Replace(stripHTML(db2html(eName)),"[☆2015 다이어리]","")%></span> <% IF fixBanner(11,0) and eNameredsale <> "" THEN %><b class="discount color-red"><%=eNameredsale%></b><% end if %></b>
