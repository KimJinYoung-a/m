...
sFolder = "/chtml/main/xml/tpobanner/" & Replace(Date(),"-","") &"/"
CtrlDate = Date()
mainFile = "main_tpobanner_"&CtrlDate&".xml"
...
If fileCont<>"" Then
	...
	For each tplNodes in cTmpl
		...
		bgalt		= tplNodes.getElementsByTagName("bgalt").item(0).text
		bgurl		= tplNodes.getElementsByTagName("bgurl").item(0).text
		If CtrlDate >= startdate AND CtrlDate <= enddate Then
%>
							<div class="swiper-slide">
								<div style="background-image:url(<%= bgimg %>);">
									<p class="tpoTopLink"><a href="<%= bgurl %>"></a></p>
									<p class="overHidden">
