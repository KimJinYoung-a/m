...
if (LGD_OID="") then
    Response.write "<script type='text/javascript'>alert('01. 인증결과 실패가 발생하였습니다. 고객센터로 문의해 주세요.');</script>"
    vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "01. 인증결과 실패가 발생하였습니다. 고객센터로 문의해 주세요.")
    response.redirect fnGetUserInfo("FAILURL")
	dbget.close()	:	response.end
end if

if (LGD_RESPCODE="") then
    Response.write "<script type='text/javascript'>alert('02. 인증결과 실패가 발생하였습니다. 고객센터로 문의해 주세요.');</script>"
    vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "02. 인증결과 실패가 발생하였습니다. 고객센터로 문의해 주세요.")
    response.redirect fnGetUserInfo("FAILURL")
	dbget.close()	:	response.end
end if

if (LGD_RESPCODE<>"0000") then
    Response.write "<script type='text/javascript'>alert('["&LGD_RESPCODE&"]. "&replace(LGD_RESPMSG,"'","")&"');</script>"
    vBetweenPostReport = fnBetweenPostReport(fnGetUserInfo("signdata"), ref_Status, ref_result_str, FALSE, "["&LGD_RESPCODE&"]. "&replace(LGD_RESPMSG,"'",""))
    response.redirect fnGetUserInfo("FAILURL")
	dbget.close()	:	response.end
end if
...
'response.write vBetweenPostReport &"/" & ref_Status&"/" & ref_result_str & "<br>모바일결제"
'response.end
