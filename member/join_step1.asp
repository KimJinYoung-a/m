...
<form name="snsinfo" method="post" action="<%=M_SSLUrl%>/member/join.asp">
...
snsusermail = tenDec(snsusermail)
snsusername = tenDec(snsusername)
If application("Svr_Info")<>"Dev" Then
	...
	'        response.write "<script>alert('유효한 접근이 아닙니다.'); history.back();</>"
	'        response.end
		end if
	end if
end if

if Not(snsusermail="" or isNull(snsusermail)) Then
...
//	var usrph = frm.txCell1.value + "-" + frm.txCell2.value + "-" + frm.txCell3.value;
	...
	var rstStr = $.ajax({
		type: "POST",
		url: "/member/ajaxSendConfirmSMS2015.asp",
...
function fnConfirmSMS() {
	if($("#checkMsgHP").html() != "인증이 완료되었습니다."){
		...
		$.ajax({
			type:"POST",
			url:"/member/ajaxCheckConfirmSMS2015.asp",
...
function chkMemPwd(){
	...
	if(frm.txpass2.value !=null && frm.txpass2.value!= ""){
		//$("#pw2").hide();
		if(frm.txpass1.value != frm.txpass2.value){
...
<div class="heightGrid">
	<div class="mainSection">
		<form name="myinfoForm" method="post" action="<%=M_SSLUrl%>/member/dojoin_step2.asp" onsubmit="return false;">
		<input type="hidden" name="hideventid" value="<%= ihideventid %>">
		<input type="hidden" name="email_10x10" value="<%= email_10x10 %>">
		<input type="hidden" name="smsok" value="<%= smsok %>">
		<input type="hidden" name="email_way2way" value="<%= email_way2way %>">
		...
		<div class="container">
			...
			<div class="content" id="contentArea">
				<div class="section simpleJoinForm">
                    ...
                    <div class="login-form">
						...
						<legend class="hidden">로그인 폼</legend>
							<div class="form-group first">
								<input type="text" name="txuserid" id="txuserid" value="<%= snsusermailid %>" maxlength="15" onKeyDown="DuplicateIDCheck(this);keyCodeCheckID(event,this);" onKeyUp="DuplicateIDCheck(this);" onClick="DuplicateIDCheck(this);" onBlur="isToLowerCase(this,0); DuplicateIDCheck(this);" placeholder="아이디" autocorrect="off" autocapitalize="off">
								<!-- <label for="txuserid" id="uid"></label> -->
							...
							<div class="form-group">
								<input type="password" name="txpass1" id="txpass1" placeholder="비밀번호" maxlength="32" onKeyDown="fnCheckPwd();" onKeyUp="fnCheckPwd();" onClick="fnCheckPwd();" onBlur="fnCheckPwd();">
							...
							<div class="form-group">
								<input type="password" name="txpass2" id="txpass2" placeholder="비밀번호 확인" maxlength="32" onKeyDown="chkMemPwd();" onKeyUp="chkMemPwd();" onClick="chkMemPwd();" onBlur="chkMemPwd();">
							...
							<div class="form-group" id="smsConfirm" style="display:none">
								<input type="number" name="crtfyNo" id="certNum" maxlength="6" placeholder="인증번호를 입력해주세요" onKeyDown="maxLengthCheck(this);" onKeyUp="maxLengthCheck(this);" autocomplete="one-time-code">
                    ...
                    <div class="btnGroup">
                        <input type="submit" class="btnV16a btnRed2V16a btnLarge btnBlock" id="" onclick="FnJoin10x10(); return false;" value="가입하기">
                    </div>
				</div>
			</div>
			<!-- //content area -->
