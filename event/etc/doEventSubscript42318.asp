...
if mode="add" then
	...
	If rsget("cnt") >= 5 Then
		Response.Write  "<script language='javascript'>" &_
						"	alert('오늘은 더이상 등록이 불가능합니다');" &_
						"location.replace('/event/etc/iframe_42318.asp');" &_
						"</script>"
...
elseif mode="like" then
		...
		If rsget(0)>0 Then
			chk = False
			Response.Write "<script type='text/javascript'>alert('이미 참여하셨습니다.'); location.replace('/event/etc/iframe_42318.asp');</script>"
		...
		If chk = True Then
			...
			dbget.execute(strSql)
	Response.Write "<script type='text/javascript'> location.replace('/event/etc/iframe_42318.asp');</script>"
		End If
