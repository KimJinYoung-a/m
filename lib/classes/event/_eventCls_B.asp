...
Class ClsEvtCont
	...
	public Function fnEventISSUEList
		Dim strSql
		strSql ="[db_event].[dbo].sp_Ten_event_EventISSUE_New ('" & FECode & "','" & FEKind & "','" & FBrand & "','" & FEDispCate & "' ,'"& FDevice &"')"
		'rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdStoredProc
	...
	public Function fnEventISSUEList2
		Dim strSql
		strSql ="[db_event].[dbo].sp_Ten_event_EventISSUE_Temp ('" & FECode & "','" & FEKind & "','" & FBrand & "','" & FEDispCate & "' ,'"& FDevice &"')  "
		'rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdStoredProc
		dim rsMem : set rsMem = getDBCacheSQL(dbget,rsget,"EVT",strSql,60*30)
        if (rsMem is Nothing) then Exit function ''추가
		IF Not (rsMem.EOF OR rsMem.BOF) THEN
