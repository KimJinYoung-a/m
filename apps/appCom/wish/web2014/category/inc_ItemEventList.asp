<%
	...
	tmpSQL = "Execute [db_item].[dbo].sp_Ten_EvtByItem_Temp @vItemid=" & itemid &" , @device='A'"
'DB Ver.
...
'		rsget.close
	...
	'MEM Cache Ver.
		dim rsMem : set rsMem = getDBCacheSQL(dbget,rsget,"EVTISSUE",tmpSQL,60*10)
		...
		FOR i=0 to ubound(arrEvt,2)
			...
			SELECT CASE cStr(arrEvt(0,i))
				...
				case "1", "29" , "31"'//쇼핑 찬스, 헤이썸띵
					...
					Else
						''strLink = "<a href="""" onclick=""fnAPPpopupEvent('" & arrEvt(4,i) & "');return false;"">"
						'strLink = "<a href="""" onclick=""fnAPPpopupEvent_URL('" & wwwUrl & "/apps/appcom/wish/web2014/event/eventmain.asp?eventid=" & arrEvt(4,i) & "&"&strEvtRc & "');return false;"">"
						strLink = "<a href=""javascript:void(0)"" onclick=""fnAmplitudeEventMultiPropertiesAction('click_planevents_in_product','idx|evtcode','"& i+1 &"|" & arrEvt(4,i) & "',function(bool){ if(bool) {fnAPPpopupEvent_URL('" & wwwUrl & "/apps/appcom/wish/web2014/event/eventmain.asp?eventid=" & arrEvt(4,i) & "&"&strEvtRc & "');return false;}});"">"						
					End If

					if ubound(Split(arrEvt(2,i),"|"))> 0 Then
						strLinkName	= "<p class='pName'>"&cStr(Split(arrEvt(2,i),"|")(0)) &"</p>"
						strLinkNameSale =  "<span class=""color-red"">"&cStr(Split(arrEvt(2,i),"|")(1))&"</span>"
'						strLinkName	= "<span>"&cStr(Split(arrEvt(2,i),"|")(0)) &"</span> <em class='cRd1V16a'>"&cStr(Split(arrEvt(2,i),"|")(1))&"</em>"
					Else
					...
					If arrEvt(14,i)="True" Then
						strLinkNameTag	= strLinkNameTag&" <span class=""color-green"">"& chkIIF(arrEvt(35,i) <> "", arrEvt(35,i) & "%", "")  &" <small>쿠폰</small></span>"
					End If

					If arrEvt(15,i)="True" Then
						strLinkNameTag	= strLinkNameTag&" <span class=""color-blue"">GIFT</span>"
					End If
...
<%
	...
	IF evt_tagkind <>"" Then
		...
		If evt_tagkind = "2" Then vEvtListBody = vEvtListBody & "<strong class=""label"">"& evt_tagopt1 &"<br /> Coupon</strong>" End if
		...
		If evt_tagkind = "7" Then vEvtListBody = vEvtListBody & "<strong class=""label"">"& CLng((oItem.Prd.FOrgPrice-oItem.Prd.FSellCash)/oItem.Prd.FOrgPrice*100) &"%<br /> SALE</strong>" End if
		vEvtListBody = vEvtListBody & "<div class=""desc"">"
		vEvtListBody = vEvtListBody & "<p>"& etc_opt1 &"</p>"
		if year(eddate)>year(stdate) then
		vEvtListBody = vEvtListBody & "<div class=""date"">"& formatdate(stdate,"0000.00.00") &" ~ "& formatdate(eddate,"0000.00.00") &" <span>"& etc_opt2 &"</span></div>"
		else
