...
<%
	...
	eventEndDate	= cdate("2020-11-04")		'이벤트 종료일
	currentDate	= date()
...
'=========이벤트 시작일======================================================
		Case "2020-10-26"
            if selectedPdt = 4 then
                chkWinTime "10:40", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "12:06", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "14:43", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "20:43", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-10-27"
            if selectedPdt = 3 then
                chkWinTime "12:16", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 4 then
                chkWinTime "10:25", 30, winStartTimeLine, winEndTimeLine
				chkWinTime "14:47", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "13:29", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-10-28"
			if selectedPdt = 3 then
		...
		Case "2020-10-29"
			if selectedPdt = 4 then
				chkWinTime "02:54", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-10-30"
			if selectedPdt = 3 then
				chkWinTime "18:06", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 4 then
				chkWinTime "13:34", 30, winStartTimeLine, winEndTimeLine
				chkWinTime "15:51", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-10-31"
            if selectedPdt = 4 then
		...
		Case "2020-11-01"
            if selectedPdt = 4 then
				chkWinTime "11:35", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-11-02"
            if selectedPdt = 2 then
				chkWinTime "19:45", 30, winStartTimeLine, winEndTimeLine
			end if
            if selectedPdt = 4 then
                chkWinTime "00:04", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "06:36", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "12:13", 30, winStartTimeLine, winEndTimeLine
                chkWinTime "17:39", 30, winStartTimeLine, winEndTimeLine
			end if
		Case "2020-11-03"
            if selectedPdt = 1 then
				chkWinTime "20:12", 30, winStartTimeLine, winEndTimeLine
			end if
	End select
...
elseif mode = "pushadd" then
	...
	'// 다음날 푸쉬 신청을 했는지 확인한다.
	vQuery = "SELECT count(*) FROM [db_temp].[dbo].[tbl_pickUpEvent_Push4] WITH (NOLOCK) WHERE userid='"&LoginUserid&"' And convert(varchar(10), SendDate, 120) = '"&Left(pushDate, 10)&"' "
	...
	vQuery = " INSERT INTO [db_temp].[dbo].[tbl_pickUpEvent_Push4](userid, SendDate, Sendstatus, RegDate) VALUES('" & LoginUserid & "', '"&Left(pushDate, 10)&"', 'N', getdate())"
