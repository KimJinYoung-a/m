...
'####################################################
	...
	stockItemId = requestCheckVar(request("itemid"),20)
	
	'// stockItemId 값이 없으면 창을 닫아준다.
...
<body class="default-font body-popup">
	<%' contents %>
	<div id="content" class="content stock-inform">
					...
					<% IF (Not IsMultipleOption) Then %>
						...
						<div class="textfield search-filter">
							<div class="panelcont" style="display: block;">
								<ul class="depth1 multi-select">
									<% 
										for i=0 to oItemOption.FResultCount-1 
									%>
											<%' 품절만 표시 %>
											<% If ((oitem.Prd.IsSoldOut) or (oItemOption.FItemList(i).IsOptionSoldOut)) Then %>
												<li><a href="" value="<%=oItemOption.FItemList(i).FitemOption%>"><%=oItemOption.FItemList(i).FOptionName%></a></li>
											<% End If %>
									<% Next %>
								</ul>
							</div>
								...
								<ul class="depth1 multi-select">
									<% 
										...
										if Not rsget.Eof Then
											Do Until rsget.eof
									%>
												<li><a href="" value="<%=rsget("itemoption")%>"><%=rsget("optionname")%></a></li>
									<%
											rsget.movenext
											Loop
										End If
