...
Dim cpid , strSQL
Dim basicimage ,itemname ,itemid ,startdate ,enddate ,dealtitle ,gubun1 ,gubun2 ,itemurl
Dim sellCash ,orgPrice , sailYN ,itemcouponYn ,itemcouponvalue ,LimitYn ,itemcoupontype

For j=1 to cInt(vTerm2)
	'// 메인 데이터 접수
	sqlStr = "select t.* , i.sellCash , i.orgPrice , i.sailyn , i.itemcouponYn, i.itemcouponvalue ,i.limitYN ,i.itemcoupontype , i.basicimage "
	sqlStr = sqlStr + " , md.itemcouponidx , md.itemcoupontype as ctype , md.itemcouponvalue as cvalue "
	...
	sqlStr = sqlStr + "		and m.coupongubun = 'T' and m.openstate in ('7') "
	...
	vTotalCount = rsget.RecordCount
		basicimage		= webImgUrl & "/image/basic/" + GetImageSubFolderByItemid(db2Html(rsget("itemid"))) + "/" + db2Html(rsget("basicimage")) 
	...
	FileName = "main_todaydeal_test.xml"
