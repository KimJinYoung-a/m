...
<%
'####################################################
' Description : 상품 SVG 이미지 테스트 페이지
...
<!-- for dev msg : SVG 마스크 참조 요소
	상품 썸네일에 7가지 도형 마스크가 필요한 페이지의 경우
	body 닫히기 전에 아래 참조 요소가 꼭! 필요합니다.
-->
	...
	var app = new Vue({
		...
		created : function() {
		    ...
		    for( let i=0 ; i<shape_arr.length ; i++ ) {
		        this.products.push({
		            shape_type : shape_arr[i],
                    sell_flag : 'N',
                    adult_type : 0,
                    image_url : '//thumbnail.10x10.co.kr/webimage/image/basic600/210/B002105986.jpg'
		        });
		        this.products.push({
		            shape_type : shape_arr[i],
                    sell_flag : 'Y',
