...
<%
'#######################################################
' PageName : make_todayenjoy_xml.asp
...
<!-- #include virtual="/lib/classes/main/main_todayenjoyCls.asp" -->
<%
'' 관리자 확정시 Data 작성. 
...
'		response.End
'	elseif (oenjoyevent.FResultCount<(3)) then
'		response.write "<script>alert('[" & prevDate & "]일 적용에 필요한 데이터가 부족합니다.\n\n(※ 최소 3 건 필요. 현재 " & oenjoyevent.FResultCount & "건 등록됨)');self.close();</script>"
'		response.end
'	end if
'
'	
...
For j=1 to cInt(vTerm2)
	'// 메인 데이터 접수
	sqlStr = "select top 15 t.* , d.evt_todaybanner , d.evt_mo_listbanner "
	...
	if vTotalCount>0 then
		...
		Do Until rsget.EOF
			...
			objXMLv.childNodes(0).text = staticImgUrl & "/mobile/enjoyevent" + db2Html(rsget("evtimg"))
	...
	'// xml 최종 생성 DB-update
	sqlStr = "Update [db_sitemaster].[dbo].tbl_mobile_main_enjoyevent_new " &_
