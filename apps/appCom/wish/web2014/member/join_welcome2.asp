...
function TnCSlogin2(frm){
	...
	if($("input:checkbox[name=saved_auto]").is(":checked") == true) {
		setTimeout(function () { fnRequestLogin(frm.userid.value,frm.userpass.value,true)}, 1000);
	}else{
		setTimeout(function () { fnRequestLogin(frm.userid.value,frm.userpass.value,false)}, 1000);
	}
	return true;
}
</script>
				...
				<div class="main">
					...
					<div class="coupon-welcome">
						<div class="type">
							...
							<p class="price red">30,000<span>p</span></p>
						...
						<input type="hidden" name="backpath" value="/apps/appcom/wish/web2014/event/benefit/">
							...
							<legend class="hidden">로그인 폼</legend>
								<div class="form-group">
                                    <input type="text" name="userid" id="userid" autocorrect="off" autocapitalize="off" placeholder="아이디" required>
								</div>
                                <div class="form-group">
									<input type="password" name="userpass" id="password" autocorrect="off" autocapitalize="off" placeholder="비밀번호"  required>
								</div>
