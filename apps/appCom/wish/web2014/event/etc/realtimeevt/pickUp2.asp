...
dim numOfParticipantsPerDay, i, arrEvtInfo, itemName, snsTitle, snsText, snsIMG, limitItemCNT, prizeIMG, evtState
...
/* 2021-04-05 추가 */
.evt-jupjup .prd-img  .prd-roll {position:absolute; top:0; left:0; width:100%; height:100%;}
.evt-jupjup .prd-img  .prd-loop-area {height:24.78rem; position:absolute; left:50%; top:40%; transform:translate(-50%,0); display:flex; align-items:center;}
.evt-jupjup .prd-img  .prd-loop-area .item0 {width:17.57rem; height:24.78rem;}
.evt-jupjup .prd-img  .prd-loop-area .item01 {width:17.57rem; height:24.78rem;}
.evt-jupjup .prd-img  .prd-loop-area .item02 {width:22.22rem; height:12.22rem; margin:2rem 2rem 0 0;}
...
$(function(){
    ...
    setInterval(function(){
		...
		<% if eCode="110549" then %>
        $("#roll").attr("src","//webimage.10x10.co.kr/fixevent/event/2021/110549/m/img_item0"+rolling+".png?v=2.1").attr("class","item0"+rolling);
		<% elseif eCode="110806" then %>
		$("#roll2").attr("src","//webimage.10x10.co.kr/fixevent/event/2021/110806/m/img_item0"+rolling+".png");
		<% end if %>
    }, 1500);
});
	...
	if(itemList.length > 0){
		...
		newArr.forEach(function(item){
            ...
            tmpEl = '\
				...
				<% if eCode="110549" then %>\
				<div class="prd-img">\
					<img src="//webimage.10x10.co.kr/fixevent/event/2021/110549/m/img_item.jpg" alt="전동킥보드">\
					<div class="prd-loop-area">\
						<img src="//webimage.10x10.co.kr/fixevent/event/2021/110549/m/img_item01.png?v=2.1" alt="킥보드" id="roll" class="item0">\
					</div>\
                </div>\
				...
				<% elseif eCode="110806" then %>\
				<div class="prd-img">\
					<img src="//webimage.10x10.co.kr/fixevent/event/2021/110806/m/img_item.jpg" alt="전동킥보드">\
					<div class="prd-roll">\
						<img src="//webimage.10x10.co.kr/fixevent/event/2021/110806/m/img_item01.png?v=2.1" alt="킥보드" id="roll2">\
					</div>\
                </div>\
                <div class="limit-bar" id="limit-bar">' + item.leftItems + '대 남았습니다</div>\
                <dl class="prd-info">\
	...
	$('.lyr-prev-winners').fadeIn();
			...
			<div class="evt-jupjup<% If isTeaser Then %> teaser<% end if %>">
				...
				<div class="evt-info">
					...
					<dl class="row">
						<dt>상 품 수 량</dt>
						<dd><%=limitItemCNT%>대<br>당첨자에게는 상품 후기를 요청할 예정입니다.</dd>
					</dl>
					<dl class="notice">
						...
						<dd id="notice_list" class="notice-list">
							<ul>
								...
								<% if eCode="109708" then %>
								<li class="last">상품 상세 정보 : 맥북 에어 (스페이스그레이,  Apple M1 칩, 256GB 저장 용량)</li>
								<% elseif eCode="110031" then %>
								<li class="last">상품 상세 정보 : 갤럭시 버즈 프로 블루투스 이어폰 SM-R190 (팬텀 바이올렛)</li>
								<% elseif eCode="110113" then %>
								<li class="last">상품 상세 정보 : LG 그램 17 / 17ZD90P-GX50K(크기 : 17인치 / OS : Free Dos / CPU : i5 / RAM : 8GB / SSD : 256GB)</li>
								<% elseif eCode="110549" then %>
								<li class="last">상품 상세 정보 : 나노휠 전동킥보드 NQ-01 8Ah</li>
								<% elseif eCode="110806" then %>
								<li class="last">상품 상세 정보 : 13형 맥북 에어 (실버, Apple M1 칩, 256GB 저장 용량)</li>
								<% else %>
								<li class="last">상품 상세 정보 : <% = replace(itemName,"|"," ") %></li>
								<% end if %>
							</ul>
						</dd>
					</dl>
...
%>
				...
				<div class="guide">
					...
					<a href="" onclick="fnAPPpopupBrowserURL('기획전','http://m.10x10.co.kr/apps/appCom/wish/web2014/event/eventmain.asp?eventid=102465');" class="link" target="_blank">가이드 보러가기</a>
				</div>
