...
<!-- #include virtual="/lib/classes/event/vieweventCls.asp" -->
...
Dim etc_itemid, arrGiftBox, isNew, isOnePlusOne, isOnlyTen
Dim eval_isusing, eval_text, eval_freebie_img, eval_start, eval_end, evt_using
Dim board_isusing, board_text, board_freebie_img, board_start, board_end, saleTxt, CopyHide, topSlideCheck
...
Dim arrTextTitle, evt_html, evt_subcopyK, contentsAlign
...
if stepdiv="" then
	Call Alert_Return("허용되지 않은 접근입니다.")
	dbget.close()	:	response.End
end if
...
IF egCode = "" THEN egCode = 0
	...
	set cEvent = new ClsEvtCont
		...
		evt_using = cEvent.Fevt_using

		if evt_using<>"Y" then response.end
		...
		if stepdiv<4 and etemplate_mo="" then
				etemplate_mo="11"
				mdthememo="1"
				if ekind="5" then etemplate_mo="12"
		end if
	...
	If Trim(ekind)="13" Then
		Response.redirect ("/category/category_itemprd.asp?itemid="&etc_itemid&"&pEtr="&eCode)
		dbget.close()	:	response.End
	End If
...
<body class="default-font body-sub bg-grey"><%'!-- for dev msg : GNB메뉴에서 들어갈때만 body-main클래스가 붙습니다. 그 이외의 페이지에서는 body-sub가 붙습니다. --%>
	...
	<% End If %>
		...
		<div class="section-event event-head" id="edate">
		...
		%>
				<div class="evtContV15">
					...
					If isArray(arrGroup) Then
						For intG = 1 To UBound(arrGroup,2)
							...
							vGroupOption = vGroupOption & "<li><a href=""#group"&arrGroup(0,intG)&""" onclick=""$('#mask').hide();""><div class=""option"">" & db2html(arrGroup(1,intG)) & "</div></a></li>"
							intG = intG+intS
						Next
			...
			<% End If %>	
			<div id="group<%=arrGroup(0,intG)%>" class="items-list">
				<h3 class="groupBar">
		...
		<%	CASE "6" '수작업+상품목록 %>
            ...
            <section class="comment-eventV19">
                ...
                <ul>
                    <li>작성기간 <span class="date"><% if comm_start <> "" then %><%=Replace(comm_start,"-",".")%> ~ <%=Replace(comm_end,"-",".")%><% end if %></span></li>
                    <li>당첨자 발표 <span class="date"><% if epdate <> "" then %><%=Replace(epdate,"-",".")%><% end if %></span></li>
                </ul>
                <div class="btnGroup">
            ...
            <section class="comment-eventV19">
                ...
                <ul>
                    <li>작성기간 <span class="date"><% if eval_start <> "" then %><%=Replace(eval_start,"-",".")%> ~ <%=Replace(eval_end,"-",".")%><% End If %></span></li>
                    <li>당첨자 발표 <span class="date"><% if epdate <> "" then %><%=Replace(epdate,"-",".")%><% End If %></span></li>
                </ul>
                <div class="btnGroup">
		...
		%>
			...
			<div id="groupBar<%=arrGroup(0,intG)%>" class="items-list">
				...
				<%'//상품도 묶인 그룹으로 노출%>
				<% if stepdiv>3 then %><% sbEvtItemView_2015 %><% end if %>
			</div>
		<%
				intG = intG+intS
			Next
			...
			<div class="bxWt1V16a">
				<div class="pdtListV15a">
					<ul>
						...
						%>
						<li>
							<a href="<%=vLink%>" onclick="fnAmplitudeEventMultiPropertiesAction('click_event_recent_list','eventkind|eventcode|eventtype|categoryname|move_eventcode|list_number','<%=ekind%>|<%=eCode%>|<%=evt_type%>|<%=vArrIssue(0,intCLoop)%>|<%=intCLoop+1%>');">
								<div class="pPhoto">
...
<script>
<% 
    dim apitest : apitest = false
    dim apiurl : apiurl = ""
    IF application("Svr_Info") = "Dev" THEN
        apiurl = "http://testfapi.10x10.co.kr/api/web/v1/event/items"
    ElseIf application("Svr_Info")="staging" Then
        apiurl = "https://fapi.10x10.co.kr/api/web/v1/event/items"
    Else
        apiurl = "https://fapi.10x10.co.kr/api/web/v1/event/items"
    End If
    ...
    if apitest then
%>
    var itemparam = "?eventid=<%=eCode%>&eventGroupCode=<%=egCode%>&itemsort=<%=eitemsort%>&listtype=<%=eItemListType%>&themebarcolor=<%=replace(ThemeBarColorCode,"#","")%>&ekind=<%=ekind%>&eventtype=<%=evt_type%>&pNtr=<%=server.URLEncode(searchback_Param)%>";
...
<% else %>
    var itemparam = "?eventid=<%=eCode%>&eGC=<%=egCode%>&itemsort=<%=eitemsort%>&listtype=<%=eItemListType%>&themebarcolor=<%=replace(ThemeBarColorCode,"#","")%>&ekind=<%=ekind%>&eventtype=<%=evt_type%>&pNtr=<%=server.URLEncode(searchback_Param)%>&userid=<%= GetLoginUserID %>";

    var data_itemlist = "/event/lib/act_eventgroupjson.asp"+itemparam;
<% end if %>    
</script>
<script src="/event/lib/eventgroupitemlist2021.js?v=1.02"></script>
</body>
</html>
