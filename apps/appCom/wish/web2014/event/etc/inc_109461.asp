...
<%
	...
	IF application("Svr_Info") = "Dev" THEN
		eCode  = 104311 '// 매일리지 코드
        mobileEventCode = 90405 '// 모바일 이벤트 코드
	Else
		eCode  = 109461 '// 매일리지 코드
        mobileEventCode = 109460 '// 모바일 이벤트 코드
	End If
...
.maeileage .topic {position:relative; background:#ffb5c3;}
...
.dc-group .dc1 {top:31.6%; margin-left:-55%; width:10rem; height:6.23rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2021/109461/m/img_dc_1.png); animation-name:move4; animation-duration:25s; animation-direction:normal;}
.dc-group .dc2 {top:37.7%; margin-left:-18.7%; width:1.2rem; height:1.2rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2021/109461/m/img_dc_2.png); animation-name:move2; animation-duration:10s; animation-direction:alternate-reverse;}
.dc-group .dc3 {top:-1.7%; margin-left:23.7%; width:3rem; height:3rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2021/109461/m/img_dc_3.png);animation-name:move1; animation-duration:13s;}
.dc-group .dc4 {top:78.7%; margin-left:-45.3%; width:2.7rem; height:2.7rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2021/109461/m/img_dc_4.png); animation-name:move1; animation-duration:13s;}
.dc-group .dc5 {top:23%; margin-left:40.7%; width:1.5rem; height:3rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2021/109461/m/img_dc_5.png); animation-name:move3; animation-duration:10s;}
.dc-group .dc6 {top:88.2%; margin-left:31.7%; width:3.4rem; height:3.4rem; background-image:url(//webimage.10x10.co.kr/fixevent/event/2021/109461/m/img_dc_6.png); animation-name:move2; animation-duration:15s;}
@keyframes move1 {
...
.lyr-daily-chk .point em {font-size:4.3rem; vertical-align:middle;}
...
.lyr-daily-chk .tomorrow-chk strong {font-weight:700; font-family:'Roboto'; color:#444;}
...
.daily-board {overflow:hidden; width:100%; background:#ff7691;}
...
.daily-board li .daily-box p {color:#ce0f35; font-size:1.32rem; letter-spacing:-.13rem;}
...
.daily-board li.current:before {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-top:3px solid #4f35d6; z-index:10; transform-origin:100% 0;}
.daily-board li.current:after {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-right:3px solid #4f35d6; z-index:10; transform-origin:0 100%;}
...
.daily-board li.current .current-fr:before {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-bottom:3px solid #4f35d6; z-index:10; transform-origin:0 100%;}
.daily-board li.current .current-fr:after {content:''; display:block; position:absolute; left:-2px; top:-2px; bottom:-2px; right:-2px; border-left:3px solid #4f35d6; z-index:10; transform-origin:0 0;}
...
.daily-board li.previous .daily-box strong {color:#ffc5d1;} /* 지난회차 포인트 */
.daily-board li.done .daily-box p {color:#ffc5d1;}
...
/* 추가 */
.daily-board li.done {background:#ff345e;}
.daily-board li.pass {background:#cb96a1;}
.daily-board li.pass .daily-box p,
.daily-board li.pass .daily-box strong {color:#8e5763;}
.daily-board li.pass .daily-box strong {text-decoration: line-through;}
/* // */
...
function jsMaeilageSubmit(){
    ...
    $.ajax({
        type:"GET",
        <% if mktTest then %>
        url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript109461.asp?mode=add&checkday=<%=request("checkday")%>",
        <% else %>
        url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript109461.asp?mode=add",
        <% end if %>
        dataType: "text",
        async:false,
        cache:true,
...
function jsMaeilagePushSubmit(){
    ...
    $.ajax({
        type:"GET",
        <% if mktTest then %>
        url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript109461.asp?mode=pushadd&checkday=<%=request("checkday")%>",
        <% else %>
        url:"/apps/appCom/wish/web2014/event/etc/doEventSubscript109461.asp?mode=pushadd",
        <% end if %>
        dataType: "text",
        async:false,
        cache:true,
        ...
        error:function(jqXHR, textStatus, errorThrown){
			...
			<div class="mEvt109461 maeileage">
				<div class="topic">
					<% If Left(currenttime, 10) > Left(vEventStartDate, 10) Then %>
                    <h2><img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/tit_maeileage_2.png" alt="매일리지"></h2>
                    <% else %>
                    <h2><img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/tit_maeileage_1.png" alt="매일리지"></h2>
                    <% End If %>
					<div class="attendance">
						...
						<button type="button" id="chkAttendance" class="chk-attendance" onclick="jsMaeilageSubmit();return false;" <% If TodayDateCheck > 0 Then %>style="display:none"<% Else %><% End If %>><img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/btn_attendance.png" alt="출석 체크하기"></button>
						<span class="chk-attendance-ok" id="attendancecheckok" <% If TodayDateCheck > 0 Then %><% Else %>style="display:none"<% End If %>><img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/btn_attendance_ok.png" alt="출석 완료"></span>
					</div>

					<div class="today-noti">
						<span><img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/txt_restart.png" alt="하루라도 놓치면 100p부터 시작"></span>
						<img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/txt_noti.png" alt="소멸됩니다">
					</div>
					<div class="dc-group">
				...
				<div id="lyrDailyChk" class="lyr-daily-chk">
                    <% If Not(Left(currenttime, 10) >= vEventEndDate) Then %>
                        <div>
                            ...
                            <button type="button" class="btn-noti-request" onclick="jsMaeilagePushSubmit();return false;">내일의 매일리지 알림 신청</button>
                            <button type="button" id="btnClose" class="btn-close">닫기</button>
                            <img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/lyr_bg.png" alt="">
                        </div>
					<% Else %>
                        <div class="lyr-finish">
                            ...
                            <strong class="point"><em id="today-mileage-value"></em>p</strong>
                            <button type="button" id="btnClose" class="btn-close">닫기</button>
                            <img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/lyr_bg_finish.png" alt="">
                        </div>
                    <% End If %>
				</div>
				<div id="mask"></div>
				...
				<div class="step-process">
                    <% If IsUserLoginOK() Then %>
                        ...
                        <%
                            ...
                            vQuery = vQuery & "	From db_temp.[dbo].[tbl_maeliageDate109461] m "
                            rsget.CursorLocation = adUseClient
                            rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
                            If Not(rsget.bof Or rsget.eof) Then
                                Do Until rsget.eof
				...
				<div class="push-request">
					<button type="button" onclick="jsMaeilagePushSubmit();return false;">
						<img src="//webimage.10x10.co.kr/fixevent/event/2021/109461/m/btn_push_alarm.png" alt="내일도 매일리지 알림 받기">
					</button>
				</div>
                 <% End If %>
				...
				<div class="noti">
					...
					<ul>
						...
						<li>- 지급된 마일리지는 스페셜 마일리지로, 2021년 2월 28일 23:59까지 사용 가능합니다. 미사용 시 자동으로 소멸됩니다.</li>
					</ul>
				</div>
			</div>
