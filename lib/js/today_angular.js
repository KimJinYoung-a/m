...
appfilter.filter("nl2br2",function($sce){
    return function(text){
        if(text) return $sce.trustAsHtml(text.replace('&lt;br&gt;', '<br/>'));
    }
});
...
app.controller('footsliderB', function($scope,$http) {
    $scope.footsliders = [];
		$http({
			method: 'POST', //방식
			url: '/chtml/main/loader/2016loader/json_main_loader.asp?poscode=2071', /* 통신할 URL */
			headers: {'Content-Type': 'application/json; charset=utf-8'} //헤더
		})
		.success(function(data, status, headers, config) {
			if( data ) {
