...
Class ClsShoppingChance
 ...
 Public FFixEvtCode '// 상단고정 이벤트 코드

	'###fnGetBannerList : 배너리스트  ###(2016-05-02 FSCTypegb 추가 유태욱)
	public Function fnGetBannerList(isApp)
	Dim  strSql, strSqlCnt

		strSqlCnt ="EXEC [db_event].[dbo].[sp_Ten_event_shoppingchance_listCnt_Mobile_new_chuseok2021] '"&FSCType&"', '"&FSCTypegb&"', '"&FSCategory&"','"&FSCateMid&"','"&FEScope&"'" & CHKIIF(FUserID<>"",",'"&FUserID&"'",",''") & "" & CHKIIF(Fis2014renew="o",",'o'","") & ",'"&isApp&"'"
		...
		IF FTotCnt > 0 THEN
			strSql = "EXEC [db_event].[dbo].[sp_Ten_event_shoppingchance_list_Mobile_new_chuseok2021] "&FCPage&", "&FPSize&",'"&FSCType&"', '"&FSCTypegb&"','"&FSCategory&"','"&FSCateMid&"','"&FEScope&"','"&FselOp&"'" & CHKIIF(FUserID<>"",",'"&FUserID&"'",",''") & "" & CHKIIF(Fis2014renew="o",",'o'","") & "," & FFixEvtCode & ",'"&isApp&"'"
	...
	public Function fnGetTopFixBanner
		Dim  strSql
		strSql = "EXEC [db_event].[dbo].[sp_Ten_event_shoppingchance_list_Mobile_chuseok2021_Top_Fix] " & FFixEvtCode & ",'"&FSCategory&"'"

		If FUserID<>"" Then '// My 일경우 실시간
			rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdText
			IF Not (rsget.EOF OR rsget.BOF) THEN
				fnGetTopFixBanner = rsget.GetRows()
			END IF
			rsget.close
		Else
			Dim rsMem : Set rsMem = getDBCacheSQL(dbget,rsget,"SPCSCS",strSql,60*5)
			if (rsMem is Nothing) then Exit Function ''추가

			IF Not (rsMem.EOF OR rsMem.BOF) THEN
				fnGetTopFixBanner = rsMem.GetRows()
			END IF
			rsMem.close
		End If
	End Function
...
Public Function fngetDiaryEvtItemHtml(eventid)
	...
	For intLoop =0 To UBound(arrListItem,2)
		...
		resultHtml = resultHtml &"		</div>"
		If isapp = "1" Then
		resultHtml = resultHtml &"		<a href="""" onclick=""fnAPPpopupAutoUrl('"& link &"');return false;"" class=""prd_link""><span class=""blind"">상품 바로가기</span></a>"
		Else
