...
'	Response.end
	...
	IF Not rsget.Eof Then
		...
		vBody = vBody & "	<span class=""moreBtn""><a href=""/shoppingtoday/shoppingchance_allevent.asp?scT=&disp="&vCateCode&""">이벤트 리스트로 이동</a></span>" & vbCrLf
		vBody = vBody & "	<ul class=""evtList"">" & vbCrLf

		i = 1
		Do Until rsget.Eof
		
		vBody = vBody & "		<li onclick=""javascript:location.href='" & rsget("evt_link") & "'"">" & vbCrLf
...
'	        Set fso = Nothing
			...
			fso.Charset = "utf-8"
			fso.WriteText (vBody)
			fso.SaveToFile server.mappath("/chtml/dispcate/2014main/")+ "\"&"catemain_eventbanner_"&vCateCode&".html", 2
			Set fso = nothing
	    end if
	End If
	rsget.close()
...
<script>alert("2014 Mobile Version OK!!");</script>
<form name="refreshFrm" method="post" action="http://<%=CHKIIF(application("Svr_Info")="Dev","testm","m1")%>.10x10.co.kr/chtml/dispcate/catemain_eventbanner_appmake.asp">
