...
'		    dbget.CommitTrans
	...
	Else
		...
		dbget.Execute vQuery
		dbget.close()
		Response.write "<script language='javascript'>alert('기프티콘에 조회 후 정상적인 쿠폰이 아닙니다. 고객센터로 문의해 주세요.');document.location.href = '/';</script>"
		Response.End
		'################################################################# [XML 통신 결과 실패] #################################################################
	End IF
%>
