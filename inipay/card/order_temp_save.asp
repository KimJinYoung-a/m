...
Dim vQuery, vQuery1, vIdx, vPGoods, sqlStr
	vIdx 	= ""
...
vQuery = vQuery & "gift_kind_option, checkitemcouponlist, packtype, mid, chkKakaoSend, userDevice, dGiftCode, DiNo, ordersheetyn"
...
vQuery = vQuery & "'" & vGift_kind_option & "', '" & vCheckitemcouponlist & "', '" & vPacktype & "', '" & vMid & "', '" & vChkKakaoSend & "', '" & vUserDevice & "', '" & vDGiftCode & "', '" & vDiNo & "', '" & ordersheetyn & "' "
vQuery = vQuery & ")"
...
%>
	...
	<% if (vTn_paymethod="400") then %>
    	...
    	<input type=hidden name="P_MID" value="INIpayTest"><!--INIpayTest-->
    	<% else %>
    	<input type=hidden name="P_MID" value="teenteen10">
    	<% end if %>
	...
	<input type="hidden" name="P_NOTI_URL" value="<%=wwwURL%>/inipay/card/mx_rnoti.asp">
	...
	if (flgDevice="I") then  ''아이폰이고.
	    if (InStr(uAgent,"safari")>0) then ispASyncCase=true  ''사파리인경우만, 다음앱, 네이버앱은 Sync 방식.방식.
	end if
	%>
	
	<% if device="A" then 'APP에서 결제라면 APP내용 추가 %>
	<INPUT TYPE="hidden" name="P_RESERVED" value="below1000=Y&twotrs_isp=Y&block_isp=Y&twotrs_isp_noti=N&cp_yn=N&apprun_check=Y&bank_receipt=N&ismart_use_sign=Y&app_scheme=tenwishapp://">
	<% else %>
	    ...
	    <% else %>
    	    ...
    	    <INPUT TYPE="hidden" name="P_RESERVED" value="below1000=Y&twotrs_isp=Y&block_isp=Y&twotrs_isp_noti=N&cp_yn=N&apprun_check=Y&bank_receipt=N&ismart_use_sign=Y&app_scheme=daumapps://open">
    	    <% else %>
	        <INPUT TYPE="hidden" name="P_RESERVED" value="below1000=Y&twotrs_isp=Y&block_isp=Y&twotrs_isp_noti=N&cp_yn=N&apprun_check=Y&bank_receipt=N&ismart_use_sign=Y&extension_enable=Y">
	        <% end if %>
	    <% end if %>
	<% end if %>
	...
	<% IF application("Svr_Info")="Dev" THEN %>
		...
		************************************************************************************-->
		<input type="hidden" name="P_NEXT_URL" value="http://testm.10x10.co.kr/inipay/card/mx_rnext.asp">
    <% ELSE %>
		<!--************************************************************************************
		안심클릭, 가상계좌, 휴대폰, 문화상품권, 해피머니 사용시 필수 항목 - 인증결과를 해당 url로 post함, 즉 이 URL이 화면상에 보여지게 됨
		************************************************************************************-->
		<input type="hidden" name="P_NEXT_URL" value="https://m.10x10.co.kr/inipay/card/mx_rnext.asp">
	<% END IF %>
