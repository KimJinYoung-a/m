...
Class RealtimeEventCls
	...
	Private sub insertLog()
		...
		elseif evtKind = 2 then
			sqlStr = "INSERT INTO db_log.[dbo].[tbl_caution_event_log] (evt_code , userid , refip ,value1, value2, value3, device)" & vbcrlf
			sqlStr = sqlStr & " VALUES("& evtCode &", '"& userid &"' ,'"&refip&"','"&randomNumber&"', '"&drawResult&"','"& selectedPdt &"', '"&selectedPouch&"')"
			dbget.execute sqlStr
		end if
	...
	'당일 응모 내역 체크
	public function isParticipationDayBase(numOfTry, checkprd)
		dim result, sqlstr, icnt
		result = false

		sqlstr = "select count(*) as icnt FROM [db_event].[dbo].[tbl_event_subscript] with(nolock) WHERE evt_code="& evtCode &" and userid='"&userid&"' and sub_opt3 = 'try' and sub_opt2='" & checkprd & "'"
		rsget.Open sqlstr, dbget, 1
		IF Not rsget.EOF THEN
			icnt = rsget("icnt")
		end if
