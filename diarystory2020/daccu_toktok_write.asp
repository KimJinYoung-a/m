...
<%
    ...
    If not(IsUserLoginOK()) Then
        Response.Write "<script>alert('로그인이 필요한 서비스 입니다.');history.back();</script>"
        Response.End
    End If
...
<link rel="stylesheet" type="text/css" href="/lib/css/diary2020.css?v=1.14" />
    ...
    $('.dctem-thumb').click(function(e){
        ...
        setTimeout(() => {
            if (confirm('이곳에 상품을 등록하시겠습니까?')) {
                fnOpenModal("/diarystory2020/daccu_toktok_myorder.asp?posX="+posX+"&posY="+posY+"&MasterIdxUseItem="+$('#daccuTokMasterIdx').val());
		...
		$('#ajaxform').ajaxSubmit({
			...
			success: function(responseText, statusText){
				...
				if(resultObj.response=="fail") {
                    ...
                    $(".loadingV19").hide();
                    $(".dctem-top .dctem-regist").eq($file_idx).show();                    
				}
				$("#fileupload").val("");
			},
			//ajax error
