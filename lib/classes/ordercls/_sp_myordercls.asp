...
'			end if
	...
	public Sub GetOrderDetail()
	    ...
	    if (FRectOldjumun<>"") then
	        sqlStr = " exec [db_order].[dbo].sp_Ten_OrderDetailList '" & FRectOrderserial & "',0"
        else
            sqlStr = " exec [db_order].[dbo].sp_Ten_OrderDetailList '" & FRectOrderserial & "',1"
        end if
...
Function GetStockOutCancelBeasongPay(orderserial)
	...
	vQuery = vQuery & " 			, sum(case when d.itemid <> 0 then 1 else 0 end) as itemCnt "
	vQuery = vQuery & " 			, sum(case when d.itemid <> 0 and IsNull(m.code, '') = '05' then 1 else 0 end) as stockOutItemCnt "
	vQuery = vQuery & " 		from "
