...
ElseIf eCode = "85159" THEN		'2018-03-30 정태훈 수정 // 웨딩 이벤트의 경우 웨딩 메인으로 보낸다
	...
	dbget.close()	:	response.End
elseif eCode = "2072911" Then
	response.redirect("/apps/appcom/wish/web2014/category/category_itemprd.asp?itemid=2072911")
	dbget.close()	:	response.End
END IF
...
IF egCode = "" THEN egCode = 0
				...
				If arrAddbanner(1,intAi) <> "" Then
					If arrAddbanner(0,intAi) = "1" And ( CStr(Date()) >= CStr(arrAddbanner(4,intAi)) and CStr(Date()) <= CStr(arrAddbanner(5,intAi))) Then
						If InStr(arrAddbanner(3,intAi),"/event/")>0 Then
							tArea = tArea & "<a href='/apps/appcom/wish/web2014"& arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						...
						ElseIf InStr(arrAddbanner(3,intAi),"#replyList")>0 Then
							tArea = tArea & "<a href='"& arrAddbanner(3,intAi) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						ElseIf InStr(arrAddbanner(3,intAi),"#replyPrdList")>0 Then
							tArea = tArea & "<a href='"& arrAddbanner(3,intAi) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						else
							tArea = tArea & "<a href='' "& chkiif(arrAddbanner(3,intAi) <> "","","_") &"onclick=""fnAPPpopupAutoUrl('"& arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode) &"');return false;""><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						End If
					ElseIf arrAddbanner(0,intAi) = "2" And ( CStr(Date()) >= CStr(arrAddbanner(4,intAi)) and CStr(Date()) <= CStr(arrAddbanner(5,intAi))) Then
						If InStr(arrAddbanner(3,intAi),"/event/")>0 Then
							mArea = mArea & "<a href='/apps/appcom/wish/web2014"& arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						ElseIf InStr(arrAddbanner(3,intAi),"#group")>0 Then
						...
						ElseIf InStr(arrAddbanner(3,intAi),"#replyPrdList")>0 Then
							mArea = mArea & "<a href='"& arrAddbanner(3,intAi) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						Else
							mArea = mArea & "<a href='' "& chkiif(arrAddbanner(3,intAi) <> "","","_") &"onclick=""fnAPPpopupAutoUrl('"& arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode) &"');return false;""><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						End If
					ElseIf arrAddbanner(0,intAi) = "3" And ( CStr(Date()) >= CStr(arrAddbanner(4,intAi)) and CStr(Date()) <= CStr(arrAddbanner(5,intAi))) Then
						If InStr(arrAddbanner(3,intAi),"/event/")>0 Then
							bArea = bArea & "<a href='/apps/appcom/wish/web2014"& arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						ElseIf InStr(arrAddbanner(3,intAi),"#group")>0 Then
						...
						ElseIf InStr(arrAddbanner(3,intAi),"#replyPrdList")>0 Then
							bArea = bArea & "<a href='"& arrAddbanner(3,intAi) &"'><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						Else
							bArea = bArea & "<a href='' "& chkiif(arrAddbanner(3,intAi) <> "","","_") &"onclick=""fnAPPpopupAutoUrl('"& arrAddbanner(3,intAi) & gaparamchk(arrAddbanner(3,intAi),"&pEtr="&eCode) &"');return false;""><img src='"& arrAddbanner(1,intAi) &"' alt='"& arrAddbanner(2,intAi) &"'></a>"
						End If
					End If
				End If
			Next
    ...
    if datediff("d",esdate,date)<0 and GetLoginUserLevel<>"7" then
        response.redirect("/apps/appcom/wish/web2014/shoppingtoday/shoppingchance_allevent.asp")
        dbget.close()   :   response.End
    end If

	'// 상품이벤트(kind13)일경우 상품상세로 리다이렉션
	If Trim(ekind)="13" Then
		Response.redirect ("/apps/appcom/wish/web2014/category/category_itemprd.asp?itemid="&etc_itemid&"&pEtr="&eCode & gaparamStr)
		dbget.close()	:	response.End
	End If
...
$(function(){
	...
	<% Else %>
		fnAPPchangPopCaption('기획전');
	<% end if %>
	fnAmplitudeEventMultiPropertiesAction("view_event","eventkind|eventcode|eventtype|categoryname","<%=ekind%>|<%=eCode%>|<%=evt_type%>|<%=vAmplitudeCategoryName%>");
});
...
<body class="default-font body-sub bg-grey"><!-- for dev msg : GNB메뉴에서 들어갈때만 body-main클래스가 붙습니다. 그 이외의 페이지에서는 body-sub가 붙습니다. -->
	...
	<% End If %>
        ...
        <%  dim currentDate : currentDate = date()
            If eCode = "96682" AND currentDate >= Cdate("2019-08-26") And currentDate < Cdate("2019-08-28") Then %>
            <!-- 할인쿠폰 배너 (20190826) -->
		...
		%>
            ...
            $(function(){
                ...
                $(window).scroll(function(){
					menuTop = $("#eventitemlist").offset().top;
        ...
        %>
			...
			<section class="section-event comment-event">
				...
				<p class="gift"><%=nl2br(comm_text)%></p>
				<ul>
					...
					<ul>
								...
								<div class="pdtCont">
									<div class="inner">
										...
										<div class="pTag">
											...
											<% If vArrIssue2(8,intCLoop) Then %><span class="color-blu">GIFT</span><% End If %>
										</div>
									</div>
								</div>
							</a>
						...
						<% For intCLoop = 0 To UBound(vArrIssue,2)
							...
							ELSE
								...
								vLink="/apps/appCom/wish/web2014/event/eventmain.asp?eventid=" & vArrIssue(0,intCLoop)&"&pEtr="&eCode
							END IF
						%>
						<li>
							<a href="" onclick="fnAPPpopupEvent_URL('<%=wwwURL&vLink%>'); return false;">
							<!--<a href="" onclick="fnAmplitudeEventMultiPropertiesAction('click_event_recent_list','eventkind|eventcode|eventtype|categoryname|move_eventcode|list_number','<%=ekind%>|<%=eCode%>|<%=evt_type%>|<%=vAmplitudeCategoryName%>|<%=vArrIssue(0,intCLoop)%>|<%=intCLoop+1%>',function(bool){if(bool) {fnAPPpopupEvent_URL('<%=wwwURL&vLink%>');}});return false;"> 클릭 버그때문에 제거 -->
								<div class="pPhoto">
...
<input type="hidden" id="layerItemId" value="<%=eCode%>">
<input type="hidden" id="layerItemName" value="<%=eOnlyName%>">
...
<%
    ...
    IF application("Svr_Info") = "Dev" THEN
		...
		//apiurl = "http://localhost:8080/api/web/v1/event/items"
...
<script src="/apps/appCom/wish/web2014/event/test/lib/eventgroupitemlist2021_test.js"></script>
</body>
</html>
