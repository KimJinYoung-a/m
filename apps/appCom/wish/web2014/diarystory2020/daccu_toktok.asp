...
<script type="text/javascript" src="/lib/js/jquery.masonry.min.js?v=1.00"></script>
...
function daccuTokWrite() {
    <% If IsUserLoginOK() Then %>
        fnAPPpopupBrowserURL("다꾸톡톡 쓰기","<%=wwwUrl%>/apps/appCom/wish/web2014/diarystory2020/daccu_toktok_write.asp");
        return false;
    <% Else %>
        <% if isApp=1 then %>
            parent.calllogin();
...
function daccutoktokView(midx) {
    fnAPPpopupBrowserURL("다꾸톡톡 보기","<%=wwwUrl%>/apps/appCom/wish/web2014//diarystory2020/daccu_toktok_view.asp?masterIdx="+midx);
    return false;
}

function daccuReload() {
    document.location.reload();
}
                                                ...
                                                <% If oDaccuTalkDetail.FResultCount > 0 Then %>
                                                    <% For j = 0 to oDaccuTalkDetail.FResultCount-1 %>
                                                        <li class="mark" style="top:<%=oDaccuTalkDetail.FItemList(j).FDetailYValue%>%; left:<%=oDaccuTalkDetail.FItemList(j).FDetailXValue%>%;">
                                                            <% If isapp="1" Then %>
                                                                <a href="" onclick="fnAPPpopupProduct('<%=oDaccuTalkDetail.FItemList(j).FDetailItemId%>');return false;">
                                                            <% Else %>
