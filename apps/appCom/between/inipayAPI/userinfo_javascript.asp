...
<script type='text/javascript'>
	...
	function fnGetLastOrderInfo() {
		if(vLstOrdData=="") {
            $.ajax({
        		url: "/apps/appCom/between/inipayAPI/act_lastOrderInfo.asp",
        		cache: false,
        		async: false,
	...
	function PopMobileOrder(paymethod){
		...
		if(popupMobileWindow == undefined)
		{
			if(paymethod == "400")
			{
				popupMobileWindow = window.open('/apps/appCom/between/inipayAPI/mobile/step1.asp','popupMobileWindow','');
		...
		else
		{
			try{
				if(paymethod == "400")
				{
					popupMobileWindow.location.href = "/apps/appCom/between/inipayAPI/mobile/step1.asp";
					popupMobileWindow.focus();
				}
				else
				{
		...
		*/
		...
		if(paymethod == "400"){
			...
			//document.LGD_FRM.action="/inipayAPI/xpay/payreq_crossplatform.asp"
			document.LGD_FRM.action="/apps/appCom/between/inipayAPI/xpay/payreq_crossplatform.asp"
	...
	function CheckPayMethod(comp){
		...
		if(paymethod == "400")
		{
			...
			//mobileiframe.document.location.href = "/apps/appCom/between/inipayAPI/mobile/step1.asp";
	...
	function popisp(){
		var popispwin;
		popispwin = window.open('http://www.10x10.co.kr/apps/appCom/between/inipayAPI/isp/isp.htm','popisp','scrollbars=yes,resizable=yes,width=580,height=600')
	}
	...
	//return;
		...
		if ((paymethod=="100")||(paymethod=="110")){
			...
			//frm.action = "<%=M_SSLUrl%>/inipayAPI/card/order_temp_save.asp";
			frm.action = "<%=M_SSLUrl%>/apps/appCom/between/inipayAPI/card/order_temp_save.asp";
		...
		if (paymethod=="400")
		{
			...
			//frm.action = "<%=M_SSLUrl%>/inipayAPI/xpay/order_temp_save_submit.asp";
			frm.action = "<%=M_SSLUrl%>/apps/appCom/between/inipayAPI/xpay/order_temp_save_submit.asp";
			$(window).unbind('beforeunload');		// 언로드 이벤트 제거
			frm.submit();
		}
		...
		if (paymethod=="7"){
			...
			if (ret){
				...
				frm.target = "";
				//frm.action = "/inipayAPI/AcctResult.asp";
				frm.action = "/apps/appCom/between/inipayAPI/AcctResult.asp";
				$(window).unbind('beforeunload');		// 언로드 이벤트 제거
				frm.submit();
			}
    ...
    function AssignBonusCoupon(bool,icoupontype,icouponvalue){
        ...
        if ((icoupontype=="2")&&(icouponvalue*1>0)){
            $.ajax({
        		url: "/apps/appcom/between/inipayAPI/getPCpndiscount.asp?icoupontype="+icoupontype+"&icouponvalue="+icouponvalue+"&jumunDiv=<%=jumunDiv%>",
        		cache: false,
        		async: false,
        		success: function(message) {
