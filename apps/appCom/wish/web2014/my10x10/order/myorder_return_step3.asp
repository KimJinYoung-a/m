...
if (myorder.FResultCount<1) or (myorderdetail.FResultCount<1) Then
	response.write "<script>alert('잘못된 접속입니다.');fnAPPclosePopup();</script>"
    dbget.close()	:	response.End
...
function RecalcuReturnPrice(frm){
    ...
    if ((refunddeliverypay - refundbeasongpay) > 0) {
   		imsgstr += "<dl class='pPrice'><dt>반품배송비 차감</dt><dd><span>" + plusComma(refunddeliverypay - refundbeasongpay) + "원</span></dd></dl> ";
    } else if ((refunddeliverypay - refundbeasongpay) < 0) {
		imsgstr += "<dl class='pPrice'><dt>배송비 환급</dt><dd><span>" + plusComma(-1 * (refunddeliverypay - refundbeasongpay)) + "원</span></dd></dl> ";
	}

    if (refundCoupon > 0) {
         imsgstr += "<dl class='pPrice'><dt>쿠폰 할인차감</dt><dd><span>" + plusComma(refundCoupon) + "원</span></dd></dl> ";
    }

    if (refundAllat > 0) {
        imsgstr += "<dl class='pPrice'><dt>기타 카드할인차감</dt><dd><span>" + plusComma(refundAllat) + "원</span></dd></dl> ";
    }

    if (refundMile > 0) {
         imsgstr += "<dl class='pPrice'><dt>마일리지 환급</dt><dd><span>" + plusComma(refundMile) + "원</span></dd></dl> ";
    }

    if (refundGiftmoney > 0) {
         imsgstr += "<dl class='pPrice'><dt>기프트카드 환급</dt><dd><span>" + plusComma(refundGiftmoney) + "원</span></dd></dl> ";
    }

    if (refundDeposit > 0) {
         imsgstr += "<dl class='pPrice'><dt>예치금 환급</dt><dd><span>" + plusComma(refundDeposit) + "원</span></dd></dl> ";
    }
...
function regfile(fileno){
    ...
    fnOpenModal("/apps/appCom/wish/web2014/my10x10/order/myorder_return_fileup.asp?filegubun=R1&fileno="+fileno);
}
                    ...
                    <div class="grpCont grp-v18">
                        ...
                        <dl>
                            ...
                            <dd>
                                ...
                                <% if (userid<>"") then %>
                                    <span class="inp"><input type="radio" id="refundWay3" name="returnmethod" value="R910" <% if cardPartialCancelok = "Y" then %>disabled<% end if %> onClick="RecalcuReturnPrice(frmReturn);"><label for="refundWay3"> 예치금 적립</label></span>
                                <% end if %>
                            </dd>
                            <span id="divAccount1">
                                <dl>
            ...
            <% else %>
                <div class="returnNoti2">
                    <ul>
                        ...
                        <li>업체 배송 상품의 반품을 원하실 경우, 브랜드별로 따로 신청하셔야 합니다. <a href="" onclick="fnAPPpopupBrowserURL('1:1 상담','<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/qna/myqnawrite.asp?orderserial=<%=orderserial%>&qadiv=06','right','','sc');return false;">교환 문의하기</a></li>
                    </ul>
                </div>
            <% end if %>
