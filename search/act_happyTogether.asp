	...
	<div class="swiper-wrapper">
		<ul class="simpleList swiper-slide">
			...
			<% if lp>8 then Exit For %>
				<li>
					<a href="" onclick="FnGoProdItem(<%=oHTBCItem.FItemList(lp).FItemId %>,<%=itemid%>,'<%=chkIIF(oHTBCItem.FItemList(lp).FUseETC="R",vMtd(lp),"20")%>','<%=oHTBCItem.FItemList(lp).FUseETC%>','<%=rcpUid%>','<%=chkIIF(oHTBCItem.FItemList(lp).FUseETC="R",vLnk(lp),"")%>','<%=chkIIF(oHTBCItem.FItemList(lp).FUseETC="R","recopick_s","10x10_s")%>'); return false;">
...
<script>
	...
	mySwiper1 = new Swiper('.swiper1',{
		pagination:'.recopick .pagination',
