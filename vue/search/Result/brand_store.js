let store = new Vuex.Store({
    ...
    mutations : {
        ...
        SET_BRANDS_RESULT(state, data) { // 브랜드 리스트 SET
            state.brand_count = data.totalCount;
    ...
    actions : {
        READ_SEARCH_BRANDS(context) { // READ 검색결과
            const parameter = context.getters.parameter;

            if( parameter.keyword == null || parameter.keyword === '' ) {
                history.back();
            } else {
                let search_brand_apiurl = apiurlv2 + '/search/brandSearch'
                ...
                if( parameter.correct_keyword !== '' ) { // 교정무시 키워드
                    search_brand_apiurl += '&ignoreCorrectTypos=true&ignoreCorrectTyposKeyword=' + parameter.correct_keyword;
                }

                $.ajax({
                    type: "GET",
                    url: search_brand_apiurl,
                    ContentType: "json",
                    crossDomain: true,
                    xhrFields: {
                    ...
                    success: function (data) {
                        console.log(data);
                        context.commit('SET_BRANDS_RESULT', data);
