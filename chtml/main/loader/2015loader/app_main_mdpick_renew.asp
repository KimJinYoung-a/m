...
%>
			<% If i Mod 5 = 0 Then %>
			<%
				If gubun=1 Then linkurl = "onclick=""fnAPPpopupMdPick_URL('http://m.10x10.co.kr/apps/appcom/wish/web2014/mdpicklist/index.asp');return false;"" " End if
				If gubun=2 Then linkurl = "onclick=""fnAPPpopupNEW_URL('http://m.10x10.co.kr/apps/appcom/wish/web2014/newitem/newitem.asp');return false;"" " End if
				If gubun=3 Then linkurl = "onclick=""fnAPPpopupSALE_URL('http://m.10x10.co.kr/apps/appcom/wish/web2014/sale/saleitem.asp');return false;"" " End if
				If gubun=4 Then linkurl = "onclick=""fnAPPpopupBest_URL('http://m.10x10.co.kr/apps/appcom/wish/web2014/award/awarditem.asp');return false;"" " End if
			%>
			<div class="curationSetV15a <%=chkiif(gubun = 2 Or gubun = 4,"setLt","setRt")%>">
				<div class="listTitV15a <%=chkiif(gubun = 4," bestCurateV15a","")%>" <%=linkurl%>>
					<div class="titContV15a">
						<div>
							<% If gubun=1 Then %>
							<h2 class="cur01V15a">MD'S PICK</h2><!-- for dev msg : 차례대로 cur01V15a~cur05V15a 클래스명 붙여주세요 -->
						...
						<% If gubun=1 Then %><a href="" onclick="return false;" class="pdtMoreV15a"><span>MD' PICK 더보기</span></a><% End If %>
						<% If gubun=2 Then %><a href="" onclick="return false;" class="pdtMoreV15a"><span>ROOKIE! 더보기</span></a><% End If %>
						<% If gubun=3 Then %><a href="" onclick="return false;" class="pdtMoreV15a"><span>CHANCE 더보기</span></a><% End If %>
						<% If gubun=4 Then %><a href="" onclick="return false;" class="pdtMoreV15a"><span>BEST 더보기</span></a><% End If %>
						<% If gubun=4 Then %>
			...
			<% End If %>
					<li <% If LimitYn="Y" And limitdispyn="Y" And limitno = 0 Then %>class="soldOut"<% End If %><% If LimitYn="Y" And limitno > 0 And limitno <= 10 And limitdispyn = "Y" Then %>class="notiTagV15a"<% End If %>>
						<a href="" onclick="fnAPPpopupProduct_URL('http://m.10x10.co.kr/apps/appCom/wish/web2014/category/category_itemPrd.asp?itemid=<%=itemid%><%=chkiif(gubun=1 Or gubun=2,gaParam1,gaParam2)%>');return false;">
							<div class="pPhoto">
