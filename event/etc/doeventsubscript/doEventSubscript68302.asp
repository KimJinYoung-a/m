...
if mode="order" then
	...
	sqlstr = sqlstr & " where m.regdate >= '2015-12-01'"
...
elseif mode="giftinsert" then
	...
	if totalcnt=0 and subtotalprice=0 then
		Response.Write "07||12월 구매내역이 있어야 응모할수 있습니다."
		dbget.close() : Response.End
	end if
	...
	if isApp="1" then
		sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', '"& left(currenttime,10) &"', "& selval &", '', 'A')" + vbcrlf
	else
		sqlstr = sqlstr & " VALUES("& eCode &", '" & userid & "', '"& left(currenttime,10) &"', "& selval &", '', 'M')" + vbcrlf
	end if
