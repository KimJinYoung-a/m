...
$(function() {
	setTimeout(function(){fnAmplitudeEventAction("view_dramazone","","","");}, 100);
	...
	<div id="content" class="content sbsDrama">
		...
		<div class="sbs-top">
			...
			<ul>
				<li class="<%=chkiif(idx = 0 ,"on","")%>"><a href="/apps/appcom/wish/web2014/dramazone/">ALL</a></li>
				<%
					if isarray(arrList) Then
						For jcnt = 0 To ubound(arrList,2)
							If CInt(idx) = CInt(arrList(0,jcnt)) Then dramatitle = arrList(2,jcnt) End If
							Response.write  "<li class='"& chkiif(CInt(idx) = CInt(arrList(0,jcnt)),"on","") &"'><a href='/apps/appcom/wish/web2014/dramazone/?idx="& arrList(0,jcnt) &"' onclick=fnAmplitudeEventAction('click_dramazonetop','idx','"&arrList(0,jcnt)&"','');>"& arrList(2,jcnt) &"</a></li>"
						Next
					End If
				%>
			</ul>
			...
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<a href="/apps/appCom/wish/web2014/event/eventmain.asp?eventid=91346" onclick="fnAPPpopupEvent('91346'); return false;" class="mApp">
					...
					<div class="swiper-slide">
						<a href="/apps/appCom/wish/web2014/event/eventmain.asp?eventid=90398" onclick="fnAPPpopupEvent('90398'); return false;" class="mApp">
...
<% end if %>
		<div id="dramalist" v-if="show" v-cloak>
			<div class="section" v-for="(item,dindex) in sliced" :item="item" :key="item.id" v-if="sliced" >
				...
				<div class="info">
					...
					<a @click="fnAPPpopupAutoUrl(item.dramaurl);" class="icon-share-wht icon"></a>
				</div>
				<div class="desc" v-html="item.contents"></div>
				<ul>
					<item-list v-for="(sub, index) in item.dramaitem" :sub="sub" :idx="index" :isapp="1" :limit="item.rowLimit"></item-list>
				</ul>
				<p class="more" @click="viewMore(item.dramaitem.length, dindex)" v-if="item.dramaitem.length > 4 && item.viewFlag == 1">상품 더보기...</p>								
				<a :href="`javascript:fnAPPpopupBrowserURL('이벤트','<%=wwwUrl%>/apps/appCom/wish/web2014/event/eventmain.asp?eventid=${item.evtcode}','right','','sc');`" v-if="isDateValid(item.evtsdt, item.evtedt)">
	...
	<script src="/vue/dramazone.js?v=1.04"></script>
	<script>
	$(function(){
		// dramatitle
		<% if idx = 0 then  %>
