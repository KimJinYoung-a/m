...
<%
	...
	IF application("Svr_Info") = "Dev" THEN
		eCode  = 89171 '// 1차 텐텐절 매일리지 코드
        mobileEventCode = 89169 '// 모바일 이벤트 코드
	Else
		eCode  = 89076 '// 1차 텐텐절 매일리지 코드
...
'	currenttime = "2018-10-24 오전 10:03:35" '// 테스트 코드 변경해야됨
    ...
    snpLink        = Server.URLEncode("http://m.10x10.co.kr/event/eventmain.asp?eventid=89076")
    snpPre        = Server.URLEncode("10x10 이벤트")
    snpImg        = Server.URLEncode("http://webimage.10x10.co.kr/eventIMG/2018/89074/etcitemban20180917180929.JPEG")
    If isapp = "1" Then
        appfblink    = "http://m.10x10.co.kr/apps/appCom/wish/web2014/event/eventmain.asp?eventid=89076"
    Else
    ...
    If isapp = "1" Then '앱일경우
        kakaolink_url = "http://m.10x10.co.kr/apps/appcom/wish/web2014/event/eventmain.asp?eventid=89076"
    Else '앱이 아닐경우
...
.bnr-next-play .line {display:block; position:absolute; left:0; top:0; width:100%; height:100%; background:url(http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/bg_circle_line.png) 0 0 no-repeat; background-size:100% 100%;}
...
<script type="text/javascript">
	$(function() {
        ...
        fnAmplitudeEventMultiPropertiesAction('view_maeliage17th','eventcode|chasu','<%=ecode%>|2','');
	});
	function jsMaeilageSubmit(){

        fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_submit','eventcode|chasu','<%=ecode%>|2','');
		...
		$.ajax({
			type:"GET",
			url:"/apps/appCom/wish/web2014/event/17th/doEventSubscript89076.asp?mode=add",
			dataType: "text",
			async:false,
			cache:true,
	...
	function jsMaeilagePushSubmit(){
        ...
        fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_push','eventcode|chasu','<%=ecode%>|2','');
		...
		$.ajax({
			type:"GET",
			url:"/apps/appCom/wish/web2014/event/17th/doEventSubscript89076.asp?mode=pushadd",
			dataType: "text",
			async:false,
			cache:true,
			success : function(Data, textStatus, jqXHR){
	...
	function snschk(snsnum) {
		if(snsnum=="fb"){
			<% if isapp then %>
			fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_sns','eventcode|chasu|snstype','<%=ecode%>|2|fb', function(bool){if(bool) {fnAPPShareSNS('fb','<%=appfblink%>');}});
			return false;
			<% else %>
			popSNSPost('fb','<%=snpTitle%>','<%=snpLink%>','','');
			fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_sns','eventcode|chasu|snstype','<%=ecode%>|2|fb');
			<% end if %>
		}else if(snsnum=="pt"){
			popSNSPost('pt','<%=snpTitle%>','<%=snpLink%>','','<%=snpImg%>');
		}else if(snsnum=="ka"){
			<% if isapp then %>
			fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_sns','eventcode|chasu|snstype','<%=ecode%>|2|ka', function(bool){if(bool) {fnAPPshareKakao('etc','<%=kakaotitle%>\n<%=kakaodescription%>','<%=kakaolink_url%>','<%=kakaolink_url%>','<%="url="&kakaolink_url%>','<%=kakaoimage%>','','','','');}});
			return false;
			<% else %>
			event_sendkakao('<%=kakaotitle%>' ,'<%=kakaodescription%>', '<%=kakaoimage%>' , '<%=kakaolink_url%>' );
			fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_sns','eventcode|chasu|snstype','<%=ecode%>|2|ka');
			<% end if %>
		}
	}
    ...
    <div class="mEvt89076 maeileage">
        ...
        <div class="attendance">
            ...
            <div class="get-mileage">
				<% If left(trim(currenttime),10)>="2018-10-18" and left(trim(currenttime),10) < "2018-10-24" Then %>
					<p><img src="http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/txt_play_date2_note.png" alt="2회차 오픈 10월24일" /></p>
				<% Else %>
					<p><img src="http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/txt_play_date2.png" alt="2회차 10월24일 ~ 31일" /></p>
				<% End If %>
                <div onclick="fnAmplitudeEventMultiPropertiesAction('click_maeliage17th_my10x10','eventcode|chasu','<%=ecode%>|2', function(bool){if(bool) {fnAPPpopupBrowserURL('마일리지 내역','<%=wwwUrl%>/apps/appCom/wish/web2014/offshop/point/mileagelist.asp','right','','sc');}});return false;">
                    <strong><span>현재까지 받은</span><span>마일리지</span></strong>
                    <p><em id="MileagePointCurrent"><%=FormatNumber(TotalMileage, 0)%></em>M</p>
                </div>
				<% If left(trim(currenttime),10)>="2018-10-18" and left(trim(currenttime),10) < "2018-10-24" Then %>
                	<button type="button" id="chkAttendance" class="chk-attendance"><img src="http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/btn_attendance_notyet.png" alt="오늘도 출석 체크!" /></button>
				<% Else %>
            ...
            <p class="bnr-next-play">
                <img src="http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/bnr_circle_1th_v2.png" alt="1회차 완료 10월10일~17일" />
        ...
        <div id="lyrDailyChk" class="lyr-daily-chk">
            ...
            <% Else %>
                <%' 10월31일 2회차 완료일 노출 레이어 %>
                <div class="end-info">
                    <p class="today-chk"><%=CInt(Mid(currenttime, 6, 2))%>월<%=CInt(Mid(currenttime, 9, 2))%>일 출석체크 완료</p>
                    <strong class="point"><span id="today-mileage-value"></span><br />지급 완료</strong>
                    <button type="button" id="btnClose" class="btn-close"><img src="http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/btn_lyr_close2.png" alt="닫기" /></button>
                    <img src="http://webimage.10x10.co.kr/fixevent/event/2018/17th/88939/m/bg_layer_finish.png" alt="" />
                </div>
            <% End If %>
        </div>
        <div id="mask"></div>
        ...
        <div class="step-process">
            <% If IsUserLoginOK() Then %>
				...
				<%
					...
					vQuery = vQuery & "	From db_temp.[dbo].[tbl_maeliageDate17th_Second] m "
					rsget.CursorLocation = adUseClient
					rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
					If Not(rsget.bof Or rsget.eof) Then
						Do Until rsget.eof
        ...
        <% if not(currenttime>="2018-10-31") then %>
			<% If left(trim(currenttime),10)>="2018-10-18" and left(trim(currenttime),10) < "2018-10-24" Then %>
			<% Else %>
