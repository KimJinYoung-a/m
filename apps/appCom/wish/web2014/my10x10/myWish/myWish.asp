...
<%
	Dim vUserId	: vUserId = requestCheckVar(request("ucid"),1200)
...
$(function() {
	...
	$('.wishEditBtn').click(function(e){
		...
		{
			e.preventDefault();
			fnAPPpopupBrowserURL("위시 폴더","<%=wwwUrl%>/apps/appcom/wish/web2014/common/popWishFolder.asp?ErBValue=10&vWUserId=<%=vUserId%>","","wishfolder");
		}
		else
		{
			if ($(this).attr("id")=="md")
			...
			else if ($(this).attr("id")=="mo")
			{
				...
				if (confirm("선택하신 위시를 이동하시겠습니까?")==true){
					fnAPPpopupBrowserURL("위시 폴더","<%=wwwUrl%>/apps/appcom/wish/web2014/common/popWishFolder.asp?ErBValue=11&mfuid=<%=vUserId%>&mfcpg=<%=CurrPage%>&mffIdx=<%=vtFIdx%>&mfvlg=<%=viewListGubun%>&folderAction=Change&bagarray="+itemSplitData+"","","wishfolder");
				}
				else {
					return;
				}
...
function goModifyCompleteMove(fi)
{
	top.location.href='/apps/appcom/wish/web2014/my10x10/mywish/mywish.asp?vlg=wlist&fidx='+fi;
}
</script>
</head>
<body>
				...
				<div class="myWish">
					<div class="myBox">
						...
						<div class="pvtImgWrap">
							...
							<% If vUserId <> getEncLoginUserID() Then %>
								<p class="circleBtn mateView">
									...
									<a href="" onclick="fnAPPpopupBrowserURL('메이트','<%=wwwUrl%>/apps/appcom/wish/web2014/my10x10/myWish/MateSelectList.asp?uid=<%=Server.Urlencode(tenEnc(vUserId))%>','','matefriend');return false;">
									<% else %>
								...
								<p class="circleBtn followView <% If Trim(FollowerChk)="1" Then %>following<% End If %>">
									<a href="" onclick="FollowInsertDeleteAjax('<%=getEncLoginUserID()%>','<%=vUserId%>');">
							...
							<ul class="myBadgeView">
								<a href="" onclick="fnAPPpopupBrowserURL('뱃지','<%=wwwUrl%>/apps/appcom/wish/web2014/my10x10/myWish/popBadge.asp?uid=<%=Server.UrlEncode(tenEnc(vUserId))%>','','badge'); return false;">
								<%
									for i = 0 to UBound(arrBadgeList,2)
										If i>3 Then 
											Exit For
						...
						<ul class="myWishFollow">
							...
							<li>
								<a href="" onclick="fnAPPpopupBrowserURL('팔로워','<%=wwwUrl%>/apps/appcom/wish/web2014/my10x10/myWish/popFollower.asp?uid=<%=Server.UrlEncode(tenEnc(vUserId))%>','','follower'); return false;">
									<p id="followCnt"><%=FormatNumber(FollowerCnt, 0)%></p> <span>팔로워</span>
								</a>
							</li>
							<li>
								<a href="" onclick="fnAPPpopupBrowserURL('팔로잉','<%=wwwUrl%>/apps/appcom/wish/web2014/my10x10/myWish/popFollowing.asp?uid=<%=Server.UrlEncode(tenEnc(vUserId))%>','','following'); return false;">
					...
					<% If Now() > #12/14/2015 00:00:00# AND Now() < #12/21/2015 00:00:00# Then %>
						<p><a href="/apps/appCom/wish/web2014/event/eventmain.asp?eventid=67490"><img src="http://webimage.10x10.co.kr/eventIMG/2015/67490/m/bnr_wish.png" alt="" /></a></p>
					<% End if %>

					<% If Now() >= #02/10/2016 10:00:00# AND Now() < #02/15/2016 00:00:00# Then %>
						<p class="tMar15"><a href="/apps/appCom/wish/web2014/event/eventmain.asp?eventid=68889" target="_top"><img src="http://webimage.10x10.co.kr/eventIMG/2016/68889/m/bnr.png" alt="새해 위시 이벤트 오늘은 WISH를 털날" /></a></p>
					<% End if %>
					
					<% If Trim(viewListGubun) = "" Or Trim(viewListGubun)="wlist" Then %>
						<!-- #include virtual="/apps/appcom/wish/web2014/my10x10/mywish/inc_pWishList.asp" -->
					<% ElseIf Trim(viewListGubun)="flist" Then %>
						<!-- #include virtual="/apps/appcom/wish/web2014/my10x10/mywish/inc_folderList.asp" -->
					<% Else %>
						<!-- #include virtual="/apps/appcom/wish/web2014/my10x10/mywish/inc_pWishList.asp" -->
					<% End If %>
				</div>
