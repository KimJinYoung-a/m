...
<div class="heightGrid">
	<div class="container">
		...
		<div class="content picDiary bgGry" id="contentArea">
			<div class="playCont">
				<span class="type" onclick="fnAPPopenerJsCallClose('jsChangeType(\'<%= playcode %>\')'); return false;">그림일기</span>
				<p class="tit"><strong><%= oplaydetail.FOneItem.ftitle %></strong><span><%=oplaydetail.FOneItem.Fviewno%></span></p>
				<p id="mywish<%=oPictureDiary.FOneItem.fpdidx%>" class="circleBox wishView <%=chkiif(oPictureDiary.FOneItem.Fchkfav > 0 ,"wishActive","")%>" <% If getloginuserid <> "" Then %>onclick="TnAddPlaymywish('<%=playcode%>','<%=oPictureDiary.FOneItem.fpdidx%>','<%= oplaydetail.FOneItem.Fviewno %>');"<% Else %>onclick="jsChklogin();"<% End If %>><span>찜하기</span></p>
			</div>
			<div class="myDiary">
