...
If fileCont<>"" Then
	...
	For each tplNodes in cTmpl
		...
		If CDate(CtrlDate) >= CDate(startdate) AND CDate(CtrlDate) <= CDate(enddate) Then
			...
			End If
%>
		<li><a href="" onclick="fnAPPpopupAutoUrl('<%= link %><%=gaParam%>');return false;"><img src="<%= image %>" alt="<%= altname %>" /><!--<p><span><%=texttitle1%></span><span><%=texttitle2%></span><% If saleflag <> "" Then %> <em><%=saletext%></em><% End If %></p>--></a></li>
<%
			i = i + 1
