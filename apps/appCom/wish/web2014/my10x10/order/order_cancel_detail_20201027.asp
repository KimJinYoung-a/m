...
<script type="text/javascript">
	function GotoCSCenter() {
    	location.href = "/apps/appCom/wish/web2014/my10x10/qna/myqnalist.asp";
    }
		...
		<div class="myOrderView myOrderCancel">
			<div class="myTenNoti">
				...
				<ul class="listRound ftMidSm2 lh12 tMar05 c999">
					...
					%>
					<li>상품 일부만 취소하고자 하시는 경우 고객행복센터로 문의 바랍니다. <a href="#" onclick="fnAPPpopupBrowserURL('<%=CHKIIF(IsVIPUser()=True,"VIP ","")%>1:1 상담','<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/qna/myqnawrite.asp?qadiv=04&orderserial=<%=orderserial%>'); return false;" class="cRd1" style="white-space:nowrap;">1:1상담 바로가기 &gt;</a></li>
				</ul>
				<% end if %>
			</div>

			<div class="orderSummary box3">
				<p class=""><%=formatdate(myorder.FOneItem.Fregdate,"0000.00.00")%><em class="cBk1">l</em>주문번호 <b><%=orderserial%></b></p>
			...
			<ul class="commonTabV16a tMar1r <%=chkIIF(myorder.FOneItem.IsPayed,"grid4","grid3")%>">
			...
			<div class="inner10">
			<!-- 주문상품 -->
			<div id="myorderTab1" class="lyOrdTab">
				<div class="cartGroup">
					<div class="groupCont">
						<ul>
						<% for i=0 to myorderdetail.FResultCount-1
							...
							if (vIsShowItem = True) and (mode = "stockoutcancel" and myorderdetail.FItemList(i).Fmibeasoldoutyn <> "Y") then
								vIsShowItem = False
							end if
							If vIsShowItem Then
								...
								<div class="box3">
									...
									<div class="pdtInfo">
										<dl class="pPrice">
											<dt>판매가</dt>
											<dd>
												<span><%= FormatNumber(myorderdetail.FItemList(i).getItemcostCouponNotApplied,0) %><%= CHKIIF(myorderdetail.FItemList(i).IsMileShopSangpum,"Pt","원") %></span>
										...
										<dl class="pPrice">
											...
											<dd>
												<span><%= FormatNumber(myorderdetail.FItemList(i).FItemCost*vCurrItemNo,0) %><%= CHKIIF(myorderdetail.FItemList(i).IsMileShopSangpum,"Pt","원") %></span>
												<% if (myorderdetail.FItemList(i).IsSaleBonusCouponAssignedItem) then %>
										...
										<dl class="pPrice">
											...
											%>
												...
												<span class="button btS1 btRed  cWh1 btnCancel"><a href="#" onclick="fnAPPpopupBrowserURL('<%=CHKIIF(IsVIPUser()=True,"VIP ","")%>1:1 상담','<%=wwwUrl%>/apps/appCom/wish/web2014/my10x10/qna/myqnawrite.asp?qadiv=04&orderserial=<%=orderserial%>&itemid=<%= myorderdetail.FItemList(i).FItemid %>&orderdetailidx=<%= myorderdetail.FItemList(i).fidx %>'); return false;" class="cRd1" style="white-space:nowrap;">1:1 상담</a></span>
											<% else %>
			...
			<div id="myorderTab2" class="lyOrdTab" style="display:none;">
				<!-- 구매자 -->
			...
			<div id="myorderTab3" class="lyOrdTab" style="display:none;">
				<!-- 결제 -->
				<div class="groupTotal box3 bgWht tMar15">
					<dl class="pPrice">
						<dt>결제방법</dt>
			...
			<div class="btnWrap">
				<p class="tMar30"><span class="button btB1 btRed cWh1 w100p"><a href="" onclick="AllCancelProc(document.frmCancel);return false;"><%= cancelbuttontitle %></a></span></p>
			</div>
			<% end if %>
			</div>
		</div>
