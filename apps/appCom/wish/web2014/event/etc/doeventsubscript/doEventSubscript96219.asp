...
<%
	...
	'// 이벤트시작시간
	vEventStartDate = "2019-07-24"

    '// 마일리지를 받거나 마일리지 받기 취소는 8월 1일까지 유효함
    If Trim(alramAction)="action" Or Trim(alramAction)="nopush" Then
        '// 이벤트종료시간
        vEventEndDate = "2019-08-01"
    Else
        '// 이벤트종료시간
        vEventEndDate = "2019-07-31"
    End If
    ...
    Select Case Trim(alramAction)
        ...
        Case "action"
            ...
            If sub_opt2 <> "1" Then
                Response.write "ERR|마일리지 푸시 알림을 신청한 사람만 받을 수 있습니다."
                dbget.close()	:	response.End
            Else
                '// 오늘 푸시가 발송되었는지 확인한다.
                ...
                '// 아예 푸시 발송 리스트에 없다면
                Else
                    Response.write "ERR|푸시 알림은 매일 아침 10시에 발송됩니다.>?n해당일의 푸시 알림을 받으신 후 다시 확인해주세요."
                    dbget.close()	:	response.End
                End If
