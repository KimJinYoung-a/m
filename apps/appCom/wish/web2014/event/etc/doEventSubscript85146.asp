...
'	currenttime = "2018-04-12 오전 10:03:35"
	...
	Select Case Trim(mode)
		...
		Case "pushadd"
			...
			'// 다음날 푸쉬 신청을 했는지 확인한다.
			vQuery = "SELECT count(*) FROM db_temp.[dbo].[tbl_maeliagePush] WITH (NOLOCK) WHERE userid='"&userid&"' And convert(varchar(10), SendDate, 120) = '"&Left(pushDate, 10)&"' "
			rsget.CursorLocation = adUseClient
			rsget.Open vQuery, dbget, adOpenForwardOnly, adLockReadOnly
			IF Not rsget.Eof Then
				If rsget(0) > 0 Then
			...
			vQuery = " INSERT INTO db_temp.[dbo].[tbl_maeliagePush](userid, SendDate, Sendstatus, RegDate) VALUES('" & userid & "', '"&Left(pushDate, 10)&"', 'N', getdate())"
			dbget.Execute vQuery
