...
<body class="default-font body-popup">
    <!-- 포토후기리스트 -->
    <div class="content pop-photo-album">
        <ul>
        <%
            '// 참고
            '0-idx
        ...
        %>
            <% for imgI = 0 to ubound(arrList,2) %>        
                <% If arrList(15,imgI) <> "" Then %>
                    <li><a href="" onclick="fnAPPpopupBrowserURL('포토후기','<%=wwwUrl%>/apps/appCom/wish/web2014/category/pop_ItemEvalPhotoDetail.asp?itemid=<%=itemid%>&idx=<%=arrList(0,imgI)%>'); return false;"><span class="thumbnail"><i style="background-image:url(<%=getStonReSizeImg("http://imgstatic.10x10.co.kr/goodsimage/"&GetImageSubFolderByItemid(itemid)&"/"&arrList(15,imgI),200,"",95)%>);"></i></span></a></li>
                <% End If %>
            <% next %>
        </ul>
    </div>
