...
Class ClsEvtCont
	...
	public Function fnEventISSUEList
		...
		rsget.Open strSql, dbget, adOpenForwardOnly, adLockReadOnly, adCmdStoredProc
		IF Not (rsget.EOF OR rsget.BOF) THEN
			fnEventISSUEList = rsget.GetRows()
		END IF
		rsget.close
	End Function
			...
			<% If (intI mod 2) = 0 Then %>
				<div class="evtPdtListWrap">
					<div class="pdtListWrap">
						<ul class="pdtList">
			<% End If %>
							<li onclick="location.href='/category/category_itemPrd.asp?itemid=<% = cEventItem.FCategoryPrdList(intI).Fitemid %>&flag=e';">
								<div class="pPhoto">
...
%>
			<% If (intI mod 2) = 0 Then %>
					...
					<div class="pdtListWrap">
						<ul class="pdtList">
			<% End If %>
							<li onclick="fnAPPpopupProduct('<% = cEventItem.FCategoryPrdList(intI).Fitemid %>');">
								<div class="pPhoto">
									<% IF cEventItem.FCategoryPrdList(intI).IsSoldOut Then %><p><span><em>품절</em></span></p><% End if %>
									<% if (eItemListType = "1") or (eItemListType = "2") then %>
