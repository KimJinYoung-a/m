								...
								<li>
									...
									<div class="vote">
										...
										<% Else %>
											...
											<% else %>
												<button type="button" id="mybtn<%=rsget("evtcom_idx")%>" style="outline:none;" onclick="goVoteChk('<%=rsget("evtcom_idx")%>', '<%=ReceiveDate%>','1');return false;" <% If rsget("userchk") >= 1 Then %> class="on" <% End If %>>투표하기</button>
											<% End If %>
										<% End If %>
