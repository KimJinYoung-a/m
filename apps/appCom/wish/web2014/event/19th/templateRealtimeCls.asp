...
Class RealtimeEventCls
	...
	public sub execDrawEvent()
		...
		'스태프인지 확인
		if isStaff() then
			'날짜가 지정되어 있으면 테스트 이므로 스태프 체크 해제
			If FRectCurrentDate = "" Then
				execResult(0)
				totalResult = B02
				exit sub
			End If
	...
	private function chkItemDate()
        ...
        If Trim(FRectCurrentDate) <> "" Then
		    sqlStr = sqlStr & "    and open_date < '"&FRectCurrentDate&"' " &vbcrlf
        Else
	...
	Private function isWinnerExistsInTime()
        ...
        If Trim(FRectCurrentDate) <> "" Then
            sqlStr = sqlStr &" and regdate > dateadd(day, datediff(day, 0, '"&FRectCurrentDate&"'), 0) + '" & startWinTimeOption & "'" & vbcrlf
            sqlStr = sqlStr &" and regdate < dateadd(day, datediff(day, 0, '"&FRectCurrentDate&"'), 0) + '" & EndWinTimeOption & "'" & vbcrlf
        Else
	...
	public function isParticipationDayBase(numOfTry)
        ...
        If Trim(FRectCurrentDate) <> "" Then
            sqlstr = sqlstr & " and datediff(day,regdate,'"&FRectCurrentDate&"') = 0 "
        Else
            sqlstr = sqlstr & " and datediff(day,regdate,getdate()) = 0 "            
        End If
		rsget.Open sqlstr, dbget, 1
		IF Not rsget.EOF THEN
			icnt = rsget("icnt")
	...
	public function getEventObjList(evtCode)
		...
		sqlStr = sqlStr & " , option4 "	'이벤트 상품 수량
		sqlStr = sqlStr & " , option5		 "	'이벤트 상품 정렬번호
        If Trim(FRectCurrentDate) <> "" Then
		    sqlStr = sqlStr & " , case when convert(varchar, open_date, 23) <= convert(varchar, '"&FRectCurrentDate&"', 23) then 1 else 0 end isOpen"	'7
		    sqlStr = sqlStr & " , case when convert(varchar, open_date, 23) = convert(varchar, '"&FRectCurrentDate&"', 23) then 1 else 0 end todayOpen"	'8
        Else
        ...
        End If
        sqlStr = sqlStr & " , option6		 "	'상품실가격
        sqlStr = sqlStr & " , option7		 "	'이벤트 할인가격
	...
	public function getEventObjOLDList(evtCode)
        ...
        If Trim(FRectCurrentDate) <> "" Then
			sqlStr = sqlStr & " , case when convert(varchar, o.open_date, 23) <= convert(varchar, '"&FRectCurrentDate&"', 23) then 1 else 0 end isOpen"	'7
			sqlStr = sqlStr & " , case when convert(varchar, o.open_date, 23) = convert(varchar, '"&FRectCurrentDate&"', 23) then 1 else 0 end todayOpen"	'8
		Else
			sqlStr = sqlStr & " , case when convert(varchar, o.open_date, 23) <= convert(varchar, getdate(), 23) then 1 else 0 end isOpen"	'7
	...
	Private sub execResult(isWin)
        ...
        If Trim(FRectCurrentDate) <> "" Then
            sqlStr = sqlStr & " VALUES("& evtCode &", '"& userid &"', '"& isWin &"', '"& selectedPdt &"','try','"& device &"','"&FRectCurrentDate&"')"
        Else
		    sqlStr = sqlStr & " VALUES("& evtCode &", '"& userid &"', '"& isWin &"', '"& selectedPdt &"','try','"& device &"', getdate())"
        End If
		dbget.execute sqlStr
	...
	public function snsShare()
		...
		if not isSnsShared() Then
			sqlStr = "INSERT INTO [db_event].[dbo].[tbl_event_subscript] (evt_code, userid, sub_opt1, sub_opt3, device, regdate)" & vbcrlf
            If Trim(FRectCurrentDate) <> "" Then
			    sqlStr = sqlStr & " VALUES("& evtCode &", '"& userid &"', '"& snsType &"','share','"&device&"', '"&FRectCurrentDate&"')"
            Else
			    sqlStr = sqlStr & " VALUES("& evtCode &", '"& userid &"', '"& snsType &"','share','"&device&"', getdate())"
            End If
			dbget.execute sqlStr
		end if
	end function
...
'=============================================================데이터===
	...
	public function getOldEventList()
		...
		sqlStr = " SELECT top 4 e.evt_code, e.listimg, e.itemea, o.option1, o.option6, o.option7"
		sqlStr = sqlStr & " FROM db_event.dbo.tbl_realtime_event_list as e with(nolock)"
		sqlStr = sqlStr & " left join db_event.dbo.tbl_realtime_event_obj as o with(nolock) on e.evt_code=o.evt_code"
