...
class CEvaluateSearcherItem
	...
	public Function getImageUrl1()
		If Flinkimg1 <> "" Then 
			getImageUrl1 = staticImgUpUrl&"/goodsimage/\" + GetImageSubFolderByItemid(Fitemid) + "\" + Flinkimg1
		End If 
	end function 
	
	public Function getImageUrl2()
		If Flinkimg3 <> "" Then 
			getImageUrl2 =	staticImgUpUrl&"/goodsimage/\" + GetImageSubFolderByItemid(Fitemid) + "\" + Flinkimg2
		End If 
	end function 
	
	public Function getImageUrl3()
		If Flinkimg3 <> "" Then 
			getImageUrl3 =	staticImgUpUrl&"/goodsimage/\" + GetImageSubFolderByItemid(Fitemid) + "\" + Flinkimg3
		End If 
	end function 
	'new ver2017
...
Class CEvaluateSearcher
	...
	'' Top N개 sql2012 신규
	public Sub GetTopEventGoodUsingList_new()
		dim sqlStr,i
	...
	Public Sub NotEvalutedItemList()
		...
		sqlStr = "" &_
				...
				"	and d.currstate>=7 " &_
				" JOIN [db_item].[dbo].tbl_Item i  "&_
				" 	on d.itemid=i.itemid and i.isusing = 'Y'  "&_
...
'e.idx is null
		sqlStr = " " &_
				...
				" , m.orderserial ,m.regdate, i.basicimage, i.evalcnt, IsNull(c.favcount, 0) as favcount " &_
				" FROM [db_order].[dbo].tbl_Order_Master m  "&_
				" JOIN [db_order].[dbo].tbl_Order_Detail d  "&_
				" 	on m.OrderSerial= d.OrderSerial and m.sitename='10x10' and m.ipkumdiv>=7  "&_
				" 	and m.cancelyn='N' and m.jumundiv<>9 and d.cancelyn<>'Y'  "&_
	...
	Public Sub getNotEvaluatedItem()
		...
		sqlStr = " " &_
			...
			" ,i.itemid,i.sellcash,i.itemname,i.brandname , i.listimage , i.listImage120, i.basicimage " &_
			" ,o.optionname " &_
			...
			IF not rsget.eof THEN 
					...
					FEvalItem.FBasicImage 		= "http://webimage.10x10.co.kr/image/basic/" + GetImageSubFolderByItemid(rsget("itemid")) + "/" + rsget("basicimage")
...
'
'		sqlStr = "SELECT " & _
'				 " 	count(*) AS totalcnt, isNull(SUM(CASE WHEN i.evalcnt > 0 THEN " & FRectMileage & " ELSE " & (FRectMileage*2) & " END),0) AS totalgetmile, " & _
'				 " 	isNull(sum(Case When i.evalcnt>0 then 1 else 0 end),0) AS cnt, isNull(sum(Case When i.evalcnt=0 then 1 else 0 end),0) AS firstcnt " & _
'				 " FROM [db_order].[dbo].[tbl_Order_Master] AS m " & _
'				 "	INNER JOIN [db_order].[dbo].[tbl_Order_Detail] AS d on m.userid = '" & FRectUserID & "' and m.OrderSerial= d.OrderSerial " & _
'				 "			and m.sitename = '10x10' and m.ipkumdiv>=7 and m.cancelyn = 'N' and m.jumundiv <> 9 and m.jumundiv <> 6 " & _
'				 "			and d.cancelyn <> 'Y' and d.itemid not in (0,100) and IsNull(m.userDisplayYn, 'Y') = 'Y' " & _
'				 "			and d.currstate>=7 " &_
'				 "	INNER JOIN [db_item].[dbo].[tbl_Item] AS i on d.itemid = i.itemid and i.isusing = 'Y' " & _
'				 "	LEFT JOIN [db_board].[dbo].[tbl_Item_Evaluate] AS e on e.UserID = '" & FRectUserID & "' and m.OrderSerial = e.OrderSerial " & _
'				 "			and d.Itemid = e.itemid and d.ItemOption = e.ItemOption " & _
'				 " WHERE e.idx is null"
'			rsget.open sqlStr ,dbget,1
''response.write sqlStr
'			IF not rsget.eof THEN
...
'				FItemList(i).Fitemid    = rsget("itemid")
	...
	Public Sub NotEvalutedItemListNew()
		...
		addSql = ""
		addSql = addSql + "FROM "
		addSql = addSql + "	[db_order].[dbo].tbl_Order_Master m "
		addSql = addSql + "	JOIN [db_order].[dbo].tbl_Order_Detail d "
		addSql = addSql + "	on "
		addSql = addSql + "		1 = 1 "
		addSql = addSql + "		and m.userid = '" + CStr(FRectUserID) + "' "
		addSql = addSql + "		and m.OrderSerial= d.OrderSerial "
		addSql = addSql + "		and m.sitename = '10x10' "
		addSql = addSql + "		and m.ipkumdiv>=7 "
		addSql = addSql + "		and m.cancelyn = 'N' "
		addSql = addSql + "		and m.jumundiv <> 9 "
		addSql = addSql + "		and m.jumundiv <> 6 "
		addSql = addSql + "		and d.cancelyn <> 'Y' "
		addSql = addSql + "		and d.itemid <> 0 "
		addSql = addSql + "		and d.itemid <> 100 "
		addSql = addSql + "		and d.currstate>=7 "
		addSql = addSql + "		and IsNull(m.userDisplayYn, 'Y') = 'Y' "
		addSql = addSql + "	JOIN [db_item].[dbo].tbl_Item i "
		addSql = addSql + "	on "
		addSql = addSql + "		d.itemid = i.itemid and i.isusing = 'Y' "

		if (FCateCode <> "") then
		    addSql = addSql + " 	and i.dispcate1='" + CStr(FCateCode) + "' "    
'			addSql = addSql + " 	JOIN db_item.dbo.tbl_display_cate_item ct "
'			addSql = addSql + " 	on "
'			addSql = addSql + " 		1 = 1 "
'			addSql = addSql + " 		and Left(catecode, 3) = '" + CStr(FCateCode) + "' "
'			addSql = addSql + " 		and i.itemid = ct.itemid "
		end if

		addSql = addSql + "	LEFT JOIN [db_item].[dbo].tbl_item_Contents c "
		addSql = addSql + "	on "
		addSql = addSql + "		d.itemid = c.itemid "
		addSql = addSql + "	LEFT JOIN db_board.[dbo].tbl_Item_Evaluate e "
		addSql = addSql + "	on "
		addSql = addSql + "		1 = 1 "
		addSql = addSql + "		and e.UserID = '" + CStr(FRectUserID) + "' "
		addSql = addSql + "		and m.OrderSerial = e.OrderSerial "
		addSql = addSql + "		and d.Itemid = e.itemid "
		addSql = addSql + "		and d.ItemOption = e.ItemOption "
		addSql = addSql + "WHERE "
		addSql = addSql + "	e.idx is null "
		...
		sqlStr = sqlStr + " i.itemid , i.sellcash , i.itemname , i.brandname , (Case When isNull(i.frontMakerid,'')='' then i.makerid else i.frontMakerid end) as makerid "
		sqlStr = sqlStr + " , i.listimage120, i.listimage, i.icon2image, d.oitemdiv as itemdiv, i.evalcnt, IsNull(c.favcount, 0) as favcount "
		sqlStr = sqlStr + " , d.itemoption , d.itemoptionname as optionname "
		sqlStr = sqlStr + " , m.orderserial ,m.regdate, i.itemscore, i.basicimage "
	...
	Public Sub EvalutedItemListNew()
		...
		addSql = ""
		addSql = addSql + " FROM "
		addSql = addSql + " 	db_board.[dbo].tbl_Item_Evaluate e "
		addSql = addSql + " 	JOIN db_item.[dbo].tbl_item i "
		addSql = addSql + " 	on "
		addSql = addSql + " 		e.itemid=i.itemid "
		addSql = addSql + " 	LEFT JOIN db_item.[dbo].tbl_item_option o "
		addSql = addSql + " 	on "
		addSql = addSql + " 		1 = 1 "
		addSql = addSql + " 		and e.itemid=o.itemid "
		addSql = addSql + " 		and e.itemoption=o.itemoption "

		if (FCateCode <> "") then
			addSql = addSql + " 	JOIN db_item.dbo.tbl_display_cate_item c "
			addSql = addSql + " 	on "
			addSql = addSql + " 		1 = 1 "
			addSql = addSql + " 		and Left(catecode, 3) = '" + CStr(FCateCode) + "' "
			addSql = addSql + " 		and i.itemid = c.itemid "
		end if

		addSql = addSql + " left join db_board.dbo.tbl_Item_Evaluate_exclude ee"
		addSql = addSql + " 	on i.itemid=ee.itemid"
		addSql = addSql + " WHERE "
		addSql = addSql + " 	1 = 1 "
		addSql = addSql + " 	and e.userid = '" + CStr(FRectUserID) + "' "
		addSql = addSql + " 	and e.isusing = 'Y' "
		addSql = addSql + " 	and e.itemid <> 100 "

		'// ====================================================================
		sqlStr = " select Count(distinct e.idx) as TotalCnt , Ceiling(cast(count(distinct e.idx) as Float)/" & Cstr(FPageSize) & ") as TotalPage "
		...
		'// ====================================================================
		sqlStr = " SELECT TOP " + CStr(FPageSize*(FCurrPage)) + " "
		sqlStr = sqlStr + " e.idx , e.gubun , e.contents ,  e.regdate , e.orderserial,e.itemoption "
		sqlStr = sqlStr + " , e.file1 , e.file2 , e.file3 ,e.file4 , e.file5 "
		sqlStr = sqlStr + " , isnull(e.TotalPoint,0) as TotalPoint"
		sqlStr = sqlStr + " , isnull(e.Point_function,0) as Point_function "
		sqlStr = sqlStr + " , isnull(e.Point_Design,0) as Point_Design "
		sqlStr = sqlStr + " , isnull(e.Point_Price,0) as Point_Price "
		sqlStr = sqlStr + " , isnull(e.Point_satisfy,0) as Point_satisfy "
		sqlStr = sqlStr + " , i.itemid , i.itemname , i.sellcash , (Case When isNull(i.frontMakerid,'')='' then i.makerid else i.frontMakerid end) as makerid , i.brandname , i.listimage120 , i.listimage , i.itemdiv "
		sqlStr = sqlStr + " , o.optionname, (case when isnull(ee.itemid,'') <> '' then 'Y' else 'N' end) as Eval_excludeyn, i.basicimage "
