    ...
    template :'\
                <li v-bind:class="soldOut">\
					<a @click="itemUrl(itemId)">\
						...
						<div class="desc">\
							<div class="price">{{totalPrice}}<span>{{saleInfo}}</span></div>\
							<div class="name">{{itemName}}</div>\
							<div class="brand">{{brandName}}</div>\
							<div class="review">\
								<span v-if="totalPoint > 79" class="rating"><i v-bind:style="{width : totalPoint +\'%\'}"></i></span>\
								<span v-if="evalCount > 4" class="counting">{{evaluateCount}}</span>\
							</div>\
						</div>\
					</a>\
				</li>\
