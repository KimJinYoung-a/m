...
<%
	Dim vItemID, lp, i, vWishArr, vSearchItemID, vUserID, vRect
	...
	For i=0 To oDoc.FResultCount-1	
		...
		linkUrl = "/category/category_itemPrd.asp?itemid="& oDoc.FItemList(i).FItemID 
		...
		'### 상품코드 ###
		if adultChkFlag then
			vItemBtn(i) = "<td><a href=""javascript:confirmAdultAuth('"&linkUrl&"')"" class=""btn-get"">구매하기</a></td>"
		else
			vItemBtn(i) = "<td><a href=""/category/category_itemPrd.asp?itemid=" & oDoc.FItemList(i).FItemID & "&pRtr=" & vRect & "&rc=cpr_"&(i+1)&"_"&oDoc.FResultCount&""" class=""btn-get"">구매하기</a></td>"			
		end if
	Next
	
%>
	<div class="layerPopup default-font">
		<header class="tenten-header header-popup header-white fixed-top">
