	...
	Select Case Trim(sGubun)
		...
		Case "jibun" '// 지번 주소로 검색했을때
			...
			if oDoc.FTotalCount>0 Then
				...
				Response.write "OK|"&tmpconfirmVal&"|"& chkiif(oDoc.FTotalCount>PageSize,PageSize,oDoc.FTotalCount) &"|"&tmppagingVal
				Response.End
			Else
		...
		Case "RoadBnumber" '// 도로명 주소에 도로명 + 건물번호로 검색했을때
			...
			If Not(rsEVTget.bof Or rsEVTget.eof) Then
				...
				rsEVTget.movenext
				Loop
				Response.write "OK|"&tmpconfirmVal&"|"& chkiif(tmpsReturnCnt>PageSize,PageSize,tmpsReturnCnt)
				Response.End
			Else
				Response.write "OK|<li class='nodata'>검색된 주소가 없습니다.</li>"
				Response.End
