...
<title>10x10: 장바구니</title>
<style type="text/css">
.btn-top {z-index:500;}
</style>
<script type="text/javascript" src="/lib/js/shoppingbag_script.js?v=1.3"></script>
<script type="text/javascript" src="/lib/js/itemPrdDetail.js"></script>
					...
					if(message.substr(0,2)=="OK") {
						...
						if(message.substr(3,message.length)=="0") {
							...
							$("form[name='baguniFrm'] li").eq(idx).fadeOut(0,function(){
								...
								// 장바구니 수 변경
								$(".header .btnCartGo dfn").html(message.substr(3,message.length));
								// 총계 재계산
								fnSelCalculate();
							});
						}
	...
	function addWishSelected(){
		<% If IsUserLoginOK() Then ''ErBValue.value -> 공통파일의 구분값 (장바구니는 5) %>
		    ...
		    if (confirm('선택 상품을 위시리스트에 추가하시겠습니까?')){
		    	document.frmWishPop.bagarray.value = ArrayFavItemID;
		    	document.frmWishPop.folderAction.value = 'AddFavItems';
				frmWishPop.submit();
		    }
		<% Else %>
	...
	$(function(){
		...
		if (CStr(GetCartCount)<>CStr(iTotalItemCount)) and not(IsForeignDlv) then
			...
			response.write "$('#ibgaCNT').html('"&iTotalItemCount&"');"
		end if
		%>

		//Top버튼 위치 기본값
		$(".btn-top").addClass("down");
		...
		$(window).scroll(function(){
			...
			if (vSpos < docuH){
				...
				$(".cartFloatBarV16a").fadeOut("fast",function(){
					$(".btn-top").removeClass("down");
				});
			}
		});

		// 상품갯수 직접입력시 플로팅바 컨트롤
		if( document.baguniFrm !== undefined ) {
		...
		<%' amplitude 이벤트 로깅 %>
			//tagScriptSend('', 'shoppingbag', '', 'amplitude');
		...
		<div class="container">
			...
			<% if Not(oshoppingbag.IsShoppingBagVoid) or IsForeignDlv or IsQuickDlv then %>
				<div class="tabV16a">
					...
					<span class="grid3 <%=CHKIIF(IsLocalDlv,"current","")%>"><a href="/inipay/shoppingbag.asp">국내배송</a></span>
					<span class="grid3 <%=CHKIIF(IsQuickDlv,"current","")%>"><a href="/inipay/ShoppingBag.asp?bTp=q">바로배송</a></span>
					<span class="grid3 <%=CHKIIF(IsForeignDlv,"current","")%>"><a href="/inipay/ShoppingBag.asp?bTp=f">해외배송</a></span>
				    <% else %>
				    <span class="grid2 <%=CHKIIF(IsLocalDlv,"current","")%>"><a href="/inipay/shoppingbag.asp">국내배송</a></span>
					<span class="grid2 <%=CHKIIF(IsForeignDlv,"current","")%>"><a href="/inipay/ShoppingBag.asp?bTp=f">해외배송</a></span>
				    <% end if %>
				</div>
			<% end if %>
			<%
			...
			%>
				<div class="emptyMsgV16a <%=chkIIF(IsForeignDlv or IsQuickDlv,"emptyIntlV16a","emptyDomsV16a")%>">
					<div>
						...
						<p><button type="button" class="btnV16a btnRed2V16a" onclick="location.href='/'">쇼핑하러 가기</button></p>
					</div>
				</div>
			<%
				Else
			...
			%>
				<div class="cartV16a">
					...
					<%
						''IDX 0: 티켓, 1:Present상품, 2:현장수령 상품, 3:텐바이텐 배송, 4:업체 배송 5:업체 개별 배송 6:업체 착불배송, 8:이니렌탈 상품
						dim Mix, MidxSub
						MidxSub = 0
				...
				%>
                            <script>
                                ...
                                appier_shoppingbag_product.product_url = "/category/category_itemPrd.asp?itemid=<%= oshoppingbag.FItemList(i).FItemID %>&gaparam=cart_list";
							...
							<li class="bxWt1V16a <%=chkIIF(oshoppingbag.FItemList(i).ISsoldOut or TicketBookingExired,"soldoutV16a","") & chkIIF(oshoppingbag.FItemList(i).IsMileShopSangpum,"mileageV16a","") %>" mix="<%=CHKIIF(Mix=5,Mix+CnPls+MidxSub,Mix)%>">
								<div class="pdtWrapV16a">
									...
									<div class="pdtInfoV16a">
										<p class="pdtPicV16a">
											<span>품절</span>
											<a href="/category/category_itemPrd.asp?itemid=<% = oshoppingbag.FItemList(i).FItemID %>&gaparam=cart_list"><img src="<%= oshoppingbag.FItemList(i).FImageList %>" alt="<%= replace(oshoppingbag.FItemList(i).FItemName,"""","") %>" /></a>
										</p>
									...
									<div class="pdtPriceV16a">
										<p>
											...
											<button type="button" class="btnV16a btnGrnV16a" onclick="jsDownCouponShoppingbag('prd','<%= oshoppingbag.FItemList(i).FCurrItemCouponIdx %>','S');return false;"><span class="cWh1">쿠폰 다운</span></button>
											<% end if %>
										</p>
										<% If oshoppingbag.FItemList(i).IsRentalItem Then %>
									...
									<div class="pdtOdrTxtV16a">
										<dl>
											<dt>
												<span>주문제작문구</span>
												<button type="button" class="btnV16a btnLGryV16a btnReqEditItem" <%=chkIIF(oshoppingbag.FItemList(i).ISsoldOut or TicketBookingExired,"disabled=""disabled""","")%> onclick="fnOpenModal('/inipay/Pop_EditItemRequireAjax.asp?itemid=<%= oshoppingbag.FItemList(i).FItemid %>&itemoption=<%= oshoppingbag.FItemList(i).FItemoption %>&eqno='+$('.btnReqEditItem').index(this));">수정</button>
											</dt>
											<dd class="reqrText">
				...
				%>
						...
						<div class="bxLGy2V16a grpTotalV16a">
							...
							<% Case 5 %>
								...
								<p class="tMar0-5r fs1-1r cMGy1V16a"><%=Replace(oshoppingbag.FParticleBeasongUpcheList(j).getDeliveryPayDispHTML,"<strong>"&oshoppingbag.FParticleBeasongUpcheList(j).FSocName_Kor&"</strong>","<strong class=""txtLine""><a href=""/brand/brand_detail2020.asp?brandid=" & pmakerid & """>"&oshoppingbag.FParticleBeasongUpcheList(j).FSocName_Kor&"</a></strong>")%></p>
			...
			%>
				...
				%>
					...
					<div class="bnr18th">
						<a href="/my10x10/couponbook.asp" onclick="fnAmplitudeEventMultiPropertiesAction('click_shoppingbag_coupon_banner','','',);">
                    ...
                    <!-- <a href="/diarystory2022/" class="bnr_shoppingbag_dr">
			...
			<form name="NextFrm" method="post" action="<%= Replace(wwwUrl,"http:","http:") %>/inipay/userinfo.asp">
			<input type="hidden" name="sitename" value="10x10">
			<input type="hidden" name="jumundiv" value="1">
			<input type="hidden" name="bTp" value="<%=bTp%>">
			...
			<form name="frmWishPop" method="get" action="/common/popWishFolder.asp" style="margin:0px;">
			<input type="hidden" name="itemid" value="">
