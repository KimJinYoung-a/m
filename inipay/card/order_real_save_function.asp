...
Function OrderRealSaveProc(vIdx)
	...
	Dim vPgGubun, vAppVer, vPDiscount, device
	...
	IF vResult = "ok" Then
		...
		IF Not rsget.EOF THEN
			...
			vAppVer                 = rsget("userDevice")           '' 앱버전 기존 rdsite에 추가.
			vPDiscount              = rsget("pDiscount")            '' pg사 할인 금액.
			if isNULL(vPgGubun) then vPgGubun=""
			if isNULL(vAppVer) then vAppVer=""    
			if isNULL(vPDiscount) then vPDiscount=0
			
			vAppVer = Trim(Right(vAppVer,5))                        ''A1.65  // I1.94
		...
		IF vResult = "ok" Then
			...
			If vRdsite = "app_wish" Then
				iorderParams.Frdsite    	= "app_wish"
			ElseIf vRdsite = "app_wish2" Then
				iorderParams.Frdsite		= "app_wish2"                ''&vAppVer       ''2015/08/04
			Else
						...
						IF vResult = "ok" Then
							...
							IF vResult = "ok" Then
								...
								if (CLng(oshoppingbag.getTotalCouponAssignPrice(packtype) + iorderParams.fpojangcash - iorderParams.Fmiletotalprice-iorderParams.Fcouponmoney-iorderParams.FallatDiscountprice-iorderParams.Fspendtencash-iorderParams.Fspendgiftmoney) <> CLng(subtotalprice)) then
									...
									if (vIdx<>4980278) and (vIdx<>4871312) and (vIdx<>4884051) and (vIdx<>4267870) then
									    vResult = "x9"
								    end if
								end if
