		...
		<div class="container bgGry">
			...
			<div class="content evtDetailV15" id="contentArea">
				...
				<div class="evtHeadV15">
					...
					<div class="btnWrap">
						<p class="evtShareV15"><a href="" onClick="popSNSShare('<%=eOnlyName%>','<%=wwwUrl&CurrURLQ()%>','10x10 이벤트','<%=snpImg%>'); return false;">공유</a></p>
				...
				<div class="section-event event-head">
					<p class="date"><%=Replace(esdate,"-",".")%> ~ <%=Replace(right(eedate,5),"-",".")%></p>
					<div class="btnGroup">
				...
				<% End If %>
				<% If isArray(arrGroup) Then %>
				<div class="evtPdtListWrapV15<% If UBound(arrGroup, 2)>0 Then %> evtPdtListWrapV15a<% Else %> evtPdtListWrapV15b<% End If %>">
				...
				%>
					...
					<% End If %>
					<div id="groupBar<%=arrGroup(0,intG)%>" class="evtPdtListV15">
						<h3 class="groupBar">
				...
				Else
				%>
					<div id="groupBar0" class="evtPdtListV15">
							<% sbEvtItemView_2015 %>
					</div>
				<%
				End If
