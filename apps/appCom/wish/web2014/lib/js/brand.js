...
function ShowCategory(cate1,isend) {
	$.ajax({
			url: "/apps/appCom/wish/web2014/category/incCategory_cate_ajax.asp?disp="+cate1,
			cache: false,
...
{
	$.ajax({
			url: "/apps/appCom/wish/web2014/category/category_list_ajax.asp?disp="+cate1+"&cpg="+page+"&srm="+sort+"&imgsize="+imgsize+"",
			cache: false,
			success: function(message)
			{
...
{
	location.href="/apps/appCom/wish/web2014/category/category_list.asp?disp="+cate1+"&cpg="+page+"&srm="+sort+"&imgsize="+imgsize;
}

//1depth 대카테고리 페이지 이동		//2013.12.18 한용민 생성
function moveCategorymain()
{
	location.href="/apps/appCom/wish/web2014/category/category_main.asp"
}

//1depth 이후 2depth 부터 카테고리 페이지 이동		//2013.12.18 한용민 생성
function moveCategorysub(cate1)
{
	location.href="/apps/appCom/wish/web2014/category/category_sub.asp?disp="+cate1;
}

//브랜드 카테고리에 상품 리스트 보기
function moveBrandCategoryList(cate1,page,sort,imgsize,makerid){
	location.href="/apps/appCom/wish/web2014/street/street_brand.asp?disp="+cate1+"&cpg="+page+"&srm="+sort+"&imgsize="+imgsize+"&makerid="+makerid;
}

//나의찜브랜드 리스트 보기.
function ShowMyZzimBrand() {
	$.ajax({
			url: "/apps/appCom/wish/web2014/category/category_mybrandlist_ajax.asp",
			cache: false,
			success: function(message)
			{
...
function ShowBrandDetail(makerid){
	top.location.href = "/apps/appCom/wish/web2014/street/street_brand.asp?makerid="+makerid+"";
}
