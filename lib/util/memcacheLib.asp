...
'exit function
	...
	cdoMessage.SubJect 	= "["&date()&"] MemCached (M) ÆäÀÌÁö ¿À·ù ¹ß»ý"
...
function fnMemValDecrypt(v)
    ...
    Obj.RegisterKey  = "63918C68A2D78AF7-5755"
    Obj.CipherAlgorithm = 1    '18,1 fast
    fnMemValDecrypt = Obj.DecryptString(replace(v,G_DUMINVAL,""),"")
    set Obj = nothing
end function
 ...
 'dim otime : otime=Timer()
    ...
    memVal = omem.get(fnHashMemKey(ipreFix,iKeyStr))
        if Err Then
            ...
            debugWrite "err mem;"
            call OnErrNotiByVal((fnHashMemKey(ipreFix,iKeyStr))&":"&iKeyStr)
            omem.close()
            set omem = Nothing
            On Error Goto 0
            Exit function
    ...
    public function GetResult_TotalCount(byRef iTotalCount)
        if (FuseMemcache) then
            if (FMemExists) and (FQueryType=1) then
                iTotalCount = FMemVal
                if (iTotalCount="") then iTotalCount=0 ''2014/12/19 Ãß°¡
                Exit function
            end if
    ...
    public function GetResult_RowSize(byRef iResultcount) ''°Ë»ö°á°ú¼ö
        if (FuseMemcache) then
            if (FMemExists) and (FQueryType=2) then
                ...
                if (iResultcount="") then iResultcount=0 ''2014/12/19 Ãß°¡
                Exit function
            end if
        end if
