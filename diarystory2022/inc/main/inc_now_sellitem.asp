        ...
        %>
			<article class="prd_item<% if soldList(i).IsSoldOut then %> soldout<% end if %>">
				...
				<div class="prd_info">
					...
					<div class="prd_badge<% if fnGetGiftiCon(soldList(i).Fdeliverytype,soldList(i).Forgprice,soldList(i).Fitemid) and giftCheck and (fnGetDeliveryFreeiCon(soldList(i).Fdeliverytype,soldList(i).Fsellcash,soldList(i).FdefaultFreeBeasongLimit)) then %> badge_two<% end if %>">
                ...
                <%if isapp = 1 then%>
                <a href="javascript:TnGotoAPPDiaryProduct('<%=soldList(i).Fitemid%>');" onclick="fnAmplitudeEventAction('click_diarystory_justsold','item_id','<%=soldList(i).Fitemid%>');" class="prd_link"><span class="blind">상품 바로가기</span></a>
                <%else%>
                <a href="javascript:TnGotoDiaryProduct(<%=soldList(i).Fitemid%>);" onclick="fnAmplitudeEventAction('click_diarystory_justsold','item_id','<%=soldList(i).Fitemid%>');" class="prd_link"><span class="blind">상품 바로가기</span></a>
                <% end if %>
